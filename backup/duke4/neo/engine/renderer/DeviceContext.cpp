...
idDeviceContextOptimized deviceContextLocal;
idDeviceContextOptimized* deviceContext = &deviceContextLocal;
...
void idDeviceContext::DrawWinding(idWinding& w, const idMaterial* mat) {
	...
	idVec4 currentColor = deviceContext->GetColor();
...
void idDeviceContext::SetColorLocal(float r, float g, float b, float a) {
	tr.guiModel->SetColor(r, g, b, a);
}
...
int idDeviceContextOptimized::DrawText(float x, float y, float scale, idVec4 color, const char* text, float adjust, int limit, int style, int cursor) {
	...
	while (charIndex < drawText.Length()) {
		...
		if (!ClippedCoords(&drawX, &drawY, &w, &h, &s, &t, &s2, &t2)) {
			...
			idDrawVert* verts = tr.guiModel->AllocTris(4, quadPicIndexes, 6, glyphInfo.material, 0);
			if (verts != NULL) {
				verts[0].xyz[0] = x1;
				verts[0].xyz[1] = y1;
				verts[0].xyz[2] = 0.0f;
				verts[0].st = idVec2(s, t);
				verts[0].color[0] = currentColor.x;
				verts[0].color[1] = currentColor.y;
				verts[0].color[2] = currentColor.z;
				verts[0].color[3] = currentColor.w;

				verts[1].xyz[0] = x2;
				verts[1].xyz[1] = y1;
				verts[1].xyz[2] = 0.0f;
				verts[1].st = idVec2(s2, t);
				verts[1].color[0] = currentColor.x;
				verts[1].color[1] = currentColor.y;
				verts[1].color[2] = currentColor.z;
				verts[1].color[3] = currentColor.w;

				verts[2].xyz[0] = x2;
				verts[2].xyz[1] = y2;
				verts[2].xyz[2] = 0.0f;
				verts[2].st = idVec2(s2, t2);
				verts[2].color[0] = currentColor.x;
				verts[2].color[1] = currentColor.y;
				verts[2].color[2] = currentColor.z;
				verts[2].color[3] = currentColor.w;

				verts[3].xyz[0] = x1;
				verts[3].xyz[1] = y2;
				verts[3].xyz[2] = 0.0f;
				verts[3].st = idVec2(s, t2);
				verts[3].color[0] = currentColor.x;
				verts[3].color[1] = currentColor.y;
				verts[3].color[2] = currentColor.z;
				verts[3].color[3] = currentColor.w;
			}
		}
