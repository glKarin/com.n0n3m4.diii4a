...
========================
*/
ID_INLINE uint32 MsecToSamples( uint32 msec, uint32 sampleRate ) { return ( msec * ( sampleRate / 100 ) ) / 10; }
ID_INLINE uint32 SamplesToMsec( uint32 samples, uint32 sampleRate ) { return sampleRate < 100 ? 0 : ( samples * 10 ) / ( sampleRate / 100 ); }
...
========================
*/
ID_INLINE float DBtoLinear( float db ) { return idMath::Pow( 2.0f, db * ( 1.0f / 6.0f ) ); }
ID_INLINE float LinearToDB( float linear ) { return ( linear > 0.0f ) ? ( idMath::Log( linear ) * ( 6.0f / 0.693147181f ) ) : -999.0f; }
...
#else
struct idSoundVoice { int i ;};
struct idSoundHardware { int i ;};
#endif

#include "../external/oggvorbis/ogg/ogg.h"
...
public:
	...
	virtual int				PlayShaderDirectly(const idSoundShader* shader, int channel = -1);
