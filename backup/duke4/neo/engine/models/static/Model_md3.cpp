...
void idRenderModelMD3::InitFromFile( const char *fileName ) {
	...
	//k64 if (!size || size<0 ) 
	if ( size <= sizeof(md3Header_t) ) //k64
	{
		return;
	}
...
// jmarshall end
	...
	for( i = 0; i < md3->numSurfaces; i++ ) {
		...
		//k64 surf.shader = shaders->shader;
		...
		R_BoundTriSurf( tri );
		surf.id = staticModel->NumSurfaces(); // DG: make sure to initialize id; FIXME: or just set id to 0? //k64
...
idBounds idRenderModelMD3::Bounds(const struct renderEntity_t *ent) const {
	...
	//k frame += (int)ent->shaderParms[SHADERPARM_MD3_FRAME]; // DG: use bounds of current frame //k64
