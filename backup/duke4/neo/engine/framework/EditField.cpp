...
void idEditField::Draw( int x, int y, int width, bool showCursor, const idMaterial *shader ) {
	...
	if ( !showCursor || (int)(com_ticNumber >> 4) & 1) {
		deviceContext->DrawText(0, y + SMALLCHAR_HEIGHT, CONSOLE_FONT_SCALE, colorWhite, va("> %s", str), 1.0f, 0, true);
		return;
	}
	...
	deviceContext->DrawText(0, y + SMALLCHAR_HEIGHT, CONSOLE_FONT_SCALE, colorWhite, va("> %s_", str), 1.0f, 0, true);
}
