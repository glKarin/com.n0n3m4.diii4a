...
void R_ImageList_f( void ) {
	...
	for ( i = 0; i < tr.numImages; i++ )
	{
		...
		{
#if !defined(__ANDROID__) //karin: GL_RGBA on OpenGL ES1.1
			...
			case GL_RGBA:
				...
				estSize *= 4;
				break;
#if !defined(__ANDROID__) //karin: GL_RGB on OpenGL ES1.1
			case GL_RGB5:
			case GL_RGB8:
#endif
			case GL_RGB:
...
#if !defined(__ANDROID__) //karin: only use GL_RGBA on OpenGL ES1.1, because internalFormat == dataFormat, and data is 4-bytes component
	if ( lightMap )
		return GL_RGB;
