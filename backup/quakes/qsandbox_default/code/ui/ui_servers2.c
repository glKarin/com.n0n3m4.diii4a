...
static const char *master_itemsru[] = {
	"Локальные",
	"Любимые",
	0
};

static const char *servertype_itemsru[] = {
	"Все",
	"Все Против Всех",
	"Командный Бой",
	"Турнир",
...
static const char *sortkey_itemsru[] = {
	"Имя Сервера",
	"Имя Карты",
	"Открытые Слоты",
	"Режим Игры",
...
static void ArenaServers_UpdateMenu( void ) {
	...
	if( g_arenaservers.numqueriedservers > 0 ) {
		...
		if( g_arenaservers.refreshservers && ( g_arenaservers.currentping <= g_arenaservers.numqueriedservers ) ) {
			...
			Com_sprintf( g_arenaservers.status.string, MAX_STATUSLENGTH, "%d of %d QSandbox Servers.", g_arenaservers.currentping, g_arenaservers.numqueriedservers);
			g_arenaservers.statusbar.string  = "Press SPACE to stop";
			}
			if(cl_language.integer == 1){
			Com_sprintf( g_arenaservers.status.string, MAX_STATUSLENGTH, "%d из %d QSandbox Серверов.", g_arenaservers.currentping, g_arenaservers.numqueriedservers);
			g_arenaservers.statusbar.string  = "Нажмите SPACE для остановки";
			}
		...
		if( g_arenaservers.refreshservers ) {
			...
			if(cl_language.integer == 1){
			strcpy( g_arenaservers.status.string,"Поиск Серверов..." );
			g_arenaservers.statusbar.string = "Нажмите SPACE для остановки";
			}
...
if(cl_language.integer == 0){
   		Com_sprintf( buff, MAX_LISTBOXWIDTH, "%-20.20s %-12.12s %2d/%2d %-30.30s %3s %s%3d",
...
void ArenaServers_SetType( int type )
{
	...
	if(cl_language.integer == 1){
	strcpy(g_arenaservers.status.string,"нажмите Обновить для обновления списка");
	}
}
...
static void ArenaServers_MenuInit( void ) {
	...
	g_arenaservers.password.field.maxchars		= 32;
	...
	Menu_AddItem( &g_arenaservers.menu, (void*) &g_arenaservers.password );
