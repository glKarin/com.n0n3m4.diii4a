...
vec3 heatHazeFragmentShader(
	...
	vec2 displacementTex = 2.0 * displacementRawTex.xy - vec2(1.0);
	...
	if (saveForeground) {
		vec2 depthRenderSizeRatio = vec2(textureSize(currentDepth, 0)) * invRenderSize;
	...
	if (blurDistance > 0.0) {
		vec3 sum = vec3(0.0);
		vec2 pos;
		pos = displacedPos + vec2(blurDistance, 0.0) * invRenderSize;
		sum += texture(currentRender, pos).rgb;
		pos = displacedPos + vec2(0.0, blurDistance) * invRenderSize;
		sum += texture(currentRender, pos).rgb;
		pos = displacedPos + vec2(-blurDistance, 0.0) * invRenderSize;
		sum += texture(currentRender, pos).rgb;
		pos = displacedPos + vec2(0.0, -blurDistance) * invRenderSize;
