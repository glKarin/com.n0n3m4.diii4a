...
bool rvGEItemPropsTextPage::Init ( void )
{
	...
	SendMessage ( GetDlgItem ( mPage, IDC_GUIED_ITEMTEXTFONT ), CB_ADDSTRING, 0, (LPARAM)"<default>" );
	...
	for ( i = 0; i < folders->GetNumFiles(); i++ ) {
		...
		SendMessage ( GetDlgItem ( mPage, IDC_GUIED_ITEMTEXTFONT ), CB_ADDSTRING, 0, (LPARAM)folders->GetFile(i) );
	}
...
bool rvGEItemPropsTextPage::SetActive ( void )
{
	...
	font.StripQuotes ( );
	font.StripPath ( );
	fontSel = SendMessage ( GetDlgItem ( mPage, IDC_GUIED_ITEMTEXTFONT ), CB_FINDSTRING, -1, (LPARAM)font.c_str () );
...
bool rvGEItemPropsTextPage::KillActive ( void )
{
	...
	if ( IsDlgButtonChecked ( mPage, IDC_GUIED_USETEXT ) )
	{
		...
		else
		{
			...
			SendMessage ( GetDlgItem ( mPage, IDC_GUIED_ITEMTEXTFONT ), CB_GETLBTEXT, fontSel, (LPARAM)fontName );
...
bool GEItemPropsDlg_DoModal ( HWND parent, idWindow* window, idDict& dict )
{
	...
	propsp[RVITEMPROPS_GENERAL].lParam		= (LPARAM)new rvGEItemPropsGeneralPage ( &dict, wrapper->GetWindowType ( ) );
	...
	propsp[RVITEMPROPS_IMAGE].lParam		= (LPARAM)new rvGEItemPropsImagePage ( &dict );;
	...
	propsp[RVITEMPROPS_TEXT].lParam			= (LPARAM)new rvGEItemPropsTextPage ( &dict );;
	...
	propsp[RVITEMPROPS_KEYS].lParam			= (LPARAM)new rvGEItemPropsKeysPage ( &dict, wrapper );
