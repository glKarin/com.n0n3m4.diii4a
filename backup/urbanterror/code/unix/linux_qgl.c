...
//GLX Functions
#if !defined(__ANDROID__) //karin: X11
#if !defined(USE_SDL_VIDEO)
...
void ( APIENTRY * qglMultiTexCoord4f )( GLenum texture, GLfloat s, GLfloat t, GLfloat r, GLfloat q );
...
static void ( APIENTRY * dllMultiTexCoord4f )(GLenum texture, GLfloat s, GLfloat t, GLfloat r, GLfloat q);

#define SIG( ... ) do { GL_CheckError(); fprintf( stderr, __VA_ARGS__ ); fprintf( stderr, "\n" ); } while(0)
static void GL_CheckError(void)
{
    GLenum err;
    while((err = dllGetError()) != GL_NO_ERROR)
    {
        switch( err ) {
...
{
	SIG( "glAlphaFunc( 0x%x, %f )", (unsigned int) func, ref );
...
{
	SIG( "glBegin( 0x%x )", (unsigned int) mode );
	dllBegin( mode );
}

static void APIENTRY logBindTexture(GLenum target, GLuint texture)
{
	SIG( "glBindTexture( 0x%x, %u )", (unsigned int) target, (unsigned int) texture );
...
{
	SIG( "glBlendFunc( 0x%x, 0x%x )", (unsigned int) sfactor, (unsigned int) dfactor );
	dllBlendFunc( sfactor, dfactor );
}

static void APIENTRY logCallList(GLuint list)
{
	SIG( "glCallList( %u )", (unsigned int) list );
	dllCallList( list );
}
...
{
    SIG( "glColor4f( %f,%f,%f,%f )", red, green, blue, alpha );
	dllColor4f( red, green, blue, alpha );
}
static void APIENTRY logColor4fv(const GLfloat *v)
{
    SIG( "glColor4fv( %f,%f,%f,%f )", v[0], v[1], v[2], v[3] );
	dllColor4fv( v );
}
...
{
    SIG( "glDisable( 0x%x )", (unsigned int) cap );
	dllDisable( cap );
}

static void APIENTRY logDisableClientState(GLenum array)
{
    SIG( "glDisableClientState(%x)", array );
...
static void APIENTRY logDrawElements(GLenum mode, GLsizei count, GLenum type, const void *indices)
{
	SIG( "glDrawElements" );
    int arr[100];
    arr[0] = dllIsEnabled(GL_VERTEX_ARRAY);
    arr[1] = dllIsEnabled(GL_COLOR_ARRAY);
    arr[2] = dllIsEnabled(GL_TEXTURE_COORD_ARRAY);
    dllGetIntegerv(GL_TEXTURE_BINDING_2D, arr+3);
    arr[4] = dllIsTexture(arr[3]);
    fprintf(stderr, "DDD ");
    for(int i = 0; i < 5; i++)
        fprintf(stderr, "%d, ", arr[i]);
    fprintf(stderr, "\n");
    dllEnable(GL_TEXTURE_2D);
    dllDisableClientState(GL_COLOR_ARRAY);
    //dllDisableClientState(GL_TEXTURE_COORD_ARRAY);
    static int c[4] = {0};
    c[0] = (c[0] + 10) % 255;
    c[1] = (c[1] + 20) % 255;
    c[2] = (c[2] + 50) % 255;
#define FFF(x) (float)x/255.0f
    dllColor4f(FFF(c[0]), FFF(c[1]), FFF(c[2]), 1);
...
{
    SIG( "glEnable( 0x%x )", (unsigned int) cap );
	dllEnable( cap );
}

static void APIENTRY logEnableClientState(GLenum array)
{
	SIG( "glEnableClientState(%x)", array );
...
{
    SIG( "glHint( 0x%x, 0x%x )", (unsigned int) target, (unsigned int) mode );
	dllHint( target, mode );
}
...
{
	SIG( "glOrtho(%f, %f, %f, %f, %f, %f)", left, right, bottom, top, zNear, zFar );
...
{
    SIG( "glPolygonMode( 0x%x, 0x%x )", (unsigned int) face, (unsigned int) mode );
...
{
    SIG( "glTexEnvf( 0x%x, 0x%x, %f )", (unsigned int) target, (unsigned int) pname, param );
...
{
    SIG( "glTexEnvi( 0x%x, 0x%x, 0x%x )", (unsigned int) target, (unsigned int) pname, (unsigned int) param );
...
{
    SIG( "glTexParameterf( 0x%x, 0x%x, %f )", (unsigned int) target, (unsigned int) pname, param );
...
{
    SIG( "glTexParameteri( 0x%x, 0x%x, 0x%x )", (unsigned int) target, (unsigned int) pname, (unsigned int) param );
...
static void APIENTRY logMultiTexCoord4f(GLenum texture, GLfloat s, GLfloat t, GLfloat r, GLfloat q)
{
    SIG( "glMultiTexCoord4f" );
    dllMultiTexCoord4f( texture, s, t, r, q );
}
...
#elif defined(__ANDROID__) //karin: EGL
#define GPA( a ) Q3E_GET_PROC_ADDRESS( a )
#else
...
#ifdef USE_OPENGLES //karin: GLES compat
