...
qhandle_t RE_RegisterShaderNoMip( const char *name ) {
	...
	if(!name) {
		Com_Printf( "Shader name is NULL\n" );
		return 0;
	}
	if ( strlen( name ) >= MAX_QPATH ) {
		Com_Printf( "Shader name exceeds MAX_QPATH\n" );
    ...
    if(strstr(name, "i/assets/") == name)
        Q_snprintf(newPath, MAX_OSPATH, "u%s", name);
    else if(strstr(name, "/assets/") == name)
        Q_snprintf(newPath, MAX_OSPATH, "ui%s", name);
    else if(strstr(name, "assets/") == name)
        Q_snprintf(newPath, MAX_OSPATH, "ui/%s", name);
    else
        Q_snprintf(newPath, MAX_OSPATH, "%s", name);

	sh = R_FindShader( newPath, LIGHTMAP_2D, qfalse );
	...
	// something calls RE_RegisterShader again with
	// the same name, we don't try looking for it again
    fprintf(stderr, "FFF %s : %s -> %d\n", name, newPath, sh->defaultShader);
	if ( sh->defaultShader ) {
		return 0;
	}
