...
// Q3Rally Code Start - increased max
...
// Q3Rally Code Start
	else if ( le->leMarkType == LEMT_OIL ) {

		radius = 8 + (rand()&15);
		CG_ImpactMark( cgs.media.oilMarkShader, trace->endpos, trace->plane.normal, random()*360,
			1,1,1,1, qtrue, radius, qfalse );
	}
// Q3Rally Code END
...
// Q3Rally Code Start : put oil sound in?
	else if ( le->leBounceSoundType == LEBS_OIL ) {
...
//	VectorCopy( le->refEntity.origin, oldOrigin );
	...
	VectorSet(mins, -le->radius, -le->radius, -le->radius);
	VectorSet(maxs, le->radius, le->radius, le->radius);

	if( le->leFlags & LEF_SCALE_FADE_OUT )
	{
		float	c = 1.0f;

		c = ( cg.time - le->startTime ) / (float)( le->endTime - le->startTime );

		le->refEntity.shaderRGBA[0] = 255;
		le->refEntity.shaderRGBA[1] = (int)(210 * ( 1.0f - c ));
		le->refEntity.shaderRGBA[2] = 0;

		c = 1.0f;

		if ( le->fadeInTime > le->startTime && le->fadeInTime < le->endTime && cg.time > le->fadeInTime )
		{
			// fade / grow time
			c = 1.0f - (float) ( cg.time - le->fadeInTime ) / ( le->endTime - le->fadeInTime );
		}

		le->refEntity.shaderRGBA[3] = (int)(255 * c * le->color[3]);

		if ( !( le->leFlags & LEF_PUFF_DONT_SCALE ) ) {
			le->refEntity.radius = le->radius * c;
		}
	}

//	if ( le->pos.trType == TR_STATIONARY ) {
	if ( le->pos.trType == TR_STATIONARY && !(le->leFlags & LEF_SCALE_FADE_OUT) ) {
// Q3Rally Code END
...
// Q3Rally Code Start
	if( le->leFlags & LEF_SCALE_FADE_OUT )
		BG_EvaluateTrajectory( &le->pos, cg.time - le->startTime > 50 ? cg.time - 50 : le->startTime, oldOrigin );
// Q3Rally Code END
...
//		trap_R_AddRefEntityToScene( &le->refEntity );
		if( le->leFlags & LEF_SCALE_FADE_OUT )
			CG_Draw3DLine( oldOrigin, newOrigin, le->refEntity.shaderRGBA[0] / 255.0f, le->refEntity.shaderRGBA[1] / 255.0f, le->refEntity.shaderRGBA[2] / 255.0f, le->refEntity.shaderRGBA[3] / 255.0f );
		else
			trap_R_AddRefEntityToScene( &le->refEntity );
// Q3Rally Code END
...
//	trap_R_AddRefEntityToScene( &le->refEntity );
	if( le->leFlags & LEF_SCALE_FADE_OUT )
		CG_Draw3DLine( oldOrigin, trace.endpos, le->refEntity.shaderRGBA[0] / 255.0f, le->refEntity.shaderRGBA[1] / 255.0f, le->refEntity.shaderRGBA[2] / 255.0f, le->refEntity.shaderRGBA[3] / 255.0f );
	else
		trap_R_AddRefEntityToScene( &le->refEntity );
// Q3Rally Code END
}
...
static void CG_AddMoveScaleFade( localEntity_t *le ) {
	...
	if ( len < le->radius ) {
// Q3Rally Code Start - keep the entity, just dont draw it
//		CG_FreeLocalEntity( le );
// Q3Rally Code END
		return;
	}
...
void CG_AddInvulnerabilityJuiced( localEntity_t *le ) {
	...
	if ( t > 5000 ) {
		le->endTime = 0;
// Q3Rally Code Start: set to -1 maybe?
//		CG_GibPlayer( le->refEntity.origin );
		CG_GibPlayer( le->refEntity.origin, vec3_origin, 0 );
// Q3Rally Code END
	}
	else {
		trap_R_AddRefEntityToScene( &le->refEntity );
