...
// Q3Rally Code Start
static void CG_HUDDown_f( void ) {
	cg.showHUD = qtrue;
}

static void CG_HUDUp_f( void ) {
	cg.showHUD = qfalse;
}

static void CG_ControlMode_f( void ) {
	int mode;

	mode = cgs.clientinfo[cg.snap->ps.clientNum].controlMode + 1;
	...
	switch (mode){
	case CT_MOUSE:
		Com_Printf("Control Mode: Mouse\n");
		trap_SendConsoleCommand("-strafe");
		break;
	case CT_JOYSTICK:
		Com_Printf("Control Mode: Joystick\n");
		trap_SendConsoleCommand("+strafe");
		break;
	}

	trap_Cvar_Set("cg_controlMode", va("%i", mode));
}

// UPDATE: Temp for testing
static void CG_PhysicsDebug_f( void ) {
	trap_SendClientCommand(va("pDebug %s", CG_Argv(1)));
	cg.pDebug = atoi(CG_Argv(1));
}

static void CG_NextBezierPoint_f( void )
{
	cg.currentBezierPoint++;
}

static void CG_PrevBezierPoint_f( void )
{
	cg.currentBezierPoint--;
}

static void CG_MoveBezierPoint_f( void )
{
	char	buffer[256];
	vec3_t	delta;
/*
	...
	for (i = 0; i < MAX_ENTITIES; i++)
	{
		...
		if (cent->currentState.weapon != cg.currentBezierPoint) continue;

		cent->bezierPos[0] += atof(CG_Argv(1));
		cent->bezierPos[1] += atof(CG_Argv(2));
		cent->bezierPos[2] += atof(CG_Argv(3));
...
*/
	...
	Com_sprintf( buffer, sizeof(buffer), "moveBPoint %i %f %f %f", cg.currentBezierPoint, delta[0], delta[1], delta[2] );
	Com_Printf( "Sending console command: '%s'\n", buffer );
	trap_SendClientCommand( buffer );
}

static void CG_MoveBezierHandle_f( void )
{
	char	buffer[256];
	vec3_t	delta;
/*
	...
	for (i = 0; i < MAX_ENTITIES; i++)
	{
		...
		cent->bezierDir[0] += atof(CG_Argv(1));
		cent->bezierDir[1] += atof(CG_Argv(2));
		cent->bezierDir[2] += atof(CG_Argv(3));
...
*/
	...
	delta[1] = atof(CG_Argv(2));
	delta[2] = atof(CG_Argv(3));
	Com_sprintf( buffer, sizeof(buffer), "moveBHandle %i %f %f %f", cg.currentBezierPoint, delta[0], delta[1], delta[2] );
	Com_Printf( "Sending console command: '%s'\n", buffer );
	trap_SendClientCommand( buffer );
}
...
// Q3Rally Code Start
	{ "+hud", CG_HUDDown_f },
	{ "-hud", CG_HUDUp_f },
	{ "controlMode", CG_ControlMode_f },
	{ "pDebug", CG_PhysicsDebug_f },
	{ "nextBPoint", CG_NextBezierPoint_f },
