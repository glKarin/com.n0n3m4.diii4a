...
//		if ( item->giType == IT_WEAPON && item->giTag == weaponNum ) {
		if ( (item->giType == IT_WEAPON || item->giType == IT_RFWEAPON) && item->giTag == weaponNum ) {
// END
...
// Q3Rally Code Start
		if ( item->giTag == PW_HASTE && item->world_model[2] ) {
			itemInfo->models[2] = trap_R_RegisterModel( item->world_model[2] );
		}
// END
	}
}
...
Caused by an EV_ALTFIRE_WEAPON event
================
*/
void CG_FireAltWeapon( centity_t *cent ) {
	...
	if ( ent->weapon >= WP_NUM_WEAPONS ) {
		CG_Error( "CG_FireAltWeapon: ent->weapon >= WP_NUM_WEAPONS" );
		return;
	}
	weap = &cg_weapons[ ent->weapon ];
...
*/
	AngleVectors( cent->currentState.apos.trBase, forward, NULL, up );
	VectorMA( muzzle, CAR_HEIGHT/2, up, muzzle );
// END
	VectorMA( muzzle, 14, forward, muzzle );
