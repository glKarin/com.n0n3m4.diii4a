...
// STONELANCE
vec4_t menu_back_color       = {0.0, 0.0, 0.0, 0.5};
// END
...
sfxHandle_t ListBox_Key( menulist_s *l, int key );
// END1
...
// STONELANCE
#define		DEFAULT_BACK_TRANSITION		1000000
// END
...
//	if (l->generic.flags & QMF_SCROLL_ONLY){
		l->generic.flags |= QMF_SILENT;
//	}
}
		...
		case K_MOUSE1:
			if (l->generic.flags & QMF_HASMOUSEFOCUS)
			{
				...
				if (l->scrollbarAlignment & SB_LEFT){
					...
					if (!(l->scrollbarAlignment & SB_HIDE) || l->numitems > l->height)
						w -= SB_WIDTH;
				}
				if (l->scrollbarAlignment & SB_RIGHT){
					if (!(l->scrollbarAlignment & SB_HIDE) || l->numitems > l->height)
						w -= SB_WIDTH;
				}
				...
				if ((l->scrollbarAlignment & SB_RIGHT) || (l->scrollbarAlignment & SB_LEFT)){
					if ((l->scrollbarAlignment & SB_HIDE) && (l->numitems <= l->height))
						return (menu_null_sound);
				}

				x = l->generic.left;
				tmbsize = (int)((h - 2*SB_WIDTH) * (l->height / (float)l->numitems));
				tmbsize = tmbsize < SB_WIDTH ? SB_WIDTH : tmbsize;
				tmbsize = tmbsize > (h - 2*SB_WIDTH) ? (h - 2*SB_WIDTH) : tmbsize;

				tmbpos = (int)((h - tmbsize - 2*SB_WIDTH) * (l->top / (float)(l->numitems - l->height)));

				if (l->scrollbarAlignment & SB_RIGHT){
					x += w;
				}

				if (UI_CursorInRect( x, y, SB_WIDTH, SB_WIDTH )){
					ListBox_Key(l, K_UPARROW);
				}
				if (UI_CursorInRect( x, y + h - SB_WIDTH, SB_WIDTH, SB_WIDTH )){
					ListBox_Key(l, K_DOWNARROW);
				}
				if (UI_CursorInRect( x, y + SB_WIDTH, SB_WIDTH, tmbpos )){
					ListBox_Key(l, K_PGUP);
				}
				if (UI_CursorInRect( x, y + SB_WIDTH + tmbpos + tmbsize, SB_WIDTH, (h - 2*SB_WIDTH) - (tmbpos + tmbsize))){
					ListBox_Key(l, K_PGDN);
				}
				if (UI_CursorInRect( x, y + SB_WIDTH + tmbpos, SB_WIDTH, tmbsize)){
					// thumb was clicked
					l->dragY = uis.cursory - (y + SB_WIDTH + tmbpos);
				}
...
/*
=================
ListBox_DrawScrollBar
=================
*/
static void ListBox_DrawScrollBar( menulist_s *l, int x ){
	int			y, h;
	float*		color;
	qboolean	hasfocus;
	int			tmbpos;
	...
	if (hasfocus && UI_CursorInRect( x, y, SB_WIDTH, SB_WIDTH ) && l->top > 0)
	{	
		color = pulse_color;
		color[3] = 0.5+0.5*sin(uis.realtime/PULSE_DIVISOR);
	...
	if (hasfocus && UI_CursorInRect( x, y + h - SB_WIDTH, SB_WIDTH, SB_WIDTH ) && (l->top + l->height < l->numitems))
	{	
		color = pulse_color;
		color[3] = 0.5+0.5*sin(uis.realtime/PULSE_DIVISOR);
	...
	UI_DrawHandlePic( x, y + SB_WIDTH + tmbpos, SB_WIDTH, SB_WIDTH/2, uis.sbtop );
	UI_DrawHandlePic( x, y + SB_WIDTH + tmbpos + 8, SB_WIDTH, tmbsize - SB_WIDTH, uis.sbmid );
	UI_DrawHandlePic( x, y + SB_WIDTH + tmbpos + (tmbsize - 8), SB_WIDTH, SB_WIDTH/2, uis.sbbot );
}
...
static void ListBox_Draw( menulist_s *l )
{
	...
	hx = l->generic.left;
	hw = (l->generic.right - l->generic.left) - SB_WIDTH;

	UI_FillRect(l->generic.left, l->generic.top, l->generic.right - l->generic.left, h, color_dim);

	if (l->dragY != -1 && !trap_Key_IsDown( K_MOUSE1 )){
		l->dragY = -1;
	}
	if (l->dragY != -1){
		tmbsize = (int)((h - 2*SB_WIDTH) * (l->height / (float)l->numitems));
		...
		tmbpos = (uis.cursory - l->dragY);
		tmbpos = tmbpos < y + SB_WIDTH ? y + SB_WIDTH : tmbpos;
		tmbpos = tmbpos > (y + h - SB_WIDTH - tmbsize) ? (y + h - SB_WIDTH - tmbsize) : tmbpos;
		tmbpos -= y + SB_WIDTH;

		l->top = (int)((l->numitems - l->height) * (float)tmbpos / (h - tmbsize - 2*SB_WIDTH));
	}

	// draw scrollbar
	if (l->scrollbarAlignment & SB_LEFT){
		w -= (int)(SB_WIDTH / (float)SMALLCHAR_WIDTH);

		if (!(l->scrollbarAlignment & SB_HIDE) || l->numitems > l->height)
			ListBox_DrawScrollBar(l, x);

		hx += SB_WIDTH;
	}
	else if (l->scrollbarAlignment & SB_RIGHT){
		w -= (int)(SB_WIDTH / (float)SMALLCHAR_WIDTH);
		x = l->generic.right - SB_WIDTH;
	...
	for( column = 0; column < l->columns; column++ ) {
		...
		for( i = base; i < base + l->height; i++) {
			...
			if (i == l->curvalue && !(l->generic.flags & QMF_SCROLL_ONLY))
			{
				UI_FillRect(hx, y, hw, SMALLCHAR_HEIGHT, listbar_color);
				color = text_color_highlight;
...
// STONELANCE - menu transitions
	if (menu->transition && menu->changeMenu){
		if (uis.transitionIn){
			if ((uis.realtime - uis.transitionIn) > MENU_TRANSITION_TIME){
				uis.tFrac = 1.0f;
				menu->transition(uis.tFrac);
				uis.transitionIn = 0;
			}
			else {
				uis.tFrac = (uis.realtime - uis.transitionIn) / (float)MENU_TRANSITION_TIME;
				menu->transition(uis.tFrac);
			}
		}
		else if (uis.transitionOut){
			if ((uis.realtime - uis.transitionOut) > MENU_TRANSITION_TIME){
				uis.tFrac = 0.0f;
				menu->transition(uis.tFrac);
				uis.transitionOut = 0;
				if (menu->transitionMenu == DEFAULT_BACK_TRANSITION){
					UI_PopMenu();
				}
				else
					menu->changeMenu( menu->transitionMenu );
				return;
			}
			else {
				uis.tFrac = 1.0 - ((uis.realtime - uis.transitionOut) / (float)MENU_TRANSITION_TIME);
				menu->transition(uis.tFrac);
			}
		}
		else {
...
// FIXME: enable this once all the menu transitions are done
			if (m->transition && uis.menusp > 1 && uis.stack[uis.menusp-2]->transition){
				m->transitionMenu = DEFAULT_BACK_TRANSITION;
...
// STONELANCE
	uis.sbtop			= trap_R_RegisterShaderNoMip( "menu/scrollbar/scrollbart" );
	uis.sbmid			= trap_R_RegisterShaderNoMip( "menu/scrollbar/scrollbarm" );
	uis.sbbot			= trap_R_RegisterShaderNoMip( "menu/scrollbar/scrollbarb" );
	uis.sb_u0			= trap_R_RegisterShaderNoMip( "menu/scrollbar/scrollarrow_u0" );
	uis.sb_u1			= trap_R_RegisterShaderNoMip( "menu/scrollbar/scrollarrow_u1" );
	uis.sb_d0			= trap_R_RegisterShaderNoMip( "menu/scrollbar/scrollarrow_d0" );
	uis.sb_d1			= trap_R_RegisterShaderNoMip( "menu/scrollbar/scrollarrow_d1" );

	uis.port			= trap_R_RegisterShaderNoMip( "menu/art/menu_port" );
	uis.select			= trap_R_RegisterShaderNoMip( "menu/art/menu_select" );
	uis.selected		= trap_R_RegisterShaderNoMip( "menu/art/menu_selected" );
	uis.arrow_l0		= trap_R_RegisterShaderNoMip( "menu/art/arrow_l0" );
	uis.arrow_l1		= trap_R_RegisterShaderNoMip( "menu/art/arrow_l1" );
	uis.arrow_r0		= trap_R_RegisterShaderNoMip( "menu/art/arrow_r0" );
	uis.arrow_r1		= trap_R_RegisterShaderNoMip( "menu/art/arrow_r1" );

	// car model stuff
	uis.turboModel = trap_R_RegisterModel( "models/powerups/turbo/turbocar.md3" );
	uis.headLightGlow = trap_R_RegisterModel( "gfx/flares/h_lite.md3" );
	uis.brakeLightGlow = trap_R_RegisterModel( "gfx/flares/b_lite.md3" );
	uis.reverseLightGlow = trap_R_RegisterModel( "gfx/flares/r_lite.md3" );

  uis.cursorModel		= trap_R_RegisterModel( "menu/models/wrench.md3" );
  uis.cursorShader	= trap_R_RegisterShaderNoMip( "menu/models/wrench" );
  // END
...
//	uis.menuBackNoLogoShader = trap_R_RegisterShaderNoMip( "menubacknologo" );
	...
	if (!uis.menuBackShader){
		int		numItems, backPic;
		char	pics[256][MAX_QPATH];
