...
qboolean	G_TryPushingEntity( gentity_t *check, gentity_t *pusher, vec3_t move, vec3_t amove ) {
	...
	if ( check->client ) {
		...
		// STONELANCE - FIXME:
/*
		Com_Printf("Trying to move player, changing car angles\n");

		check->client->ps.viewangles[YAW] += ANGLE2SHORT(amove[YAW]);
		AnglesToOrientation(check->client->ps.viewangles, check->client->car.sBody.t);
*/
	}
	else {
...
// STONELANCE
/*
		Com_Printf("Trying to move player, changing car velocity\n");

		VectorCopy(pusher->s.pos.trDelta, check->client->car.sBody.v);

		VectorCopy(check->client->ps.origin, check->client->car.sBody.r);
		VectorAdd(check->client->car.sPoints[0].r, move, check->client->car.sPoints[0].r);
		VectorAdd(check->client->car.sPoints[1].r, move, check->client->car.sPoints[1].r);
		VectorAdd(check->client->car.sPoints[2].r, move, check->client->car.sPoints[2].r);
		VectorAdd(check->client->car.sPoints[3].r, move, check->client->car.sPoints[3].r);

		VectorAdd(check->client->car.sPoints[0].r, move2, check->client->car.sPoints[0].r);
		VectorAdd(check->client->car.sPoints[1].r, move2, check->client->car.sPoints[1].r);
		VectorAdd(check->client->car.sPoints[2].r, move2, check->client->car.sPoints[2].r);
		VectorAdd(check->client->car.sPoints[3].r, move2, check->client->car.sPoints[3].r);
*/
//		PM_CalculateSecondaryQuantities(&check->client->car, &check->client->car.sBody, check->client->car.sPoints);
// END
