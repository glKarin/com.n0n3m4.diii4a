...
// STONELANCE - FIXME: temp
	if ( gametype == GT_RACING_DM )
	{
		return qfalse;
	}
// END
...
//		vis = BotEntityVisible(bs->entitynum, bs->eye, bs->viewangles, f, i);
		vis = BotEntityVisible(bs->entitynum, bs->eye, bs->cur_ps.viewangles, f, i);
// END
		if (vis <= 0) continue;
...
//	enemyvisible = BotEntityVisible(bs->entitynum, bs->eye, bs->viewangles, 360, bs->enemy);
	enemyvisible = BotEntityVisible(bs->entitynum, bs->eye, bs->cur_ps.viewangles, 360, bs->enemy);
// END
...
//			if (InFieldOfVision(bs->viewangles, 20, bs->ideal_viewangles)) {
			if (InFieldOfVision(bs->cur_ps.viewangles, 20, bs->ideal_viewangles)) {
// END
				trap_EA_Attack(bs->client);
			}
		}
