...
#define _K_D3_MOD
	...
	if(fsgame.Length())
	{
		...
		common->Printf("[Harmattan]: Load dynamic library %s!\n", Load_Result[gameDLL ? 0 : 1]);
	}
	...
	if(!gameDLL)
	{
		...
		if(fsgame.Length())
		{
...
#endif
			{
				...
				gameDLL = sys->DLL_Load(_ANDROID_DLL_PATH "libgame" _DLL_FPU ".so");
				common->Printf("[Harmattan]: Load dynamic library %s!\n", Load_Result[gameDLL ? 0 : 1]);
			}
	}
	//k
