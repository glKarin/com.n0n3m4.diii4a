...
void sdLODEntity::FreeModelDefs() {
	for ( int i = 0; i < modelDefHandles.Num(); i++ ) {
		...
		if ( modelDefHandle != -1 ) {
			...
			renderEntity_t *re = (renderEntity_t *)gameRenderWorld->GetRenderEntity( modelDefHandle );
#else
...
void sdLODEntity::PostMapSpawn() {
	...
	for ( int i = 0; i < modelID.Num(); i++ ) {
		...
		if ( ID != -1 ) {
#ifdef _ETQW //karin: discard const
			renderEntity_t *re = (renderEntity_t *)gameRenderWorld->GetRenderEntity( modelDefHandles[i] );
#else
