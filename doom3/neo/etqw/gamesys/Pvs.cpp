...
pvsStack_t *idPVS::FloodPassagePVS_r( pvsPortal_t *source, const pvsPortal_t *portal, pvsStack_t *prevStack ) const {
	...
	/* 64long */int *sourceVis, *passageVis, *portalVis, *mightSee, *prevMightSee, more;
	...
	for ( i = 0; i < area->numPortals; i++ ) {
		...
		prevMightSee = reinterpret_cast</* 64long */int *>(prevStack->mightSee);
		passageVis = reinterpret_cast</* 64long */int *>(passage->canSee);
		sourceVis = reinterpret_cast</* 64long */int *>(source->vis);
		mightSee = reinterpret_cast</* 64long */int *>(stack->mightSee);

		more = 0;
		// use the portal PVS if it has been calculated
		if ( p->done ) {
			portalVis = reinterpret_cast</* 64long */int *>(p->vis);
...
int idPVS::AreaPVSFromPortalPVS( void ) const {
	...
	for ( i = 0; i < numAreas; i++ ) {
		...
		for ( j = 1; j < area->numPortals; j++ ) {
			p1 = reinterpret_cast</* 64long */int *>(area->portals[0]->vis);
			p2 = reinterpret_cast</* 64long */int *>(area->portals[j]->vis);
...
void idPVS::Init( void ) {
	...
	areaVisLongs = areaVisBytes/sizeof(/* 64long */int);
	...
	portalVisLongs = portalVisBytes/sizeof(/* 64long */int);
...
pvsHandle_t idPVS::SetupCurrentPVS( const int *sourceAreas, const int numSourceAreas, const pvsType_t type ) const {
	...
	if ( type != PVS_CONNECTED_AREAS ) {
		...
		for ( i = 1; i < numSourceAreas; i++ ) {
			...
			vis = reinterpret_cast</* 64long */int*>(areaPVS + sourceAreas[i] * areaVisBytes);
			pvs = reinterpret_cast</* 64long */int*>(currentPVS[handle.i].pvs);
...
pvsHandle_t idPVS::MergeCurrentPVS( pvsHandle_t pvs1, pvsHandle_t pvs2 ) const {
	int i;
	/* 64long */int *pvs1Ptr, *pvs2Ptr, *ptr;
	pvsHandle_t handle;
	...
	ptr = reinterpret_cast</* 64long */int*>(currentPVS[handle.i].pvs);
	pvs1Ptr = reinterpret_cast</* 64long */int*>(currentPVS[pvs1.i].pvs);
	pvs2Ptr = reinterpret_cast</* 64long */int*>(currentPVS[pvs2.i].pvs);
...
void idPVS::DrawPVS( const idVec3 &source, const pvsType_t type ) const {
	...
	for ( j = 0; j < numAreas; j++ ) {
		...
		for ( i = 0; i < n; i++ ) {
			...
			offset = portal.plane.Normal() * 4.0f;
