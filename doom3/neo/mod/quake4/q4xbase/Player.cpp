...
#ifdef _Q4XBASExxx //karin: create std def if missing
		if ( !decl ) {
			G_CREATE_DEF_T(const idDeclEntityDef, decl, DECL_ENTITYDEF, "player_viewweapon", "def", "def",
				"spawnclass", "rvViewWeapon"
				);
		}
#endif
#ifdef _Q4XBASE //karin: missing std def
		if ( !decl ) {
			gameLocal.Warning( "entityDef not found: player_viewweapon, using built-in" );

			args.Clear();
			args.Set("spawnclass", "rvViewWeapon");

			args.Set( "name", va( "%s_weapon", name.c_str() ) );
			args.SetInt( "instance", instance );
			args.Set( "classname", "player_viewweapon" );
			spawn = NULL;
			gameLocal.SpawnBuiltinEntityDef( args, &spawn );
		}
		else
		{
			args.Set( "name", va( "%s_weapon", name.c_str() ) );
...
#ifdef _Q4XBASExxx //karin: create std def if missing
		if ( !decl ) {
			G_CREATE_DEF_T(const idDeclEntityDef, decl, DECL_ENTITYDEF, "player_animatedentity", "def", "def",
...
#ifdef _Q4XBASE //karin: missing std def
		if ( !decl ) {
			gameLocal.Warning( "entityDef not found: player_animatedentity, using built-in" );

			args.Clear();
			args.Set("spawnclass", "idAnimatedEntity");
			args.Set( "networkSync", "1" );

			args.Set( "name", va( "%s_weapon_world", name.c_str() ) );
			args.SetInt( "instance", instance );
			args.Set( "classname", "player_animatedentity" );
			spawn = NULL;
			gameLocal.SpawnBuiltinEntityDef( args, &spawn );
		}
		else
...
#ifdef _Q4XBASE //karin: create std def if missing
		if ( !model ) {
			const rvDeclPlayerModel *_model;
			G_CREATE_DEF_T(rvDeclPlayerModel, _model, DECL_PLAYER_MODEL, "model_player_marine", "def", "def",
				...
				"team",			"marine",
				"description",		"#str_108003"
			);
			model = const_cast<rvDeclPlayerModel *>(_model);
		}
#endif
		if( !model ) {
