...
const idDecl * idGameLocal::CreateDef(declType_t type, const char *name, const char *parms[], const char *folder, const char *extension) const
{
	const idDecl *decl = declManager->FindType(type, name, false);
	if(decl) {
		return decl;
	}

	gameLocal.Printf("Creating built-in def %s(%d)\n", name, type);

	idDict dict;
	const char **ptr = parms;
	while (*ptr)
	{
		dict.Set(ptr[0], ptr[1]);
		ptr += 2;
	}

	decl = declManager->AddDeclDef(name, type, dict, false, folder, extension);
	if(!decl) {
		gameLocal.Warning("Create def %s(%d) fail", name, type);
		return NULL;
	}

	gameLocal.Printf("Create def %s(%d)\n", name, type);
	return decl;
}

/*
===================
idGameLocal::SpawnBuiltinEntityDef
===================
*/
bool idGameLocal::SpawnBuiltinEntityDef( const idDict &args, idEntity **ent ) {
	const char	*classname;
	const char	*spawn;
	idTypeInfo	*cls;
	idClass		*obj;
