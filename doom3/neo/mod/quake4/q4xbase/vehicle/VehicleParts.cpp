...
void rvVehicleThruster::RunPhysics ( void ) {
	...
	if(harm_g_normalizeMovementDirection.GetBool())
	{
		...
		GAME_SETUPCMDDIRECTION(position->mInputCmd, forward, backward, right, left);
		switch ( key )	{
			case KEY_FORWARD:
				if(forward || backward)
...
CLASS_DECLARATION( rvVehicleWeapon, riVCWMissileTurret )
END_CLASS



class riVCWPulseCannon : public rvVehicleWeapon {
public:

	CLASS_PROTOTYPE( riVCWPulseCannon );
};

CLASS_DECLARATION( rvVehicleWeapon, riVCWPulseCannon )
END_CLASS
// Awakening END
