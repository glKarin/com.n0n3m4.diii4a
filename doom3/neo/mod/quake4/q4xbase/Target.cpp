...
/*
================
idTarget_ObjectiveBeacon::idTarget_ObjectiveBeacon
================
*/
idTarget_ObjectiveBeacon::idTarget_ObjectiveBeacon( void ) {
}

#define OBJECTIVE_BEACON_TEXT GetName() // "Objective Beacon"

void idTarget_ObjectiveBeacon::ShowText(void) const
{
	float scale = 0.25f;
	int distance = -1;
	idPlayer *player = gameLocal.GetLocalPlayer();
	if (player)
	{
		float dist = GetPhysics()->GetOrigin().Dist(player->GetPhysics()->GetOrigin());
		scale = Max(0.25f, dist / 1000.0f);
		distance = idMath::FtoiFast(dist * 0.0254f/* DOOM_TO_METERS, but not defined in q4sdk */);
	}

	const char *loc = OBJECTIVE_BEACON_TEXT;
	idLocationEntity *ent = gameLocal.LocationForPoint(GetPhysics()->GetOrigin());
	if (ent)
	{
		const char *text = ent->GetLocation();
		if (text && text[0])
			loc = text;
	}

	const char *text = va("%s\n%dm", loc, distance);
	gameRenderWorld->DrawText( text, GetPhysics()->GetOrigin(), scale, colorWhite, player ? player->viewAngles.ToMat3() : GetPhysics()->GetAxis().Transpose(), 1 );
}

/*
================
idTarget_ObjectiveBeacon::Think
================
*/
void idTarget_ObjectiveBeacon::Think( void ) {
	if ( thinkFlags & TH_THINK ) {
		ShowText();
	} else {
		BecomeInactive( TH_ALL );
...
================
*/
void idTarget_ObjectiveBeacon::ClientPredictionThink( void ) {
	if ( thinkFlags & TH_THINK ) {
		ShowText();
	} else {
		BecomeInactive( TH_ALL );
