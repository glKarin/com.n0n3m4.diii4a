...
// RAVEN END
	...
	virtual allowReply_t	RepeaterAllowClient( int clientId, int numClients, const char *IP, const char *guid, bool repeater, const char *password, const char *privatePassword, char reason[MAX_STRING_CHARS] ) { idStr::Copynz( reason, "#str_107239" /* zinx - FIXME - not banned... */, /*//k sizeof(reason)*/sizeof(char) * MAX_STRING_CHARS ); return ALLOW_NO; };
...
#endif
#ifdef _Q4XBASE //karin: for missing def
	const idDecl *          CreateDef( declType_t type, const char *name, const char *parms[], const char *folder = NULL, const char *extension = NULL ) const;
	bool					SpawnBuiltinEntityDef( const idDict &args, idEntity **ent );
#endif
...
#ifdef _Q4XBASE //karin: for missing def
#if !defined(_MSC_VER)
#define G_CREATE_DEF(decl, type, name, folder, extension, args...) \
const char *_name_##type##parms[] = { \
NULL, ##args \
, NULL \
}; \
decl = gameLocal.CreateDef(type, name, _name_##type##parms + 1, folder, extension);

#define G_CREATE_DEF_T(T, decl, type, name, folder, extension, args...) \
const char *_name_##type##parms[] = { \
NULL, ##args \
, NULL \
}; \
decl = static_cast<const T *>(gameLocal.CreateDef(type, name, _name_##type##parms + 1, folder, extension));
#else
#define G_CREATE_DEF(decl, type, name, folder, extension, ...) \
const char *_name_##type##parms[] = { \
__VA_ARGS__ \
, NULL \
}; \
decl = gameLocal.CreateDef(type, name, _name_##type##parms, folder, extension);

#define G_CREATE_DEF_T(T, decl, type, name, folder, extension, ...) \
const char *_name_##type##parms[] = { \
__VA_ARGS__ \
, NULL \
}; \
decl = static_cast<const T *>(gameLocal.CreateDef(type, name, _name_##type##parms, folder, extension));
#endif
#endif
