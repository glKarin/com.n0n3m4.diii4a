...
int idAFAttachment::StartLipSyncing( const char *speechDecl ) 
{
	...
	if( speechDecl[0] ) 
	{
		...
		const idSoundShader *voShader = declManager->FindSound( lipSync->GetName() );
		StartSoundShader( voShader, SND_CHANNEL_VOICE, refSound.parms.soundShaderFlags | SSF_IS_VO, false, &length );

		// karin: 汉化项目 —— 语音字幕（文本经 GetTranscribeText 自动本地化，按声音衰减做可听性门控）
		// decl 不存在时 FindLipSync 返回默认空 decl → GetTranscribeText 为空 → 原本静默丢弃无日志
		// (诊断盲区)。此处补 debug 日志，便于定位缺 decl 的 AI 对白（2026-07-31）
		{
			const char *subText = lipSync->GetTranscribeText();
			if ( subText && subText[0] ) {
				gameSubtitles.AddFromEntity( GetBody(), subText, length, voShader );
			} else if ( harm_g_subtitleDebug.GetBool() ) {
				if ( !declManager->FindLipSync( speechDecl, false ) )
					gameLocal.Printf( "[SUB] lipsync: no decl for '%s'\n", speechDecl );
				else
					gameLocal.Printf( "[SUB] lipsync: empty transcribe for '%s'\n", speechDecl );
			}
		}
