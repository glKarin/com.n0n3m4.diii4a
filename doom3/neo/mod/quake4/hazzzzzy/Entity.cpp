...
#ifdef _QUAKE4
// karin: hit \xE6\x97\xA5\xE5\xBF\x97\xE5\xBC\x80\xE5\x85\xB3
static idCVar harm_g_hitLog( "harm_g_hitLog", "0", CVAR_GAME | CVAR_BOOL, "debug: log every entity that takes damage" );
#endif
...
bool idEntity::StartSound( const char *soundName, const s_channelType channel, int soundShaderFlags, bool broadcast, int *length ) {
	...
	int __subLen = 0;
	bool __sndResult = StartSoundShader( shader, channel, soundShaderFlags, broadcast, length ? length : &__subLen );
	int __actualLen = length ? *length : __subLen;

	// karin: \xE5\xAD\x97\xE5\xB9\x95\xE6\x8C\x82\xE9\x92\xA9\xE2\x80\x94\xE2\x80\x94snd_*/lipsync_* \xE9\x94\xAE\xE7\x9A\x84\xE5\xA3\xB0\xE9\x9F\xB3\xEF\xBC\x88\xE5\x8A\xA8\xE7\x94\xBB\xE5\xB8\xA7\xE4\xBA\x8B\xE4\xBB\xB6 + \xE8\x84\x9A\xE6\x9C\xAC startSound\xEF\xBC\x89\xE3\x80\x82
	// AI::Speak \xE8\xB5\xB0 StartSoundShader \xE7\x9B\xB4\xE6\x8E\xA5\xEF\xBC\x88\xE4\xB8\x8D\xE7\xBB\x8F StartSound\xEF\xBC\x89\xEF\xBC\x8C\xE4\xB8\x8D\xE4\xBC\x9A\xE9\x87\x8D\xE5\xA4\x8D\xE3\x80\x82
	if ( __sndResult && __actualLen > 0 && !( soundShaderFlags & SSF_IS_VO ) && shader ) {
		const char *sndName = shader->GetName();
		idStr sndNameStr2 = sndName;
		sndNameStr2.ToLower();
		// \xE8\xB7\xB3\xE8\xBF\x87\xE7\x96\xBC\xE7\x97\x9B/\xE6\xAD\xBB\xE4\xBA\xA1\xE5\xA3\xB0\xE9\x9F\xB3\xEF\xBC\x8C\xE4\xB8\x8D\xE5\x87\xBA\xE5\xAD\x97\xE5\xB9\x95
		if ( sndNameStr2.Find( "pain" ) < 0 && sndNameStr2.Find( "death" ) < 0 ) {
			...
			if ( ls && ls->GetTranscribeText() && ls->GetTranscribeText()[0] ) {
				gameSubtitles.AddFromEntity( this, ls->GetTranscribeText(), __actualLen, shader, NULL );
			}
		}
	}
...
#ifdef _QUAKE4
	// karin: hit \xE6\x97\xA5\xE5\xBF\x97\xEF\xBC\x9A\xE8\xAE\xB0\xE5\xBD\x95\xE5\x8F\x97\xE5\x87\xBB\xE5\xAE\x9E\xE4\xBD\x93\xE7\x9A\x84\xE5\x90\x8D\xE7\xA7\xB0\xE3\x80\x81\xE7\xB1\xBB\xE5\x9E\x8B\xE5\x92\x8C\xE4\xBC\xA4\xE5\xAE\xB3\xE5\xAE\x9A\xE4\xB9\x89
	if ( harm_g_hitLog.GetBool() ) {
		const char *clsName = GetClassname();
		const char *entName = GetName();
		gameLocal.Printf( "[HIT] ent='%s' cls='%s' dmgDef='%s' loc=%d\n",
...
// RAVEN END
	...
	// karin: 能进入此函数的实体都已通过 bleed 检查（会流血的有机体）。idTech4A++ 材质
	// surfacetype 读取异常导致 metal 误判冒火花——强制走 flesh 喷血。
	if ( idStr::Cmp( materialType, "flesh" ) != 0 ) {
		materialType = "flesh";
	}
...
void idEntity::Event_StartSound( const char *soundName, int channel, int netSync ) {
	...
	// karin: 字幕挂钩已移至 idEntity::StartSound（统一覆盖帧事件 + 脚本事件）
...
void idAnimatedEntity::AddDamageEffect( const trace_t &collision, const idVec3 &velocity, const char *damageDefName, idEntity* inflictor ) {
    ...
    if( !collision.c.material )
        return;
	// karin: hit \xE6\x97\xA5\xE5\xBF\x97\xEF\xBC\x9A\xE8\xAE\xB0\xE5\xBD\x95\xE5\x8F\x97\xE5\x87\xBB\xE6\x95\x88\xE6\x9E\x9C\xEF\xBC\x88\xE6\xAD\xA4\xE5\x87\xBD\xE6\x95\xB0\xE5\xA4\x84\xE7\x90\x86\xE5\x96\xB7\xE8\xA1\x80/\xE8\xB4\xB4\xE8\x8A\xB1\xE6\x95\x88\xE6\x9E\x9C\xEF\xBC\x89
	if ( harm_g_hitLog.GetBool() ) {
		int surfType = collision.c.material ? collision.c.material->GetSurfaceType() : -1;
		gameLocal.Printf( "[HIT] AddDamageEffect ent='%s' cls='%s' bleed=%d dmgDef='%s' surfType=%d mat='%s'\n",
			GetName() ? GetName() : "(null)",
			GetClassname() ? GetClassname() : "(null)",
			spawnArgs.GetBool( "bleed" ),
			damageDefName ? damageDefName : "(null)",
			surfType,
			collision.c.material ? collision.c.material->GetName() : "(null)" );
	}
#endif
	const idDeclEntityDef *def = gameLocal.FindEntityDef( damageDefName, false );
	if ( def == NULL || !def->dict.GetBool ( "bleed" ) ) {
...
int	idAnimatedEntity::GetDefaultSurfaceType( void ) const {
	// karin: idActor 子类自行覆盖为 FLESH。此处只处理非角色的 idAnimatedEntity。
	// bleed=true 或 rvHealingStation（生命补给器）→ FLESH（喷血），其余 → METAL（火花）。
	// rvVehicle 继承 idActor 但应喷火花，由 HitScan/Projectile 的 spawnclass 检查排除。
	const char *cls = spawnArgs.GetString( "spawnclass", "" );
	if ( spawnArgs.GetBool( "bleed" ) || idStr::Icmp( cls, "rvHealingStation" ) == 0 ) {
