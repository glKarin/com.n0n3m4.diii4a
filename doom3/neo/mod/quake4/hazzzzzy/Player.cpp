...
// HAZ: gib visual test demo (1=standard 8, 2=medium 10, 3=large 12 big_)
idCVar harm_g_gibTest( "harm_g_gibTest", "0", CVAR_GAME | CVAR_INTEGER, "spawn giblets in front of player to preview gib (1=small,2=medium,3=large)" );
idCVar harm_g_killSentry( "harm_g_killSentry", "0", CVAR_GAME | CVAR_BOOL, "remove all sentries in current map (fix sentry physics stutter)" );
...
static void ClearSaveMessageGui( idUserInterface *gui ) {
	if ( !gui || !gui->GetDesktop() ) {
		return;
	}

	idWinVar *visible = gui->GetDesktop()->GetWinVarByName( "quicksave_msg::visible", true );
	if ( visible ) {
		visible->Set( "0" );
		visible->SetEval( false );
	}
}
...
// \xE5\x87\x86\xE5\xBF\x83\xE5\x91\xBD\xE4\xB8\xAD\xE5\x8F\x98\xE7\xBA\xA2\xEF\xBC\x9A\xE5\x88\xA9\xE7\x94\xA8 cursor.gui weaponChange \xE4\xBA\x8B\xE4\xBB\xB6\xEF\xBC\x88set matcolor = $gui::crossColor\xEF\xBC\x89
// \xE4\xB8\x8D\xE9\x9C\x80\xE8\xA6\x81\xE6\x94\xB9 cursor.gui \xE6\x88\x96\xE7\xBC\x96\xE8\xAF\x91 Quake4.exe\xEF\xBC\x8C\xE7\xBA\xAF q4game.dll \xE5\xB1\x82\xE9\x9D\xA2\xE5\xAE\x9E\xE7\x8E\xB0
static idStr	harm_origCrossColor;
static int		harm_hitFlashTime = 0;
#endif
...
void idPlayer::Restore( idRestoreGame *savefile ) {
	...
	// "游戏已保存"是瞬时提示，不应随 GUI 状态从旧存档恢复；否则它没有
	// 对应的 SaveMessage 计时起点，现有 watchdog 无法将其清除。
	ClearSaveMessageGui( hud );
	ClearSaveMessageGui( objectiveSystem );
...
void idPlayer::DrawShadow( renderEntity_t *headRenderEnt ) {
	if ( gameLocal.isMultiplayer && g_skipPlayerShadowsMP.GetBool() ) {
		...
		renderEntity.suppressShadowInViewID	= entityNumber+1;
 		if ( headRenderEnt ) {
#ifdef _MOD_FULL_BODY_AWARENESS
			if(!harm_pm_fullBodyAwareness.GetBool() || pm_thirdPerson.GetBool() || IsInVehicle() || zoomed || focusUI || !harm_pm_fullBodyAwarenessHeadVisible.GetBool())
#endif
...
void idPlayer::DrawHUD( idUserInterface *_hud ) {
	...
	if ( gameLocal.isMultiplayer ) {
		...
		// karin: cursor.gui 的命中变红条件读 gui::mp，必须也给 cursor 设值（用 int 避免 bool→"false" 字符串不匹配）
		if ( cursor ) {
			cursor->SetStateInt( "mp", 1 );
		}
	} else {
		_hud->SetStateBool( "mp", false );
		if ( cursor ) {
			cursor->SetStateInt( "mp", 0 );
		}
	}

	// karin: 准心命中变红恢复——150ms 后恢复 crossColor 原色
	if ( harm_hitFlashTime > 0 && gameLocal.time - harm_hitFlashTime > 150 ) {
		if ( cursor ) {
			cursor->SetStateString( "crossColor", harm_origCrossColor );
			cursor->HandleNamedEvent( "weaponChange" );
		}
		harm_hitFlashTime = 0;
	}
...
void idPlayer::UpdateFocusCharacter( idEntity* newEnt ) {
	...
	// karin: \xE5\x87\x86\xE5\xBF\x83\xE5\x90\x8D\xE5\xAD\x97\xE4\xBA\xBA\xE5\x90\x8D\xE6\xB1\x89\xE5\x8C\x96 + \xE5\xB0\x8F\xE9\x98\x9F\xE5\x90\x8D\xE6\xB1\x89\xE5\x8C\x96
	{
		idStr npcName( common->GetLocalizedString(newEnt->spawnArgs.GetString( "npc_name", "Joe" )) );
		rvSubtitles::LocalizeText( npcName );
		cursor->SetStateString( "npc", npcName.c_str() );
		idStr npcDesc( common->GetLocalizedString(newEnt->spawnArgs.GetString( "npc_description", "" )) );
		rvSubtitles::LocalizeText( npcDesc );
		cursor->SetStateString( "npcdesc", npcDesc.c_str() );
	}
...
void idPlayer::Think( void ) {
	// HAZ: gib test demo — spawn giblets in front to preview the visual effect
	if ( harm_g_gibTest.GetInteger() > 0 ) {
		int tier = harm_g_gibTest.GetInteger();
		harm_g_gibTest.SetInteger( 0 );
		static const char *stdGibs[] = {
			"moveable_gib_head_pork","moveable_gib_torso_pork","moveable_gib_rup_arm_pork",
			"moveable_gib_left_waist_pork","moveable_gib_rup_leg_pork","moveable_gib_lup_leg_pork",
			"moveable_gib_pelvis_pork","moveable_gib_skull","moveable_gib_rup2_leg_pork",
			"moveable_gib_rup_arm_pork"
		};
		static const char *bigGibs[] = {
			"moveable_gib_big_head_pork","moveable_gib_big_torso_pork","moveable_gib_big_rup_arm_pork",
			"moveable_gib_big_left_waist_pork","moveable_gib_big_rup_leg_pork","moveable_gib_big_lup_leg_pork",
			"moveable_gib_big_rup2_leg_pork","moveable_gib_big_pelvis_pork","moveable_gib_big_torso_pork",
			"moveable_gib_big_rup_arm_pork","moveable_gib_big_left_waist_pork","moveable_gib_skull"
		};
		const char **gibs = ( tier >= 3 ) ? bigGibs : stdGibs;
		int count = ( tier >= 3 ) ? 12 : ( ( tier == 2 ) ? 10 : 8 );
		idVec3 fwd = viewAngles.ToForward();
		idVec3 center = GetPhysics()->GetOrigin() + fwd * 256.0f + idVec3( 0, 0, 32 );
		for ( int i = 0; i < count; i++ ) {
			idDict args;
			args.Set( "classname", gibs[i] );
			args.Set( "origin", center.ToString() );
			idEntity *g = NULL;
			if ( gameLocal.SpawnEntityDef( args, &g ) && g && g->GetPhysics() ) {
				float ang = i * ( 2.0f * 3.14159265f / count );
				idVec3 vel( idMath::Cos( ang ), idMath::Sin( ang ), 0.0f );
				vel *= 300.0f;
				vel.z += 150.0f;
				g->GetPhysics()->SetLinearVelocity( vel );
				g->PostEventSec( &EV_Remove, 3.0f );
			}
		}
	}
	// HAZ: kill all sentries (fix sentry physics stutter)
	if ( harm_g_killSentry.GetBool() ) {
		harm_g_killSentry.SetBool( false );
		int killed = 0;
		for ( int i = 0; i < gameLocal.num_entities; i++ ) {
			idEntity *ent = gameLocal.entities[i];
			if ( ent && ent->IsType( idAI::GetClassType() ) ) {
				if ( idStr::Icmp( ent->spawnArgs.GetString( "spawnclass", "" ), "rvMonsterSentry" ) == 0 ) {
					ent->PostEventMS( &EV_Remove, 0 );
					killed++;
				}
			}
		}
		gameLocal.Printf( "[KILLSENTRY] removed %d sentries\n", killed );
	}
	SaveMessageWatchdog();
#ifdef _MOD_FULL_BODY_AWARENESS
	if(!harm_pm_fullBodyAwareness.GetBool() || pm_thirdPerson.GetBool() || IsInVehicle() || zoomed || focusUI)
...
#undef _HARM_Q4_PLAYERMODEL_HEAD_JOINT
		...
		trace_t trace;
		gameLocal.TraceBounds(this, trace, firstPersonViewOrigin_orig, firstPersonViewOrigin, bounds, MASK_SOLID, this);
...
void idPlayer::SetLastHitTime( int time, bool armorHit ) {
	...
	if ( lastHitTime != time ) {
		if ( cursor ) {
			// karin: 准心命中变红——设 crossColor=红色，触发 weaponChange（设 matcolor=红色），再触发 weaponHit（缩放）
			// 150ms 后在 DrawHUD 中恢复 crossColor 原色并触发 weaponChange
			if ( harm_hitFlashTime == 0 ) {
				harm_origCrossColor = cursor->GetStateString( "crossColor" );
			}
			cursor->SetStateString( "crossColor", "1, 0, 0, 1" );
			cursor->HandleNamedEvent( "weaponChange" );
			cursor->HandleNamedEvent( "weaponHit" );
			harm_hitFlashTime = gameLocal.time;
		}
		if ( gameLocal.isMultiplayer ) {			
...
=====================
*/
// 2026-07-18 用户反馈"游戏已保存有时不消失"：hud.gui 里 onNamedEvent saveMessage
// 设 quicksave_msg::visible=1 + onTime 3000 ms 自清；但游戏 pause/loading 期间
// gui 时间线可能被跳过 → onTime 不触发 → 提示卡住。DLL 侧记录最近触发时刻，
// Think 里 4 秒后强制清 gui state 兜底。
static int g_saveMessageStartMs = 0;
...
void idPlayer::SaveMessage( void ) {
	...
	g_saveMessageStartMs = gameLocal.time;
#endif
}

// 强制清 quicksave_msg 兜底（Think 每帧调用）
void idPlayer::SaveMessageWatchdog( void ) {
#ifndef _XENON
	if ( g_saveMessageStartMs && gameLocal.time - g_saveMessageStartMs > 4000 ) {
		ClearSaveMessageGui( GetHud() );
		ClearSaveMessageGui( objectiveSystem );
		g_saveMessageStartMs = 0;
	}
#endif
}
