...
void rvModviewModel::Event_Speak ( const char* lipsync ) {
	...
	if ( head ) {
		...
		int length = 0;
		StartSoundShader (declManager->FindSound ( lipsync ), SND_CHANNEL_VOICE, 0, false, &length );
		const rvDeclLipSync *ls = declManager->FindLipSync( lipsync, false );
		if ( ls ) {
			const char *subText = ls->GetTranscribeText();
			if ( subText && subText[0] ) {
				gameSubtitles.AddFromEntity( this, subText, length );
			} else if ( harm_g_subtitleDebug.GetBool() ) {
				gameLocal.Printf( "[SUB] modmodel: empty transcribe for '%s'\n", lipsync );
			}
		} else if ( harm_g_subtitleDebug.GetBool() ) {
			gameLocal.Printf( "[SUB] modmodel: no lipsync decl for '%s'\n", lipsync );
		}
	}
}
