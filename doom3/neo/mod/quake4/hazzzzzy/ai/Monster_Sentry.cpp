...
void rvMonsterSentry::Think ( void ) {
	// idTech4A++ 物理保护：Sentry 飞行速度在引擎物理积分中可能溢出，
	// 触发 TestHugeTranslation 后卡在异常位置→碰撞/寻路计算爆炸→3FPS。
	// 钳制速度 + 位置保护（离出生点太远则重置）。
	idVec3 sentryVel = physicsObj.GetLinearVelocity();
	float sentryMaxSpeed = Max( move.fly_speed * 3.0f, 500.0f );
	float sentrySpeedSqr = sentryVel.LengthSqr();
	if ( sentrySpeedSqr > Square( sentryMaxSpeed ) ) {
		sentryVel *= sentryMaxSpeed * idMath::InvSqrt( sentrySpeedSqr );
		physicsObj.SetLinearVelocity( sentryVel );
	}
	// 位置保护：TestHugeTranslation 残留导致 Sentry 卡在异常位置
	idVec3 sentryOrg = physicsObj.GetOrigin();
	idVec3 sentrySpawnOrg;
	if ( spawnArgs.GetVector( "origin", "0 0 0", sentrySpawnOrg ) ) {
		if ( ( sentryOrg - sentrySpawnOrg ).LengthSqr() > Square( 5000.0f ) ) {
			physicsObj.SetOrigin( sentrySpawnOrg );
