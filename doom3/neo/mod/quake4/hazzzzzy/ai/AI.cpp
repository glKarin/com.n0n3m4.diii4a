...
// HAZ: auto-gib enemies on death (skips boss to preserve death animation, skips friendly marines)
idCVar harm_g_gibOnDeath( "harm_g_gibOnDeath", "0", CVAR_GAME | CVAR_BOOL | CVAR_ARCHIVE, "auto-gib Strogg enemies on death (skip boss and friendly)" );
...
void idAI::Killed( idEntity *inflictor, idEntity *attacker, int damage, const idVec3 &dir, int location ) {
	...
	// HAZ: auto-gib on death — weapon-gated, size-scaled, bio Strogg only.
	if ( harm_g_gibOnDeath.GetBool() && team == AITEAM_STROGG ) {
		const char *gibcls = spawnArgs.GetString( "spawnclass", "" );
		// exclude: bosses (preserve scripted death) + mechanical/flyer/device + splitting monster
		static const char *noGibClass[] = {
			"rvMonsterBossMakron", "rvMonsterNetworkGuardian",
			"rvMonsterHarvester", "rvMonsterHeavyHoverTank",
			"rvMonsterStreamProtector", "rvMonsterFailedTransfer",
			"rvMonsterRepairBot", "rvMonsterStroggFlyer",
			"rvMonsterStroggHover", "rvMonsterTeleportDropper",
			"rvMonsterTurret", "rvMonsterConvoyGround",
			"rvMonsterConvoyHover", NULL
		};
		bool skipGib = false;
		for ( int i = 0; noGibClass[i]; i++ ) {
			if ( idStr::Icmp( gibcls, noGibClass[i] ) == 0 ) { skipGib = true; break; }
		}
		if ( !skipGib ) {
			// weapon whitelist: only shotgun/grenade/nailgun/railgun/rocket/lightning gib
			const char *dmg = spawnArgs.GetString( "harm_lastDmgDef", "" );
			bool canGibWpn = ( idStr::Icmpn( dmg, "damage_pellet", 13 ) == 0
				|| idStr::Icmpn( dmg, "damage_grenade", 14 ) == 0
				|| idStr::Icmpn( dmg, "damage_nail", 11 ) == 0
				|| idStr::Icmpn( dmg, "damage_railgun", 14 ) == 0
				|| idStr::Icmpn( dmg, "damage_rocket", 13 ) == 0
				|| idStr::Icmpn( dmg, "damage_lightninggun", 19 ) == 0 );
			if ( canGibWpn ) {
				// size tier by def health (unaffected by difficulty — skill only scales AI aggression)
				int mh = spawnArgs.GetInt( "health", "100" );
				bool large = ( mh > 500 );
				bool medium = ( mh > 200 );
				if ( !spawnArgs.GetString( "def_dropGibItem1", "" )[0] ) {
					spawnArgs.Set( "gib", "1" );
					spawnArgs.Set( "fx_gib", "effects/monsters/gib.fx" );
					if ( large ) {
						// 15 chunks (heavy: recognizable parts — head/torso/skull/brain/leg/pelvis + repeats)
						spawnArgs.Set( "def_dropGibItem1", "moveable_gib_head_pork" );
						spawnArgs.Set( "def_dropGibItem2", "moveable_gib_torso_pork" );
						spawnArgs.Set( "def_dropGibItem3", "moveable_gib_skull" );
						spawnArgs.Set( "def_dropGibItem4", "moveable_gib_brain1" );
						spawnArgs.Set( "def_dropGibItem5", "moveable_gib_brain2" );
						spawnArgs.Set( "def_dropGibItem6", "moveable_gib_lup_leg_pork" );
						spawnArgs.Set( "def_dropGibItem7", "moveable_gib_pelvis_pork" );
						spawnArgs.Set( "def_dropGibItem8", "moveable_gib_rup_arm_pork" );
						spawnArgs.Set( "def_dropGibItem9", "moveable_gib_rup_arm_pork" );
						spawnArgs.Set( "def_dropGibItem10", "moveable_gib_left_waist_pork" );
						spawnArgs.Set( "def_dropGibItem11", "moveable_gib_torso_pork" );
						spawnArgs.Set( "def_dropGibItem12", "moveable_gib_head_pork" );
						spawnArgs.Set( "def_dropGibItem13", "moveable_gib_brain1" );
						spawnArgs.Set( "def_dropGibItem14", "moveable_gib_torso_pork" );
						spawnArgs.Set( "def_dropGibItem15", "moveable_gib_skull" );
					} else if ( medium ) {
						// 12 chunks (elite: recognizable base + repeats)
						spawnArgs.Set( "def_dropGibItem1", "moveable_gib_head_pork" );
						spawnArgs.Set( "def_dropGibItem2", "moveable_gib_torso_pork" );
						spawnArgs.Set( "def_dropGibItem3", "moveable_gib_skull" );
						spawnArgs.Set( "def_dropGibItem4", "moveable_gib_brain1" );
						...
						// 10 chunks (small: head/torso/skull/brain/leg/pelvis + 2 arms + flesh)
						spawnArgs.Set( "def_dropGibItem1", "moveable_gib_head_pork" );
						spawnArgs.Set( "def_dropGibItem2", "moveable_gib_torso_pork" );
						spawnArgs.Set( "def_dropGibItem3", "moveable_gib_skull" );
						spawnArgs.Set( "def_dropGibItem4", "moveable_gib_brain1" );
				...
				// defer to next frame — spawning inside Killed chain stalls scripted sequences
				spawnArgs.Set( "harm_gibDir", dir.ToString() );
				PostEventMS( &EV_Gib, 0, "damage_gib" );
			}
		}
	}
}
...
bool idAI::SetEnemy( idEntity *newEnemy ) {
	...
	// karin: AI 首次发现敌人时为 alert 声音添加字幕（snd_alert -> lipsync decl -> 字幕）
	if ( !oldEnemy && newEnemy && enemy.ent == newEnemy ) {
		const char *alertSnd = spawnArgs.GetString( "snd_alert", "" );
		if ( alertSnd && *alertSnd ) {
			const rvDeclLipSync *ls = declManager->FindLipSync( alertSnd, false );
			if ( ls && ls->GetTranscribeText() && ls->GetTranscribeText()[0] ) {
				// 从 spawnclass 推断怪物中文名（步兵/机炮兵等），默认"敌军"
				const char *alertSpeaker = "\xE6\x95\x8C\xE5\x86\x9B";
				const char *alertCls = spawnArgs.GetString( "spawnclass", "" );
				static const struct { const char *cls; const char *cn; } alertMonMap[] = {
					{ "rvMonsterGrunt", "\xE6\xAD\xA5\xE5\x85\xB5" },
					{ "rvMonsterGunner", "\xE6\x9C\xBA\xE7\x82\xAE\xE5\x85\xB5" },
					{ "rvMonsterBerserker", "\xE7\x8B\x82\xE6\x88\x9C\xE5\xA3\xAB" },
				...
				for ( int ami = 0; alertMonMap[ami].cls; ami++ ) {
					if ( idStr::Icmp( alertCls, alertMonMap[ami].cls ) == 0 ) {
						alertSpeaker = alertMonMap[ami].cn;
						break;
					}
				}
				if ( !( gameLocal.inCinematic && !cinematic ) ) {
					gameSubtitles.Add( alertSpeaker, ls->GetTranscribeText(), 3000, SUB_COLOR_ENEMY );
				}
			} else if ( harm_g_subtitleDebug.GetBool() ) {
				gameLocal.Printf( "[SUB] alert: no lipsync decl for '%s'\n", alertSnd );
			}
		}
	}
