...
void idProjectile::DefaultDamageEffect( const trace_t &tr, const idVec3 &velocity, const char *damageDefName ) {
	...
	// karin: idActor/idAFAttachment 命中强制走 flesh 效果（喷血），不读 tr.c.materialType
	...
	bool harm_isActor = harm_isOrganic || (!harm_isVehicle && ent->IsType( idActor::GetClassType() )) || ent->IsType( idAFAttachment::GetClassType() ) || (!harm_isVehicle && ent->IsType( idAnimatedEntity::GetClassType() ));
	if ( harm_isActor ) {
		harm_matType = declManager->FindMaterialType( "flesh" );
	} else if ( ent->IsType( idAnimatedEntity::GetClassType() ) ) {
		harm_matType = declManager->FindMaterialType( "metal" );
	}
	...
	if ( harm_isActor ) {
		...
		const idDecl *actorImpactEffect = gameLocal.GetEffect( spawnArgs, "fx_impact_actor", harm_matType );
	...
	if ( ent->IsType( idMover::GetClassType ( ) ) ) {
		ent->PlayEffect( gameLocal.GetEffect( spawnArgs, "fx_impact", harm_matType ), tr.c.point, axis, false, vec3_origin, true );
	} else {
		gameLocal.PlayEffect( gameLocal.GetEffect( spawnArgs, "fx_impact", harm_matType ), tr.c.point, axis, false, vec3_origin, true );
	}
}
