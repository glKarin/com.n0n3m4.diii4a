...
ID_INLINE bool TestHugeTranslation( trace_t &results, const idClipModel *mdl, const idVec3 &start, const idVec3 &end, const idMat3 &trmAxis ) {
	if ( mdl != NULL && ( end - start ).LengthSqr() > Square( CM_MAX_TRACE_DIST ) ) {
		...
		// karin: hub1/hub2 关卡 rvMonsterSentry 物理故障（坐标/速度溢出）。
		// 清零速度 + 禁用碰撞。卡顿不在 RunFrame 内（0.1ms），而在引擎主循环
		// 的物理世界更新环节（RunFrame 之后）——对异常碰撞模型做海量碰撞检测。
		// SetContents(0) 让物理世界更新永久跳过此实体 → 不卡。
		// 代价：触发此保护的实体穿墙/无法被击中，但物理状态已异常本就不正常。
		idEntity *ent = mdl->GetEntity();
		if ( ent ) {
			static int hugeTranslationWarnCount = 0;
			if ( hugeTranslationWarnCount < 5 ) {
				hugeTranslationWarnCount++;
				gameLocal.Printf( "huge translation for clip model %d on entity %d '%s'\n", mdl->GetId(), ent->entityNumber, ent->GetName() );
			}
			ent->GetPhysics()->SetLinearVelocity( vec3_zero );
