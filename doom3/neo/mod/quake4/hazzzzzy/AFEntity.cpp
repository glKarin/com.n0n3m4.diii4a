...
// RAVEN END
	...
	for ( i = 0; i < list.Num(); i++ ) {
		if ( gibNonSolid ) {
			...
			// HAZ: directional gib spray — chunks fly toward enemy back (bullets)
			// or blast tail (explosives), with upward arc so they launch not roll
			idVec3 useDir = dir;
			spawnArgs.GetVector( "harm_gibDir", "", useDir );
			velocity = list[i]->GetPhysics()->GetAbsBounds().GetCenter() - entityCenter;
			velocity.NormalizeFast();
			if ( useDir.LengthSqr() > 0.001f ) {
				useDir.NormalizeFast();
				velocity = velocity * 0.3f + useDir * 0.9f;
				velocity.z += 0.6f;		// upward arc so chunks fly up and out
			} else {
				velocity.z += 0.5f;
			}
			velocity.NormalizeFast();
			list[i]->GetPhysics()->SetLinearVelocity( velocity * 380.0f );
		}
