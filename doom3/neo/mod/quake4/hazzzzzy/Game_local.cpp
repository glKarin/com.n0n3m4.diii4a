...
void idGameLocal::HandleMainMenuCommands( const char *menuCommand, idUserInterface *gui ) {
	// karin: 设置页分辨率/比例 onActionRelease 触发。session 的 HandleMainMenuCommands
	// 不转发未知命令给 cmdSystem，但每个菜单命令都会调 game->HandleMainMenuCommands。
	// 在此识别 harm_applyVideo，直接调用解析函数（不经控制台命令：主菜单时 Clear
	// 未执行、命令未注册）。解析 harm_g_resIndex → r_mode -1+custom+vid_restart。
	if ( !idStr::Icmp( menuCommand, "harm_applyVideo" ) ) {
...
// RAVEN END
		...
		if ( !reflect || ent->fl.takedamage ) {
			...
			// karin: idActor 命中强制走默认 flesh 效果（血），不读 tr.c.materialType
			// （idTech4A++ 材质 type 读取异常，Strogg 模型被误判 metal 冒火花）
			const rvDeclMatType *harm_matType = tr.c.materialType;
			bool harm_isVehicle = ( idStr::Icmpn( ent->spawnArgs.GetString( "spawnclass", "" ), "rvVehicle", 8 ) == 0 );
			bool harm_isOrganic = ent->spawnArgs.GetBool("bleed");
			if ( harm_isOrganic || (ent->IsType ( idActor::GetClassType() ) && !harm_isVehicle) ) {
				harm_matType = declManager->FindMaterialType( "flesh" );
			} else if ( ent->IsType ( idAnimatedEntity::GetClassType() ) && !harm_isVehicle ) {
				harm_matType = declManager->FindMaterialType( "flesh" );
			}
			...
			if ( !g_perfTest_weaponNoFX.GetBool() ) {
				if ( ent->CanPlayImpactEffect( owner, ent ) ) {
					if ( ent->IsType( idMover::GetClassType( ) ) ) {
						ent->PlayEffect( GetEffect( hitscanDict, "fx_impact", harm_matType ), collisionPoint, axis, false, vec3_origin, false, EC_IMPACT, hitscanTint );
					} else {
						gameLocal.PlayEffect( GetEffect( hitscanDict, "fx_impact", harm_matType ), collisionPoint, axis, false, vec3_origin, false, false, EC_IMPACT, hitscanTint );
					}
				}
			}
