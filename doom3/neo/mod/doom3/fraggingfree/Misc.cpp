...
//ff1.4 start - added for LM
/*
===============================================================================
idFuncShootProjectile
===============================================================================
*/
const idEventDef EV_ShootProjectile( "<shootProjectile>" );

CLASS_DECLARATION( idStaticEntity, idFuncShootProjectile )
	EVENT( EV_Activate,				idFuncShootProjectile::Event_Activate )
	EVENT( EV_ShootProjectile,		idFuncShootProjectile::Event_ShootProjectile )
	END_CLASS
...
void idFuncShootProjectile::Spawn() {
	respawnDelayMs = spawnArgs.GetInt( "spawn_delay_ms" );
	launchPower = spawnArgs.GetFloat( "launch_power", "1" );
	projDefName = spawnArgs.GetString( "def_projectile" );
	active = false;
}

/*
===============
idFuncShootProjectile::Event_ShootProjectile
===============
*/
void idFuncShootProjectile::Event_ShootProjectile() {
	idVec3			dir;
	idVec3			firePos;
	idEntity		*target;
	...
	target = targets[0].GetEntity();
	if ( target == NULL ) {
		return;
	}
	
	firePos = GetPhysics()->GetOrigin();
	dir = SysGetAimDir( firePos, target );

	if ( SysFireProjectile( projDefName , firePos, dir, launchPower ) != NULL ) {
		PostEventMS( &EV_ShootProjectile, respawnDelayMs );
	}
}
...
void idFuncShootProjectile::Save( idSaveGame *savefile ) const {
	savefile->WriteInt( respawnDelayMs );
	savefile->WriteFloat( launchPower );
	savefile->WriteString( projDefName );
	savefile->WriteBool( active );
}
...
void idFuncShootProjectile::Restore( idRestoreGame *savefile ) {
	savefile->ReadInt( respawnDelayMs );
	savefile->ReadFloat( launchPower );
	savefile->ReadString( projDefName );
	savefile->ReadBool( active );
}
...
void idFuncShootProjectile::Event_Activate( idEntity *activator ) {
	active = !active;
	if ( active ) {
		ProcessEvent( &EV_ShootProjectile );
	} else {
		CancelEvents( &EV_ShootProjectile );
	}
}
