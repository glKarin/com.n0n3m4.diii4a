...
void idMoveable::Save( idSaveGame *savefile ) const {
	...
	attacker.Save( savefile ); //ff1.4 - was: savefile->WriteObject( attacker );
#endif
...
void idMoveable::Restore( idRestoreGame *savefile ) {
	...
	attacker.Restore( savefile ); //was: savefile->ReadObject( reinterpret_cast<idClass *&>( attacker ) );
#endif
...
bool idMoveable::Collide( const trace_t &collision, const idVec3 &velocity ) {
	...
	if ( !gameLocal.isClient && canDamage && gameLocal.time > nextDamageTime ) {
		...
		if ( hasDamage || hasMonsterDamage ) {
			attackerEnt = attacker.GetEntity(); //ff1.4
			...
			if ( ent && v > minDamageVelocity ) {
				...
				if ( ent->IsType( idAI::Type ) && hasMonsterDamage ) {
#ifdef _D3XP
					if ( attackerEnt ) {
						ent->Damage( this, attackerEnt, dir, monsterDamage, f, INVALID_JOINT );
					}
					else {
					...
					if ( attackerEnt ) {
						ent->Damage( this, attackerEnt, dir, damage, f, INVALID_JOINT );
					}
					else {
...
void idExplodingBarrel::Killed( idEntity *inflictor, idEntity *attacker, int damage, const idVec3 &dir, int location ) {
	...
	if ( f > 0.0f && state == NORMAL ) {
		state = BURNING;
		SetAttacker( attacker ); //ff1.4 - store attacker for later
...
void idExplodingBarrel::Event_Explode() {
	if ( state == NORMAL || state == BURNING ) {
		state = BURNEXPIRED;
		Killed( NULL, attacker.GetEntity(), 0, vec3_zero, 0 ); //ff1.4 use attacker set when the exploding event fired
	}
}
