...
void idProjectile::Spawn( void ) {
	...
	mNoExplodeDisappear = spawnArgs.GetBool( "no_explode_disappear", mNoExplodeDisappear ? "1" : "0" ); //added for LM
	mTouchTriggers = spawnArgs.GetBool( "touch_triggers", mTouchTriggers ? "1" : "0" ); //added for LM
...
bool idProjectile::Collide( const trace_t &collision, const idVec3 &velocity ) {
	...
	if ( !gameLocal.mpGame.IsGametypeCoopBased() && gameLocal.isMultiplayer && ent->IsType( idDoor::Type ) && !static_cast< idDoor * >(ent)->IsOpen() && !ent->spawnArgs.GetBool( "no_touch" ) ) {
...
void idProjectile::AddDefaultDamageEffect( const trace_t &collision, const idVec3 &velocity ) {
	...
	if ( gameLocal.isServer && ((fl.networkSync && !gameLocal.mpGame.IsGametypeCoopBased()) || (fl.coopNetworkSync && gameLocal.mpGame.IsGametypeCoopBased())) ) {
		idBitMsg	msg;
