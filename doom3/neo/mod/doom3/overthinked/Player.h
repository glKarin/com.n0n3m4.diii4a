...
//	void					AddPickupName( const char *name, const char *icon );
    ...
    bool					UseAmmo( ammo_t type, int amount, bool add ); //clear up messy ammo count for reusable clips, 0x29a
...
public:
    ...
    **/
    ...
    int                     acidPool;           // amount of acid that takes health over time
    int                     pendingHealth; // medkits will now have latency, 0x29a
    int                     nextHealingTime;
    int                     nextOverdoseTime;
    int                     nextBleedTime; // Marine will bleed out to death, too
    float                   nextAcidTime; // Acid will burn over time
...
// sikk---> Crosshair Positioning
    ...
    //int                     GetCurrentWeapon2( void ) { return currentWeapon; };
...
private:
    ...
    void					WeaponMeleeFunction( bool keyTapped ); // New
