...
#endif
	FinishSurfaces();
#ifdef _SPLASHDAMAGExxx //karin: only create it when collision model loading if have not surfaces
	else
	{
		srfTriangles_t *tri = R_AllocStaticTriSurf();
		...
		surf.id = 0;
		surf.geometry = tri;
		surf.material = declManager->FindMaterial(CM_DEFAULT_COLLISION_SHADER);
		AddSurface(surf);
	}
#endif
}
