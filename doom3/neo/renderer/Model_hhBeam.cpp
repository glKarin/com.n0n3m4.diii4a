...
//			surf.shader = tr.defaultMaterial;
	...
	for(int i = 0; i < declBeam->numBeams; i++)
	{
		UpdateSurface(renderEntity, i, renderEntity->beamNodes, staticModel->Surface(i));
		id++;
	}
	for(int i = 0; i < declBeam->numBeams; i++)
	{
		for(int m = 0; m < 2; m++)
		{
			if(!declBeam->quadShader[i][m])
				continue;
			UpdateQuadSurface(renderEntity, i, m, renderEntity->beamNodes, staticModel->Surface(id++));
		}
	}
