...
void ImGui_ImplSDLGPU3_DestroyDeviceObjects()
{
    ...
    if (bd->VertexShader)
    {
        SDL_ReleaseGPUShader(v->GpuDevice, bd->VertexShader);
        bd->VertexShader = nullptr;
    }
    if (bd->FragmentShader)
    {
        SDL_ReleaseGPUShader(v->GpuDevice, bd->FragmentShader);
    ...
    if (bd->Pipeline)
    {
        SDL_ReleaseGPUGraphicsPipeline(v->GpuDevice, bd->Pipeline);
        bd->Pipeline = nullptr;
    }
}
