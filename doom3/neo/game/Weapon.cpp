...
void idWeapon::TestWorldWeaponTransform(const idCmdArgs &args)
{
	idPlayer	*player;
	if(args.Argc() < 2)
	{
	...
	idEntity *ent = weapon->worldModel.GetEntity();
	if(!ent)
		return;
...
void idWeapon::InitWorldModel(const idDeclEntityDef *def)
{
    ...
    {
        cmdSystem->AddCommand("testWeaponWorld", idWeapon::TestWorldWeaponTransform, CMD_FL_GAME, "");
