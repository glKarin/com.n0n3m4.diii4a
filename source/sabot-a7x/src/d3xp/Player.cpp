...
// cusTom3 - moved this #endif up one line (and brace) - oops!
#endif
	} 
}
...
#ifdef _D3XP // cusTom3 - xp change not wrapped and won't build without define
...
idPlayer::~idPlayer() {
	...
	weapon = NULL;
// TinMan: Split off _D3XP, was mixed in with ctf
#ifdef _D3XP
	if ( enviroSuitLight.IsValid() ) {
...
#endif // cusTom3 - inserted to remove else from #ifdef - needs to be else either way
	} else {
#ifdef _D3XP // cusTom3 - restart #ifdef after the else
...
#ifdef _D3XP // cusTom3 - xp changed unwrapped
	if ( ( ammoavailable != -1 ) && ( ammoavailable < 0 ) ) {
#else  // TODO: cusTom3 - not sure if this is a bug fix change, more likely xp changed ammo handling and didn't need to subtract inclip
...
#ifdef _D3XP // cusTom3 - wrapped this function definition in #ifdef seeing as how its declaration was :)
...
#ifdef _D3XP // cusTom3 - wrapped this case in #ifdef - enviroSuitLight undefined without _D3XP :)
		case IMPULSE_25: {
...
void idPlayer::EvaluateControls( void ) {
	...
	// id: in MP, idMultiplayerGame decides spawns
	if ( forceRespawn && !gameLocal.isMultiplayer && !g_testDeath.GetBool() ) {
		// id: in single player, we let the session handle restarting the level or loading a game
...
void idPlayer::UpdateHud( void ) {
	...
	if ( c > 0 ) {
		if ( gameLocal.time > inventory.nextItemPickup ) {
			...
			int i;
// cusTom3 - count is replacing harcoded magic number 5 in original - moved definition out of #ifdef
			int count = 5; 
#ifdef _D3XP
			if(gameLocal.isMultiplayer) {
				count = 3;
...
#endif
			for ( i = 0; i < count, i < c; i++ ) { //_D3XP - cusTom3 - now works without _D3XP
				...
				} else 	if ( inventory.nextItemNum > count ) { //_D3XP - cusTom3 - now works without _D3XP
					inventory.nextItemNum = 1;
...
// cusTom3 - this was in original as well - removing #ifdef _D3XP
