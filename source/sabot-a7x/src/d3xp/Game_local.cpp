...
#ifndef MOD_BOTS // cusTom3 - aas extensions - moved to where used below, then moved out for MOD_BOTS
	int i;
#endif
...
void idGameLocal::LocalMapRestart( ) {
	...
	#else // cusTom3 - original version below
	for ( i = 0; i < MAX_CLIENTS; i++ ) {
		if ( entities[ i ] && entities[ i ]->IsType( idPlayer::Type ) ) {
			static_cast< idPlayer * >( entities[ i ] )->PrepareForRestart();
		}
...
#ifdef _D3XP // cusTom3 - .slow.time again ;(
...
// TODO: cusTom3 - mucho different then Game version? leaving for now
	idPlayer *   player;
