...
void idMultiplayerGame::UpdateScoreboard( idUserInterface *scoreBoard, idPlayer *player ) {
	int i, j, iline, k;
	idStr gameinfo;
// #ifdef _D3XP - cusTom3 removed, needed to build
	idStr livesinfo;
	idStr timeinfo;
// #endif - cusTom3
...
// TODO: cusTom3 - removed the #ifdef _D3XP find out if it was a fix - else it was in original (some more to follow below)
...
// #ifdef _D3XP - removed by cusTom3
			needRestart = gameLocal.NeedRestart();
			cmdSystem->BufferCommandText( CMD_EXEC_NOW, "rescanSI" );
			if ( needRestart ) {
				cmdSystem->BufferCommandText( CMD_EXEC_APPEND, "nextMap" );
...
void idMultiplayerGame::UpdateMainGui( void ) {
	...
	if ( IsGametypeTeamBased() ) {
		...
		// TODO: cusTom3 - looks like a fix was added here - check for null pointer not in original, leaving for now
		if ( p ) {
...
#ifdef CTF // cusTom3 - assert with GAME_COUNT if CTF, else use old assert
			assert( vote_gameTypeIndex > 0 && vote_gameTypeIndex < GAME_COUNT );
#else
			assert( vote_gameTypeIndex >= 0 && vote_gameTypeIndex <= 3 );
#endif // cusTom3 - this strcpy looks like optimization for commented out switch, removed from #ifdef CTF so used always
...
/*
	...
	CTF code.
	cusTom3 says thanks for the comment - and he will not substantially clutter the code for these like 
	he did for many other changes which didn't build 
*/
