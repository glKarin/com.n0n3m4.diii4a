/*  This is a cut-down version of loader.h from cctools-895,
    ...
    Comment reformatting for line length may have left
...
* vm_prot_t type and contains the
...
/*
* The 64-bit mach header appears at the
* very beginning of object files for
...
/*
* The layout of the file depends on the filetype.
...
*
* The file type MH_OBJECT is a compact
* format intended as output of the
* assembler and input (and possibly output) of the link editor (the .o
* format).  All sections are in one unnamed
* segment with no segment padding.
* This format is used as an executable
* format when the file is so small the
* segment padding greatly increases its size.
*
* The file type MH_PRELOAD is an executable
* format intended for things that
* are not executed under the kernel (proms,
* stand alones, kernels, etc).  The
* format can be executed under the kernel
* but may demand paged it and not
...
#define MH_NO_HEAP_EXECUTION 0x1000000    /* When this bit is set,
    the OS will
...
* of the commands is given by the sizeofcmds
* field in the mach_header.  All
* load commands must have as their first two
* fields cmd and cmdsize.  The cmd
...
* of the particular load command structure
* plus anything that follows it that
* is a part of the load command (i.e.
* section structures, strings, etc.).  To
* advance to the next load command the
* cmdsize can be added to the offset or
* pointer of the current load command.
*  The cmdsize for 32-bit architectures
* MUST be a multiple of 4 bytes and for
* 64-bit architectures MUST be a multiple
* of 8 bytes (these are forever the maximum
* alignment of any load commands).
* The padded bytes must be zero.
*  All tables in the object file must also
* follow these rules so the file can be memory mapped.
*  Otherwise the pointers
* to these tables will not work well or at all
* on some machines.  With all
...
/*
* After MacOS X 10.1 when a new load command
* is added that is required to be
* understood by the dynamic linker for the
* image to execute properly the
* LC_REQ_DYLD bit will be or'ed into the load
* command constant.  If the dynamic
* linker sees such a load command it it does
* not understand will issue a
* "unknown load command required for execution"
* error and refuse to use the
* image.  Other load commands without this bit
* that are not understood will
* simply be ignored.
*/
...
* union.  The strings are stored just after
* the load command structure and
* the offset is from the start of the load
* command structure.  The size
...
* vmsize, maybe equal to or larger than the amount
* to map from this file,
* filesize.  The file is mapped starting at fileoff
* to the beginning of
* the segment in memory, vmaddr.
*  The rest of the memory of the segment,
* if any, is allocated zero fill on demand.
*  The segment's maximum virtual
* memory protection and initial virtual memory
* protection are specified
* by the maxprot and initprot fields.
*  If the segment has sections then the
* section structures directly follow the segment
* command and their size is
...
/*
* The 64-bit segment load command indicates
* that a part of this file is to be
* mapped into a 64-bit task's address space.
*  If the 64-bit segment has
* sections then section_64 structures directly
* follow the 64-bit segment
...
* all of their segments with the proper sections
* in each, and padded to the
* specified segment alignment when produced by
* the link editor.  The first
* segment of a MH_EXECUTE and MH_FVMLIB format file
* contains the mach_header
* and load commands of the object file before its
* first section.  The zero
* fill sections are always last in their segment
* (in all formats).  This
* allows the zeroed segment padding to be mapped
* into memory where zero fill
* sections might be. The gigabyte zero fill sections,
* those with the section
* type S_GB_ZEROFILL, can only be in a segment
* with sections of this type.
* These segments are then placed after all other segments.
*
* The MH_OBJECT format has all of its sections in one segment for
* compactness.  There is no padding to a specified
* segment boundary and the
* mach_header and load commands are not part of the segment.
*
* Sections with the same section name, sectname,
* going into the same segment,
* segname, are combined by the link editor.
*  The resulting section is aligned
* to the maximum alignment of the combined sections
* and is the new section's
* alignment.  The combined sections are aligned to
* their original alignment in
* the combined section.  Any padded bytes to get
* the specified alignment are
* zeroed.
*
* The format of the relocation entries referenced by
* the reloff and nreloc
* fields of the section structure for mach
* object files is described in the
* header file <reloc.h>.
*/
...
/*
* The flags field of a section structure is
* separated into two parts a section
* type and section attributes.
*  The section types are mutually exclusive (it
* can only have one type) but the section attributes
* are not (it may have more
* than one attribute).
*/
...
#define    S_LITERAL_POINTERS    0x5
    ...
    /*  literals */
/*
* For the two types of symbol pointers
* sections and the symbol stubs section
* they have indirect symbol table entries.
*  For each of the entries in the
* section the indirect symbol table entries,
* in corresponding order in the
* indirect symbol table, start at the
* index stored in the reserved1 field
* of the section structure.  Since the indirect symbol table entries
* correspond to the entries in the section
* the number of indirect symbol table
* entries is inferred from the size of the
* section divided by the size of the
* entries in the section.
*  For symbol pointers sections the size of the entries
* in the section is 4 bytes and for symbol stubs
* sections the byte size of the
...
/*
* Constants for the section attributes part of the
* flags field of a section
* structure.
*/
...
* sections in that segment must have this attribute.
*  No section other than
* a section marked with this attribute may reference
* the contents of this
* section.  A section with this attribute may
* contain no symbols and must have
* a section type S_REGULAR.
*  The static linker will not copy section contents
...
* mostly meaningless to the
* link-editor.  But there are few things to support traditional UNIX
* executables that require the link-editor and assembler
* to use some names
* agreed upon by convention.
*
* The initial protection of the "__TEXT" segment
* has write protection turned
* off (not writeable).
*
* The link-editor will allocate common symbols at
* the end of the "__common"
* section in the "__DATA" segment.
*  It will create the section and segment
* if needed.
*/

/* The currently known segment names and the
* section names in those segments */
...
/*
* Fixed virtual memory shared libraries are
* identified by two things.  The
* target pathname (the name of the library
* as found for execution), and the
* minor version number.
*  The address of where the headers are loaded is in
...
/*
* A fixed virtual shared library (filetype == MH_FVMLIB
* in the mach header)
* contains a fvmlib_command (cmd == LC_IDFVMLIB) to
...
* time a library was
* built and copied into user so it can be use to determined
* if the library used
...
struct dylib {
    ...
    TYP(compatibility_version,4);
    /* library's compatibility vers number*/
};

/*
* A dynamically linked shared library (filetype == MH_DYLIB
* in the mach header)
* contains a dylib_command (cmd == LC_ID_DYLIB) to
* identify the library.
* An object that uses a dynamically linked shared library
* also contains a
...
/*
* A dynamically linked shared library may be a
* subframework of an umbrella
* framework.  If so it will be linked with "-umbrella
* umbrella_name" where
* Where "umbrella_name" is the name of the umbrella framework.
* A subframework
* can only be linked against by its umbrella framework
* or other subframeworks
* that are part of the same umbrella framework.
*  Otherwise the static link
* editor produces an error and states to link against
* the umbrella framework.
* The name of the umbrella framework for subframeworks
* is recorded in the
* following structure.
*/
...
/*
* For dynamically linked shared libraries that
* are subframework of an umbrella
* framework they can allow clients other than
* the umbrella framework or other
* subframeworks in the same umbrella framework.
*  To do this the subframework
* is built with "-allowable_client client_name"
* and an LC_SUB_CLIENT load
* command is created for each -allowable_client flag.
*  The client_name is
* usually a framework name.
*  It can also be a name used for bundles clients
...
/*
* A dynamically linked shared library may be a
* sub_umbrella of an umbrella
* framework.  If so it will be linked with
...
* listed as sub_umbrella frameworks to be
* implicited linked in.  Any other
* dependent dynamic libraries will not be linked
* it when -twolevel_namespace
* is in effect.  The primary library recorded
* by the static linker when
* resolving a symbol in these libraries will
* be the umbrella framework.
* Zero or more sub_umbrella frameworks may be
* use by an umbrella framework.
* The name of a sub_umbrella framework is
* recorded in the following structure.
*/
...
/*
* A dynamically linked shared library may be a
* sub_library of another shared
* library.  If so it will be linked with
...
* and those frameworks
* listed as sub_umbrella frameworks and libraries
* listed as sub_libraries to
* be implicited linked in.  Any other dependent
* dynamic libraries will not be
* linked it when -twolevel_namespace is in effect.
*  The primary library
* recorded by the static linker when resolving a
* symbol in these libraries
* will be the umbrella framework (or dynamic library).
* Zero or more sub_library
* shared libraries may be use by an umbrella framework
* or (or dynamic library).
* The name of a sub_library framework is recorded
* in the following structure.
* For example /usr/lib/libobjc_profile.A.dylib
* would be recorded as "libobjc".
*/
...
* prebound to its dynamic libraries has one of
* these for each library that
* the static linker used in prebinding.
*  It contains a bit vector for the
* modules in the library.
*  The bits indicate which modules are bound (1) and
* which are not (0) from the library.
*  The bit for module 0 is the low bit
...
* dylinker_command to identify
* the name of the dynamic linker (LC_LOAD_DYLINKER).
*  And a dynamic linker
* contains a dylinker_command to identify the dynamic
* linker (LC_ID_DYLINKER).
* A file can have at most one of these.
* This struct is also used for the
* LC_DYLD_ENVIRONMENT load command and
...
/*
* Thread commands contain machine-specific
* data structures suitable for
* use in the thread state primitives.
*  The machine specific data structures
* follow the struct thread_command as follows.
* Each flavor of machine specific data structure
* is preceded by an unsigned
* long constant for the flavor of that data structure, an uint32_t
* that is the count of longs of the size of the state
* data structure and then
* the state data structure follows.  This triple may be
* repeated for many
* flavors.  The constants for the flavors, counts and
* state data structure
* definitions are expected to be in the header file
* <machine/thread_status.h>.
* These machine specific data structures sizes must be multiples of
* 4 bytes  The cmdsize reflects the total size of the thread_command
* and all of the sizes of the constants for the flavors,
* counts and state
* data structures.
*
* For executable objects that are unix processes there will be one
* thread_command (cmd == LC_UNIXTHREAD) created for it
* by the link-editor.
* This is the same as a LC_THREAD, except that a stack
* is automatically
...
/*
* The routines command contains the address of the
* dynamic shared library
* initialization routine and an index into the module
* table for the module
* that defines the routine.  Before any modules are
* used from the library the
* dynamic linker fully binds the module that defines
* the initialization routine
* and then calls it.  This gets called before any
...
/*
* The symtab_command contains the offsets and sizes
* of the link-edit 4.3BSD
* "stab" style symbol table information as described
* in the header files
* <nlist.h> and <stab.h>.
*/
...
/*
* This is the second set of the symbolic information
* which is used to support
* the data structures for the dynamically link editor.
*
* The original set of symbolic information in the
* symtab_command which contains
* the symbol and string tables must also be present
* when this load command is
* present.  When this load command is present the symbol
* table is organized
* into three groups of symbols:
*    local symbols (static and debugging symbols) - grouped by module
*    defined external symbols - grouped by module
*       (sorted by name if not lib)
*    undefined external symbols
*    (sorted by name if MH_BINDATLOAD is not set,
*                         and in order the were seen by the static
*                    linker if MH_BINDATLOAD is set)
* In this load command there are offsets
* and counts to each of the three groups
* of symbols.
*
* This load command contains a the offsets
* and sizes of the following new
* symbolic information tables:
*    table of contents
*    module table
*    reference symbol table
*    indirect symbol table
* The first three tables above (the table of contents,
* module table and
* reference symbol table) are only present if the
* file is a dynamically linked
* shared library.  For executable and object modules, which are files
* containing only one module, the information that
* would be in these three
* tables is determined as follows:
*     table of contents - the defined external symbols
*         are sorted by name
*    module table - the file contains only one module
*         so everything in the file is part of the module.
*    reference symbol table - is the defined and
* undefined external symbols
*
* For dynamically linked shared library files this
* load command also contains
* offsets and sizes to the pool of relocation entries for all sections
* separated into two groups:
*    external relocation entries
*    local relocation entries
* For executable and object modules the
* relocation entries continue to hang
...
struct dysymtab_command {
    ...
    /*
    * The symbols indicated by symoff and nsyms
    * of the LC_SYMTAB load command
    * are grouped into the following three groups:
    *    local symbols (further grouped by the module they are from)
    *    defined external symbols (further grouped
    * by the module they are from)
    *    undefined symbols
    *
    * The local symbols are used only for debugging.
    *  The dynamic binding
    * process may have to use them to indicate to the
    * debugger the local
    * symbols for a module that is being bound.
    *
    * The last two groups are used by the dynamic
    * binding process to do the
    * binding (indirectly through the module table
    ...
    */
    ...
    /*
    * For the for the dynamic binding process to
    * find which module a symbol
    * is defined in the table of contents is used
    * (analogous to the ranlib
    * structure in an archive) which maps defined
    * external symbols to modules
    * they are defined in.  This exists only in a
    * dynamically linked shared
    * library file.  For executable and object modules
    * the defined external
    ...
    */
    ...
    /*
    * To support dynamic binding of "modules" (whole
    * object files) the symbol
    * table must reflect the modules that the file was
    * created from.  This is
    * done by having a module table that has indexes
    * and counts into the merged
    * tables for each module.
    *  The module structure that these two entries
    * refer to is described below.
    *  This exists only in a dynamically linked
    * shared library file.  For executable and object
    * modules the file only
    * contains one module so everything in the file belongs
    * to the module.
    */
    ...
    *  references.
    * This exists only in a dynamically linked shared
    *  library file.  For
    ...
    */
    ...
    /*
    * The sections that contain "symbol pointers" and
    *  "routine stubs" have
    * indexes and (implied counts based on the size of
    *  the section and fixed
    * size of the entry) into the "indirect symbol" table
    *  for each pointer
    * and stub.  For every section of these two types the
    *  index into the
    * indirect symbol table is stored in the section header
    *  in the field
    * reserved1.  An indirect symbol table entry is simply
    *  a 32bit index into
    * the symbol table to the symbol that the pointer or
    *  stub is referring to.
    * The indirect symbol table is ordered to match the
    *  entries in the section.
    */
    TYP(indirectsymoff,4);
    ...
    *  library need to be
    * accessed efficiently.  Since the relocation entries
    *  can't be accessed
    * through the section headers for a library file
    *  they are separated into
    * groups of local and external entries further
    *  grouped by module.  In this
    * case the presents of this load command who's extreloff, nextrel,
    * locreloff and nlocrel fields are non-zero indicates
    *  that the relocation
    * entries of non-merged sections are not referenced
    *  through the section
    * structures (and the reloff and nreloc fields
    *  in the section headers are
    * set to zero).
    *
    * Since the relocation entries are not accessed
    *  through the section headers
    * this requires the r_address field to be something other
    * than a section
    * offset to identify the item to be relocated.
    * In this case r_address is
    * set to the offset from the vmaddr of the first
    * LC_SEGMENT command.
    * For MH_SPLIT_SEGS images r_address is set to
    * the the offset from the
    * vmaddr of the first read-write LC_SEGMENT command.
    *
    * The relocation entries are grouped by module and the
    * module table
    * entries have indexes and counts into them for the
    * group of external
    ...
    */
    ...
    /*
    * All the local relocation entries are
...
struct dylib_module_64 {
    ...
    TYP(objc_module_info_size,4); /* for this module size of */
        /*  the (__OBJC,__module_info) section */
    TYP(objc_module_info_addr,8);
        /* for this module address of the start of */
        /*  the (__OBJC,__module_info) section */
};

/*
* The entries in the reference symbol
* table are used when loading the module
* (both by the static and dynamic link
* editors) and if the module is unloaded
* or replaced.  Therefore all external
* symbols (defined and undefined) are
* listed in the module's reference table.
*  The flags describe the type of
* reference that is being made.
*  The constants for the flags are defined in
...
/*
* The twolevel_hints_command contains the
* offset and number of hints in the
...
/*
* The entries in the two-level namespace lookup
* hints table are twolevel_hint
* structs.  These provide hints to the
* dynamic link editor where to start
* looking for an undefined symbol in a two-level namespace image.  The
* isub_image field is an index into the sub-images (sub-frameworks and
* sub-umbrellas list) that made up the
* two-level image that the undefined
* symbol was found in when it was built by the static link editor.  If
* isub-image is 0 the the symbol is expected
* to be defined in library and not
* in the sub-images.
* If isub-image is non-zero it is an index into the array
* of sub-images for the umbrella with the first
* index in the sub-images being
* 1. The array of sub-images is the ordered
*    list of sub-images of the umbrella
*    that would be searched for a symbol that
*    has the umbrella recorded as its
*    primary library.
* The table of contents index is an index into the
* library's table of contents.
* This is used as the starting point of the
...
/*
* The prebind_cksum_command contains the value of
* the original check sum for
* prebound files or zero.
* When a prebound file is first created or modified
* for other than updating its prebinding information
* the value of the check sum
* is set to zero.
* When the file has it prebinding re-done and if the value of
* the check sum is zero the original check sum is
* calculated and stored in
* cksum field of this load command in the output file.
* If when the prebinding
* is re-done and the cksum field is non-zero it
* is left unchanged from the
* input file.
*/
...
/*
* The uuid load command contains a single 128-bit
* unique random number that
...
/*
* The rpath_command contains a path which
* at runtime should be added to
...
/*
* The encryption_info_command_64 contains
* the file offset and size of an
...
struct dyld_info_command {
    ...
    /*
    * Dyld rebases an image whenever dyld loads it at
    * an address different
    * from its preferred address.  The rebase information is a stream
    * of byte sized opcodes whose symbolic names start
    * with REBASE_OPCODE_.
    ...
    * requires any pointers to be initialized to symbols
    * in other images.
    ...
    */
    ...
    /*
    * Some uses of external symbols do not need to be
    * bound immediately.
    ...
    * information (name, address, flags) in one
    * small, contiguous range.
    * The export area is a stream of nodes.
    * The first node sequentially
    * is the start node for the trie.
    *
    * Nodes for a symbol start with a uleb128 that is the length of
    * the exported symbol information for the string so far.
    * If there is no exported symbol, the node starts with
    * a zero byte.
    ...
    * then following
    * the flags is two uleb128s: the stub offset and
    * the resolver offset.
    * The stub is used by non-lazy pointers.  The resolver is used
    * by lazy pointers and must be called to get
    * the actual address to use.
    *
...
/*
* The linker_option_command contains linker
* options embedded in object files.
*/
...
* The symbol roots of the symbol segments must
* also be aligned properly
* in the file.  So the requirement of keeping the offsets aligned to a
* multiple of a 4 bytes translates to the length field of the symbol
* roots also being a multiple of a long.
* Also the padding must again be
...
* The strings are null terminated and the size of
...
/*
* The fvmfile_command contains a reference to a
* file to be loaded at the
* specified virtual address.  (Presently, this command is
* reserved for internal use.  The kernel ignores
* this command when loading a program into
* memory).
*/
