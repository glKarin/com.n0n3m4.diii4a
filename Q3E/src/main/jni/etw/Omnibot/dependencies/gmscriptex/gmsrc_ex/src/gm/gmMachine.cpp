...

{
	...
	m_global = AllocTableObject(); // Alloc global table
...
void gmMachine::RegisterLibrary(gmFunctionEntry * a_functions, int a_numFunctions, const char * a_asTable, bool a_newTable)
{
	...
	for(int index = 0; index < a_numFunctions; ++index )
	{
		...
		funcObj->m_cFunctor = a_functions[index].m_functor;
...
{
	for(int i = 0; i < a_numFunctions; ++i)
	{
		gmFunctionObject* funcObj = AllocFunctionObject(a_functions[i].m_function);
		funcObj->m_cFunctor = a_functions[i].m_functor;
...
#if GM_USE_INCGC
void gmMachine::RegisterUserCallbacks(gmType a_type, gmGCTraceCallback a_gcTrace, gmGCDestructCallback a_gcDestruct, gmAsStringCallback a_asString, gmDebugChildInfoCallback a_dbgInfo)
{
...
#else //GM_USE_INCGC
void gmMachine::RegisterUserCallbacks(gmType a_type, gmGarbageCollectCallback a_mark, gmGarbageCollectCallback a_gc, gmAsStringCallback a_asString, gmDebugChildInfoCallback a_dbgInfo)
{
...
bool gmMachine::RegisterTypeOperator(gmType a_type, gmOperator a_operator, gmFunctionObject * a_function, gmOperatorFunction a_nativeFunction, gmFunctionEntry *a_entry)
{
	//  if(a_type < GM_USER) return false; //Prevent overriding default operators
	if(a_entry)
	{
		gmFunctionObject* funcObj = AllocFunctionObject(a_entry->m_function);
		funcObj->m_cFunctor = a_entry->m_functor;
		RegisterTypeOperator(a_type, a_operator, funcObj);
	}
	else if(a_function)
	{
...
gmThread * gmMachine::CreateThread(int * a_threadId, gmuint8 a_priority)
{
	...
	thread->SetPriority(a_priority);
	if(a_priority)
	{
		bool inserted = false;
		for(gmThread *tit = m_runningThreads.GetFirst(); 
			m_runningThreads.IsValid(tit); 
			tit = m_runningThreads.GetNext(tit)) 
		{
			if(tit->GetPriority() < a_priority)
			{
				inserted = true;
				m_runningThreads.InsertBefore(tit,thread);
				break;
			}			
		}

		if(!inserted)
			m_runningThreads.InsertLast(thread);
	}
	else
...
gmType gmMachine::GetTypeId(const char * a_typename) const
{
	...
	{
		gmStringObject* pObject = m_types[id].m_name;
		if( strcmp(pObject->GetString(), a_typename) == 0 )
		{
			return id;
		}
	}
...
{    
	if( (a_type >= 0) && (a_type < (int)m_types.Count()) )
	{
		return m_types[a_type].m_ParentType;
	}
	return GM_NULL; 
}

void gmMachine::SetBaseForType(gmType a_type, gmType a_base)
{    
	if( (a_type >= 0) && (a_type < (int)m_types.Count()) && 
		(a_base >= 0) && (a_base < (int)m_types.Count()))
	{
		m_types[a_type].m_ParentType = a_base;

		// Copy variables over
		gmTableIterator tIt;
		gmTableNode *pNode = m_types[a_base].m_variables->GetFirst(tIt);
		while(pNode)
		{
			m_types[a_type].m_variables->Set(this,pNode->m_key,pNode->m_value);
			pNode = m_types[a_base].m_variables->GetNext(tIt);
		}
	}
}
...
gmVariable gmMachine::Lookup(const char *a_string, gmTableObject *a_tbl)
{
	...
	if(pCurrentTable)
	{
		return pCurrentTable->Get(this, pStart);
	}
...
const char *gmMachine::ReverseLookup(const char *a_string, const gmVariable &_value)
{
	...
	{
		gmTableIterator tIt;
		gmTableNode *pNode = pCurrentTable->GetFirst(tIt);
		while(pNode)
		{
			if(!gmVariable::Compare(pNode->m_value,_value))
			{
				return pNode->m_key.GetCStringSafe();
			}
			pNode = pCurrentTable->GetNext(tIt);
		}
	}
