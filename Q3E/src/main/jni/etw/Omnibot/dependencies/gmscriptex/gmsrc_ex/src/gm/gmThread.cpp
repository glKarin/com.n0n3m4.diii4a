		...
		case BC_FORK :
			{
				...
				// http://www.gmscript.com/gamemonkey/forum/viewtopic.php?f=6&t=603
...
gmThread::State gmThread::PushStackFrame(int a_numParameters, const gmuint8 ** a_ip, const gmuint8 ** a_cp)
{
	...
	if(fn->m_cFunction || fn->m_cFunctor)
	{
		//
		...
		int result = 0;
		if(fn->m_cFunctor)
			result = (*fn->m_cFunctor)(this);
		else if(fn->m_cFunction)
			result = fn->m_cFunction(this);		
...
void gmThread::LogLineFile()
{
	...
	if(m_base >= 2 && GetFunction()->m_type == GM_FUNCTION)
	{
		...
		if(fn)
		{
			...
			{
				char buffer[512];
				gmGetLineFromString(source, line, buffer, 512);
