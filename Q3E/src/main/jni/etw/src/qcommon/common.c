...
#if !defined(_DIII4A) //karin: do not check last pid
 ...
 * @param[in] nodefaults
 */
void Com_WriteConfigToFile(const char *filename, qboolean nodefaults)
{
	fileHandle_t f;
	...
	Key_WriteBindings(f);
	Cvar_WriteVariables(f, nodefaults);
	FS_FCloseFile(f);
}
...
void Com_WriteConfiguration(void)
{
...

	{
		...
		if (cl_profileStr[0])
		{
			Com_WriteConfigToFile(va("profiles/%s/%s", cl_profileStr, CONFIG_NAME), qfalse);
		}
		else
		{
			Com_WriteConfigToFile(CONFIG_NAME, qfalse);
		}
	}
}
...
void Com_WriteConfig_f(void)
{
	...
	if (Cmd_Argc() < 2)
	{
		Com_Printf("Usage: writeconfig [options] <filename>\n"
		           "Use '-nd' or '-no-defaults' to omit writing cvars at default values.\n");
		return;
	}

	if (!Q_stricmp(Cmd_Argv(arg), "-nd") || !Q_stricmp(Cmd_Argv(arg), "-no-defaults"))
	{
		nodefaults = qtrue;
		arg++;
	}

	Q_strncpyz(filename, Cmd_Argv(arg), sizeof(filename));
