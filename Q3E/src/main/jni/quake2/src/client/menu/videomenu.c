...
static int gamma2br (float g)
{
return (int)((2.1-g)*10+0.5);
}

static float br2gamma (int x)
{
return 2.1-x/10.0;
}
...
BrightnessCallback(void *s)
{
	...
	//float gamma = br2gamma(slider->curvalue);
...
ApplyChanges(void *unused)
{
	...
	if (gamma2br(vid_gamma->value) != s_brightness_slider.curvalue)
	{
		float gamma = br2gamma(s_brightness_slider.curvalue);
		Cvar_SetValue("vid_gamma", gamma);
		restart = true;
	}
...
VID_MenuInit(void)
{
	...
	s_brightness_slider.curvalue = gamma2br(vid_gamma->value);
