...
R_DrawGLPoly(glpoly_t *p)
{
	...
	qglTexCoordPointer( 2, GL_FLOAT, VERTEXSIZE*sizeof(float), &v[3]);
        qglVertexPointer( 3, GL_FLOAT, VERTEXSIZE*sizeof(float), v);
	...
	for (i = 0; i < MAX_LIGHTMAPS; i++)
	{
			 ...
			 surf = surf->lightmapchain)
		{
			...
			for ( ; p; p = p->chain)
			{
				...
				{
					float vertx[4][3];
					vertx[0][0]=p->verts[0][0];
					vertx[0][1]=p->verts[0][1];
					vertx[0][2]=p->verts[0][2];
	
					vertx[1][0]=p->verts[j-1][0];
					vertx[1][1]=p->verts[j-1][1];
					vertx[1][2]=p->verts[j-1][2];

					vertx[2][0]=p->verts[j][0];
					vertx[2][1]=p->verts[j][1];
					vertx[2][2]=p->verts[j][2];

					vertx[3][0]=p->verts[0][0];
					vertx[3][1]=p->verts[0][1];
					vertx[3][2]=p->verts[0][2];

					qglVertexPointer( 3, GL_FLOAT, 0, vertx );
        				qglDrawArrays( GL_LINE_STRIP, 0, 4);
				}
			}
		}
	}
...
R_DrawGLPolyChain(glpoly_t *p, float soffset, float toffset)
{
	if ((soffset == 0) && (toffset == 0))
	{
		for ( ; p != 0; p = p->chain)
		{
			...
			qglEnableClientState( GL_TEXTURE_COORD_ARRAY );
			qglTexCoordPointer( 2, GL_FLOAT, VERTEXSIZE*sizeof(float), &v[5]);
        		qglVertexPointer( 3, GL_FLOAT, VERTEXSIZE*sizeof(float), v);
       			qglDrawArrays( GL_TRIANGLE_FAN, 0, p->numverts);
			qglDisableClientState( GL_TEXTURE_COORD_ARRAY );
...
R_DrawBrushModel(entity_t *e)
{
	...
	else
	{
		...
		else
		{
			...
			qglTexEnvi(GL_TEXTURE_ENV, GL_SRC0_RGB, GL_TEXTURE);
			qglTexEnvi(GL_TEXTURE_ENV, GL_SRC1_RGB, GL_PREVIOUS);
			qglTexEnvi(GL_TEXTURE_ENV, GL_COMBINE_ALPHA, GL_MODULATE);
			qglTexEnvi(GL_TEXTURE_ENV, GL_SRC0_ALPHA, GL_TEXTURE);
			qglTexEnvi(GL_TEXTURE_ENV, GL_SRC1_ALPHA, GL_PREVIOUS);
		}

		if (gl_overbrightbits->value)
		{
			qglTexEnvi(GL_TEXTURE_ENV, GL_RGB_SCALE,
...
R_DrawWorld(void)
{
	...
	if (qglMTexCoord2fSGIS)
	{
		...
		else
		{
			...
			if (gl_overbrightbits->value)
			{
				qglTexEnvi(GL_TEXTURE_ENV, GL_RGB_SCALE, gl_overbrightbits->value);
			}
		}
