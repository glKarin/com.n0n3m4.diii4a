...
R_EmitWaterPolys(msurface_t *fa)
{
	...
	for (bp = fa->polys; bp; bp = bp->next)
	{
		...
		float vertx[p->numverts][3];
		float texx[p->numverts][2];
		...
		for (i = 0, v = p->verts[0]; i < p->numverts; i++, v += VERTEXSIZE)
		{
			...
			texx[i][0]=s;texx[i][1]=t;
			vertx[i][0]=v[0];vertx[i][1]=v[1];vertx[i][2]=v[2];
		}

		qglEnableClientState(GL_TEXTURE_COORD_ARRAY);
		qglTexCoordPointer(2, GL_FLOAT, 0, texx);
...
static float skyvWTF[4][3];
static float skytWTF[4][2];

void
R_MakeSkyVec(int wtfpos,float s, float t, int axis)
{
	vec3_t v, b;
	int j, k;
	...
	t = 1.0 - t;
	skytWTF[wtfpos][0]=s;skytWTF[wtfpos][1]=t;
	skyvWTF[wtfpos][0]=v[0];skyvWTF[wtfpos][1]=v[1];skyvWTF[wtfpos][2]=v[2];
}
...
R_DrawSkyBox(void)
{
	...
	for (i = 0; i < 6; i++)
	{
		...
		R_MakeSkyVec(0,skymins[0][i], skymins[1][i], i);
		R_MakeSkyVec(1,skymins[0][i], skymaxs[1][i], i);
		R_MakeSkyVec(2,skymaxs[0][i], skymaxs[1][i], i);
		R_MakeSkyVec(3,skymaxs[0][i], skymins[1][i], i);

		qglEnableClientState(GL_TEXTURE_COORD_ARRAY);
		qglTexCoordPointer(2, GL_FLOAT, 0, skytWTF);
	       	qglVertexPointer( 3, GL_FLOAT, 0, skyvWTF);
