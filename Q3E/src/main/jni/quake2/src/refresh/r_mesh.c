		...
		while (1)
		{
				...
				(RF_SHELL_RED | RF_SHELL_GREEN | RF_SHELL_BLUE))
			{
				do
				{
					index_xyz = order[2];
					order += 3;
					colors[count0-count][0]=shadelight[0];
					colors[count0-count][1]=shadelight[1];
					colors[count0-count][2]=shadelight[2];
					colors[count0-count][3]=alpha;
					
					vertx[count0-count][0]=s_lerped[index_xyz][0];
					vertx[count0-count][1]=s_lerped[index_xyz][1];
					vertx[count0-count][2]=s_lerped[index_xyz][2];
				}
				while (--count);
			}
			else
			{
				...
				{
					/* texture coordinates come from the draw list */
					texcoords[count0-count][0]=((float *)order)[0];
					texcoords[count0-count][1]=((float *)order)[1];
					index_xyz = order[2];
					order += 3;
					...
					colors[count0-count][0]=l*shadelight[0];
					colors[count0-count][1]=l*shadelight[1];
					colors[count0-count][2]=l*shadelight[2];
					colors[count0-count][3]=alpha;
...
R_DrawAliasShadow(dmdl_t *paliashdr, int posenum)
{
	...
	while (1)
	{
		...
		do
		{
			...
			point[2] = height;
			vertx[count0-count][0]=point[0];
			vertx[count0-count][1]=point[1];
			vertx[count0-count][2]=point[2];
...
R_DrawAliasModel(entity_t *e)
{
	...
	{
		extern void R_MYgluPerspective(double fovy, double aspect,
