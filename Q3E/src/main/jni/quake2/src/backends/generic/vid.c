...
#include <stdbool.h>
void (*AndroidSetResolution_p)(int aw,int ah);
...
VID_LoadRefresh(char *name)
{
	...
	path = Sys_DLLDefaultPath();//Cvar_Get("basedir", ".", CVAR_NOSET)->string;
	snprintf(fn, MAX_OSPATH, "%s/lib%s", path, name);
	...
	extern void (*Q3E_KeyEvent_p)(int state,int key,int character);
	extern void (*Q3E_MotionEvent_p)(float dx, float dy);
	Q3E_KeyEvent_p=Sys_GetProcAddress(reflib_library, "KeyEventHidden");
	Q3E_MotionEvent_p=Sys_GetProcAddress(reflib_library, "MotionEventHidden");

	AndroidSetResolution_p=Sys_GetProcAddress(reflib_library, "AndroidSetResolution");
	extern int screen_width;
	extern int screen_height;
	(*AndroidSetResolution_p)(screen_width,screen_height);

	extern int gl_format;
	extern int gl_msaa;
    void (*SetGLParms)(int f, int msaa);
    SetGLParms = Sys_GetProcAddress(reflib_library, "R_SetGLParms");
	SetGLParms(gl_format, gl_msaa);
