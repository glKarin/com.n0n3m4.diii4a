...
#ifdef __ANDROID__ //karin: use setting size, not real size
	int width, height;
	QVk_GetDrawableSize(&width, &height);
	VkExtent2D extent;
	extent.width = Q_max(surfaceCaps.minImageExtent.width,
						 Q_min(surfaceCaps.maxImageExtent.width, width));
	extent.height = Q_max(surfaceCaps.minImageExtent.height,
						  Q_min(surfaceCaps.maxImageExtent.height, height));
#else
...
#ifdef __ANDROID__ //karin: do not rorate
	scCreateInfo.preTransform = VK_SURFACE_TRANSFORM_IDENTITY_BIT_KHR;
	//scCreateInfo.oldSwapchain = NULL; // destroy before manually
#endif
