...
#include <sys/stat.h>
#include <unistd.h>
static const char * FS_CheckFile(const char *path) {
    if (!path || !*path) {
        return "error";
    }
...
#define FS_DEBUG_BEGIN() Sys_Printf(">>>>FS_NativePath(%s,%s,%d) -> %d|%s|%d\n", fname,(relativeto<=FS_PUBBASEGAMEONLY?fs_relative[relativeto]:NULL),fordisplay,com_homepathenabled,host_parms.binarydir,com_installer)
#define FS_DEBUG_END(x) Sys_Printf("%2d: FS_NativePath(%s,%s,%d) -> %s|%d : %s\n\n", x,fname,(relativeto<=FS_PUBBASEGAMEONLY?fs_relative[relativeto]:NULL),fordisplay,out,outlen,FS_CheckFile(out))
#else
#define FS_DEBUG_BEGIN()
#define FS_DEBUG_END(x)
#endif
#endif
