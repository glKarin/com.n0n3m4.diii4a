...
// lightgrid atlas
	...
	float invXZ = ( 1.0 / ( float(lightGridBounds[0] * lightGridBounds[2]) ) );
	float invY = ( 1.0 / float(lightGridBounds[1]) );
	...
	for( int i = 0; i < 3; i++ )
	{
		...
		v = max( 0.0, lightOrigin[i] * ( 1.0 / float(lightGridSize[i]) ) );
		gridCoord[i] = int( floor( v ) );
		frac[ i ] = v - float(gridCoord[ i ]);
	...
	*/
	...
	for( int i = 0; i < 8; i++ )
	{
		...
		atlasOffset.x = float( gridCoord2[0] * gridStep[0] + gridCoord2[2] * gridStep[1] ) * invXZ;
		atlasOffset.y = ( float(gridCoord2[1]) * invY );
		...
		probeTopLeftPosition.x = float( gridCoord2[0] * gridStep[0] + gridCoord2[2] * gridStep[1] ) * rpScreenCorrectionFactor.z + rpScreenCorrectionFactor.z * 0.5;
		probeTopLeftPosition.y = float( gridCoord2[1] ) * rpScreenCorrectionFactor.z + rpScreenCorrectionFactor.z * 0.5;
