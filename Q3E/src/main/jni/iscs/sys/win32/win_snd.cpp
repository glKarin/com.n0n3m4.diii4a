...
// DirectX SDK
#include <DxErr.h> // BOYETTE NOTE IMPORTANT: was able to solve the DxErr.h error by downloading direct x 9 SDK or older (download here: https://www.microsoft.com/en-us/download/details.aspx?id=6812) // this link will probably not work eventually as microsoft changes these all the time. If that happens then, just search for Windows DirectX SDK (June 2010).
// AND THEN IF YOU GET AN ERROR FOR atlthunk.lib - then just set it to be ignored as it is not actually used and thus is not necessary to compile: Configuration Properties->Linker->Input->Ignore specific library : atlthunk.lib
