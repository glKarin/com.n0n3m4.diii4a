...
void idServerScan::GUIAdd( int id, const networkServer_t server ) {
	...
	if ( d3xp ) {
		...
		// could have a specific icon for this case // BOYETTE NOTE TODO: we should replace these doom icons with icarus icons
...
bool idServerScan::IsFiltered( const networkServer_t server ) {
	...
	// filter based on the game doom or XP // BOYETTE NOTE: it should all be the same now
