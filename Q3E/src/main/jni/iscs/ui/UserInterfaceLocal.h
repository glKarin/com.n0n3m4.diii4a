...
// boyette space command begin
	virtual void				SetCursorImage( int cursortochangeto);
	virtual void				ScrollListGUIToBottom( idStr winName );
	virtual void				ScrollListGUIToTop( idStr winName );
	virtual void				BeginMultiselectBox();
	virtual void				EndMultiselectBox();
	virtual void				LoopThroughChildrenAndRunScript( idWindow* Parent );
	//bool						multiselect_box_active;
	idRectangle					MultiSelectBox;

	drawWin_t*					listwindow;

	virtual bool				HasEditWindowAsFocus();
	virtual void				ClearFocusWindow();

	virtual void				UpdateTooltip();
	virtual void				GetTooltipText( idWindow* Parent );
...
private:
	...
	float						previous_cursorX_delta;
	float						previous_cursorY_delta;
