...
public :
	...
	virtual void			Launch( const idVec3 &start, const idVec3 &dir, const idVec3 &pushVelocity, const float timeSinceFire = 0.0f, const float launchPower = 1.0f, const float dmgPower = 1.0f, const float setGravity = 0.0f );
...
protected:
	...
	struct projectileFlags_s {
		...
		// boyete space command begin
		bool				ship_to_ship_projectile		: 1;
	...
	typedef enum {
		// must update these in script/icarus_defs.script if changed
		SPAWNED = 0,
		CREATED = 1,
		LAUNCHED = 2,
		FIZZLED = 3,
...
public:
	...
	void					Spawn( void );
	virtual void			Think( void );
	virtual void			Launch( const idVec3 &start, const idVec3 &dir, const idVec3 &pushVelocity, const float timeSinceFire = 0.0f, const float power = 1.0f, const float dmgPower = 1.0f, const float setGravity = 0.0f );
...
public :
	...
	void					Spawn( void );
	virtual void			Think( void );
	// BOYETTE NOTE - the setGravity parameter needed to be added to this Launch function as well(after we added it to the base class idProjectile Launch function) to get the beams and splash damage to work properly - not sure why yet.
	virtual void			Launch( const idVec3 &start, const idVec3 &dir, const idVec3 &pushVelocity, const float timeSinceFire = 0.0f, const float launchPower = 1.0f, const float dmgPower = 1.0f, const float setGravity = 0.0f );
	virtual void			Explode( const trace_t &collision, idEntity *ignore );
...
class sbFoodProjectile : public idProjectile {
public :
	CLASS_PROTOTYPE( sbFoodProjectile );

							sbFoodProjectile();
	virtual					~sbFoodProjectile();
