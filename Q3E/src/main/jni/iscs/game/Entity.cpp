...
const idEventDef EV_EndTransporterMaterialShaderEffect( "<endTransporterMaterialShaderEffect>" );
...
// BOYETTE SPACE COMMAND BEGIN
	...
	EVENT( EV_EndTransporterMaterialShaderEffect,		idEntity::Event_EndTransporterMaterialShaderEffect )
// BOYETTE SPACE COMMAND END
END_CLASS
...
void idGameEdit::ParseSpawnArgsToRenderEntity( const idDict *args, renderEntity_t *renderEntity ) {
	...
	if ( temp[0] != '\0' ) {
		...
		// BOYETTE SPACE COMMAND BEGIN
		renderEntity->originalCustomSkin = declManager->FindSkin( temp );
		// BOYETTE SPACE COMMAND END
	} else if ( modelDef ) {
		renderEntity->customSkin = modelDef->GetDefaultSkin();
		// BOYETTE SPACE COMMAND BEGIN
		renderEntity->originalCustomSkin = modelDef->GetDefaultSkin();
		// BOYETTE SPACE COMMAND END
	}
...
idEntity::idEntity() {
	...
	noGrab = false;
	// boyette space command begin
	stargridpositionx = 0;		// boyette mod
	stargridpositiony = 0;		// boyette mod
	stargriddestinationx = 0;	// boyette mod
	stargriddestinationy = 0;	// boyette mod
	team	= 0;
	entity_max_health	= 0;
...
// boyette important end

// boyette spaceship command begin
	was_killed = false;
	spawnArgs.GetInt( "team", "0", team );
	// boyette note - little messy - but it works. If there is no "entity_max_health" key it will just use the "health" key - which is the safest assumption.
...
idEntity::~idEntity( void ) {
	...
	//file->Printf( name + "::~idEntity begin\n" );
...
void idEntity::Save( idSaveGame *savefile ) const {
	...
	// BOYETTE SAVE BEGIN
	savefile->WriteInt( captaintestnumber );

	savefile->WriteInt( stargridpositionx );
	savefile->WriteInt( stargridpositiony );
	savefile->WriteInt( stargriddestinationx );
	savefile->WriteInt( stargriddestinationy );

	savefile->WriteBool( was_killed );

	savefile->WriteBool( was_just_damaged );
	savefile->WriteBool( was_just_repaired );

	savefile->WriteInt( entity_max_health );
...
void idEntity::Restore( idRestoreGame *savefile ) {
	...
	savefile->ReadInt( captaintestnumber );

	savefile->ReadInt( stargridpositionx );
	savefile->ReadInt( stargridpositiony );
	savefile->ReadInt( stargriddestinationx );
	savefile->ReadInt( stargriddestinationy );

	savefile->ReadBool( was_killed );

	savefile->ReadBool( was_just_damaged );
	savefile->ReadBool( was_just_repaired );

	savefile->ReadInt( entity_max_health );
...
void idEntity::TakeRemoteRenderViewScreenshot( const char* screenshot_filename, int image_width, int image_height ) {
	...
	bool needs_crop = ( image_width != SCREEN_WIDTH || image_height != SCREEN_HEIGHT );

	if ( needs_crop ) {
		//renderSystem->CropRenderSize( 256, 256, true ); // originally
		renderSystem->CropRenderSize( image_width, image_height );
	}
	gameRenderWorld->RenderScene( &fullView );
	renderSystem->CaptureRenderToFile( screenshot_filename );
	if ( needs_crop ) {
		renderSystem->UnCrop();
	}
}
...
renderView_t *idEntity::GetRenderView( void ) {
	...
	renderView->fov_x = 100; // BOYETTE NOTE TODO IMPORTANT: was originally 120 - we should make this adjustable with a spawnarg - maybe called "remote_render_fov"
	renderView->fov_y = 100; // BOYETTE NOTE TODO IMPORTANT: was originally 120 - we should make this adjustable with a spawnarg - maybe called "remote_render_fov"
...
================
*/
bool idEntity::StartSoundShader( const idSoundShader *shader, const s_channelType channel, int soundShaderFlags, bool broadcast, int *length, bool allowSlow ) {
	float diversity;
	int len;
	...
	len = refSound.referenceSound->StartSound( shader, channel, diversity, soundShaderFlags, allowSlow );
	if ( length ) {
		*length = len;
	}
...
bool idEntity::GetFloorPos( float max_dist, idVec3 &floorpos ) const {
	...
	// boyette general improvement begin
	if ( !GetPhysics()->HasGroundContacts() && GetPhysics()->GetClipModel()->IsTraceModel() ) {
	// boyette general improvement end
...
/*
================
idEntity::Event_EndTransporterMaterialShaderEffect
================
*/
void idEntity::Event_EndTransporterMaterialShaderEffect( void ) {
	EndTransporterMaterialShaderEffect();
}
// BOYETTE SPACE COMMAND END
...
// BOYETTE SPACE COMMAND BEGIN
void idEntity::SetRenderEntityGuisStrings( const char* varName, const char* value ) {
	if ( renderEntity.gui[0] ) {
...
void idEntity::SetRenderEntityGuisBools( const char* varName, bool value ) {
	if ( renderEntity.gui[0] ) {
...
void idEntity::SetRenderEntityGuisInts( const char* varName, int value ) {
	if ( renderEntity.gui[0] ) {
...
void idEntity::SetRenderEntityGuisFloats( const char* varName, float value ) {
	if ( renderEntity.gui[0] ) {
...
void idEntity::HandleNamedEventOnGuis( const char* eventName ) {
	if ( renderEntity.gui[0] ) {
...
void idEntity::ActivateGuis( bool activate, int time ) {
	for ( int i = 0; i < MAX_RENDERENTITY_GUI; i++ ) {
		if ( renderEntity.gui[i] ) {
			renderEntity.gui[i]->Activate( activate, time );
		}
	}
}
void idEntity::ActivateGuisIfNotActive( bool activate, int time ) {
	for ( int i = 0; i < MAX_RENDERENTITY_GUI; i++ ) {
		if ( renderEntity.gui[i] && !renderEntity.gui[i]->Active() ) {
			renderEntity.gui[i]->Activate( activate, time );
		}
	}
}

void idEntity::BeginTransporterMaterialShaderEffect( const idVec3& theme_colors ) {
	idEntity *ent;
	idEntity *next;

	renderEntity.noShadow = true;
	renderEntity.shaderParms[ SHADERPARM_TIME_OF_DEATH ] = gameLocal.time * 0.001f;
	renderEntity.shaderParms[ 0 ] = theme_colors.x;
	renderEntity.shaderParms[ 1 ] = theme_colors.y;
	renderEntity.shaderParms[ 2 ] = theme_colors.z;
	UpdateVisuals();

	for( ent = GetNextTeamEntity(); ent != NULL; ent = next ) {
		next = ent->GetNextTeamEntity();
		if ( ent->GetBindMaster() == this ) {
			ent->BeginTransporterMaterialShaderEffect( theme_colors );
		}
	}
}
void idEntity::EndTransporterMaterialShaderEffect() {
	idEntity *ent;
	idEntity *next;
...
void idEntity::ScheduleEndTransporterMaterialShaderEffect( int ms_from_now ) {
	PostEventMS( &EV_EndTransporterMaterialShaderEffect, ms_from_now );
}

void idEntity::TemporarilySetThisEntityGravityToZero( int ms_to_stay_at_zero ) {
	GetPhysics()->SetGravity(idVec3(0,0,0));
	CancelEvents( &EV_ResetGravity );
	PostEventMS( &EV_ResetGravity, ms_to_stay_at_zero );
}

/*
================
idEntity::DoStuffAfterAllMapEntitiesHaveSpawned

put stuff in here that needs to be done after all map entities have spawned
================
*/
void idEntity::DoStuffAfterAllMapEntitiesHaveSpawned() {
	// implemented in subclasses
}
/*
================
idEntity::DoStuffAfterPlayerHasSpawned

put stuff in here that needs to be done after the player has spawned
================
*/
void idEntity::DoStuffAfterPlayerHasSpawned() {
	// implemented in subclasses
}
