...
void CMediaPreviewDlg::Refresh() {
	if (mode == GUIS) {
		const idMaterial *mat = declManager->FindMaterial("textures/materials_used_in_source/guisurfs/guipreview");
		mat->SetGui( media );
		drawMaterial.setMedia("textures/materials_used_in_source/guisurfs/guipreview");
...
void CMediaPreviewDlg::OnMouseMove(UINT nFlags, CPoint point)
{
	if (mode == GUIS) {
		...
		if (gui) {
			...
			memset( &ev, 0, sizeof( ev ) );
			ev.evType = SE_MOUSE;
			ev.evValue = (point.x / rct.Width()) * 1280.0f;
			ev.evValue2 = (point.y / rct.Height()) * 960.0f;
			gui->HandleEvent(&ev, 0);
		}
	}
