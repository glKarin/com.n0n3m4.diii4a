...
static	void RemoveIfColinear( optVertex_t *ov, optIsland_t *island ) {
	...
	if ( !e2 ) {
		...
		// BOYETTE DMAP COMMENT OUT // common->Printf( "WARNING: vertex with only one edge\n" );
		return;
	}
...
static void LinkTriToEdge( optTri_t *optTri, optEdge_t *edge ) {
	if ( ( edge->v1 == optTri->v[0] && edge->v2 == optTri->v[1] )
		...
		if ( edge->backTri ) {
			// BOYETTE DMAP COMMENT OUT // common->Printf( "Warning: LinkTriToEdge: already in use\n" );
			return;
		}
		edge->backTri = optTri;
		return;
...
//DrawEdges( island );
	...
	if ( !opposite ) {
		// BOYETTE DMAP COMMENT OUT // common->Printf( "Warning: BuildOptTriangles: couldn't locate opposite\n" );
		return;
	}
...
static void AddOriginalTriangle( optVertex_t *v[3] ) {
	...
	if ( !IsTriangleValid( v[0], v[1], v[2] ) ) {
		// BOYETTE DMAP COMMENT OUT // common->Printf( "WARNING: backwards triangle in input!\n" );
		return;
	}
...
void	OptimizeGroupList( optimizeGroup_t *groupList ) {
	...
	// BOYETTE DMAP COMMENT OUT //common->Printf( "----- OptimizeAreaGroups Results -----\n" );
	// BOYETTE DMAP COMMENT OUT //common->Printf( "%6i tris in\n", c_in );
	// BOYETTE DMAP COMMENT OUT //common->Printf( "%6i tris after edge removal optimization\n", c_edge );
	// BOYETTE DMAP COMMENT OUT //common->Printf( "%6i tris after final t junction fixing\n", c_tjunc2 );
}
