...
guiPoint_t	idRenderWorldLocal::GuiTrace( qhandle_t entityHandle, const idVec3 start, const idVec3 end ) const {
	...
	for ( j = 0 ; j < model->NumSurfaces() ; j++ ) {
		...
		shader = R_RemapShaderBySkin( surf->shader, def->parms.customSkin, def->parms.customShader, def->parms.originalCustomSkin );
		if ( !shader ) {
			continue;
		}
...
const char* playerModelExcludeList[] = {
	"models/md5/characters/milwaukee_player/PlayerMesh1.md5mesh",
	NULL
};

const char* playerMaterialExcludeList[] = {
	"textures/materials_used_in_source/muzzle_smoke_puff",
	NULL
};
...
===============
*/
const idMaterial *R_RemapShaderBySkin( const idMaterial *shader, const idDeclSkin *skin, const idMaterial *customShader, const idDeclSkin *original_skin ) {
	...
	if ( skin && original_skin && original_skin != skin ) {
		return skin->RemapShaderBySkins( shader, original_skin );
	}
