...
idCVar r_customWidth( "r_customWidth", "1080", CVAR_RENDERER | CVAR_ARCHIVE | CVAR_INTEGER, "custom screen width. set r_mode to -1 to activate" );
idCVar r_customHeight( "r_customHeight", "1920", CVAR_RENDERER | CVAR_ARCHIVE | CVAR_INTEGER, "custom screen height. set r_mode to -1 to activate" );
idCVar r_singleTriangle( "r_singleTriangle", "0", CVAR_RENDERER | CVAR_BOOL, "only draw a single triangle per primitive" );
idCVar r_checkBounds( "r_checkBounds", "0", CVAR_RENDERER | CVAR_BOOL, "compare all surface bounds with precalculated ones" );

// BOYETTE SPACE COMMAND EXPERIMENTAL RESOLUTION BEGIN
idCVar r_mode_experimental( "r_mode_experimental", "0", CVAR_RENDERER | CVAR_ARCHIVE | CVAR_BOOL, "0 = 1920x1080 16:9, 1 = multiple possible resolutions 4:3" );
// BOYETTE SPACE COMMAND EXPERIMENTAL RESOLUTION END
...
idCVar r_useExpensiveShadowOptimizations( "r_useExpensiveShadowOptimizations", "0", CVAR_RENDERER | CVAR_BOOL | CVAR_ARCHIVE, "use (in real time) the expensive shadows normally reserved for the dmap generated shadows" );
idCVar m_menu_acceleration( "m_menu_acceleration", "0.35", CVAR_SYSTEM | CVAR_ARCHIVE | CVAR_FLOAT, "how much acceleration is applied to the mouse on the menus" );
...
/*
==================
R_SetDisplayDeviceSupportedResolutionsInfoStrings

==================
*/
void R_SetDisplayDeviceSupportedResolutionsInfoStrings( void ) {
	common->Printf( "Number of VID MODES: &i \n", s_numVidModes );
	//common->Printf( "common->displayDeviceSupportedResolutions = " + common->displayDeviceSupportedResolutions + "\n" );
	//common->Printf( "common->displayDeviceSupportedRModes = " + common->displayDeviceSupportedRModes + "\n" );
	common->displayDeviceSupportedResolutions.Clear(); // this is done earlier - so probably not necessary - but just for good measure
	common->displayDeviceSupportedRModes.Clear(); // this is done earlier - so probably not necessary - but just for good measure
	std::vector<int> supported_r_modes;
	for ( int i = 0; i < s_numVidModes; i++ ) {
		if ( CheckIfScreenResolutionIsSupported( r_vidModes[i].width, r_vidModes[i].height ) ) {
			supported_r_modes.push_back(i);
		}
	}
	for ( unsigned int i = 0; i < supported_r_modes.size(); i++ ) {
		if ( supported_r_modes.back() == supported_r_modes[i] ) {
			common->displayDeviceSupportedResolutions.Append( idStr(r_vidModes[supported_r_modes[i]].width) + "x" + idStr(r_vidModes[supported_r_modes[i]].height) );
			common->displayDeviceSupportedRModes.Append( idStr(supported_r_modes[i]) );
		} else {
			common->displayDeviceSupportedResolutions.Append( idStr(r_vidModes[supported_r_modes[i]].width) + "x" + idStr(r_vidModes[supported_r_modes[i]].height) + ";" );
			//common->displayDeviceSupportedResolutions.Append( ";" );
			common->displayDeviceSupportedRModes.Append( idStr(supported_r_modes[i]) + ";" );
		}
	}
}
...
void R_VidRestart_f( const idCmdArgs &args ) {
	...
	if ( full ) {
		...
		// BOYETTE SPACE COMMAND EXPERIMENTAL RESOLUTION BEGIN
		if ( cvarSystem->GetCVarBool( "r_mode_experimental" ) ) {
			cvarSystem->SetCVarInteger( "r_customHeight", 1080 );
			cvarSystem->SetCVarInteger( "r_customWidth", 1920 );
			cvarSystem->SetCVarInteger( "r_mode", -1 );
			//cvarSystem->SetCVarInteger( "r_aspectRatio", 1 ); // makes the FOV super wide - might call this "biscuit" mode
			...
			if ( cvarSystem->GetCVarInteger( "r_mode" ) == -1 ) {
				...
				} else if ( renderSystem->CheckIfResolutionModeIsSupported(5) ) {
					cvarSystem->SetCVarInteger( "r_mode", 5, CVAR_ARCHIVE );
				} else if ( renderSystem->CheckIfResolutionModeIsSupported(3) ) {
					cvarSystem->SetCVarInteger( "r_mode", 4, CVAR_ARCHIVE );
				} else if ( renderSystem->CheckIfResolutionModeIsSupported(3) ) {
					cvarSystem->SetCVarInteger( "r_mode", 3, CVAR_ARCHIVE );
				} else if ( renderSystem->CheckIfResolutionModeIsSupported(2) ) {
					cvarSystem->SetCVarInteger( "r_mode", 2, CVAR_ARCHIVE );
				} else if ( renderSystem->CheckIfResolutionModeIsSupported(1) ) {
					cvarSystem->SetCVarInteger( "r_mode", 1, CVAR_ARCHIVE );
				} else {
					cvarSystem->SetCVarInteger( "r_mode", 0, CVAR_ARCHIVE );
				}

				cvarSystem->SetCVarInteger( "r_aspectRatio", 0 ); // makes the FOV super wide - might call this "biscuit" mode
			}
		}
		// BOYETTE SPACE COMMAND EXPERIMENTAL RESOLUTION END
...
/*
========================
idRenderSystemLocal::CheckIfResolutionModeIsSupported
========================
*/
bool idRenderSystemLocal::CheckIfResolutionModeIsSupported( int r_mode_num ) {
	if ( r_mode_num >= s_numVidModes || r_mode_num < 0 ) {
		return false;
	}

	if ( CheckIfScreenResolutionIsSupported( r_vidModes[r_mode_num].width, r_vidModes[r_mode_num].height ) ) {
		return true;
	} else {
		return false;
	}
