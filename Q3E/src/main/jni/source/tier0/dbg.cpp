...
#ifdef _DIII4A //karin: output log to stdout
#define _fprintf(f, ...) do { \
...
void CDbgLogger::Init(const char *logfile)
{
	...
	if( file )
	{
		...
		_fprintf(file, ">>> Engine(arch:%s commit:" GIT_COMMIT_HASH ") started at %s\n", GetProcessorArchName(), szTime);
#else
		_fprintf(file, ">>> Engine(arch:%s) started at %s\n", GetProcessorArchName(), szTime);
#endif

#ifdef GNUC
		_fprintf(file, "Compiler version: %s\n", __VERSION__);
#endif
		_fprintf(file, "Compiler CFLAGS: %s\n", WAF_CFLAGS);
		_fprintf(file, "Compiler LDFLAGS: %s\n", WAF_LDFLAGS);
		fflush(file);
...
void CDbgLogger::Write(const char *data)
{
	...
	if( file )
	{
		_fprintf(file, "[%.4f] ", Plat_FloatTime() - flStartTime);
		_fprintf(file, "%s", data);
		fflush(file);
	}
