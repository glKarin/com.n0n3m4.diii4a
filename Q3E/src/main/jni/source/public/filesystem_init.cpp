...
FSReturnCode_t FileSystem_GetFileSystemDLLName( char *pFileSystemDLL, int nMaxLen, bool &bSteam )
{
	...
	// Assume we'll use local files
#ifdef _DIII4A //karin: add source_ prefix on dll
    if(executablePath && executablePath[0])
	    Q_snprintf( pFileSystemDLL, nMaxLen, "%s%clibsource_filesystem_stdio" DLL_EXT_STRING, executablePath, CORRECT_PATH_SEPARATOR );
    else
	    Q_snprintf( pFileSystemDLL, nMaxLen, "libsource_filesystem_stdio" DLL_EXT_STRING );
#else
