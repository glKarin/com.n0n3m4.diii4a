...
void qh_checkconnect (void /* qh newfacet_list */) {
  ...
  FORALLnew_facets {
    if (newfacet->visitid == qh visit_id)
      break;
    ivp_message( "qhull error: f%d is not attached to the new facets\n",
         newfacet->id);
    errfacet= newfacet;
  }
  if (errfacet)
...
vertexT *qh_find_newvertex (vertexT *oldvertex, setT *vertices, setT *ridges) {
  ...
  if (qh IStracing >= 4) {
    ...
    FOREACHvertex_(vertices) 
      ivp_message( "v%d ", vertex->id);
    FOREACHridge_(ridges)
      ivp_message( "r%d ", ridge->id);
    ivp_message( "\n");
  }
#endif
  FOREACHvertex_(vertices) 
...
facetT *qh_findbestneighbor(facetT *facet, realT *distp, realT *mindistp, realT *maxdistp) {
  ...
  *distp= REALmax;
  ...
  if (!bestfacet) {
    ivp_message( "qhull internal error (qh_findbestneighbor): no neighbors for f%d\n", facet->id);
...
void qh_forcedmerges(boolT *wasmerge) {
  ...
  FOREACHmerge_(othermerges) {
    ...
    if (!qh_setin (facet2->neighbors, facet1)) {
      ivp_message( "qhull internal error (qh_forcedmerges): f%d and f%d had a duplicate ridge but as f%d and f%d they are no longer neighbors\n",
...
int qh_merge_degenredundant (void) {
  ...
  while ((merge= (mergeT*)qh_setdellast (qh degen_mergeset))) {
    ...
    if (mergetype == MRGredundant) {
      ...
      while (facet2->visible) {
        if (!facet2->f.replace) {
          ivp_message( "qhull internal error (qh_merge_degenredunant): f%d redundant but f%d has no replacement\n",
	       facet1->id, facet2->id);
          qh_errexit2 (qh_ERRqhull, facet1, facet2);
        }
...
void qh_mergecycle (facetT *samecycle, facetT *newfacet) {
  ...
  if (traceonce) {
    ivp_message( "qh_mergecycle: end of trace facet\n");
...
void qh_mergecycle_all (facetT *facetlist, boolT *wasmerge) {
  ...
  for (facet= facetlist; facet && (nextfacet= facet->next); facet= nextfacet) {
    ...
    if (!facet->mergehorizon) {
      ivp_message( "qh_mergecycle_all: f%d without normal\n", facet->id);
      qh_errexit (qh_ERRqhull, facet, NULL);
    }
    horizon= SETfirstt_(facet->neighbors, facetT);
	...
	if (vertex != apex)
      ...
      for (same= facet->f.samecycle; same;  /* FORALLsame_cycle_(facet) */
	   same= (same == facet ? 0 :nextsame)) { /* ends at facet */
...
void qh_mergecycle_facets (facetT *samecycle, facetT *newfacet) {
  ...
  for (same= samecycle->f.samecycle; same; same= (same == samecycle ?  0 : next)) {
...
void qh_mergecycle_ridges(facetT *samecycle, facetT *newfacet) {
  ...
  FORALLsame_cycle_(samecycle) {
	...
	ivp_message( "qhull internal error (qh_mergecycle_ridges): bad ridge r%d\n", ridge->id);
...
void qh_mergefacet(facetT *facet1, facetT *facet2, realT *mindist, realT *maxdist, boolT mergeapex) {
  ...
  if (qh build_cnt >= qh RERUN) {
    if (mindist && (-*mindist > qh TRACEdist || *maxdist > qh TRACEdist)) {
      ...
      traceonce= True;
      ivp_message( "qh_mergefacet: ========= trace wide merge #%d (%2.2g) for f%d into f%d, last point was p%d\n", zzval_(Ztotmerge),
  ...
  if (qh IStracing >= 2) {
    ...
    ivp_message( "qh_mergefacet: #%d merge f%d into f%d, mindist= %2.2g, maxdist= %2.2g\n", 
    zzval_(Ztotmerge), facet1->id, facet2->id, mergemin, mergemax);
  }
#endif /* !qh_NOtrace */
  if (facet1 == facet2 || facet1->visible || facet2->visible) {
    ivp_message( "qhull internal error (qh_mergefacet): either f%d and f%d are the same or one is a visible facet\n",
	     facet1->id, facet2->id);
    qh_errexit2 (qh_ERRqhull, facet1, facet2);
  }
...
try 'C-0.001' instead of 'C-0.1' or 'A-0.999' instead of 'A-0.9'.\n", 
    ...
    if (qh hull_dim >= 5 && !qh MERGEexact)
      ivp_message( "Option 'Qx' may avoid this problem.\n");
  ...
  if (traceonce) {
    ivp_message( "qh_mergefacet: end of wide tracing\n");
    qh IStracing= tracerestore;
  }
} /* mergefacet */
...
void qh_mergevertex_neighbors(facetT *facet1, facetT *facet2) {
  ...
  trace4((qh ferr, "qh_mergevertex_neighbors: merge vertex neighbors of f%d and f%d\n",
	  facet1->id, facet2->id));
  if (qh tracevertex) {
    ivp_message( "qh_mergevertex_neighbors: of f%d and f%d at furthest p%d f0= %p\n",
...
void qh_mergevertices(setT *vertices1, setT **vertices2) {
  ...
  if (newsize < qh_setsize (mergedvertices)) {
    ivp_message( "qhull internal error (qh_mergevertices): facets did not share a ridge\n");
...
vertexT *qh_rename_sharedvertex (vertexT *vertex, facetT *facet) {
  ...
  if (qh_setsize (vertex->neighbors) == 2) {
    ...
    if (!neighborA) {
      ivp_message( "qhull internal error (qh_rename_sharedvertex): v%d's neighbors not in f%d\n",
        vertex->id, facet->id);
...
void qh_renamevertex(vertexT *oldvertex, vertexT *newvertex, setT *ridges, facetT *oldfacet, facetT *neighborA) {
  ...
  if (!oldfacet) {
    zinc_(Zrenameall);
    if (istrace)
      ivp_message( "qh_renamevertex: renamed v%d to v%d in several facets\n",
    ...
    zinc_(Zrenameshare);
    if (istrace)
      ivp_message( "qh_renamevertex: renamed v%d to v%d in oldfacet f%d\n", 
    ...
    if (istrace || qh IStracing)
      ivp_message( "qh_renamevertex: renamed pinched v%d to v%d between f%d and f%d\n", 
...
/*-<a                             href="qh-c.htm#merge"
  ...
  design:
    if qh.cos_max is defined
      if the angle between facets is too shallow
    ...
    if (facet2 != qh tracefacet)
      qh_errprint ("TRACE", qh tracefacet, 
        (qh tracevertex && qh tracevertex->neighbors) ? 
           SETfirstt_(qh tracevertex->neighbors, facetT) : 0,
        0, qh tracevertex);      
  }
  if (qh tracevertex) {
    if (qh tracevertex->deleted)
      ivp_message( "qh_tracemerge: trace vertex deleted at furthest p%d\n",
	    qh furthest_id);
    else
