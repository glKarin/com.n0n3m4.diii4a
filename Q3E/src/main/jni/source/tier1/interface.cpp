...
#ifdef _DIII4A //karin: export Sys_GetProcAddress
void *Q3E_GetProcAddress( HMODULE hModule, const char *pName )
{
	return (void *)dlsym( (void *)hModule, pName );
}
#endif
...
#endif
	HMODULE hDLL = NULL;
#ifdef _DIII4A //karin: add source_ prefix to dll
	char sourceModuleName[1024] = { 0 };
    if(!strchr(pModuleName, '/') && !strchr(pModuleName, '\\'))
    {
        const char *offset = pModuleName;
        if(!strncmp(offset, "lib", 3)) // not starts with lib
            offset += 3;
        if(!strncmp(offset, "source_", 7)) // not starts with source_
            offset += 7;
		Q_snprintf( sourceModuleName, sizeof(sourceModuleName), "libsource_%s", offset );
        pModuleName = sourceModuleName;
    }
#endif
...
#ifdef _DIII4A //karin: check exists if using absolution path
...
#ifdef _DIII4A //karin: print dll path
