...
void idUsercmdGenLocal::KeyMove( void ) {
	...
	{
		//togglecrouch behavior.
		if (ButtonState( UB_UP ) && toggled_crouch.on)
		{
			heldJump = true;
			toggled_crouch.Clear();
		}
		if (!ButtonState( UB_UP ) && heldJump)
		{
			heldJump = false;
		}
...
void idUsercmdGenLocal::CmdButtons( void ) {
	...
	//bc
	if ( ButtonState ( UB_PICKER ))
	{
		cmd.buttons |= BUTTON_PICKER;
	}

	if ( ButtonState ( UB_ROTATE ))
	{
		cmd.buttons |= BUTTON_ROTATE;
	}
