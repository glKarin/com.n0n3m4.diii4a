...
idCVar com_developer( "com_developer", "0", CVAR_INTEGER|CVAR_SYSTEM|CVAR_NOCHEAT, "developer mode" );
...
public:
	...
	//bc
	virtual idStr			GetModDescription( idStr descPath );
...
const char *idFileSystemLocal::BuildOSPath( const char *base, const char *game, const char *relativePath ) {
	...
	if ( fs_caseSensitiveOS.GetBool()  || com_developer.GetBool() ) {
		...
		if ( strlen( relativePath ) > 0 && strstr( relativePath, GAME_NAME ) == NULL ) // if the game name is in the path 
		{
...
idModList *idFileSystemLocal::ListMods( void ) {
	...
	list->mods.Insert( "" );
	list->descriptions.Insert( "Quadrilateral Cowboy" );
...
void idFileSystemLocal::SetupGameDirectories( const char *gameName ) {
	...
	// accept abs path
	if (gameName && strlen(gameName) > 2
	&& (gameName[0] == '/' || gameName[0] == '\\' || gameName[1] == ':') ) {
		AddGameDirectory( gameName, "." );
	}
	else {
		// setup cdpath
...
idStr idFileSystemLocal::GetModDescription(idStr descPath)
{
	const int 	MAX_DESCRIPTION = 256;
	char 		desc[ MAX_DESCRIPTION ];

	FILE *f = OpenOSFile( descPath, "r" );
	
	if ( f )
	{
		if ( fgets( desc, MAX_DESCRIPTION, f ) )
		{
			//common->Printf("FOUND MOD %s\n", desc);
			fclose( f );
		}
		else
		{
			common->DWarning( "STEAM: error reading %s", descPath.c_str() );
			fclose( f );
		}
	}
