...
const idEventDef EV_CallFunctionArg( "callFunctionArg", "sd" );
const idEventDef EV_SetNeverDormant( "setNeverDormant", "d" );

const idEventDef EV_SetFrobbable( "setFrobbable", "d" ); //bc
const idEventDef EV_deck_print( "deck_print", "s" ); 
const idEventDef EV_deck_printline( "deck_printline", "s" ); 
const idEventDef EV_deck_cls( "deck_cls" );
const idEventDef EV_SetCameraTarget( "setCameraTarget", "e" );
const idEventDef EV_ClearCameraTarget( "clearCameraTarget", NULL );
const idEventDef EV_getNearestEnemy( "getNearestEnemy", NULL, 'e' );
const idEventDef EV_canSeeEntity( "canSeeEntity", "e", 'd' );
const idEventDef EV_GetIndex( "getIndex", NULL, 'd' );
const idEventDef EV_SetSolid( "setSolid", "d" ); 
const idEventDef EV_SetClipModel( "setClipModel", "s" ); 

//bc flite
//const idEventDef EV_flitespeech( "flitespeech", "s", 'd' );
...
ABSTRACT_DECLARATION( idClass, idEntity )
	...
	EVENT( EV_CallFunctionArg,		idEntity::Event_CallFunctionArg )
	EVENT( EV_SetNeverDormant,		idEntity::Event_SetNeverDormant )

	EVENT( EV_SetFrobbable,			idEntity::Event_SetFrobbable ) //bc
	EVENT( EV_deck_print,			idEntity::Event_deck_print)
	EVENT( EV_deck_printline,		idEntity::Event_deck_printline)
	EVENT( EV_SetSolid,				idEntity::Event_SetSolid) //bc
	EVENT( EV_SetClipModel,			idEntity::Event_SetClipModel) //bc

	//bc flite
	//EVENT( EV_flitespeech,			idEntity::Event_flitespeech) //bc

	
	EVENT( EV_deck_cls,				idEntity::Event_deck_cls)
	EVENT( EV_SetCameraTarget,		idEntity::Event_SetCameraTarget)
	EVENT( EV_ClearCameraTarget,	idEntity::Event_ClearCameraTarget)
	EVENT( EV_getNearestEnemy,		idEntity::Event_getNearestEnemy)
...
void idEntity::Save( idSaveGame *savefile ) const {
	...
	//BC
	savefile->WriteBool(noGrab);
	savefile->WriteBool(isFrobbable);
	savefile->WriteInt(uiAlarmTimer);
	savefile->WriteInt(uiHackTimer);
	savefile->WriteInt(lastThrowTime);
}
...
void idEntity::Restore( idRestoreGame *savefile ) {
	...
	//BC
	savefile->ReadBool(noGrab);
	savefile->ReadBool(isFrobbable);
	savefile->ReadInt(uiAlarmTimer);
	savefile->ReadInt(uiHackTimer);
	savefile->ReadInt(lastThrowTime);
}
...
void idEntity::Present( void ) {
	...
	if ( cameraTarget && gameLocal.InPlayerPVS( this ) ) {
		...
		if ( spawnArgs.GetBool( "isfreezeframe", "0" ))
		{
			renderEntity.isFreezeFrame = 1;
		}
	}
...
bool idEntity::RunPhysics( void ) {
	...
	//BC commenting this out might break everything.
	//if ( gameLocal.isClient )
	{
		//return true;
	}
	...
	for ( part = this; part != NULL; part = part->teamChain ) {
		...
		if ( part->physics )
		{
			...
			//gameLocal.Printf("     Starttime: %d  reachedTime: %d  endTime: %d\n",  startTime, reachedTime, endTime);
...
void idEntity::Damage( idEntity *inflictor, idEntity *attacker, const idVec3 &dir, 
	...
	if (damage > 0)
		attacker->DamageFeedback( this, inflictor, damage ); //bc
...
void idEntity::Event_CallFunctionArg( const char *funcname, int arg )
{
	idThread*		thread;
	const function_t *funcCall;


	funcCall = this->scriptObject.GetFunction( funcname );

	if (!funcCall)
	{
		common->Printf("CallFunctionArg: cannot find function '%s', arg '%d'\n", funcname, arg);
		return;
	}
	
	// start a thread that will run immediately and be destroyed
	thread = new idThread( funcCall );
	thread->CallFunctionArgs(funcCall, true, "ef", this, (float)arg);
	thread->Execute();
	delete thread;	
}
...
void idEntity::Event_getNearestEnemy( )
{
	idEntity	*ent;
	idActor		*actor;
	float		bestDist;
	...
	{
		if (!ent || ent->fl.hidden || ent->fl.isDormant || !ent->IsType(idActor::Type) || ent->IsType(idPlayer::Type))
			continue;
		...
		delta = this->GetPhysics()->GetOrigin() - actor->GetPhysics()->GetOrigin();
		dist = delta.LengthSqr();

		if ( ( dist < bestDist ) && CanSeeEntity( actor ))
		{
			bestDist = dist;
			bestEnemy = actor;
		}
...
void idEntity::Event_canSeeEntity( idEntity *ent )
{
	if ( !ent )
	{
		idThread::ReturnInt( false );
...
bool idEntity::CanSeeEntity( idEntity *other )
{
	trace_t		tr;
	idVec3		selfPos;
	idVec3		otherPos;	

	selfPos = this->GetPhysics()->GetOrigin();
	otherPos = this->GetPhysics()->GetOrigin();

	//aim for center of mass.
	selfPos.z += this->GetPhysics()->GetBounds().GetCenter().z;
	otherPos.z += other->GetPhysics()->GetBounds().GetCenter().z;

	gameLocal.clip.TracePoint( tr, selfPos, otherPos, MASK_OPAQUE, this );
	if ( tr.fraction >= 1.0f || ( gameLocal.GetTraceEntity( tr ) == other) )
		return true;
...
{
	ent->cameraTarget = ent; //assign the cameraTarget of the cameraTarget (yup)
	this->cameraTarget = ent; //assign the cameraTarget of this entity.

	UpdateVisuals();  //force camera update.
}

void idEntity::Event_ClearCameraTarget(  )
{
	this->cameraTarget = NULL; //assign the cameraTarget of this entity.
	this->renderEntity.remoteRenderView = NULL;
...
/*bc flite
void idEntity::Event_flitespeech( const char *text )
{
	soundSystem->SynthesizeSpeech( text );

	int length;
	this->StartSoundShader( declManager->FindSound( "flite" ), SCHANNEL_ANY, 0, false, &length );

	idThread::ReturnInt(length);
}
*/

void idEntity::Event_SetClipModel( const char *clipModelName )
{
	idTraceModel trm;

	if ( !collisionModelManager->TrmFromModel( clipModelName, trm ) )
	{
		gameLocal.Error( "SetClipModel '%s': cannot load collision model %s", name.c_str(), clipModelName );
		return;
	}
	else
	{
		this->GetPhysics()->SetClipModel( new idClipModel( trm ), 0.5f );
	}
}
...
//print string to user deck GUI.
void idEntity::Event_deck_print( const char* text )
{
	if ( gameLocal.GetLocalPlayer()->deckList == NULL)
		return;

	//BC 7-22-2016 fixed possible deck gui crash when loading savegame.
	if (!gameLocal.GetLocalPlayer()->deckList->IsConfigured())
		return;

	gameLocal.GetLocalPlayer()->deckList->Push( text );
}

void idEntity::Event_deck_printline( const char* text )
{
	if ( gameLocal.GetLocalPlayer()->deckList == NULL)
		return;
	...
	int totalCount = gameLocal.GetLocalPlayer()->deckList->Num();
	gameLocal.GetLocalPlayer()->deckList->Add(totalCount - 1, text);
}


void idEntity::Event_deck_cls( void )
{
	if ( gameLocal.GetLocalPlayer()->deckList == NULL)
		return;
	...
	gameLocal.GetLocalPlayer()->deckList->Clear();	
}
...
int idAnimatedEntity::Event_PlayAnim( const char* animname, int animBlendFrames)
{
	int anim;
	int channel = 0;
	int animDoneTime = 0;
