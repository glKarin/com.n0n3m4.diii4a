    ...
    while(TRUE)
    {
      ...
      wait(m_fMoveFrequency) {
        ...
        on (EBegin) : {
          ...
          // if between two fire/hit monents
          } else {
...
//**********************************************************
  ...
  ChargeHitEnemy(EVoid) 
  {
    ...
    wait(GetModelObject()->GetAnimLength(m_iChargeHitAnimation)) {
      on (EBegin) : { resume; }
      on (ETimer) : { stop; }
      // ignore damages
      on (EDamage) : { resume; }
    ...
    if (!GetModelObject()->IsAnimFinished()) {
      ...
      wait(GetModelObject()->GetCurrentAnimLength() - GetModelObject()->GetPassedTime()) {
        on (EBegin) : { resume; }
        on (ETimer) : { stop; }
        // if timer expired
        on (EReminder) : {
