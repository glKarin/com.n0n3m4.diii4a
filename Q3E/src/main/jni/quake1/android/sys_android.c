...
// Do not catch signal
qbool no_handle_signals = false;

// enable redirect stdout/stderr to file
static qbool redirect_output_to_file = true;

// app paused
volatile qbool paused = false;

// Continue when missing OpenGL context
volatile qbool continue_when_no_gl_context = false;

// using mouse
qbool mouse_available = false;
...
static volatile qbool window_changed = false;
...
static qbool 			signaled[ MAX_TRIGGER_EVENTS ];
static qbool			waiting[ MAX_TRIGGER_EVENTS ];
...
void Android_GrabMouseCursor(qbool grabIt)
{
...
{
    Con_Printf("Call JNI::tmpfile.\n");
    FILE *f = itmpfile ? itmpfile() : NULL;
    if (!f) {
        Con_Printf("JNI::tmpfile failed: %s", strerror(errno));
    }
    return f;
}
...
void Q3E_MotionEvent(float dx, float dy)
{
    ...
    if (in_windowmouse_x>screen_width-1) in_windowmouse_x=screen_width-1;
    in_windowmouse_y += dy;
    if (in_windowmouse_y<0) in_windowmouse_y=0;
    if (in_windowmouse_y>screen_height-1) in_windowmouse_y=screen_height-1;
}
