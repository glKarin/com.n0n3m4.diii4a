...
#if !defined(USE_OPENGLES) //karin: use glDrawArrays on GLES 1.1
    qglBegin(GL_QUADS);
...
void DrawLineLoop(const vec2_t* points, int count, int stipple_factor, int stipple_mask) {
...
#if !defined(USE_OPENGLES) //karin: not support LineStipple on GLES1
	if (stipple_factor) {
	...
	GLfloat *vtx = malloc(sizeof(GLfloat) * count * 2);

	for (i = 0; i < count; i++) {
		vtx[i * 2] = points[i][0];
		vtx[i * 2 + 1] = points[i][1];
	}

	qglVertexPointer  ( 2, GL_FLOAT, 0, vtx );
	qglDrawArrays( GL_LINE_LOOP, 0, 4 );
	if (text)
		qglEnableClientState( GL_TEXTURE_COORD_ARRAY );
	if (glcol)
		qglEnableClientState( GL_COLOR_ARRAY );
