...
extern "C" float Android_GetConsoleMaxHeightFrac(float frac);
#endif
...
#ifdef __ANDROID__ //karin: limit console max height
        float frac = Android_GetConsoleMaxHeightFrac(0.5f);		// half screen
        if(frac < 0.5f)
        {
            rect.size.height = (float)uid.vidHeight * frac;
        }
#endif
    }
