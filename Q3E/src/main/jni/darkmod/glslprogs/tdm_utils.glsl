...
float sampleZWithDerivs(sampler2D depthTexture, mat4 projectionMatrix, vec2 tcPoint, vec2 tcDelta, out vec2 Zderivs) {
	// set cross pattern
	vec2 tcMoreX = tcPoint + vec2(tcDelta.x, 0.0);
	vec2 tcLessX = tcPoint - vec2(tcDelta.x, 0.0);
	vec2 tcMoreY = tcPoint + vec2(0.0, tcDelta.y);
	vec2 tcLessY = tcPoint - vec2(0.0, tcDelta.y);
