...
vec3 CubeMapDirectionToUv(vec3 v, out int faceIdx) {
	...
	if(maxV == v.x) {
		...
		v1 = v.zyx * vec3(1.0, -1.0, 1.0);
		faceIdx = 1;
	}
	else if(maxV == v.y) {
		v1 = v.xzy * vec3(1.0, 1.0, -1.0);
		faceIdx = 2;
	}
	else if(maxV == -v.y) {
		v1 = v.xzy * vec3(1.0, -1.0, 1.0);
...
#endif
	vec2 wgt = fract(shadow2d * float(texSize.x) - 0.5); // float(texSize)
...
float computeShadowMapCoefficient(
	...
	vec3 nonCollinear = vec3(1.0, 0.0, 0.0);
	if (absL.x == maxAbsL)
		nonCollinear = vec3(0.0, 1.0, 0.0);
