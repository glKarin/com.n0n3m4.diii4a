...
BOBrick::BOBrick( BOEntity *_ent, float _x, float _y, float _width, float _height ) {
	...
	ent->SetMaterial( "textures/materials_used_in_source/busting_out_game/brick" );
...
void idGameBustOutWindow::CommonInit() {
	...
	// Precache images
	declManager->FindMaterial( "textures/materials_used_in_source/busting_out_game/ball" );
	declManager->FindMaterial( "textures/materials_used_in_source/busting_out_game/doublepaddle" );
	declManager->FindMaterial( "textures/materials_used_in_source/busting_out_game/powerup_big_paddle" );
	declManager->FindMaterial( "textures/materials_used_in_source/busting_out_game/powerup_multiball" );
	declManager->FindMaterial( "textures/materials_used_in_source/busting_out_game/brick" );

	// Precache sounds
	declManager->FindSound( "source_default_arcade_ballbounce" );
	declManager->FindSound( "source_default_arcade_brickhit" );
	declManager->FindSound( "source_default_arcade_missedball" );
	declManager->FindSound( "source_default_arcade_sadsound" );
	declManager->FindSound( "source_default_arcade_extraball" );
	declManager->FindSound( "source_default_arcade_powerup" );
	...
	paddle->ent->SetMaterial( "textures/materials_used_in_source/bear_shooting_game/paddle" );
}
...
void idGameBustOutWindow::UpdateScore() {
	...
	if ( gameScore >= nextBallScore ) {
		...
		// Play sound
		session->sw->PlayShaderDirectly( "source_default_arcade_extraball", S_UNIQUE_CHANNEL );
...
void idGameBustOutWindow::LoadBoardFiles( void ) {
	...
	for ( i=0; i<numLevels; i++ ) {
		byte *pic;
		idStr	name = "textures/images_used_in_source/level";
		name += (i+1);
		name += ".tga";
...
BOEntity * idGameBustOutWindow::CreateNewBall( void ) {
	...
	ball->SetMaterial( "textures/materials_used_in_source/busting_out_game/ball" );
...
BOEntity * idGameBustOutWindow::CreatePowerup( BOBrick *brick ) {
	...
	switch( powerEnt->powerup ) {
		case POWERUP_BIGPADDLE:
			powerEnt->SetMaterial( "textures/materials_used_in_source/busting_out_game/powerup_big_paddle" );
			break;
		case POWERUP_MULTIBALL:
			powerEnt->SetMaterial( "textures/materials_used_in_source/busting_out_game/powerup_multiball" );
			break;
		default:
...
void idGameBustOutWindow::UpdatePowerups( void ) {
	...
	for ( int i=0; i < powerUps.Num(); i++ ) {
		...
		if ( collision != COLLIDE_NONE ) {
			...
			// Play the sound
			session->sw->PlayShaderDirectly( "source_default_arcade_powerup", S_UNIQUE_CHANNEL );
...
void idGameBustOutWindow::UpdatePaddle( void ) {
	...
	if ( bigPaddleTime > gui->GetTime() ) {
		...
		paddle->ent->SetMaterial( "textures/materials_used_in_source/busting_out_game/doublepaddle" );
	} else {
...
void idGameBustOutWindow::UpdateBall( void ) {
	...
	for ( ballnum = 0; ballnum < balls.Num(); ballnum++ ) {
		...
		if ( playSoundBounce ) {
			session->sw->PlayShaderDirectly( "source_default_arcade_ballbounce", bounceChannel );
		} else if ( playSoundBrick ) {
			session->sw->PlayShaderDirectly( "source_default_arcade_brickhit", bounceChannel );
		}
	...
	if ( ballsInPlay == 0 ) {
		if ( ballsRemaining == 0 ) {
			...
			// Game Over sound
			session->sw->PlayShaderDirectly( "source_default_arcade_sadsound", S_UNIQUE_CHANNEL );
		} else {
			ballsRemaining--;

			// Ball was lost, but game is not over
			session->sw->PlayShaderDirectly( "source_default_arcade_missedball", S_UNIQUE_CHANNEL );
		}
