...
const char *idListWindow::HandleEvent(const sysEvent_t *event, bool *updateVisuals) {
	...
	if ( event->evType == SE_KEY ) {
		...
		if ( key == K_MOUSE1 || key == K_MOUSE2 ) {
			...
			if ( scroller->Contains(gui->CursorX(), gui->CursorY()) ) {
				// boyette space command begin - we don't want the multiselect box to do anything when the scroller is clicked on or affected.
	...
	// boyette space command begin - we don't want the multiselect box to do anything with listdefs as of right now.
	if ( gui->multiselect_box_active == true && allow_multiselect_box_events ) {
		gui->multiselect_box_active = false;
		gui->EndMultiselectBox();
	}
...
{
	const char *thumbImage = "ui/assets/scrollbar_thumb.tga";
	const char *barImage = "ui/assets/scrollbarv.tga";
	const char *scrollerName = "_scrollerWinV";

	if (horizontal) {
		barImage = "ui/assets/scrollbarh.tga";
