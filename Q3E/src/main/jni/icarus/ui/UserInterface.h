...
const int TOOLTIP_ACTIVATION_DELAY_MS = 250;
const float TOOLTIP_LETTER_WIDTH = 8.92f; // BOYETE NOTE IMPORTANT: if the textScale of the tooltip window is changed on a GUI then this will need to be changed accordingly - currently it is set to 0.37
const float TOOLTIP_TEXT_PADDING = 14.0f;
const float TOOLTIP_BOX_HEIGHT = 27.0f; // BOYETE NOTE IMPORTANT: If this is change on a GUI then this will need to be changed accordingly - currently it is set to 27
...
// boyette space command begin
	virtual void				SetCursorImage( int cursortochangeto ) = 0;
	virtual void				ScrollListGUIToBottom( idStr winName ) = 0;
	virtual void				ScrollListGUIToTop( idStr winName ) = 0;
	virtual void				BeginMultiselectBox() = 0;
	virtual void				EndMultiselectBox() = 0;
	virtual void				LoopThroughChildrenAndRunScript( idWindow* Parent ) = 0;
	bool						multiselect_box_active;
	float						multiselect_box_origin_x;
	float						multiselect_box_origin_y;
	virtual int					GetTime() const = 0;
	
	virtual bool				HasEditWindowAsFocus() = 0;
	virtual void				ClearFocusWindow() = 0;

	int							time_Activated;
	int							frame_Activated;

	virtual void				UpdateTooltip() = 0;
	virtual void				GetTooltipText( idWindow* Parent ) = 0;
	int							last_cursor_move_time;
	float						old_cursor_x;
	float						old_cursor_y;
	bool						tooltip_checked_this_mouse_position;
	idStr						tooltip_text;
	bool						tooltip_visible;
	float						tooltip_width;
	float						tooltip_x_offset;
	float						tooltip_y_offset;

	bool						handle_time_commands_when_not_active;
...
public:
	...
	virtual void				WritePrecacheCommands( idFile *f ) = 0;
								...
								// Sets the size for 1280x960 adjustment.
