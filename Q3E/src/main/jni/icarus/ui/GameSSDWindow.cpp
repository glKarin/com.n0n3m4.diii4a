...
#define V_WIDTH 1280.0f
#define V_HEIGHT 960.0f
...
void SSDPowerup::OnHit(int key) {
	...
	if(powerupState == POWERUP_STATE_CLOSED) {
		...
		game->entities.Append(explosion);
		game->PlaySound("source_default_arcade_explode");
...
void idGameSSDWindow::CommonInit() {
	...
	// Precache sounds
	declManager->FindSound( "source_default_arcade_blaster" );
	declManager->FindSound( "source_default_arcade_capture " );
	declManager->FindSound( "source_default_arcade_explode" );
...
void idGameSSDWindow::FireWeapon(int key) {
	...
	if(key == K_MOUSE1) {
		...
		PlaySound("source_default_arcade_blaster");
...
void idGameSSDWindow::HitAsteroid(SSDAsteroid* asteroid, int key) {
	...
	if(asteroid->health <= 0) {
		...
		PlaySound("source_default_arcade_explode");
...
void idGameSSDWindow::AstronautStruckPlayer(SSDAstronaut* astronaut) {
	...
	entities.Append(explosion);
	PlaySound("source_default_arcade_capture");
