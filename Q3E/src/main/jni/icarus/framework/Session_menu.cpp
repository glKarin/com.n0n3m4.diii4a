...
void idSessionLocal::StartMenu( bool playIntro ) {
	...
	if(fileSystem->HasD3XP()) {
		guiMainMenu->SetStateString("game_list", "Icarus SCS" ); // BOYETTE NOTE: was originally guiMainMenu->SetStateString("game_list", common->GetLanguageDict()->GetString( "#str_07202" ) );
	} else {
		guiMainMenu->SetStateString("game_list", "Icarus SCS" ); // BOYETTE NOTE: was originally guiMainMenu->SetStateString("game_list", common->GetLanguageDict()->GetString( "#str_07212" ) );
	}
...
void idSessionLocal::SetGUI( idUserInterface *gui, HandleGuiCommand_t handle ) {
	...
	if ( guiActive && game && guiActive == game->PlayerGUIOverlay() ) {
		cmd = guiActive->HandleEvent( &ev, game->GetTimeGroupTime(1) );
		guiActive->Activate( true, game->GetTimeGroupTime(1) );
	} else {
...
/*
===============
idSessionLocal::SetGUIActive
===============
*/
void idSessionLocal::SetGUIActive( idUserInterface *gui ) {
	guiActive = gui;
}
...
void idSessionLocal::SetSaveGameGuiVars( void ) {
	...
	guiActive->SetStateString( "loadgame_shot", "textures/images_used_in_source/blank_level_shot.tga" );
...
void idSessionLocal::SetMainMenuGuiVars( void ) {
	...
	// BOYETTE BEGIN
	guiMainMenu->SetStateString( "game_version", "VERSION: " + idStr(ENGINE_VERSION) + "." + idStr(BUILD_NUMBER) ); 
	// BOYETTE END
...
#endif
	guiMainMenu->SetStateString( "browser_levelshot", "textures/images_used_in_source/default_map_splashscreen.tga" );
	...
	// BOYETTE BEGIN
	guiMainMenu->SetStateString( "display_device_supported_resolutions", common->displayDeviceSupportedResolutions );
	guiMainMenu->SetStateString( "display_device_supported_r_modes", common->displayDeviceSupportedRModes );

	guiMainMenu->SetStateBool( "not_in_iron_man_mode", !cvarSystem->GetCVarBool( "g_inIronManMode" ) );
	// BOYETTE END
...
/*
==============
idSessionLocal::ReloadMaterialImages
==============
*/
void idSessionLocal::ReloadMaterialImages( const char *material_name ) {
	const idMaterial *material;
	material = declManager->FindMaterial( material_name );
	if ( material ) {
...
void idSessionLocal::HandleRestartMenuCommands( const char *menuCommand ) {
	...
	for( icmd = 0; icmd < args.Argc(); ) {
		...
		// BEGIN BOYETTE ADDED 07 14 2016: - because saves don't work and and we autosave sometimes - it was causing it to attempt to load the autosave and then it was crashing - this just assumes there is no save and just reloads the map
		// BOYETTE NOTE TODO IMPORTANT: when we get saves working - we can make sure to change the restart.gui back to using "restart" instead of "plain_restart" 
		if ( !idStr::Icmp( cmd, "plain_restart" ) ) {
			//if ( !LoadGame( GetAutoSaveName( mapSpawnData.serverInfo.GetString("si_map") ) ) ) {
				// If we can't load the autosave then just restart the map
				// BOYETTE ADJUSTED SCREEN WIPE BEGIN
				restartingCurrentMap = true;
...
void idSessionLocal::HandleMainMenuCommands( const char *menuCommand ) {
	...
	for( icmd = 0; icmd < args.Argc(); ) {
		...
		if ( !idStr::Icmp( cmd, "startGame" ) ) {
			//cvarSystem->SetCVarInteger( "g_skill", guiMainMenu->State().GetInt( "skill" ) ); // BOYETTE NOTE IMPORTANT: this is why the medical module was killing us. Fixed it so all map loading on the gui set the skill to 1 - also commented this out to avoid any future potential problems with it - might revisit oncve we start working on difficulty levels.
			if ( icmd < args.Argc() ) {
				StartNewGame( args.Argv( icmd++ ) );

				if ( icmd < args.Argc() ) {
					if ( !idStr::Icmp(args.Argv( icmd++ ),"ironman") ) {
						cvarSystem->SetCVarBool( "g_inIronManMode", true );
					}
				} else {
				...
				StartNewGame( "radiant1" ); // BOYETTE NOTE: was originally: StartNewGame( "game/mars_city1" );
#else
				StartNewGame( "radiant1" ); // BOYETTE NOTE: was originally: StartNewGame( "game/demo_mars_city1" );
#endif
			}
			...
			/* // BOYETTE NOTE IMPORTANT BEGIN: this is where the intro video is played - currently it is just a moving starfield with the icarus symbol flying towards the screen - commented it out because the video resoution is too low for this to look good
...
// BOYETTE SPACE COMMAND BEGIN
		if ( !idStr::Icmp( cmd, "OpenGameWebsite" ) ) {
#ifdef _WIN32
			ShellExecute(NULL, "open", "http://midwestvideogames.com", NULL, NULL, SW_SHOWNORMAL); // we could do SW_SHOWMAXIMIZED instead - not a big deal
			common->Quit();
			continue;
#endif
		}
		if ( !idStr::Icmp( cmd, "OpenGameUpdateWebsite" ) ) {
#ifdef _WIN32
			ShellExecute(NULL, "open", "http://www.midwestvideogames.com/updates/", NULL, NULL, SW_SHOWNORMAL); // we could do SW_SHOWMAXIMIZED instead - not a big deal
			common->Quit();
			continue;
#endif
		}
		if ( !idStr::Icmp( cmd, "OpenGameGreenlightPage" ) ) {
#ifdef _WIN32
			ShellExecute(NULL, "open", "http://steamcommunity.com/sharedfiles/filedetails/?id=785909840", NULL, NULL, SW_SHOWNORMAL); // we could do SW_SHOWMAXIMIZED instead - not a big deal
			common->Quit();
			continue;
#endif
		}
...
void idSessionLocal::HandleInGameCommands( const char *menuCommand ) {
	...
	if ( !idStr::Icmp( cmd, "close" ) ) {
		if ( guiActive ) {
			...
			if ( guiActive && game && guiActive == game->PlayerGUIOverlay() ) {
				cmd = guiActive->HandleEvent( &ev, game->GetTimeGroupTime(1) );
				guiActive->Activate( false, game->GetTimeGroupTime(1) );
			} else {
				cmd = guiActive->HandleEvent( &ev, com_frameTime );
...
void idSessionLocal::DispatchCommand( idUserInterface *gui, const char *menuCommand, bool doIngame ) {
	...
	} else if ( gui == game->PlayerGUIOverlay() ) {
		if ( game ) {
			game->HandleGuiCommandsOnPlayer(menuCommand);
		}
		//game->PlayerGUIOverlay()->
		// these commands are handled on idPLayer
...
void idSessionLocal::MenuEvent( const sysEvent_t *event ) {
	...
	if ( guiActive && game && guiActive == game->PlayerGUIOverlay() ) {
		...
		if ( event->evType == SE_KEY ) {
			common->Printf( "event->evType == SE_KEY\n" );
			if ( event->evValue2 == 1 ) {
				common->Printf( "event->evValue2 == 1\n" );
				if ( event->evValue == idKeyInput::StringToKeyNum(idKeyInput::KeysFromBinding("_impulse41")) ) {
					common->Printf( "event->evValue == idKeyInput::StringToKeyNum(idKeyInput::KeysFromBinding(impulse41))\n" );
					if ( usercmdGen->GetDirectUsercmd().impulse == IMPULSE_41 ) {
						common->Printf( "usercmdGen->GetDirectUsercmd().impulse == IMPULSE_41\n" );
						if ( !guiActive->HasEditWindowAsFocus() ) {
							common->Printf( "!guiActive->HasEditWindowAsFocus()\n" );
							if ( (idLib::frameNumber - guiActive->frame_Activated) > 1 ) {
								common->Printf( "(idLib::frameNumber - guiActive->frame_Activated) > 1\n" );
							}
						}
					}
				}
		...
		*/
		if ( event->evType == SE_KEY && event->evValue2 == 1 && event->evValue == K_ESCAPE ) {
			guiActive->ClearFocusWindow();
			game->HandleESC( NULL );
			return;
		} else if ( event->evType == SE_KEY && event->evValue2 == 1 && event->evValue == idKeyInput::StringToKeyNum(idKeyInput::KeysFromBinding("_impulse41")) && usercmdGen->GetDirectUsercmd().impulse == IMPULSE_41 && !guiActive->HasEditWindowAsFocus() && (idLib::frameNumber - guiActive->frame_Activated) > 1 ) { //&& (game->GetTimeGroupTime(1) - guiActive->time_Activated) > 400 ) { // boyette note: this works fine but there might be a better way to do this - // the framenumber stuff is to fix a problem with the menu closing right on the next frame when the captain menu is opened.
			guiActive->ClearFocusWindow();
			game->HandleESC( NULL );
			return;
		} else {
			menuCommand = guiActive->HandleEvent( event, game->GetTimeGroupTime(1) );
		}
	} else {
...
void idSessionLocal::GuiFrameEvents() {
	...
	ev.evType = SE_NONE;
	if ( guiActive && game && guiActive == game->PlayerGUIOverlay() ) {
		cmd = gui->HandleEvent( &ev, game->GetTimeGroupTime(1) );
	} else {
