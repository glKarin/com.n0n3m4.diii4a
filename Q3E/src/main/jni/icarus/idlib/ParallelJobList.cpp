...
const char * jobNames[] = {
	...
	ASSERT_ENUM_STRING( JOBLIST_GAME_LOGIC,			2 ),
		...
		// execute the next job
		{
			...
			if ( jobs_longJobMicroSec.GetInteger() > 0 ) {
				if ( jobEnd - jobStart > jobs_longJobMicroSec.GetInteger()
					...
					// BOYETTE NOTE IMPORTANT: Printf is not thread safe. It will cause a thread to deadlock. The application might still run but the thread will be deadlocked.
