...
// BOYETTE SPACE COMMAND BEGIN
	session->writeDemo->WriteInt( (int&)ent->originalCustomSkin );
...
// BOYETTE SPACE COMMAND BEGIN
	if ( ent->originalCustomSkin ) {
		session->writeDemo->WriteHashString( ent->originalCustomSkin->GetName() );
	}
// BOYETTE SPACE COMMAND END
	if ( ent->hModel ) {
...
void	idRenderWorldLocal::ReadRenderEntity() {
	...
	// BOYETTE SPACE COMMAND BEGIN
	session->readDemo->ReadInt( (int&)ent.originalCustomSkin );
	// BOYETTE SPACE COMMAN
	...
	// BOYETTE SPACE COMMAND BEGIN
	if ( ent.originalCustomSkin ) {
		ent.originalCustomSkin = declManager->FindSkin( session->readDemo->ReadHashString() );
	}
	// BOYETTE SPACE COMMAND END
	if ( ent.hModel ) {
