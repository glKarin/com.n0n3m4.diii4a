...
void idRenderWorldLocal::AddWorldModelEntities() {
	...
	for ( i = 0 ; i < numPortalAreas ; i++ ) {
		...
		for ( int j = 0; j < hModel->NumSurfaces(); j++ ) {
			...
			if ( surf->shader->GetName() == idStr( "textures/smf/portal_sky" ) || surf->shader->GetName() == idStr( "textures/materials_used_in_source/portal_sky" ) ) { // BOYETE NOTE: added textures/materials_used_in_source/portal_sky - we can get rid of textures/smf/portal_sky from this if statement when it is no longer used in any maps - it is identical to textures/smf/portal_sky
