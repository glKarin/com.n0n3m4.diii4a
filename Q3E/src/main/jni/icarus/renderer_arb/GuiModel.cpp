...
void idGuiModel::EmitFullScreen( void ) {
	...
	// qglOrtho( 0, 1280, 960, 0, 0, 1 );		// always assume 1280x960 virtual coordinates
	viewDef->projectionMatrix[0] = 2.0f / 1280.0f;
	viewDef->projectionMatrix[5] = -2.0f / 960.0f;
...
x/y/w/h are in the 0,0 to 1280,960 range
=============
*/
void idGuiModel::DrawStretchPic( float x, float y, float w, float h, float s1, float t1, float s2, float t2, const idMaterial *hShader ) {
	...
	DrawStretchPic( &verts[0], &indexes[0], 4, 6, hShader, false, 0.0f, 0.0f, 1280.0f, 960.0f );
}
