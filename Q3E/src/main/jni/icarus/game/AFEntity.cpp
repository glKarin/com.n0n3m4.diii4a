...
void idHarvestable::Think() {
	...
	if(startTime && gameLocal.time - startTime > giveDelay && ! given) {
	...
	if(startTime && gameLocal.time - startTime > removeDelay) {
...
void idHarvestable::BeginBurn() {
	...
	parent->SetShaderParm( SHADERPARM_TIME_OF_DEATH, gameLocal.time * 0.001f );
	...
	if(head) {
		...
		head->SetShaderParm( SHADERPARM_TIME_OF_DEATH, gameLocal.time * 0.001f );
	}
...
bool idHarvestable::GetFxOrientationAxis(idMat3& mat) {
	...
	if(!fxOrient.Icmp("up")) {
		...
		if ( joint != INVALID_JOINT ) {
			thePlayer->weapon.GetEntity()->GetJointWorldTransform( joint, gameLocal.time, joint_origin, joint_axis );
		} else {
