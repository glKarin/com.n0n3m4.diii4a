...
// BOYETTE SPACE COMMAND BEGIN
	int						captaintestnumber;		// boyette mod
	virtual void			RecieveVolley();		// boyette mod
	int						stargridpositionx;		// boyette mod
	int						stargridpositiony;		// boyette mod
	int						stargriddestinationx;	// boyette mod
	int						stargriddestinationy;	// boyette mod

	bool					was_killed;				// boyette mod - this will start off as true - even if health is zero we will think of all entities as starting off as initially alive. This may seem strange but we need a way to know if an entity was killed or not. This will get called in subclasses when an entity is killed.

	// boyette idDoor Custom Damage begin
	bool					was_just_damaged;
	bool					was_just_repaired;
	// boyette idDoor Custom Damage end

	int						entity_max_health;	// boyette space command mod
	
	//idEntity*				TargetEntityInSpace;			// boyette mod - not necessary - we can put it in the derived class.
	...
	virtual	void		ActivateGuis( bool activate, int time );
	virtual	void		ActivateGuisIfNotActive( bool activate, int time );

	virtual void		TakeRemoteRenderViewScreenshot( const char* screenshot_filename, int image_width = SCREEN_WIDTH, int image_height = SCREEN_HEIGHT );

	// boyette transporter material effect begin
	virtual	void		BeginTransporterMaterialShaderEffect( const idVec3& theme_colors );
	virtual	void		EndTransporterMaterialShaderEffect();
	virtual	void		ScheduleEndTransporterMaterialShaderEffect( int ms_from_now = 0 );  // boyette mod
	// boyette transporter material effect end

	virtual	void		TemporarilySetThisEntityGravityToZero( int ms_to_stay_at_zero = 1 );
...
// BOYETTE SPACE COMMAND END
	...
	int						team;					// Entities need a team so we know which ones to have the AI attack.
...
public:
	...
	bool					StartSoundShader( const idSoundShader *shader, const s_channelType channel, int soundShaderFlags, bool broadcast, int *length, bool allowSlow = true );
...
private:
	...
	void					Event_EndTransporterMaterialShaderEffect( void );
	// BOYETTE SPACE COMMAND END
};
...
ID_INLINE void SetTimeState::PushState( int timeGroup ) {
	...
	if ( !gameLocal.isMultiplayer ) {
		...
		if ( gameLocal.time == gameLocal.time ) {
			previousFast = false;
		}
		else {
			previousFast = true;
