...
const idEventDef EV_Weapon_OwnerBeginsUsingSpaceCommandTablet( "ownerBeginsUsingSpaceCommandTablet" );
const idEventDef EV_Weapon_OwnerStopsUsingSpaceCommandTablet( "ownerStopsUsingSpaceCommandTablet" );
const idEventDef EV_Weapon_OwnerActivatesSpaceCommandTabletBeam( "ownerActivatesSpaceCommandTabletBeam" );
const idEventDef EV_Weapon_OwnerDeactivatesSpaceCommandTabletBeam( "ownerDeactivatesSpaceCommandTabletBeam" );
const idEventDef EV_Weapon_OwnerSpaceCommandTabletBeamIsActive( "ownerSpaceCommandTabletBeamIsActive", NULL,'d' );
const idEventDef EV_Weapon_GiveAmmo( "giveAmmo", "d" );
...
CLASS_DECLARATION( idAnimatedEntity, idWeapon )
	...
	// boyette space command tablet begin
	EVENT( EV_Weapon_OwnerBeginsUsingSpaceCommandTablet,		idWeapon::Event_OwnerBeginsUsingSpaceCommandTablet )
	EVENT( EV_Weapon_OwnerStopsUsingSpaceCommandTablet,			idWeapon::Event_OwnerStopsUsingSpaceCommandTablet )
	EVENT( EV_Weapon_OwnerActivatesSpaceCommandTabletBeam,		idWeapon::Event_OwnerActivatesSpaceCommandTabletBeam )
	EVENT( EV_Weapon_OwnerDeactivatesSpaceCommandTabletBeam,	idWeapon::Event_OwnerDeactivatesSpaceCommandTabletBeam )
	EVENT( EV_Weapon_OwnerSpaceCommandTabletBeamIsActive,		idWeapon::Event_OwnerSpaceCommandTabletBeamIsActive )
	EVENT( EV_Weapon_GiveAmmo,									idWeapon::Event_GiveAmmo )
	// boyette space command tablet end
...
bool idWeapon::BloodSplat( float size ) {
	...
	const idMaterial *mtr = declManager->FindMaterial( "textures/materials_used_in_source/weapon_generic_blood_splat" );
...
/*
===============
idWeapon::Event_OwnerBeginsUsingSpaceCommandTablet
===============
*/
void idWeapon::Event_OwnerBeginsUsingSpaceCommandTablet( void ) {
	owner->using_space_command_tablet = true;
	owner->tabletfocusEnt = NULL; // to reset events on tablet gui
	SetRenderEntityGuisBools( "tablet_focus_entity_exists_and_is_not_door", false ); // to reset events on tablet gui
	HandleNamedEventOnGuis( "BeginUsingTablet" ); // to reset events on tablet gui
	HandleNamedEventOnGuis( "LostScannedEntity" ); // to reset events on tablet gui
	owner->UpdateSpaceCommandTabletGUIOnce(); // to reset info on tablet gui
}

/*
===============
idWeapon::Event_OwnerStopsUsingSpaceCommandTablet
===============
*/
void idWeapon::Event_OwnerStopsUsingSpaceCommandTablet( void ) {
	owner->using_space_command_tablet = false;
	owner->tabletfocusEnt = NULL; // to reset events on tablet gui
	SetRenderEntityGuisBools( "tablet_focus_entity_exists_and_is_not_door", false ); // to reset events on tablet gui
	HandleNamedEventOnGuis( "StopUsingTablet" ); // to reset events on tablet gui
	//HandleNamedEventOnGuis( "LostScannedEntity" ); // Don't need to do this as it will be done when it is raised again. to reset events on tablet gui
	owner->UpdateSpaceCommandTabletGUIOnce(); // to reset info on tablet gui
}

/*
===============
idWeapon::Event_OwnerActivatesSpaceCommandTabletBeam
===============
*/
void idWeapon::Event_OwnerActivatesSpaceCommandTabletBeam( void ) {
	owner->ActivateSpaceCommandTabletBeam();
}

/*
===============
idWeapon::Event_OwnerDeactivatesSpaceCommandTabletBeam
===============
*/
void idWeapon::Event_OwnerDeactivatesSpaceCommandTabletBeam( void ) {
	owner->DeactivateSpaceCommandTabletBeam();
}

/*
===============
idWeapon::Event_OwnerSpaceCommandTabletBeamIsActive
===============
*/
void idWeapon::Event_OwnerSpaceCommandTabletBeamIsActive( void ) {
	if ( owner && owner->space_command_tablet_beam_active ) {
...
/*
===============
idWeapon::Event_GiveAmmo
===============
*/
void idWeapon::Event_GiveAmmo( int amount ) {
	if ( gameLocal.isClient ) {
		return;
	}

	owner->inventory.GiveAmmo( ammoType, amount );
}
...
void idWeapon::Event_AddToClip( int amount ) {
	...
	ammoAvail = owner->inventory.HasAmmo( ammoType, 1 ) + AmmoInClip();
	// BOYETTE NOTE: was this originally but we changed it so the shotgun would work: // ammoAvail = owner->inventory.HasAmmo( ammoType, ammoRequired ) + AmmoInClip();
