...
idMover_Binary::idMover_Binary() {
	...
	door_disabled_time = 0;
	door_damaged_time = 0;
	parentship_security_module_efficiency_adjusted_max_health = 0;
...
void idMover_Binary::Save( idSaveGame *savefile ) const {
	...
	// BOYETTE SAVE BEGIN
	savefile->WriteInt( door_disabled_time );
	savefile->WriteInt( door_damaged_time );
	savefile->WriteInt( parentship_security_module_efficiency_adjusted_max_health );
	// BOYETTE SAVE END
}
...
void idMover_Binary::Restore( idRestoreGame *savefile ) {
	...
	savefile->ReadInt( door_disabled_time );
	savefile->ReadInt( door_damaged_time );
	savefile->ReadInt( parentship_security_module_efficiency_adjusted_max_health );
	// BOYETTE RESTORE END
}
...
void idMover_Binary::Spawn( void ) {
	...
	// boyette space command begin
	door_disabled_time = 0;
	SetShaderParm(8,(float)health / (float)entity_max_health);
	SetShaderParm(7,(float)health / (float)entity_max_health);
	door_damaged_time = 0;  // 2500 should be the maximum amount of time between attacks if an AI is trying to break through a door - similar to the way module buffing works. That we we can tell if a doors is under attack if this int is less than that.
	parentship_security_module_efficiency_adjusted_max_health = entity_max_health;
	idEntity::team = spawnArgs.GetInt( "idEntity_team", idStr(idEntity::team) );
	// boyette space command end
}
...
void idMover_Binary::SetMoverState( moverState_t newstate, int time ) {
	...
	switch( moverState ) {
		case MOVER_POS1: {
			...
			// boyette space command door status begin
			//gameLocal.Printf( "Closing ... \n" );
			if ( health <= 0 ) {
				SetShaderParm(9,0); // hide secure material
				SetShaderParm(10,1); // show disabled material
			} else {
				SetShaderParm(9,1); // show secure material
				SetShaderParm(10,0); // hide disabled material
			}
			SetShaderParm(11,0); // hide open material
			// boyette space command door status end
			break;
		}
		case MOVER_POS2: {
		...
		case MOVER_1TO2: {
			...
			// boyette space command door status begin
			//gameLocal.Printf( "Opening ... \n" );
			SetShaderParm(9,0); // hide secure material
			SetShaderParm(10,0); // hide disabled material
			if ( health <= 0 ) {
				SetShaderParm(9,0); // hide secure material
				SetShaderParm(10,1); // show disabled material
				SetShaderParm(11,0); // show open material
			} else {
				SetShaderParm(9,0); // show secure material
				SetShaderParm(10,0); // hide disabled material
				SetShaderParm(11,1); // show open material
			}
			// boyette space command door status end
			break;
		}
...
/*
================
idDoor::UpdateDoorStatusShaderParms
================
*/
void idMover_Binary::UpdateMoverStatusShaderParms() {
...
idDoor::idDoor( void ) {
	...
	was_just_damaged = false;
	was_just_repaired = false;
	door_disabled_time = 0;
	door_damaged_time = 0;
	parentship_security_module_efficiency_adjusted_max_health = 0;
}
...
void idDoor::Spawn( void ) {
	...
	if ( moveMaster == this ) {
		...
		if ( noTouch || health ) {
			...
			// boyette space command begin.
			 PostEventMS( &EV_Door_SpawnDoorTrigger, 0 ); // boyette space command - this line wasn't originally here - this line is essential for the door opening. The door spawns its trigger on spawn. This wasn't here before, so when you set the health the door wouldn't open ever. Looks like we are going to have to implement this - they must not have finished.
			// So here is how it will work. When the door first spawns and it's health is full, we will spawn the trigger that only opens for AI's on the same team. This trigger will always be present. There is already a team var so we might have to rename that idStr.
			// So only when the door's health reaches zero, will we spawn the trigger for all teams. When the doors health is fully repaired again we will destroy the trigger for all teams - and just keep the same team trigger. If the door's health reaches zero again - same process.
			// Doors should repair themselves, or the ship should repair all doors after a certain amount of time without damage. We're probably going to want to make our own derived door class.
			// idDoor* MyDoor
			// MyDoor->idEntity::team = 1; // This is how you would access the doors team number (not the team door name which is a string).
			// boyette space command end.
...
void idDoor::Use( idEntity *other, idEntity *activator ) {
	if ( gameLocal.RequirementMet( activator, requires, removeItem ) ) {
		...
		// boyette space command begin
		if ( spawnArgs.GetBool( "only_open_for_team" ) ) {
			if ( idEntity::team == activator->team ) {
				ActivateTargets( activator );
				Use_BinaryMover( activator );
				// DONE: BOYETTE NOTE TODO IMPORTANT: Also CLEAR this door entity as something on the AI and return it in the script with a custom function so that it can know to walk through it.
				if ( activator->IsType(idAI::Type) ) {
					dynamic_cast<idAI*>(activator)->EnemyDoorToAttack = NULL;
				}
			} else if ( health <= 0 ) {
				ActivateTargets( activator );
				Use_BinaryMover( activator );
			...
			} else if ( idEntity::team != activator->team ) {
				// BOYETTE NOTE TODO IMPORTANT: Play a sound here indicating that the door is locked to them.
				// DONE: BOYETTE NOTE TODO IMPORTANT: Also set this door entity as something on the AI and return it in the script with a custom function so that it can know to attack it.
				if ( activator->IsType(idAI::Type) ) {
					dynamic_cast<idAI*>(activator)->EnemyDoorToAttack = this;
				}
				// boyette door locked sound begin
				if ( activator && activator->IsType( idPlayer::Type ) && gameLocal.time > nextSndTriggerTime ) {
					gameLocal.Printf( "DOOR LOCKED TO PLAYER - PLAYING snd_locked ... \n" );
					StartSound( "snd_locked", SND_CHANNEL_ANY, 0, false, NULL );
					nextSndTriggerTime = gameLocal.time + 10000;
				}
				// boyette door locked sound end
			}
		} else {
			ActivateTargets( activator );
			Use_BinaryMover( activator );
		}
		//gameLocal.Printf( moveMaster->name );
		//if ( !spawnArgs.GetBool( "only_open_for_team" ) || ( spawnArgs.GetBool( "only_open_for_team" ) && spawnArgs.GetInt( "idEntity_team" ) == activator->team ) && ( health <= entity_max_health || ( activateChain && activateChain->health <= activateChain->entity_max_health ) ) ) {
			//ActivateTargets( activator );
			//Use_BinaryMover( activator );
		//} 
		// BOYETTE NOTE TODO: We will have to come up with
		// a system whereby if one of the doors is damaged that the other one is also damaged(need to be careful that this does not cause a recursive loop and they infinitely damage each other. And the health that is displayed on the 
		// captain gui will be a combination of both the master door's health and the slave door's health. The healing of the door should heal both of the doors.
		// Maybe if the sum of both of their healths is less than half the combination of both of their max healths, then the door will open. - I THINK THIS IS WHAT TO DO.
		// Healing of the doors should be similar to how the medical module works and it should be based on the door module's efficiency. It should heal the master door until it's health is at 100% and then it should start healing the slave door.
		// If the door module's efficiency changes or is damaged the health of the doors should change appropriately. Should be a function on idDoor to damage the doors in this fashion.
		// Might have to set the teammaster door, there might already be a spawnarg for this. Then this teammaster door should be referenced on the parent sbShip.
		// While the health is greater than half of the combination of both of their health's a lock icon overlay should be put on top of the door icons. (implying it is secure).
		// The door icon should just be a couple of upright rectangles similar to other icons. Should be small enough to fit in between door entry-ways on the ship diagram display.
		// we need to have a material on the door that will change slightly when the door is "broken" implying that it is not locked. This might have to get set in the ::Damage function. - 
		// We might need an AI script that returns the blocking entity and/or shecks if it is of type idDoor. That way if the blocking entity is type idDoor we can damage it and attempt to walk past it.
		// We will also need an AI script function that checks if idDoor entity and it's slave are at half of the cumulative max healths. The doors should not be able to get damage past this point (less than half of their cumulative max healths).
		// boyette space command end - this was originally just ActivateTargets( activator ); Use_BinaryMover( activator ); - without being nested in this custom if statement.
	} 
}
...
============
*/
void idDoor::Damage( idEntity *inflictor, idEntity *attacker, const idVec3 &dir, 
	...
	if ( damage ) {
		...
		health -= damage;
		// boyette space command begin
		health = idMath::ClampInt(0,parentship_security_module_efficiency_adjusted_max_health,health);
		// boyette space command end
		if ( damage > 0 ) {
			was_just_damaged = true;
			SetShaderParm(7,(float)health / (float)entity_max_health); // update the damage decal
		}
		if ( damage < 0 ) {
			was_just_repaired = true;
			SetShaderParm(7,1.0); // update the damage decal
		}
		SetShaderParm(8,(float)health / (float)entity_max_health); // update the health bar
		door_damaged_time = gameLocal.time;
		if ( health > 0 ) {
			SetShaderParm(9,1); // hide secure material
			SetShaderParm(10,0); // hide disabled material
		}
		if ( health <= 0 ) {
			door_disabled_time = gameLocal.time;
			if ( health < 0 ) {
				health = 0;
			}
			SetShaderParm(9,0); // hide secure material
		...
		// BOYETTEM NOTE TODO MAYBE: Might want to add a spawn here so it will only damage chained doors if we specify to do so.
		// do the damage to the master if activateChain
		if ( GetMoveMaster() && GetMoveMaster() != this ) {
			GetMoveMaster()->health -= damage;
			// boyette space command begin
			GetMoveMaster()->health = idMath::ClampInt(0,GetMoveMaster()->parentship_security_module_efficiency_adjusted_max_health,GetMoveMaster()->health);
			// boyette space command end
			if ( damage > 0 ) {
				GetMoveMaster()->was_just_damaged = true;
				GetMoveMaster()->SetShaderParm(7,(float)GetMoveMaster()->health / (float)GetMoveMaster()->entity_max_health); // update the damage decal
			}
			if ( damage < 0 ) {
				GetMoveMaster()->was_just_repaired = true;
				GetMoveMaster()->SetShaderParm(7,1.0); // update the damage decal
			}
			GetMoveMaster()->SetShaderParm(8,(float)GetMoveMaster()->health / (float)GetMoveMaster()->entity_max_health); // update the health bar
			GetMoveMaster()->door_damaged_time = gameLocal.time;
			if ( GetMoveMaster()->health > 0 ) {
				GetMoveMaster()->SetShaderParm(9,1); // show secure material
				GetMoveMaster()->SetShaderParm(10,0); // hide disabled material
			}
			if ( GetMoveMaster()->health <= 0 ) {
				GetMoveMaster()->door_disabled_time = gameLocal.time;
				if ( GetMoveMaster()->health < 0 ) {
					GetMoveMaster()->health = 0;
				}
				GetMoveMaster()->SetShaderParm(9,0); // hide secure material
				GetMoveMaster()->SetShaderParm(10,1); // show disabled material

				GetMoveMaster()->Killed( inflictor, attacker, damage, dir, location );
			} else {
				GetMoveMaster()->Pain( inflictor, attacker, damage, dir, location );
			}
				//gameLocal.Printf( GetMoveMaster()->name + "\n" );
		}
		// do the damage to the activateChain if master
		if ( GetMoveMaster() && GetMoveMaster() == this && GetActivateChain() ) {
			GetActivateChain()->health -= damage;
			// boyette space command begin
			GetActivateChain()->health = idMath::ClampInt(0,GetActivateChain()->parentship_security_module_efficiency_adjusted_max_health,GetActivateChain()->health);
			// boyette space command end
			if ( damage > 0 ) {
				GetActivateChain()->was_just_damaged = true;
				GetActivateChain()->SetShaderParm(7,(float)GetActivateChain()->health / (float)GetActivateChain()->entity_max_health); // update the damage decal
			}
			if ( damage < 0 ) {
				GetActivateChain()->was_just_repaired = true;
				GetActivateChain()->SetShaderParm(7,1.0); // update the damage decal
			}
			GetActivateChain()->SetShaderParm(8,(float)GetActivateChain()->health / (float)GetActivateChain()->entity_max_health); // update the health bar
			GetActivateChain()->door_damaged_time = gameLocal.time;
			if ( GetActivateChain()->health > 0 ) {
				GetActivateChain()->SetShaderParm(9,1); // hide secure material
				GetActivateChain()->SetShaderParm(10,0); // hide disabled material
			}
			if ( health <= 0 ) {
				GetActivateChain()->door_disabled_time = gameLocal.time;
				if ( GetActivateChain()->health < 0 ) {
					GetActivateChain()->health = 0;
				}
				GetActivateChain()->SetShaderParm(9,0); // hide secure material
				GetActivateChain()->SetShaderParm(10,1); // show disabled material

				GetActivateChain()->Killed( inflictor, attacker, damage, dir, location );
			} else {
				GetActivateChain()->Pain( inflictor, attacker, damage, dir, location );
			}
				//gameLocal.Printf( GetActivateChain()->name + "\n" );
		}
		// BOYETTEM NOTE TODO MAYBE: If we want to have doors with more than two entities then we will have to add more logic for that i.e. GetActivateChain()->GetActivateChain()->health
	}
}

/*
================
idDoor::UpdateDoorGroupsMoverStatusShaderParms
================
*/
void idDoor::UpdateDoorGroupsMoverStatusShaderParms() {
	UpdateMoverStatusShaderParms();
	if ( GetMoveMaster() && GetMoveMaster() != this ) {
		GetMoveMaster()->UpdateMoverStatusShaderParms();
	}
	if ( GetMoveMaster() && GetMoveMaster() == this && GetActivateChain() ) {
		GetActivateChain()->UpdateMoverStatusShaderParms();
	}
}

/*
================
idDoor::SetDoorGroupsHealth
================
*/
void idDoor::SetDoorGroupsHealth(int health_to_set) {
	if ( health_to_set > 0 && health_to_set > health  ) {
		SetShaderParm(7,1.0); // remove the damage decal
	}
	health = health_to_set;
	parentship_security_module_efficiency_adjusted_max_health = health_to_set;
	if ( GetMoveMaster() && GetMoveMaster() != this ) {
		if ( health_to_set > 0 && health_to_set > GetMoveMaster()->health  ) {
			GetMoveMaster()->SetShaderParm(7,1.0); // remove the damage decal
		}
		GetMoveMaster()->health = health_to_set;
		GetMoveMaster()->parentship_security_module_efficiency_adjusted_max_health = health_to_set;
	}
	if ( GetMoveMaster() && GetMoveMaster() == this && GetActivateChain() ) {
		if ( health_to_set > 0 && health_to_set > GetActivateChain()->health  ) {
			GetActivateChain()->SetShaderParm(7,1.0); // remove the damage decal
		}
		GetActivateChain()->health = health_to_set;
		GetActivateChain()->parentship_security_module_efficiency_adjusted_max_health = health_to_set;
	}
...
/*
================
idDoor::SetDoorGroupsTeam
================
*/
void idDoor::SetDoorGroupsidEntityTeam(int team_to_set) {
	idEntity::team = team_to_set; spawnArgs.SetInt("idEntity_team",team_to_set);
	if ( GetMoveMaster() && GetMoveMaster() != this ) {
		GetMoveMaster()->idEntity::team = team_to_set; GetMoveMaster()->spawnArgs.SetInt("idEntity_team",team_to_set);
	}
	if ( GetMoveMaster() && GetMoveMaster() == this && GetActivateChain() ) {
		GetActivateChain()->idEntity::team = team_to_set; GetActivateChain()->spawnArgs.SetInt("idEntity_team",team_to_set);
	}
...
/*
================
idDoor::RemoveDoorGroup
================
*/
void idDoor::RemoveDoorGroup() {
	if ( GetMoveMaster() && GetMoveMaster() == this && GetActivateChain() ) {
		GetActivateChain()->Event_Remove();
		activateChain = NULL;
	}
	if ( GetMoveMaster() && GetMoveMaster() != this ) {
		GetMoveMaster()->Event_Remove();
		moveMaster = NULL;
	}
	Event_Remove();
}
...
/*
================
idDoor::IsRelatedDoor
================
*/
bool idDoor::IsRelatedDoor(idEntity* door_to_test) {
	if ( door_to_test == this ) {
		return true;
	}
	if ( GetMoveMaster() && GetMoveMaster() != this && door_to_test == GetMoveMaster() ) {
		return true;
	}
	if ( GetMoveMaster() && GetMoveMaster() == this && GetActivateChain() && door_to_test == GetActivateChain() ) {
		return true;
	}
	return false;
}
...
/*
================
idDoor::SyncUpDoorGroupsSpawnArgs
================
*/
void idDoor::SyncUpDoorGroupsSpawnArgs() {
	if ( GetMoveMaster() && GetMoveMaster() != this ) {
		GetMoveMaster()->entity_max_health = entity_max_health;
		GetMoveMaster()->spawnArgs.SetInt( "entity_max_health", entity_max_health );
		GetMoveMaster()->health = health;
		GetMoveMaster()->spawnArgs.SetInt( "health", health );
	}
	if ( GetMoveMaster() && GetMoveMaster() == this && GetActivateChain() ) {
		GetActivateChain()->entity_max_health = entity_max_health;
		GetActivateChain()->spawnArgs.SetInt( "entity_max_health", entity_max_health );
		GetActivateChain()->health = health;
		GetActivateChain()->spawnArgs.SetInt( "health", health );
	}
}
