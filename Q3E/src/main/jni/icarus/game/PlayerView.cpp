...
idPlayerView::idPlayerView() {
	...
	tunnelMaterial = declManager->FindMaterial( "textures/materials_used_in_source/health_damage_screen_blob_tunnel" );
	armorMaterial = declManager->FindMaterial( "textures/materials_used_in_source/armor_damage_screen_blob" );
	berserkMaterial = declManager->FindMaterial( "textures/materials_used_in_source/berserk_screen_effect" );
	irGogglesMaterial = declManager->FindMaterial( "textures/materials_used_in_source/xray_blend" );
	bloodSprayMaterial = declManager->FindMaterial( "textures/materials_used_in_source/blood_spray" );
	bfgMaterial = declManager->FindMaterial( "textures/materials_used_in_source/big_fabulous_gala_screen_blob_tunnel" );
...
void idPlayerView::DamageImpulse( idVec3 localKickDir, const idDict *damageDef ) {
	...
	// screen blob
	//
// boyette mod begin // added some options - kept original behavior as the default spawnarg values.
	float	blobTime = damageDef->GetFloat( "blob_time" );
	bool	random_screen_positioning = damageDef->GetBool( "random_screen_positioning","1");
	bool	is_fullscreen = damageDef->GetBool( "is_fullscreen","0");
	if ( blobTime ) {
		screenBlob_t	*blob = GetScreenBlob();
		blob->startFadeTime = gameLocal.time;
		blob->finishTime = gameLocal.time + blobTime * g_blobTime.GetFloat() * ((float)gameLocal.msec / USERCMD_MSEC);

		const char *materialName = damageDef->GetString( "mtr_blob" );
		blob->material = declManager->FindMaterial( materialName );
		if ( random_screen_positioning ) {
			blob->x = damageDef->GetFloat( "blob_x" );
			blob->x += ( gameLocal.random.RandomInt()&63 ) - 32;
			//blob->x += ( gameLocal.random.RandomInt()&127 ) - 64; // BOYETTE NOTE TODO: this would result in more variation of the x position and might be more consistent with the new 1280 960 size. but maybe less variation is better.
			blob->y = damageDef->GetFloat( "blob_y" );
			blob->y += ( gameLocal.random.RandomInt()&63 ) - 32;
			//blob->y += ( gameLocal.random.RandomInt()&127 ) - 64; // BOYETTE NOTE TODO: this would result in more variation of the y position and might be more consistent with the new 1280 960 size. but maybe less variation is better.
		
			float scale = ( 256 + ( ( gameLocal.random.RandomInt()&63 ) - 32 ) ) / 256.0f; // BOYETTE NOTE: this results in a maximum scale modifier of 1.125 i.e. (256 +32)/256 = 1.125 and minimum of 0.875 // so it does not really need to be changed to adjust for the new 1280 960 size - it was arbitrary to begin with and there is no reason to change it.
			...
			blob->w = damageDef->GetFloat( "blob_width" ) * g_blobSize.GetFloat();
			blob->h = damageDef->GetFloat( "blob_height" ) * g_blobSize.GetFloat();
			blob->s1 = 0;
			blob->t1 = 0;
			blob->s2 = 1;
			blob->t2 = 1;
		...
		if ( is_fullscreen ) {
			blob->x = 0;
			blob->y = 0;
			blob->w = SCREEN_WIDTH * g_blobSize.GetFloat();
			blob->h = SCREEN_HEIGHT * g_blobSize.GetFloat();
			blob->s1 = 0;
			blob->t1 = 0;
			blob->s2 = 1;
			blob->t2 = 1;
...
// boyette mod end
	...
	//
	bool	trigger_health_tunnel = damageDef->GetBool( "trigger_health_tunnel","1");
	if ( trigger_health_tunnel ) {
...
void idPlayerView::SingleView( idUserInterface *hud, const renderView_t *view ) {
	...
	if ( gameLocal.portalSkyEnt.GetEntity() && gameLocal.IsPortalSkyAcive() && g_enablePortalSky.GetBool() ) {
		...
		// boyette begin
		// boyette this fixes the view portal in place - not what we want - //portalView.viewaxis = gameLocal.portalSkyEnt.GetEntity()->GetPhysics()->GetAxis();
		// boyette this fixes the view portal in place - not what we want - //portalView.viewaxis = gameLocal.portalSkyEnt.GetEntity()->spawnArgs.GetMatrix("rotation", "1 0 0 0 1 0 0 0 1");
		portalView.viewaxis = portalView.viewaxis * gameLocal.portalSkyEnt.GetEntity()->GetPhysics()->GetAxis(); // this works perfectly. Translates the rotated axis by the amount of the map editor rotation of the info_portalsky entity.
		// boyette end
...
bool FxFader::SetTriggerState( bool active ) {
	...
	switch ( state ) {
		...
		case FX_STATE_RAMPUP:	alpha = 1 - (float)( time - gameLocal.time ) / msec; break;
		case FX_STATE_RAMPDOWN:	alpha = (float)( time - gameLocal.time ) / msec; break;
	}
...
void FullscreenFX_Helltime::Initialize() {
	acInitMaterials[0] = declManager->FindMaterial( "textures/materials_used_in_source/orb_ac_init_placeholder" );
	acInitMaterials[1] = declManager->FindMaterial( "textures/materials_used_in_source/orb_ac_init_placeholder" );
	acInitMaterials[2] = declManager->FindMaterial( "textures/materials_used_in_source/orb_ac_init_placeholder" );

	acCaptureMaterials[0] = declManager->FindMaterial( "textures/materials_used_in_source/orb_ac_capture_placeholder1" );
	acCaptureMaterials[1] = declManager->FindMaterial( "textures/materials_used_in_source/orb_ac_capture_placeholder2" );
	acCaptureMaterials[2] = declManager->FindMaterial( "textures/materials_used_in_source/orb_ac_capture_placeholder3" );

	acDrawMaterials[0] = declManager->FindMaterial( "textures/materials_used_in_source/orb_ac_draw_placeholder1" );
	acDrawMaterials[1] = declManager->FindMaterial( "textures/materials_used_in_source/orb_ac_draw_placeholder2" );
	acDrawMaterials[2] = declManager->FindMaterial( "textures/materials_used_in_source/orb_ac_draw_placeholder3" );

	crCaptureMaterials[0] = declManager->FindMaterial( "textures/materials_used_in_source/orb_cr_capture_placeholder" );
	crCaptureMaterials[1] = declManager->FindMaterial( "textures/materials_used_in_source/orb_cr_capture_placeholder" );
	crCaptureMaterials[2] = declManager->FindMaterial( "textures/materials_used_in_source/orb_cr_capture_placeholder" );

	crDrawMaterials[0] = declManager->FindMaterial( "textures/materials_used_in_source/orb_cr_draw_placeholder" );
	crDrawMaterials[1] = declManager->FindMaterial( "textures/materials_used_in_source/orb_cr_draw_placeholder" );
	crDrawMaterials[2] = declManager->FindMaterial( "textures/materials_used_in_source/orb_cr_draw_placeholder" );
...
void FullscreenFX_Multiplayer::Initialize() {
	acInitMaterials		= declManager->FindMaterial( "textures/materials_used_in_source/orb_ac_init_placeholder" );
	acCaptureMaterials	= declManager->FindMaterial( "textures/materials_used_in_source/orb_ac_capture_placeholder1" );
	acDrawMaterials		= declManager->FindMaterial( "textures/materials_used_in_source/orb_ac_draw_placeholder1" );
	crCaptureMaterials	= declManager->FindMaterial( "textures/materials_used_in_source/orb_cr_capture_placeholder" );
	crDrawMaterials		= declManager->FindMaterial( "textures/materials_used_in_source/orb_cr_draw_placeholder" );
	clearAccumBuffer	= true;
}
...
void FullscreenFX_Warp::Initialize() {
	material = declManager->FindMaterial( "textures/materials_used_in_source/warp" );
...
void FullscreenFX_Warp::HighQuality() {
	...
	idVec2 center;
	int STEP = 9;
// boyette slow motion begin
	if ( !g_enableSlowmo.GetBool() ) {
		interp = ( idMath::Sin( (float)( gameLocal.time - startWarpTime ) / 1000 ) + 1 ) / 2.f;
	} else {
		interp = ( idMath::Sin( (float)( gameLocal.time - startWarpTime ) / 700 ) + 1 ) / 2.f; //PERFECT//
		//PERFECT SHORTER//interp = ( idMath::Sin( (float)( gameLocal.time - startWarpTime ) / 510 ) + 1 ) / 2.f; //PERFECT SHORTER//
		//PERFECT IMMEDIATE//interp = ( idMath::Sin( (float)( gameLocal.time - startWarpTime ) / 400 ) + 1 ) / 2.f; //PERFECT IMMEDIATE//
	}
// boyette slow motion end
...
// boyette slow motion begin
	if ( !g_enableSlowmo.GetBool() ) {
		radius = 400; // original value
	} else {
		radius = 480; // boyette value
	}
// boyette slow motion end
...
void FullscreenFX_EnviroSuit::Initialize() {
	material = declManager->FindMaterial( "textures/materials_used_in_source/environment_suit_screen_effect_placeholder" );
}
...
void FullscreenFX_DoubleVision::Initialize() {
	material = declManager->FindMaterial( "textures/materials_used_in_source/double_vision" );
}
...
bool FullscreenFX_DoubleVision::Active() {
	...
	if ( gameLocal.time < fxman->GetPlayerView()->dvFinishTime ) {
		return true;
	}
...
void FullscreenFX_DoubleVision::HighQuality() {
	int offset = fxman->GetPlayerView()->dvFinishTime - gameLocal.time;
	...
	if ( !gameLocal.isMultiplayer && gameLocal.time < player->inventory.powerupEndTime[ HELLTIME ] || gameLocal.time < player->inventory.powerupEndTime[ INVULNERABILITY ]) {
		color.y = 0;
		color.z = 0;
	}
...
void FullscreenFX_Bloom::Initialize() {
	drawMaterial		= declManager->FindMaterial( "textures/materials_used_in_source/bloom_draw" );
	initMaterial		= declManager->FindMaterial( "textures/materials_used_in_source/bloom_init" );
	currentMaterial		= NULL;	// was originally: currentMaterial = declManager->FindMaterial( "textures/materials_used_in_source/bloom_current_material" ); // BOYETTE NOTE IMPORTANT: this is was originally a reference to a material that didn't exist in the materials files. So it's value was just NULL. Not sure how this affects the bloom effect but it seems to work at least seemingly properly as just NULL.
...
/*
==================
FullscreenFX_Blur::Initialize
==================
*/
void FullscreenFX_Blur::Initialize() {
	material = declManager->FindMaterial( "textures/materials_used_in_source/blur" );
}

/*
==================
FullscreenFX_Blur::Active
==================
*/
...
/*
==================
FullscreenFX_Desaturate::Initialize
==================
*/
void FullscreenFX_Desaturate::Initialize() {
	material = declManager->FindMaterial( "textures/materials_used_in_source/desaturate" );
}

/*
==================
FullscreenFX_Desaturate::Active
==================
*/
bool FullscreenFX_Desaturate::Active() {
	idPlayer *player;

	player = fxman->GetPlayer();

	if ( player && player->desaturateEnabled ) {
		return true;
	}
...
/*
==================
FullscreenFX_Desaturate::HighQuality
==================
*/
void FullscreenFX_Desaturate::HighQuality() {
	renderSystem->SetColor4( 1, 1, 1, 1 );
	renderSystem->DrawStretchPic( 0.0f, 0.0f, 1280.0f, 960.0f, 0.0f, 0.0f, 1.0f, 1.0f, material );
}
...
void FullscreenFXManager::CreateFX( idStr name, idStr fxtype, int fade ) {
	...
	else if ( fxtype == "desaturate" ) {
		pfx = new FullscreenFX_Desaturate;
	}
	else {
		assert( 0 );
	}
...
void FullscreenFXManager::Initialize( idPlayerView *pv ) {
	// set the playerview
	playerView = pv;
	blendBackMaterial = declManager->FindMaterial( "textures/materials_used_in_source/blend_back" );
	...
	CreateFX( "desaturate", "desaturate", 0 );
