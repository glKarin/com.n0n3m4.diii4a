...
class idProjectile; // BOYETTE NOTE: added this 07 20 2016 for projectiles_to_track
...
public:
	...
	//int						team; // this is on idEntity now
...
public:
	...
	const idDict *						HealFXDef;
	idEntityPtr<idFuncEmitter>			HealFX;
	virtual void						TriggerHealFX( idStr heal_def_name );

	const idDict *						TransporterFXDef;
	idEntityPtr<idFuncEmitter>			TransporterFX;
	virtual void						TriggerTransporterFX( idStr transporter_def_name );

	idEntity*							EnemyDoorToAttack;
	bool								prevent_actor_inflicted_same_team_damage;

	idEntityPtr<idEntity>				DefAttach;

// boyette space command projectile tracking begin
    std::vector<idProjectile*>	projectiles_to_track;							// boyette mod
// boyette space command projectile tracking end

	// boyette space command end

// boyette space command kill count begin
	int						space_command_level;
	int						killCount;
