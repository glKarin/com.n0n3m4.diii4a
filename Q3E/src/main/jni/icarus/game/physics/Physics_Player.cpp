	...
	for ( bumpcount = 0; bumpcount < numbumps; bumpcount++ ) {
		...
		if ( push && trace.c.entityNum != ENTITYNUM_WORLD ) {
			...
			if ( g_allowPlayerToPushMonsters.GetBool() ) {
				pushFlags = PUSHFL_CLIP|PUSHFL_NOGROUNDENTITIES|PUSHFL_APPLYIMPULSE;
			} else {
				pushFlags = PUSHFL_CLIP|PUSHFL_ONLYMOVEABLE|PUSHFL_NOGROUNDENTITIES|PUSHFL_APPLYIMPULSE; // BOYETTE NOTE: this is how it was originally
			}
...
bool idPhysics_Player::Evaluate( int timeStepMSec, int endTimeMSec ) {
	...
	if ( IsOutsideWorld() ) {
		// BOYETTE WEIRD TEXTURE THRASHING FIX BEGIN - BOYETTE NOTE: added this check to temporarily suppress this particular warning message
		if ( gameLocal.GetLocalPlayer() && gameLocal.GetLocalPlayer()->currently_doing_weird_texture_thrash_fix ) {
		} else {
