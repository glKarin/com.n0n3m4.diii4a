...
void idForce_Grab::Evaluate( int time ) {
	...
	// boyette note - I think the fast normalize would be fine here. - But I will see.
	//distanceToGoal = forceDir.NormalizeFast();
	...
	// boyette begin - this will smooth out the object so it doesn't vibrate when it gets close to the goal position.
	if ( temp < 64.f ) {
