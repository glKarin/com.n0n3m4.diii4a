...
const char *ui_skinArgs[]			= { "skins/skins_used_in_source/player/officer_mp", "skins/skins_used_in_source/player/officer_mp_red", "skins/skins_used_in_source/player/officer_mp_blue", "skins/skins_used_in_source/player/officer_mp_green", "skins/skins_used_in_source/player/officer_mp_yellow", "skins/skins_used_in_source/player/officer_mp_purple", "skins/skins_used_in_source/player/officer_mp_grey", "skins/skins_used_in_source/player/officer_mp_orange", NULL };
...
// server info
idCVar si_name(						"si_name",					"ICARUS Server",	CVAR_GAME | CVAR_SERVERINFO | CVAR_ARCHIVE, "name of the server" );
...
// boyette space command begin - CVARS
idCVar g_stopTimeForceFrameNumUpdateDuring(		"g_stopTimeForceFrameNumUpdateDuring",	"0",	CVAR_GAME | CVAR_BOOL, "Normally during g_stoptime the frame number wouldn't update. This makes sure it updates - so story gui effects work properly." );
idCVar g_stopTimeForceRenderViewUpdateDuring(		"g_stopTimeForceRenderViewUpdateDuring",	"0",	CVAR_GAME | CVAR_BOOL, "Normally during g_stoptime the renderview wouldn't update. This makes sure it updates - so story gui videos work properly hopefully." );

idCVar g_enableCaptainGuiSlowmo(	"g_enableCaptainGuiSlowmo",	"0",			CVAR_GAME | CVAR_BOOL, "for the slow time button on the CaptainGui" );

idCVar g_enableModuleRecieveDamageProjectiles(	"g_enableModuleRecieveDamageProjectiles",	"1",			CVAR_GAME | CVAR_BOOL, "for the module recieve damage projectiles" );

idCVar g_enableReducedHullDamage(				"g_enableReducedHullDamage",			"1",			CVAR_GAME | CVAR_BOOL, "testing this to see if it can extend the druation of ship battles in a balanced way." );

idCVar g_allowPlayerToPushMonsters(				"g_allowPlayerToPushMonsters",			"0",			CVAR_GAME | CVAR_BOOL, "this will allow the player to push monsters. Added so we can avoid getting stuck behind crewmembers. Not sure yet if we want to use." );

idCVar sc_medical_officer_name(						"sc_medical_officer_name",					"Crucero",		CVAR_GAME, "name of the medical officer on the player ship" );
idCVar sc_engines_officer_name(						"sc_engines_officer_name",					"Jenson",		CVAR_GAME, "name of the engines officer on the player ship" );
idCVar sc_weapons_officer_name(						"sc_weapons_officer_name",					"Bertie",		CVAR_GAME, "name of the weapons officer on the player ship" );
idCVar sc_torpedos_officer_name(					"sc_torpedos_officer_name",					"Hogman",		CVAR_GAME, "name of the torpedos officer on the player ship" );
idCVar sc_shields_officer_name(						"sc_shields_officer_name",					"Sinolow",		CVAR_GAME, "name of the shields officer on the player ship" );
idCVar sc_sensors_officer_name(						"sc_sensors_officer_name",					"Sherm",		CVAR_GAME, "name of the sensors officer on the player ship" );
idCVar sc_environment_officer_name(					"sc_environment_officer_name",				"Hugh",		CVAR_GAME, "name of the environment officer on the player ship" );
idCVar sc_computer_officer_name(					"sc_computer_officer_name",					"Obstante",		CVAR_GAME, "name of the computer officer on the player ship" );
idCVar sc_security_officer_name(					"sc_security_officer_name",					"Bilgen",		CVAR_GAME, "name of the security officer on the player ship" );
idCVar sc_captain_officer_name(						"sc_captain_officer_name",					"Captain",		CVAR_GAME, "name of the captain officer on the player ship" );

idCVar g_enableWeirdTextureThrashingFix(	"g_enableWeirdTextureThrashingFix",	"1",			CVAR_GAME | CVAR_BOOL, "a weird and somewhat last minute hacky fix for a texture thrashing issue that seems to be common. Forces all models and associated images to load." );

idCVar g_showStargridUndiscoveredEntityIcons(	"g_showStargridUndiscoveredEntityIcons",	"0",			CVAR_GAME | CVAR_BOOL, "hides all undiscovered entities on the stargrid. May require map restart." );
idCVar g_showStargridAllEntityIcons(	"g_showStargridAllEntityIcons",	"0",			CVAR_GAME | CVAR_BOOL, "shows all discovered and undiscovered entities on the stargrid. May require map restart." );

idCVar g_allowPlayerAutoPowerMode(	"g_allowPlayerAutoPowerMode",	"0",			CVAR_GAME | CVAR_INTEGER | CVAR_ARCHIVE, "this will allow the player to use the autopower functionality to automate power allocation to ship modules." );

idCVar s_in_game_music_volume( "s_in_game_music_volume_dB", "0", CVAR_SOUND | CVAR_ARCHIVE | CVAR_FLOAT, "volume of in game space command music dB" );

idCVar g_inIronManMode(	"g_inIronManMode",				"0",			CVAR_GAME | CVAR_BOOL, "whether or not the game is in iron man mode." );
// boyette space command end - CVARS
...
idCVar mod_validSkins(				"mod_validSkins",			"skins/skins_used_in_source/player/officer_mp;skins/skins_used_in_source/player/officer_mp_green;skins/skins_used_in_source/player/officer_mp_blue;skins/skins_used_in_source/player/officer_mp_red;skins/skins_used_in_source/player/officer_mp_yellow;skins/skins_used_in_source/player/officer_mp_purple;skins/skins_used_in_source/player/officer_mp_grey;skins/skins_used_in_source/player/officer_mp_orange",		CVAR_GAME | CVAR_ARCHIVE, "valid skins for the game" );


idCVar g_grabberHoldSeconds(		"g_grabberHoldSeconds",		"3",			CVAR_GAME | CVAR_FLOAT | CVAR_CHEAT, "number of seconds to hold object" ); // original was 3
idCVar g_grabberEnableShake(		"g_grabberEnableShake",		"0",			CVAR_GAME | CVAR_BOOL | CVAR_CHEAT, "enable the grabber shake" ); // original was 1
idCVar g_grabberRandomMotion(		"g_grabberRandomMotion",	"0",			CVAR_GAME | CVAR_BOOL | CVAR_CHEAT, "enable random motion on the grabbed object" ); // original was 1
idCVar g_grabberHardStop(			"g_grabberHardStop",		"1",			CVAR_GAME | CVAR_BOOL | CVAR_CHEAT, "hard stops object if too fast" ); // original was 1 - is still 1
idCVar g_grabberDamping(			"g_grabberDamping",			"0.5",			CVAR_GAME | CVAR_FLOAT | CVAR_CHEAT, "damping of grabber" ); // original was 0.5
