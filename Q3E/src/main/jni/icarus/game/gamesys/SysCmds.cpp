...
void Cmd_Give_f( const idCmdArgs &args ) {
	...
	if ( give_all || idStr::Icmp( name, "weapons" ) == 0 ) {
		player->inventory.weapons.set(); // boyette note - this must set all the bits of the weapons int bit array to 1. Thus giving all weapons to the player. I'm not sure what cacheweapons does yet. -1 is all ones set - look into it.
		player->CacheWeapons();
