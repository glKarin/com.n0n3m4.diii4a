...
void idSaveGame::WriteRenderEntity( const renderEntity_t &renderEntity ) {
	...
	WriteSkin( renderEntity.originalCustomSkin );
...
void idRestoreGame::RestoreObjects( void ) {
	...
	//idFile *file;	
	//file = fileSystem->OpenFileAppend("save_load_working.txt");
	//file->Printf( "RestoreObjects 01\n" );
...
void idRestoreGame::ReadString( idStr &string ) {
	...
	string.Fill( ' ', len ); // boyette bit - you probably want to replace this with string.assign(len, ' ') when you make a stl string version.
...
void idRestoreGame::ReadRenderEntity( renderEntity_t &renderEntity ) {
	...
	// BOYETTE SPACE COMMAND BEGIN
	ReadSkin( renderEntity.originalCustomSkin );
	// BOYETTE SPACE COMMAND END
