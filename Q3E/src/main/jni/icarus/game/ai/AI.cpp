...
idAI::idAI() {
	...
	module_buff_amount = 0;
	sensitive_ai_blocked_behavior = false;
	ai_can_be_transported = false;
	npc_name_locked = false;
	ShipOnBoard			= NULL;
	ParentShip			= NULL;
	ship_diagram_icon			= NULL;
	ship_diagram_portrait		= NULL;
	crew_auto_mode_activated = false;
	player_follow_mode_activated = false;
	...
	uses_space_command_dialogue_behavior = false;
	ai_dialogue_gui_file = NULL;
	currently_in_dialogue_with_player = false;
	ai_dialogue_anim_to_play = "";
	waiting_until_player_talks_to_me = false;

	module_console_damage_def_name = NULL;
	module_console_repair_def_name = NULL;
	door_damage_def_name = NULL;

	handling_emergency_oxygen_situation = false;
...
idAI::~idAI() {
	// boyette space command begin
	Event_RemoveBeamToEnt();

	if ( ShipOnBoard ) {
		for ( int i = 0; i < ShipOnBoard->AIsOnBoard.size() ; i++ ) {
			if ( ShipOnBoard->AIsOnBoard[i] && ShipOnBoard->AIsOnBoard[i]->EntityCommandedToMoveTo == this ) {
				ShipOnBoard->AIsOnBoard[i]->EntityCommandedToMoveTo == NULL;
			}
		}
		ShipOnBoard->AIsOnBoard.erase(std::remove(ShipOnBoard->AIsOnBoard.begin(), ShipOnBoard->AIsOnBoard.end(), this), ShipOnBoard->AIsOnBoard.end()); // remove the ai from the list of AI's on board this ship
	}
	// boyette space command end
	...
	if ( gameLocal.GetLocalPlayer() && gameLocal.GetLocalPlayer()->grabbed_crew_member && gameLocal.GetLocalPlayer()->grabbed_crew_member == this ) {
		gameLocal.GetLocalPlayer()->grabbed_crew_member == NULL;
	}

	if ( ParentShip ) {
		ParentShip->SelectedCrewMembers.erase(std::remove(ParentShip->SelectedCrewMembers.begin(), ParentShip->SelectedCrewMembers.end(), this), ParentShip->SelectedCrewMembers.end()); // remove this ai from the list of AI's that the parentship could have selected
		
		for( int i = 0; i < MAX_CREW_ON_SHIPS; i++ ) {
			if ( ParentShip->crew[i] && ParentShip->crew[i] == this ) {
				ParentShip->crew[i] = NULL;
				break;
			}
		}
	}
...
void idAI::Save( idSaveGame *savefile ) const {
	...
	// BOYETTE SAVE BEGIN
	savefile->WriteObject( EntityCommandedToMoveTo );
	savefile->WriteBool( IsSelectedCrew );
	savefile->WriteInt( module_buff_amount );
	savefile->WriteBool( sensitive_ai_blocked_behavior );
	savefile->WriteBool( ai_can_be_transported );
	savefile->WriteBool( npc_name_locked );
	savefile->WriteString( ship_diagram_icon );
	savefile->WriteString( ship_diagram_portrait );
	savefile->WriteObject( ShipOnBoard );
	savefile->WriteObject( ParentShip );
	savefile->WriteBool( seated_in_captain_chair );
	savefile->WriteBool( working_on_console );
	savefile->WriteString( sitting_animation_name );

	savefile->WriteBool( crew_auto_mode_activated );
	savefile->WriteBool( player_follow_mode_activated );
	savefile->WriteBool( hide_weapons_skin_active );

	if ( ModuleConsoleBeam.target.GetEntity() && ModuleConsoleBeam.modelDefHandle >= 0 ) {
		savefile->WriteBool( true );
		ModuleConsoleBeam.target.Save( savefile );
		savefile->WriteRenderEntity( ModuleConsoleBeam.renderEntity );
	} else {
		savefile->WriteBool( false );
	}
	//savefile->WriteInt( ModuleConsoleBeam.modelDefHandle );
	savefile->WriteBool( module_console_beam_active );

	savefile->WriteString( module_console_damage_def_name );
	savefile->WriteString( module_console_repair_def_name );
	savefile->WriteString( door_damage_def_name );

	savefile->WriteBool( handling_emergency_oxygen_situation );

	savefile->WriteBool( uses_space_command_dialogue_behavior );

	for ( i = 0; i < MAX_DIALOGUE_BRANCHES; i++ ) {
		savefile->WriteBool( AIDialogueBranchTracker.test(i) );
	}
	savefile->WriteBool( currently_in_dialogue_with_player );

	savefile->WriteString( ai_dialogue_gui_file );
	savefile->WriteString( ai_dialogue_anim_to_play );

	savefile->WriteBool( waiting_until_player_talks_to_me );

	foodprojectile.Save( savefile );
	// BOYETTE SAVE END
}
...
void idAI::Restore( idRestoreGame *savefile ) {
	...
	savefile->ReadObject( reinterpret_cast<idClass *&>( EntityCommandedToMoveTo ) );
	savefile->ReadBool( IsSelectedCrew );
	savefile->ReadInt( module_buff_amount );
	savefile->ReadBool( sensitive_ai_blocked_behavior );
	savefile->ReadBool( ai_can_be_transported );
	savefile->ReadBool( npc_name_locked );
	savefile->ReadString( ship_diagram_icon );
	savefile->ReadString( ship_diagram_portrait );
	savefile->ReadObject( reinterpret_cast<idClass *&>( ShipOnBoard ) );
	savefile->ReadObject( reinterpret_cast<idClass *&>( ParentShip ) );
	savefile->ReadBool( seated_in_captain_chair );
	savefile->ReadBool( working_on_console );
	savefile->ReadString( sitting_animation_name );

	savefile->ReadBool( crew_auto_mode_activated );
	savefile->ReadBool( player_follow_mode_activated );
	savefile->ReadBool( hide_weapons_skin_active );

	savefile->ReadBool( hasObject );
	if ( hasObject ) {
		ModuleConsoleBeam.target.Restore( savefile );
		savefile->ReadRenderEntity( ModuleConsoleBeam.renderEntity );
	...
	//savefile->ReadRenderEntity( ModuleConsoleBeam.renderEntity );
	//savefile->ReadInt( ModuleConsoleBeam.modelDefHandle );
	//if ( ModuleConsoleBeam.modelDefHandle >= 0 ) {
	//	ModuleConsoleBeam.modelDefHandle = gameRenderWorld->AddEntityDef( &ModuleConsoleBeam.renderEntity );
	//}

	savefile->ReadBool( module_console_beam_active );

	savefile->ReadString( module_console_damage_def_name );
	savefile->ReadString( module_console_repair_def_name );
	savefile->ReadString( door_damage_def_name );

	savefile->ReadBool( handling_emergency_oxygen_situation );

	savefile->ReadBool( uses_space_command_dialogue_behavior );

	for ( i = 0; i < MAX_DIALOGUE_BRANCHES; i++ ) {
		bool bitset_value = false;
		savefile->ReadBool( bitset_value );
		AIDialogueBranchTracker.set(i,bitset_value);
	}
	savefile->ReadBool( currently_in_dialogue_with_player );

	savefile->ReadString( ai_dialogue_gui_file );
	savefile->ReadString( ai_dialogue_anim_to_play );

	savefile->ReadBool( waiting_until_player_talks_to_me );

	foodprojectile.Restore( savefile );
	// BOYETTE RESTORE END
}
...
void idAI::Spawn( void ) {
	...
	EntityCommandedToMoveTo = NULL;
	IsSelectedCrew = false;
	spawnArgs.GetInt("module_buff_amount","10",module_buff_amount );
	spawnArgs.GetBool( "sensitive_ai_blocked_behavior",	"0", sensitive_ai_blocked_behavior );
	spawnArgs.GetBool( "ai_can_be_transported",	"0", ai_can_be_transported );
	spawnArgs.GetBool( "npc_name_locked", "0", npc_name_locked );

	// space command level begin
	space_command_level = spawnArgs.GetInt("space_command_level", "1");
	space_command_level = idMath::ClampInt(1,MAX_SPACE_COMMAND_LEVEL,space_command_level);
	// get proper killCount for this level.
	if ( space_command_level == 1 ) {
		killCount = 0;
	} else if ( space_command_level == 2 ) {
		killCount = 1;
	} else if ( space_command_level == 3 ) {
		killCount = 3;
	} else if ( space_command_level == 4 ) {
		killCount = 5;
	} else if ( space_command_level == 5 ) {
		killCount = 8;
	} else if ( space_command_level == 6 ) {
		killCount = 16;
	} else if ( space_command_level == 7 ) {
		killCount = 32;
	}
	space_command_level = 1; // so the proper upgrades are made.
	HandleSpaceCommandLevel();

	//ship_diagram_icon = spawnArgs.GetString(va("ship_diagram_icon%i",space_command_level), "guis/assets/steve_captain_display/CrewDiagramIcons/CrewDiagramIconDefault.tga");
	//spawnArgs.Set("ship_diagram_icon",ship_diagram_icon);
	// space command level end

	ship_diagram_icon = spawnArgs.GetString("ship_diagram_icon", "guis/assets/steve_captain_display/CrewDiagramIcons/CrewDiagramIconDefault.tga");
	ship_diagram_portrait = spawnArgs.GetString("ship_diagram_portrait", "guis/assets/steve_captain_display/CrewPortraits/CrewPortraitDefault.tga");

	module_console_beam_active = false;

	module_console_damage_def_name = spawnArgs.GetString("module_console_damage_def","damage_module_console_default");
	module_console_repair_def_name = spawnArgs.GetString("module_console_repair_def","repair_module_console_default");
	door_damage_def_name = spawnArgs.GetString("door_damage_def","damage_door_default");

	uses_space_command_dialogue_behavior = spawnArgs.GetBool("uses_space_command_dialogue_behavior", "0");

	// ship dialogue system.
	AIDialogueBranchTracker.set(0,1); // so the first bit will spawn as set to true - all others will be false. Because we always want the dialogue to start with the first branch. When the dialoge is finished, set the last bit to true. In our logic for a hail we will check the last bit. That way hailing will just produce a "no reponse" or something like that screen if they don't want to talk. The ship will have a damage tolerance, if it goes below that amount they will give up and hail you to continue the dialogue(so the last bit will be set to false).
	ai_dialogue_gui_file = spawnArgs.GetString("ai_dialogue_gui_file","guis/steve_space_command/ai_dialogue_guis/default_ai_dialogue.gui");
	currently_in_dialogue_with_player = false;
	ai_dialogue_anim_to_play = "";
	spawnArgs.GetBool( "waiting_until_player_talks_to_me",	"0", waiting_until_player_talks_to_me );
...
void idAI::InitMuzzleFlash( void ) {
	...
	spawnArgs.GetString( "mtr_flashShader", "textures/materials_used_in_source/muzzleflash_default", &shader );
...
*/
	...
	// boyette space command begin
	// to update the beam
	if ( module_console_beam_active && ModuleConsoleBeam.target.GetEntity() != NULL ) {

		idVec3				org;
		
		org = ModuleConsoleBeam.target.GetEntity()->GetPhysics()->GetAbsBounds().GetCenter();
		...
		ModuleConsoleBeam.renderEntity.shaderParms[ SHADERPARM_BEAM_END_X ] = org.x;
		ModuleConsoleBeam.renderEntity.shaderParms[ SHADERPARM_BEAM_END_Y ] = org.y;
		ModuleConsoleBeam.renderEntity.shaderParms[ SHADERPARM_BEAM_END_Z ] = org.z;
		ModuleConsoleBeam.renderEntity.shaderParms[ SHADERPARM_RED ] = 
		ModuleConsoleBeam.renderEntity.shaderParms[ SHADERPARM_GREEN ] = 
		ModuleConsoleBeam.renderEntity.shaderParms[ SHADERPARM_BLUE ] = 
		ModuleConsoleBeam.renderEntity.shaderParms[ SHADERPARM_ALPHA ] = 1.0f;
		gameRenderWorld->UpdateEntityDef( ModuleConsoleBeam.modelDefHandle, &ModuleConsoleBeam.renderEntity );
...
bool idAI::StepDirection( float dir ) {
	...
	// boyette projectile begin
	if ( path.blockingEntity && ( ( move.moveCommand == MOVE_TO_ENEMY ) || ( move.moveCommand == MOVE_TO_ENTITY ) || ( move.moveCommand == MOVE_CLOSE_TO_ENTITY ) ) && ( path.blockingEntity == move.goalEntity.GetEntity() ) ) {
...
bool idAI::GetMovePos( idVec3 &seekPos ) {
	...
	if ( move.moveCommand == MOVE_TO_ENTITY ) {
		MoveToEntity( move.goalEntity.GetEntity() ); // boyette found reachedpos chokepoint
	}
	// boyette projectile begin
	if ( move.moveCommand == MOVE_CLOSE_TO_ENTITY ) {
		MoveCloseToEntity( move.goalEntity.GetEntity() ); // boyette found reachedpos chokepoint
	}
	// boyette projectile end
	...
	if ( gameLocal.time > move.blockTime ) {
		if ( move.moveCommand == MOVE_WANDER ) {
			...
			if( move.moveCommand == MOVE_TO_ENTITY ) {
				if ( ReachedPos( move.moveDest, move.moveCommand ) ) {
					StopMove( MOVE_STATUS_DONE );
					seekPos	= org; // boyette found reachedpos chokepoint
					return false;
				}
			}
			if( move.moveCommand == MOVE_CLOSE_TO_ENTITY ) {
				if ( CloselyReachedPos( move.moveDest, move.moveCommand ) ) {
					StopMove( MOVE_STATUS_DONE );
					seekPos	= org; // boyette found reachedpos chokepoint
					return false;
				}
...
// boyette space command begin
		if ( sensitive_ai_blocked_behavior ) {
			AI_BLOCKED = true;
		}
// boyette space command end
...
void idAI::Killed( idEntity *inflictor, idEntity *attacker, int damage, const idVec3 &dir, int location ) {
	...
	if ( gameLocal.GetLocalPlayer() ) {
		if ( gameLocal.GetLocalPlayer()->DialogueAI && gameLocal.GetLocalPlayer()->DialogueAI == this ) {
			gameLocal.GetLocalPlayer()->CloseOverlayAIDialogeGui();
		}
		// BOYETTE STEAM INTEGRATION BEGIN
#ifdef STEAM_BUILD
		if ( attacker && attacker->IsType(idPlayer::Type) && attacker == gameLocal.GetLocalPlayer() ) {
			if ( common->m_pStatsAndAchievements ) {
				common->m_pStatsAndAchievements->m_nPlayerFPSKills++;
				if ( g_enableSlowmo.GetBool() ) {
					common->m_pStatsAndAchievements->m_nPlayerFPSKillsInSlowMotion++;
				}
		...
		if ( ParentShip && gameLocal.GetLocalPlayer()->PlayerShip && ParentShip == gameLocal.GetLocalPlayer()->PlayerShip && gameLocal.GetLocalPlayer()->PlayerShip->ship_destruction_sequence_initiated == false ) {
			bool the_player_is_the_last_surviving_crewmember = true;

			for( int i = 0; i < MAX_CREW_ON_SHIPS; i++ ) {
				if ( gameLocal.GetLocalPlayer()->PlayerShip->crew[i] && gameLocal.GetLocalPlayer()->PlayerShip->crew[i] != this && gameLocal.GetLocalPlayer()->PlayerShip->crew[i]->was_killed == false ) {
					the_player_is_the_last_surviving_crewmember = false;
					break;
				}
			}

			if ( the_player_is_the_last_surviving_crewmember ) {
				gameLocal.GetLocalPlayer()->DisplayNonInteractiveAlertMessage( "^1The ^1entire ^1crew ^1of ^1your ^1ship ^1has ^1died. ^1You ^1are ^1the ^1last ^1surviving ^1crewmember." );
				gameLocal.GetLocalPlayer()->UpdateNotificationList( "^1The ^1entire ^1crew ^1of ^1your ^1ship ^1has ^1died. ^1You ^1are ^1the ^1last ^1surviving ^1crewmember." );
				// BOYETTE STEAM INTEGRATION BEGIN
#ifdef STEAM_BUILD
				if ( common->m_pStatsAndAchievements ) {
					if ( !common->m_pStatsAndAchievements->m_nTimesWasTheSoleSurvivor ) {
						common->m_pStatsAndAchievements->m_nTimesWasTheSoleSurvivor++;
						common->StoreSteamStats();
					}
				}
#endif
				...
				if ( (inflictor && inflictor->IsType(sbShip::Type)) || (inflictor && inflictor->IsType(sbModule::Type)) ) {
					gameLocal.GetLocalPlayer()->DisplayNonInteractiveAlertMessage( "^1A ^1member ^1of ^1your ^1crew, ^1" + idStr(spawnArgs.GetString("npc_name", "")) + ", ^1was ^1killed ^1by ^1ship ^1damage" + "^1." );
					gameLocal.GetLocalPlayer()->UpdateNotificationList( "^1" + idStr(spawnArgs.GetString("npc_name", "")) + ", ^1was ^1killed ^1by ^1ship ^1damage." + "^1." );
				} else if ( (attacker && attacker->IsType(sbShip::Type)) || (attacker && attacker->IsType(sbModule::Type)) ) {
					gameLocal.GetLocalPlayer()->DisplayNonInteractiveAlertMessage( "^1A ^1member ^1of ^1your ^1crew, ^1" + idStr(spawnArgs.GetString("npc_name", "")) + ", ^1was ^1killed ^1by ^1ship ^1damage" + "^1." );
					gameLocal.GetLocalPlayer()->UpdateNotificationList( "^1" + idStr(spawnArgs.GetString("npc_name", "")) + ", ^1was ^1killed ^1by ^1ship ^1damage" + "^1." );
				} else if ( inflictor && inflictor->IsType(idAI::Type) ) {
					if ( static_cast<idAI*>(inflictor)->ParentShip ) {
						gameLocal.GetLocalPlayer()->DisplayNonInteractiveAlertMessage( "^1A ^1member ^1of ^1your ^1crew, ^1" + idStr(spawnArgs.GetString("npc_name", "")) + ", ^1was ^1killed ^1by " + static_cast<idAI*>(inflictor)->spawnArgs.GetString("npc_name", "something") + " ^1of ^1the ^1" + static_cast<idAI*>(inflictor)->ParentShip->original_name + "^1." );
						gameLocal.GetLocalPlayer()->UpdateNotificationList( "^1" + idStr(spawnArgs.GetString("npc_name", "")) + ", ^1was ^1killed ^1by " + static_cast<idAI*>(inflictor)->spawnArgs.GetString("npc_name", "something") + " ^1of ^1the ^1" + static_cast<idAI*>(inflictor)->ParentShip->original_name + "^1." );
					} else {
						gameLocal.GetLocalPlayer()->DisplayNonInteractiveAlertMessage( "^1A ^1member ^1of ^1your ^1crew, ^1" + idStr(spawnArgs.GetString("npc_name", "")) + ", ^1was ^1killed ^1by " + static_cast<idAI*>(inflictor)->spawnArgs.GetString("npc_name", "something") + "^1." );
						gameLocal.GetLocalPlayer()->UpdateNotificationList( "^1" + idStr(spawnArgs.GetString("npc_name", "")) + ", ^1was ^1killed ^1by " + static_cast<idAI*>(inflictor)->spawnArgs.GetString("npc_name", "something") + "^1." );
					}
				} else if ( attacker && attacker->IsType(idAI::Type) ) {
					if ( static_cast<idAI*>(attacker)->ParentShip ) {
						gameLocal.GetLocalPlayer()->DisplayNonInteractiveAlertMessage( "^1A ^1member ^1of ^1your ^1crew, ^1" + idStr(spawnArgs.GetString("npc_name", "")) + ", ^1was ^1killed ^1by " + static_cast<idAI*>(attacker)->spawnArgs.GetString("npc_name", "something") + " ^1of ^1the ^1" + static_cast<idAI*>(attacker)->ParentShip->original_name + "^1." );
						gameLocal.GetLocalPlayer()->UpdateNotificationList( "^1" + idStr(spawnArgs.GetString("npc_name", "")) + ", ^1was ^1killed ^1by " + static_cast<idAI*>(attacker)->spawnArgs.GetString("npc_name", "something") + " ^1of ^1the ^1" + static_cast<idAI*>(attacker)->ParentShip->original_name + "^1." );
					} else {
						gameLocal.GetLocalPlayer()->DisplayNonInteractiveAlertMessage( "^1A ^1member ^1of ^1your ^1crew, ^1" + idStr(spawnArgs.GetString("npc_name", "")) + ", ^1was ^1killed ^1by " + static_cast<idAI*>(attacker)->spawnArgs.GetString("npc_name", "something") + "^1." );
						gameLocal.GetLocalPlayer()->UpdateNotificationList( "^1" + idStr(spawnArgs.GetString("npc_name", "")) + ", ^1was ^1killed ^1by " + static_cast<idAI*>(attacker)->spawnArgs.GetString("npc_name", "something") + "^1." );
					}
				} else {
					gameLocal.GetLocalPlayer()->DisplayNonInteractiveAlertMessage( "^1A ^1member ^1of ^1your ^1crew, ^1" + idStr(spawnArgs.GetString("npc_name", "")) + ", ^1was ^1killed." );
					gameLocal.GetLocalPlayer()->UpdateNotificationList( "^1" + idStr(spawnArgs.GetString("npc_name", "")) + ", ^1was ^1killed." );
				}
			}
		}
	}
...
void idAI::DirectDamage( const char *meleeDefName, idEntity *ent ) {
	...
	if ( space_command_level > 1 && ( ent->IsType(idDoor::Type) || ent->IsType(sbModule::Type) || ent->IsType(sbConsole::Type) ) ) {
		float space_command_level_damageScale = 1.0f;
		space_command_level_damageScale = idMath::Pow(SPACE_COMMAND_LEVEL_STAT_MODIFIER, space_command_level - 1.0f );
		ent->Damage( this, this, globalKickDir, meleeDefName, space_command_level_damageScale, INVALID_JOINT ); // BOYETTE NOTE: this was the original
	} else  {
		ent->Damage( this, this, globalKickDir, meleeDefName, 1.0f, INVALID_JOINT ); // BOYETTE NOTE: this was the original
	}
...
/*
=====================
idAI::GetFoodProjectile
=====================
*/
idProjectile	*idAI::GetFoodProjectile( void ) const {
	return foodprojectile.GetEntity();
}

/*
=====================
idAI::SetFoodProjectile
=====================
*/
void idAI::SetFoodProjectile( idActor *newEnemy ) {
	int enemyAreaNum;
...
/*
=====================
idAI::MoveCloseToEntity
=====================
*/
bool idAI::MoveCloseToEntity( idEntity *ent ) {
	int			areaNum;
	aasPath_t	path;
	idVec3		pos;
	...
	pos = ent->GetPhysics()->GetOrigin();
	if ( ( move.moveType != MOVETYPE_FLY ) && ( ( move.moveCommand != MOVE_CLOSE_TO_ENTITY ) || ( move.goalEntityOrigin != pos ) ) ) {
		ent->GetFloorPos( 64.0f, pos );
	}

	if ( CloselyReachedPos( pos, MOVE_CLOSE_TO_ENTITY ) ) {
		StopMove( MOVE_STATUS_DONE );
		return true;
	}
	...
	if ( ( move.moveCommand != MOVE_CLOSE_TO_ENTITY ) || ( move.goalEntity.GetEntity() != ent ) ) {
		move.startTime		= gameLocal.time;
		move.goalEntity		= ent;
		move.moveCommand	= MOVE_CLOSE_TO_ENTITY;
	}
...
/*
=====================
idAI::CloselyReachedPos
=====================
*/
bool idAI::CloselyReachedPos( const idVec3 &pos, const moveCommand_t moveCommand ) const {
	if ( move.moveType == MOVETYPE_SLIDE ) {
		idBounds bnds( idVec3( -4, -4.0f, -8.0f ), idVec3( 4.0f, 4.0f, 64.0f ) );
		bnds.TranslateSelf( physicsObj.GetOrigin() );	
		if ( bnds.ContainsPoint( pos ) ) {
		...
		if ( ( moveCommand == MOVE_TO_ENEMY ) || ( moveCommand == MOVE_CLOSE_TO_ENTITY ) ) {
			idBounds bnds( idVec3( -32.0, -32.0f, -8.0f ), idVec3( 32.0, 32.0f, 64.0f ) );
			bnds.TranslateSelf( physicsObj.GetOrigin() );	
			if ( bnds.ContainsPoint( pos ) ) {
				return true;
			}
...
float idAI::DistanceToEntity( idEntity *ent ) {
	float dist = idMath::INFINITY;
...
void idAI::SetEntityToMoveToByCommand( idEntity* ent ) {
	if (ent) {
		EntityCommandedToMoveTo = ent;
		if ( ParentShip && gameLocal.GetLocalPlayer() && gameLocal.GetLocalPlayer()->PlayerShip && gameLocal.GetLocalPlayer()->PlayerShip == ParentShip ) {
			ClearEnemy(); // BOYETTE NOTE TODO IMPORTANT: this works great for the player, hopefully it won't cause the Ship AI to keep trying to break contact if this AI is fighting an enemy. - note: this if-statement protects against that.
		}
	} else {
		EntityCommandedToMoveTo = NULL;
	}
...
sbModule* idAI::ReturnNearestShipModule() {
	idEntity	*ent;
	sbModule	*module;
	sbModule	*bestModule;
	float		bestDist;
...
sbModule* idAI::ReturnNearestDamagedShipModule() {
	idEntity	*ent;
	sbModule	*module;
	sbModule	*bestModule;
	float		bestDist;
	...
	for ( ent = gameLocal.spawnedEntities.Next(); ent != NULL; ent = ent->spawnNode.Next() ) {
		...
		dist = delta.LengthSqr();
		if ( ( dist < bestDist ) && module->health < module->entity_max_health ) {
			bestDist = dist;
			bestModule = module;
		}
	}
	...
	if ( ShipOnBoard ) {
		for ( int i = 0; i < MAX_MODULES_ON_SHIPS; i++ ) {
			if ( ShipOnBoard->consoles[i] && ShipOnBoard->consoles[i]->ControlledModule ) {
				delta = physicsObj.GetOrigin() - ShipOnBoard->consoles[i]->ControlledModule->GetPhysics()->GetOrigin();
				dist = delta.LengthSqr();
				if ( ( dist < bestDist ) && ShipOnBoard->consoles[i]->ControlledModule->health < ShipOnBoard->consoles[i]->ControlledModule->entity_max_health  ) {
					bestDist = dist;
					bestModule = ShipOnBoard->consoles[i]->ControlledModule;
				}
			}
...
sbModule* idAI::ReturnNearestUnDisabledShipModule() {
	idEntity	*ent;
	sbModule	*module;
	sbModule	*bestModule;
	float		bestDist;
	...
	for ( ent = gameLocal.spawnedEntities.Next(); ent != NULL; ent = ent->spawnNode.Next() ) {
		...
		delta = physicsObj.GetOrigin() - module->GetPhysics()->GetOrigin();
		dist = delta.LengthSqr();
		if ( ( dist < bestDist ) && module->health > 0 ) {
			bestDist = dist;
			bestModule = module;
		}
	}
	...
	if ( ShipOnBoard ) {
		for ( int i = 0; i < MAX_MODULES_ON_SHIPS; i++ ) {
			if ( ShipOnBoard->consoles[i] && ShipOnBoard->consoles[i]->ControlledModule ) {
				delta = physicsObj.GetOrigin() - ShipOnBoard->consoles[i]->ControlledModule->GetPhysics()->GetOrigin();
				dist = delta.LengthSqr();
				if ( ( dist < bestDist ) && ShipOnBoard->consoles[i]->ControlledModule->health > 0 ) {
					bestDist = dist;
					bestModule = ShipOnBoard->consoles[i]->ControlledModule;
				}
			}
...
idEntity* idAI::ReturnNearestRoomNodeToThePlayer() {
	idEntity	*ent;
	idEntity	*bestRoom_Node;
	float		bestDist;
	float		dist;
	idVec3		delta;
	...
	bestDist = idMath::INFINITY;
	bestRoom_Node = NULL;
	if ( ShipOnBoard && gameLocal.GetLocalPlayer() && gameLocal.GetLocalPlayer()->ShipOnBoard && gameLocal.GetLocalPlayer()->ShipOnBoard == ShipOnBoard ) {
		for ( int i = 0; i < MAX_ROOMS_ON_SHIPS; i++ ) {
			if ( ShipOnBoard->room_node[i] ) {
				delta = gameLocal.GetLocalPlayer()->GetPhysics()->GetOrigin() - ShipOnBoard->room_node[i]->GetPhysics()->GetOrigin();
				dist = delta.LengthSqr();
				if ( ( dist < bestDist ) ) {
					bestDist = dist;
					bestRoom_Node = ShipOnBoard->room_node[i];
				}
			}
		}
		if ( ShipOnBoard->TransporterBounds ) {
			delta = gameLocal.GetLocalPlayer()->GetPhysics()->GetOrigin() - ShipOnBoard->TransporterBounds->GetPhysics()->GetOrigin();
			dist = delta.LengthSqr();
			if ( ( dist < bestDist ) ) {
				bestDist = dist;
				bestRoom_Node = ShipOnBoard->TransporterBounds;
			}
		}
	}

	return bestRoom_Node;
}
idEntity* idAI::ReturnNearestRoomNode() {
	idEntity	*ent;
	idEntity	*bestRoom_Node;
	float		bestDist;
	float		dist;
	...
	if ( ShipOnBoard ) {
		for ( int i = 0; i < MAX_ROOMS_ON_SHIPS; i++ ) {
			if ( ShipOnBoard->room_node[i] ) {
				delta = physicsObj.GetOrigin() - ShipOnBoard->room_node[i]->GetPhysics()->GetOrigin();
				dist = delta.LengthSqr();
				if ( ( dist < bestDist ) ) {
					bestDist = dist;
					bestRoom_Node = ShipOnBoard->room_node[i];
		...
		if ( ShipOnBoard->TransporterBounds ) {
			delta = physicsObj.GetOrigin() - ShipOnBoard->TransporterBounds->GetPhysics()->GetOrigin();
			dist = delta.LengthSqr();
			if ( ( dist < bestDist ) ) {
				bestDist = dist;
				bestRoom_Node = ShipOnBoard->TransporterBounds;
...
/*
void idAI::Event_FollowMoveCommandToEntity( idEntity *ent ) {
	StopMove( MOVE_STATUS_DEST_NOT_FOUND );
	if ( ent ) {
		MoveToEntity( ent );
	}
...
void idAI::HandleSpaceCommandLevel() {
	float stat_modifier = SPACE_COMMAND_LEVEL_STAT_MODIFIER;

	int original_level = space_command_level;

	if ( killCount <= 0 ) {
		space_command_level = 1;
	} else if ( killCount > 0 && killCount <= 1 ) {
		space_command_level = 2;
	} else if ( killCount > 2 && killCount <= 4 ) {
		space_command_level = 3;
	} else if ( killCount > 4 && killCount <= 7 ) {
		space_command_level = 4;
	} else if ( killCount > 7 && killCount <= 15 ) {
		space_command_level = 5;
	} else if ( killCount > 15 && killCount <= 31 ) {
		space_command_level = 6;
	} else if ( killCount > 31 ) {
		space_command_level = 7;
		// BOYETTE STEAM INTEGRATION BEGIN
#ifdef STEAM_BUILD
		if ( common->m_pStatsAndAchievements ) {
			if ( !common->m_pStatsAndAchievements->m_nTimesMaxCrewLevelAchieved ) {
				common->m_pStatsAndAchievements->m_nTimesMaxCrewLevelAchieved++;
				common->StoreSteamStats();
			}
		}
#endif
	...
	if ( space_command_level <= original_level ) {
		return;
	}

	float original_attack_accuracy = spawnArgs.GetFloat( "attack_accuracy", "5" );
	int original_health = health;
	int original_entity_max_health = entity_max_health;
	int original_module_buff_amount = module_buff_amount;

	float attack_accuracy = spawnArgs.GetFloat( "attack_accuracy", "5" );

	spawnArgs.SetInt("space_command_level", space_command_level );

	int levels_increased = space_command_level - original_level;

	for ( int i = 0; i < levels_increased; i++ ) {
		health = health * stat_modifier;
		spawnArgs.SetInt( "health", health );
		entity_max_health = entity_max_health * stat_modifier;
		spawnArgs.SetInt( "entity_max_health", entity_max_health );
		module_buff_amount = module_buff_amount * stat_modifier;
		spawnArgs.SetInt( "module_buff_amount", module_buff_amount );

		attack_accuracy = attack_accuracy / stat_modifier;
		spawnArgs.SetFloat( "attack_accuracy", attack_accuracy );
	}
	...
	//gameLocal.Printf( "entity_max_health = %i\n", entity_max_health);
	//gameLocal.Printf( "module_buff_amount = %i\n", module_buff_amount);
	//gameLocal.Printf( "attack_accuracy = %f\n", spawnArgs.GetFloat( "attack_accuracy", "7" ));
}
