...
const int	MAX_SPACE_COMMAND_LEVEL		= 7;
const float	SPACE_COMMAND_LEVEL_STAT_MODIFIER		= 1.20f;
...
// make sure to change script/icarus_defs.script if you add any, or change their order
//
typedef enum {
	MOVE_STATUS_DONE,
	MOVE_STATUS_MOVING,
...
public:
	...
	// boyette projectile begin
	idProjectile			*GetFoodProjectile( void ) const;
	// boyette projectile end
...
// boyette AI move command begin
		...
		float					DistanceToEntity( idEntity *ent );

		idEntity*				EntityCommandedToMoveTo;
		void					SetEntityToMoveToByCommand( idEntity* ent );
		bool					IsSelectedCrew;
		int						module_buff_amount;
		bool					sensitive_ai_blocked_behavior;
		bool					ai_can_be_transported;
		bool					npc_name_locked;
		idStr					ship_diagram_icon;
		idStr					ship_diagram_portrait;
		sbShip*					ShipOnBoard;	// boyette - this is the ship the idAI is currently on board.
		sbShip*					ParentShip;		// boyette - this is the ship that the idAI is currently assigned to.
		//void					ClearEntityToMoveToByCommand();
		//idEntity*				ReturnEntityCommandedToMoveTo();
		//void					Event_FollowMoveCommandToEntity( idEntity *ent )
		bool					seated_in_captain_chair;
		bool					working_on_console;
		idStr					sitting_animation_name;

		sbModule*				ReturnNearestShipModule();
		sbModule*				ReturnNearestDamagedShipModule();
		sbModule*				ReturnNearestUnDisabledShipModule();

		idEntity*				ReturnNearestRoomNodeToThePlayer();
		idEntity*				ReturnNearestRoomNode();

		bool					crew_auto_mode_activated;
		bool					player_follow_mode_activated;
		float					GetCurrentYaw() { return current_yaw; }
		bool					hide_weapons_skin_active;
		beamTarget_t			ModuleConsoleBeam;
		bool					module_console_beam_active;

		idStr					module_console_damage_def_name;
		idStr					module_console_repair_def_name;
		idStr					door_damage_def_name;

		bool					handling_emergency_oxygen_situation;
// boyette AI move command end

// boyette space command AI dialogue begin
	// ship dialogue system:

	bool									uses_space_command_dialogue_behavior;

	std::bitset<MAX_DIALOGUE_BRANCHES>		AIDialogueBranchTracker; // so the first bit will spawn as set to true - all others will be false. Because we always want the dialogue to start with the first branch. When the dialoge is finished, set the last bit to true. That way hailing will just produce a "no reponse" or something like that screen.
	std::string								AIDialogueBranchTrackerString;
	void									OpenAIDialogueWithLocalPlayer();
	bool									currently_in_dialogue_with_player;

	idStr									ai_dialogue_gui_file;
	idStr									ai_dialogue_anim_to_play;

	bool									waiting_until_player_talks_to_me;
	
	void									Activate( idEntity *activator ); // BOYETTE NOTE IMPORTANT: this used to be protected not public
// boyette space command AI dialogue end

// boyette space command kill count begin
	virtual void			HandleSpaceCommandLevel();
...
// boyette projectile begin
	bool					MoveCloseToEntity( idEntity *ent );
// boyette projectile end

	void					SetEnemy( idActor *newEnemy ); // BOYETTE NOTE: this used to be protected not public - made it public for phenomenon_should_spawn_entity_def_on_playership
...
protected:
	...
	// boyette projectile begin
	idEntityPtr<idProjectile>	foodprojectile; // boyette -  I don't think I ever got around to using this; it wasn't necessary to do what I wanted.
	// boyette projectile end
	...
	// boyette projectile begin
	bool					CloselyReachedPos( const idVec3 &pos, const moveCommand_t moveCommand ) const;
	// boyette projectile end
	...
	//void					SetEnemy( idActor *newEnemy ); // BOYETTE NOTE: this is now public
	// boyette projectile being
	void					SetFoodProjectile( idActor *newEnemy );
	// boyette projectile end
	...
	// boyette projectile begin
	void					Event_FindFoodProjectile( int useFOV );
	// boyette projectile end
	void					Event_FindEnemyAI( int useFOV );
	// boyette projectile begin
	// boyette space command begin
	void					Event_FindNearestShipConsole( int useFOV );
	void					Event_FindNearestShipModule( int useFOV );
	void					Event_GetEntityHealth( idEntity* ent );
	void					Event_GetEntityMaxHealth( idEntity* ent );
	void					Event_IsFriendly( idEntity* ent );
	void					Event_ConsolesModuleIsBuffed( idEntity* ent );
	void					Event_BuffConsolesModule( idEntity* ent );

	void					Event_CreateBeamToEnt( idEntity* ent );
	void					Event_RemoveBeamToEnt( void );

	void					Event_ReturnModuleConsoleDamageDefName( void );
	void					Event_ReturnModuleConsoleRepairDefName( void );
	void					Event_ReturnDoorDamageDefName( void );

	void					Event_ShipOnBoardIsAtRedAlert( void );
	void					Event_FindNearestCaptainChair( int useFOV );
	void					Event_SitDownInCaptainChair( idEntity *ent );
	void					Event_StandUpFromCaptainChair( idEntity *ent );
	void					Event_IsSeatedInCaptainChair( void );

	void					Event_StartWorkingOnConsole( idEntity *ent );
	void					Event_StopWorkingOnConsole( idEntity *ent );
	void					Event_IsWorkingOnConsole( void );
	void					Event_GetSittingAnimName( void );

	void					Event_HideDefAttach( void );
	void					Event_ShowDefAttach( void );
	void					Event_IsDefAttachHidden( void );
	void					Event_ActivateHideWeaponSkin( void );
	void					Event_DeactivateHideWeaponSkin( void );
	void					Event_IsHideWeaponSkinActive( void );

	void					Event_UpdateCrewAutoModeBehavior( void );
	void					Event_IsMyCaptainChair( idEntity* ent );

	void					Event_FaceForwardOfEntity( idEntity *ent );
	void					Event_FaceAbsoluteCenterOfEntity( idEntity *ent );

	void					Event_BeginAIDialogueWithPlayer( void );
	void					Event_EndAIDialogueWithPlayer( void );
	void					Event_UsesSpaceCommandDialogueBehavior( void );
	void					Event_InAIDialogueWithPlayer( void );
	void					Event_ReturnAIDialogueAnimToPlay( void );
	void					Event_ClearAIDialogueAnimToPlay( void );
	void					Event_WaitingUntilPlayerTalksToMe( void );
	// boyette space command end
	void					Event_FindNearestFoodProjectile( int useFOV );
	// boyette projectile end
	...
	// boyette projectile being
	void					Event_SetFoodProjectile( idEntity *ent );
	// boyette projectile end
	// boyette AI move command begin
	void					Event_ReturnEntityCommandedToMoveTo( void );
	void					Event_ClearEntityToMoveToByCommand( void );

	void					Event_ReturnEnemyDoorToAttack( void );
	void					Event_ClearEnemyDoorToAttack( void );
	// boyette AI move command end
	...
	// boyette projectile begin
	void					Event_MoveCloseToEntity( idEntity *ent );
	// boyette projectile end
	...
	void					Event_FoodProjectileRange( void );
	void					Event_FoodProjectileRange2D( void );
	*/
	// boyette projectile end
	...
	// boyette projectile begin
	void					Event_GetFoodProjectile( void );
	// boyette projectile end
	...
	// boyette projectile begin
	void					Event_LocateFoodProjectile( void );
	// boyette projectile end
	...
	// boyette begin
	void					Event_DistanceToEntity( idEntity *ent );
	void					Event_DistanceToEntity2D( idEntity *ent );
	// boyette end
};
