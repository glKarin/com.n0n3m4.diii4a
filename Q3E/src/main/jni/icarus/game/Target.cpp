...
void idTarget_GiveEmail::Event_Activate( idEntity *activator ) {
	...
	if ( pda ) {
		...
		player->ShowTip( player->spawnArgs.GetString( "text_infoTitle" ), "You need a tablet in order to recieve email.", true );
	}
}
...
void idTarget_SetPrimaryObjective::Event_Activate( idEntity *activator ) {
	...
	if ( player && player->objectiveSystem ) {
		player->objectiveSystem->SetStateString( "missionobjective", spawnArgs.GetString( "text", common->GetLanguageDict()->GetString( "To Explore" ) ) );
	}
}
...
//==========================================================================
//
//	hhTarget_DisplayCaptainGui
//
//	When triggered, displays full screen captain gui
//==========================================================================
CLASS_DECLARATION( idTarget, idTarget_DisplayCaptainGui )
	EVENT( EV_Activate,			idTarget_DisplayCaptainGui::Event_Activate )
END_CLASS

void idTarget_DisplayCaptainGui::Event_Activate(idEntity *activator) {
	idPlayer *player = gameLocal.GetLocalPlayer();
	if (player && player->IsType(idPlayer::Type)) {
		static_cast<idPlayer*>(player)->SetOverlayCaptainGui();
	}
}
...
//==========================================================================
CLASS_DECLARATION( idTarget, idTarget_DisplayGui )
	EVENT( EV_Activate,			idTarget_DisplayGui::Event_Activate )
END_CLASS

void idTarget_DisplayGui::Event_Activate(idEntity *activator) {
	idPlayer *player = gameLocal.GetLocalPlayer();
	if (player && player->IsType(idPlayer::Type)) {
		static_cast<idPlayer*>(player)->SetOverlayGui( spawnArgs.GetString("gui_overlay") );
	}
}
