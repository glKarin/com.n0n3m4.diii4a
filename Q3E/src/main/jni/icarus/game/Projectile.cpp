...
void idProjectile::Create( idEntity *owner, const idVec3 &start, const idVec3 &dir ) {
	...
	this->owner = owner;
	// boyette space command projectile tracking begin
	if ( owner && owner->IsType( idActor::Type ) ) {
		static_cast<idActor*>(owner)->projectiles_to_track.push_back(this);							// boyette mod
		//gameLocal.Printf( owner->name + " is tracking " + idStr(static_cast<idActor*>(owner)->projectiles_to_track.size()) + " projectiles.\n" );
	}
	// boyette space command projectile tracking end
...
idProjectile::~idProjectile() {
	// boyette space command projectile tracking begin
	if ( owner.GetEntity() && owner.GetEntity()->IsType( idActor::Type ) ) {
		idActor* myowner = static_cast<idActor*>(owner.GetEntity());
		myowner->projectiles_to_track.erase(std::remove(myowner->projectiles_to_track.begin(), myowner->projectiles_to_track.end(), this), myowner->projectiles_to_track.end()); // remove the projectile from the projectile tracking list on the owner if the owner is an idAI.
	}
	// boyette space command projectile tracking end
...
=================
*/
void idProjectile::Launch( const idVec3 &start, const idVec3 &dir, const idVec3 &pushVelocity, const float timeSinceFire, const float launchPower, const float dmgPower, const float setGravity ) {
	float			fuse;
	float			startthrust;
	float			endthrust;
	idVec3			velocity;
	...
	if ( spawnArgs.GetFloat( "gravity" , "0", gravity ) ) {
		// it already gets set in the conditional so we don't need to do it again here.
	} else {
		gravity = setGravity;
	}
	...
	projectileFlags.ship_to_ship_projectile	= spawnArgs.GetBool( "ship_to_ship_projectile" , "0" );
...
// boyette begin - so this "physicsObj.SetAxis( axis );" is the place where you would change the beginning rotation of the launch projectile.
	// You should make a sepearate "axis" type variable and then have it read from the def file. If there is no value in the def file it should just use the orginal axis variable.
	// The reason you should have a seperate variable is because the original "axis" is used in other things like the angular/directional velocity of the projectile.

/* // if you wanted to control the initial launch rotation of the projectile with a rotation matrix you could enable this code and disable the idAngles 
	// code below it. But I personally find angles much easier to understand. Rotation matrices are a little esoteric in my opinion.
	idMat3 CustomProjectileRotation;
	if ( spawnArgs.GetMatrix( "custom_launch_rotation_matrix", "1 0 0 0 1 0 0 0 1", CustomProjectileRotation )) {
		//spawnArgs.GetMatrix( "custom_launch_rotation_matrix", "1 0 0 0 1 0 0 0 1", CustomProjectileRotation); // this is apparently not necessary. It will happen in the above if statement conditional apparently.
		physicsObj.SetAxis( CustomProjectileRotation );
	}
	else {
		physicsObj.SetAxis( axis );
	}
*/
	...
	// this code allows you to specify the initial launch rotation of the projectile with idAngles. If you would prefer to specify it with
	// a rotation matrix you could enable the code above and disable this code. But rotation matrices are pretty esoteric in my opinion.
	idMat3 CustomProjectileLaunchRotationMat;
	idAngles CustomProjectileLaunchRotationAngles;
	if ( spawnArgs.GetAngles( "custom_launch_rotation_angles", "0 0 0", CustomProjectileLaunchRotationAngles )) {
		CustomProjectileLaunchRotationAngles = spawnArgs.GetAngles( "custom_launch_rotation_angles", "0 0 0");
		CustomProjectileLaunchRotationMat = CustomProjectileLaunchRotationAngles.ToMat3() * axis; // I'm not sure yet why you neeed to multiply it by axis but it seems to work. I'll learn more about idAngles later.
		//spawnArgs.GetMatrix( "custom_spawn_rotation", "0 0 0", CustomProjectileLaunchRotationMat); // this is apparently not necessary. It will happen in the above if statement conditional apparently.
		physicsObj.SetAxis( CustomProjectileLaunchRotationMat );
	}
	else {
		physicsObj.SetAxis( axis );
	}

	// - original was just this - //physicsObj.SetAxis( axis );
// boyette end
...
bool idProjectile::Collide( const trace_t &collision, const idVec3 &velocity ) {
	...
	// boyette space command begin
	// BOYETTE NOTE - ONLY PREVENTS EXPLOSION - DOESN'T LET THE PROJECTILE PASS THROUGH - GUESS THIS ISN'T NECESSARY - although it might be a good idea to fix the logic here to make sure that the ship does not shoot itself or it's own shield entity.
	// maybe can just move the projectile a small distance forward until it get's past the ship or shield obstacle.
	// or maybe we can just make a function on the ship and the shield entity that makes them not solid for a small amount of time until the projectile passes through. Although it would be better to just allow this projectile through.
	if ( projectileFlags.ship_to_ship_projectile ) {
		/*
		if ( ent && ent->IsType(sbShip::Type) && ent == GetOwner() ) {
			return false;
		}
		if ( ent && ent->IsHidden() ) {
			return false;
		}
		if ( GetOwner() && GetOwner()->IsType(sbShip::Type) ) {
			sbShip* ShipToCheck;
			ShipToCheck = dynamic_cast<sbShip*>(GetOwner());
			if ( ShipToCheck && ShipToCheck->ShieldEntity && ent && ent == ShipToCheck->ShieldEntity ) {
				return false;
			}
		}
		*/
	...
	// BOYETTE NOTE: to prevent bleed effects when there is no team damage and friendlies shoot each other.
	//if ( owner.GetEntity() && ent && owner.GetEntity()->team == ent->team && owner.GetEntity()->IsType( idActor::Type ) && ( ent->IsType( idActor::Type ) || ( ent->IsType( idAFAttachment::Type ) && static_cast<const idAFAttachment*>(ent)->GetBody()->IsType( idActor::Type ) ) ) && !owner.GetEntity()->IsType( idPlayer::Type ) && static_cast<idActor*>(ent)->prevent_actor_inflicted_same_team_damage ) {
	if ( owner.GetEntity() && ent && owner.GetEntity()->team == ent->team && owner.GetEntity()->IsType( idActor::Type ) && ( (ent->IsType( idActor::Type ) && static_cast<idActor*>(ent)->prevent_actor_inflicted_same_team_damage) || ( ent->IsType( idAFAttachment::Type ) && static_cast<const idAFAttachment*>(ent)->GetBody()->IsType( idActor::Type ) && static_cast<idActor*>(static_cast<const idAFAttachment*>(ent)->GetBody())->prevent_actor_inflicted_same_team_damage ) ) && !owner.GetEntity()->IsType( idPlayer::Type ) ) {
		// do nothing // BOYETTE NOTE: there is similar logic elsewhere on idActor to prevent any damage from being done - this should include any damage from the Explode function.
	} else {
...
void idProjectile::Explode( const trace_t &collision, idEntity *ignore ) {
	...
	if ( testC & CONTENTS_WATER ) {
		...
		splashArgs.Set( "model", "liquid_material_projectile_impact_splash.prt" );
		...
		/* // BOYETTE NOTE: this was commented out 04/22/15. It is not necessary and we will not have the chaingun and it's bullets anyways.
    ...
    {
        light_shader = "textures/materials_used_in_source/lights/midnight_grenade";
    }
#endif        
	...
	int fxdebris = spawnArgs.GetInt( "debris_count", "0" );
	if ( fxdebris ) {
...
=================
*/
void idGuidedProjectile::Launch( const idVec3 &start, const idVec3 &dir, const idVec3 &pushVelocity, const float timeSinceFire, const float launchPower, float dmgPower, const float setGravity ) {
...
=================
*/
void idSoulCubeMissile::Launch( const idVec3 &start, const idVec3 &dir, const idVec3 &pushVelocity, const float timeSinceFire, const float launchPower, float dmgPower, const float setGravity ) {
	idVec3		newStart;
	idVec3		offs;
	idEntity	*ownerEnt;
...
=================
*/
void idBFGProjectile::Launch( const idVec3 &start, const idVec3 &dir, const idVec3 &pushVelocity, const float timeSinceFire, const float power, const float dmgPower, const float setGravity ) {
...
CLASS_DECLARATION( idProjectile, sbFoodProjectile )
...
================
*/
sbFoodProjectile::sbFoodProjectile( void ) {

}

/*
=================
sbFoodProjectile::~sbFoodrojectile
=================
*/
sbFoodProjectile::~sbFoodProjectile() {
