...
protected:
	...
	//
	// mover directions.  make sure to change script/icarus_defs.script if you add any, or change their order
	//
	typedef enum {
		DIR_UP				= -1,
		DIR_DOWN			= -2,
...
public:
	...
	int						door_disabled_time;
	int						door_damaged_time;
	virtual void			UpdateMoverStatusShaderParms();
	int						parentship_security_module_efficiency_adjusted_max_health;
...
public:
	...
	virtual	void			UpdateDoorGroupsMoverStatusShaderParms();
	virtual	void			SetDoorGroupsHealth(int health_to_set);
	virtual	void			SetDoorGroupsidEntityTeam(int team_to_set);
	virtual	void			RemoveDoorGroup();
	bool					IsRelatedDoor(idEntity* door_to_test);

	virtual	void			SyncUpDoorGroupsSpawnArgs();
	// boye idDoor Custom Damage end
