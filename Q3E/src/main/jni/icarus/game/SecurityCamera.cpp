...
void idSecurityCamera::Spawn( void ) {
	...
	if ( health && spawnArgs.GetBool( "allow_damage", "1" ) ) {
		fl.takedamage = true;
	}
	...
	if ( spawnArgs.GetBool( "no_collisions", "0" ) ) { // BOYETTE SPACE COMMAND NOTE: added this if statement for purposes of space_command_viewscreen_camera
...
/*
================
idSecurityCamera::CanSeeEntity
================
*/
bool idSecurityCamera::CanSeeEntity( idEntity* ent ) {
	int i;
	float dist;
	trace_t tr;
	idVec3 dir;
	...
	// if there is no way we can see this entity
	if ( !gameLocal.pvs.InCurrentPVS( handle, ent->GetPVSAreas(), ent->GetNumPVSAreas() ) ) {
		gameLocal.Printf( "NOT POSSIBLE TO SEE THIS ENTITY:" + ent->name + "\n" ); 
		gameLocal.pvs.FreeCurrentPVS( handle );
		return false;
	}
...
/*
================
idSecurityCamera::CanSeeAllEntityBounds
================
*/
bool idSecurityCamera::CanSeeAllEntityBounds( idEntity* ent ) {
	int i;
	float dist;
	trace_t tr;
	idVec3 dir;
	...
	// if there is no way we can see this entity
	if ( !gameLocal.pvs.InCurrentPVS( handle, ent->GetPVSAreas(), ent->GetNumPVSAreas() ) ) {
		gameLocal.Printf( "NOT POSSIBLE TO SEE THIS ENTITY'S BOUNDS:" + ent->name + "\n" ); 
		gameLocal.pvs.FreeCurrentPVS( handle );
		return false;
	}
	...
	idVec3 ent_bounds_corners[8];
	ent->GetPhysics()->GetAbsBounds().ToPoints(ent_bounds_corners);
	for ( int i = 0; i < 8; i++ ) {
		dir = ent_bounds_corners[i] - GetPhysics()->GetOrigin();
		dist = dir.Normalize();
		...
		gameLocal.clip.TracePoint( tr, GetPhysics()->GetOrigin(), ent_bounds_corners[i], MASK_OPAQUE, this );
		if ( tr.fraction == 1.0 || ( gameLocal.GetTraceEntity( tr ) == ent ) ) {
			continue;
		} else {
			gameLocal.pvs.FreeCurrentPVS( handle );
...
/*
================
idSecurityCamera::CanSeeAnyEntityBounds
================
*/
bool idSecurityCamera::CanSeeAnyEntityBounds( idEntity* ent ) {
	int i;
	float dist;
	trace_t tr;
	idVec3 dir;
