...
void Sys_CreateThread(  xthread_t function, void *parms, xthreadPriority priority, xthreadInfo &info, const char *name, xthreadInfo *threads[ORIGINAL_MAX_THREADS], int *thread_count ) {
	...
	info.name = name;
	if ( *thread_count < ORIGINAL_MAX_THREADS ) {
		threads[(*thread_count)++] = &info;
	} else {
		common->DPrintf("WARNING: ORIGINAL_MAX_THREADS reached\n");
	}
}
...
void Sys_Quit( void ) {
#ifdef STEAM_BUILD
	common->Printf("STEAM_BUILD QUIT\n");
	if ( common->m_pStatsAndAchievements ) {
		delete common->m_pStatsAndAchievements;
	}
...
bool Sys_AlreadyRunning( void ) {
	...
	if ( !win32.win_allowMultipleInstances.GetBool() ) {
		HANDLE hMutexOneInstance = ::CreateMutex( NULL, FALSE, "ICARUS" );
