...
static void GLW_GetWGLExtensionsWithFakeWindow( void ) {
	...
	// BOYETTE NOTE TODO: this is the default resolution for the safe mode for the game, might want to change this back to the original gui resolutions - but maybe it is fine at 1280x960.
...
bool CheckIfScreenResolutionIsSupported( int width_to_check, int height_to_check ) {
	...
	// check if this card/monitor can handle this resolution
	matched = false;
	for ( modeNum = 0 ; ; modeNum++ ) {
		if ( !EnumDisplaySettings( NULL, modeNum, &devmode ) ) {
			if ( matched ) {
		...
		if ( (int)devmode.dmPelsWidth == width_to_check && (int)devmode.dmPelsHeight == height_to_check && devmode.dmBitsPerPel == 32 ) {
