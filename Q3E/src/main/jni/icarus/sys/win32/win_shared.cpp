...
#undef StrCmpNI
#undef StrCmpI
#include <atlbase.h> // BOYETTE NOTE IMPORTANT: was able to solve the atlbase error by downloading Windows® Server 2003 SP1 Platform SDK and referencing "C:\Program Files\Microsoft Platform SDK\Include\atl" in the IcarussDLL Configuration Properties->VC++ directories - linker (download SPK here: https://www.microsoft.com/en-us/download/details.aspx?id=6510) (tutorial here: http://www.quantcode.com/modules/smartfaq/faq.php?faqid=99)
...
int Sys_GetVideoRam() {
	...
	if ( hr == S_OK && spInstance ) {
		...
		if ( hr == S_OK ) {
			//retSize = varSize.intVal / ( 1024 * 1024 ); // BOYETTE NOTE: this was the original
			retSize = static_cast< unsigned int >(varSize.intVal) / ( 1024u * 1024u ); // BOYETTE NOTE: thanks to DarkMod forums - I did a google search on this function not returning proper values and this solution was the first to come up
			if ( retSize == 0 ) {
				retSize = 64;
			}
		}
