	...
	while ( 1 ) {
		...
		// the wave files
			...
			if ( soundSystemLocal.soundCache && numEntries < maxSamples ) {
				...
				if ( lang.Icmp( "english" ) != 0 && token.Find( "sound/sounds_used_in_source/VO", false ) >= 0 ) {
					idStr work = token;
					work.ToLower();
					work.StripLeading( "sound/sounds_used_in_source/VO" );
					work = va( "sound/sounds_used_in_source/VO/%s/%s", lang.c_str(), work.c_str() );
