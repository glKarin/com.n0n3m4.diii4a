...
static float UNIQUE_EPSILON = 1.0f; // BOYETTE NOTE: increased in order to prevent small gaps in pre-computed shadow volumes due to floating point imprecision farther away from the origin. This does not seem to be necessary for real time shadow generation - but maybe one day we can try to add it just in case.
//static float UNIQUE_EPSILON = 0.1f; // BOYETTE NOTE: this was the original.
...
static void ClipOccluders( idVec4 *verts, glIndex_t *indexes, int numIndexes, 
	...
	// BOYETTE DMAP COMMENT OUT // common->Printf( "ClipOccluders: %i triangles\n", numTris );
	...
	// BOYETTE DMAP COMMENT OUT //common->Printf( "%i triangles completely invisible\n", numRemoved );
	// BOYETTE DMAP COMMENT OUT //common->Printf( "%i triangles completely visible\n", numComplete );
	// BOYETTE DMAP COMMENT OUT //common->Printf( "%i triangles fragmented\n", numFragmented );
	// BOYETTE DMAP COMMENT OUT //common->Printf( "%i shadowing fragments before optimization\n", numOutputTris );
}
										 ...
										 idPlane projectionPlane, idVec3 projectionOrigin )
{
	...
	// match up common verts // BOYETTE NOTE: I don't know why they don't use the word "merge" since this function is just merging shadow volume verts that are really close together. Due to floating point imprecision you can have unnecessary verts generated. UNIQUE_EPSILON is the merge distance
	UniqueVerts();
...
static void RemoveDegenerateTriangles( srfTriangles_t *tri ) {
	...
	if ( c_removed ) {
		// BOYETTE DMAP COMMENT OUT // common->Printf( "removed %i degenerate triangles from shadow\n", c_removed );
	}
}
