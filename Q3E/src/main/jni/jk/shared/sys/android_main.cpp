...
void Sys_SyncState(void)
{
	//if (setState)
	{
		...
		//int state = ((cls.state == CA_ACTIVE) && (Key_GetCatcher() == 0)) << 1;
...
==============
*/
int Sys_Dialog( const char *message, const char *title )
{
...
static void Sys_ErrorDialog( const char *error )
{
	...
	if ( fp )
	{
		...
		if ( Sys_Dialog( errorMessage, "Error" ) < 0 )
		{
...
void *Sys_LoadDll( const char *name, qboolean useSystemLib )
{
	...
	const char *binarypath = Sys_DLLDefaultPath();
	char *fn;

	Com_Printf( "Trying to load \"%s\" from \"%s\"...\n", name, binarypath );
	if(binarypath && binarypath[0])
		fn = va( "%s%c%s", binarypath, PATH_SEP, name );
	else
		fn = va( "%s", name );
...
void *Sys_LoadSPGameDll( const char *name, GetGameAPIProc **GetGameAPI )
{
	...
	const char *binarypath = Sys_DLLDefaultPath();
	if(binarypath && binarypath[0])
		Com_sprintf (filename, sizeof(filename), "%s/lib%s" DLL_EXT, binarypath, name);
	else
