...
#if !defined(_GLES) //karin: only GL_RGBA
	case GL_RGBA4:
		//"RGBA4"
		return 2;
		break;
...
//		totalFileSizeK += (image->imgfileSize+1023)/1024;
		...
		case GL_REPEAT:
			...
			break;
#if !defined(_GLES) //karin: GL_CLAMP_TO_EDGE
		case GL_CLAMP:
...
#ifdef _GLES //karin: GL_RGBA // TODO: GL_RGB on 3-components
		*pformat = GL_RGBA;
#else
...
#ifdef _GLES //karin: GL_CLAMP_TO_EDGE
...
#if !defined(_GLES) //karin: not support image border
	borderColor[0] = 1.0;
	borderColor[1] = 1.0;
	borderColor[2] = 1.0;
	borderColor[3] = 1;
...
#ifdef _GLES //karin: not support GL_RGBA16
