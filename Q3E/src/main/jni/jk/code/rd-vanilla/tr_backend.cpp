...
#if !defined(_GLES) //karin: not support glPolygonMode
	//
	// fill/line mode
	//
...
static void RB_BeginDrawingView (void) {
	...
	if ( backEnd.viewParms.isPortal ) {
		float	plane[4];
#ifdef _GLES //karin: glClipPlanef uisng GLfloat
		GLfloat plane2[4];
#else
		double	plane2[4];
#endif
...
#ifdef _GLES //karin: glCopyTexImage2D not support GL_RGBA16
...
#if !defined(_GLES) //karin: not support glDrawBuffer
...
#if !defined(_GLES) //karin: not support glReadPixels with GL_STENCIL_INDEX
...
{
#if !defined(_GLES) //karin: not support arb program
	switch ( uiType )
	{
...
{
#if !defined(_GLES) //karin: not support arb shader/glBegin/glEnd
	qglDisable (GL_CLIP_PLANE0);
...
{
#if !defined(_GLES) //karin: not support glBegin/glEnd/GL_TEXTURE_RECTANGLE_ARB
	qglDisable (GL_CLIP_PLANE0);
	GL_Cull( CT_TWO_SIDED );
