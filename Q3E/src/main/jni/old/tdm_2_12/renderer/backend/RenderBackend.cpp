...
void RenderBackend::DrawLightgem( const viewDef_t *viewDef, byte *lightgemData ) {
	...
	qglBindBuffer( GL_PIXEL_PACK_BUFFER, lightgemPbos[currentLightgemPbo] );
		...
		void *mapPtr = qglMapBufferRange( GL_PIXEL_PACK_BUFFER, 0, DARKMOD_LG_RENDER_WIDTH * DARKMOD_LG_RENDER_WIDTH * 4, GL_MAP_READ_BIT );
		if(mapPtr)
		{
			memcpy(lightgemData, mapPtr, DARKMOD_LG_RENDER_WIDTH * DARKMOD_LG_RENDER_WIDTH * 4);
		}
