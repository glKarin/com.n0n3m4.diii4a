...
//k extern void WriteDeclCache( idDemoFile* f, int demoCategory, int demoCode, declType_t  declType );
	...
	if( s_drawSounds.GetBool() && renderWorld != NULL )
	{
		for( int e = 0; e < emitters.Num(); e++ )
		{
			...
			for( int k = 0; k < emitter->channels.Num(); k++ )
			{
				...
				idStr text;
				text += va( "%s (%i %i/%i)%s", chan->soundShader->GetName(), idMath::Ftoi( emitter->spatializedDistance ), idMath::Ftoi( min ), idMath::Ftoi( max ), defaulted );
...
void idSoundWorldLocal::StartWritingDemo( idDemoFile* demo )
{
	...
	//k WriteDeclCache( writeDemo, DS_SOUND, SCMD_CACHESOUNDSHADER, DECL_SOUND );
...
bool idSoundWorldLocal::IsDukeSoundPlaying(void) {
	...
	idSoundEmitterLocal* _localsound = (idSoundEmitterLocal*)localSound;
	for (int i = 0; i < _localsound->channels.Num(); i++)
	{
		if (_localsound->channels[i] == nullptr)
			continue;

		idSoundChannel* channel = _localsound->channels[i];

		if (channel->soundShader->IsDukeVoice()) {
			if (channel->CheckForCompletion(currentTime))
			{
				return true;
			}
		}
...
========================
*/
int idSoundWorldLocal::PlayShaderDirectly(const idSoundShader* shader, int channel) {
    if (shader == NULL) {
        localSound->StopSound(channel);
        return 0;
    }
