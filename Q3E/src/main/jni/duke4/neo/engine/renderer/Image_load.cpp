...
void idImage::ActuallyLoadImage( bool fromBackEnd ) {
	...
	if ( ( common->InProductionMode() && binaryFileTime != FILE_NOT_FOUND_TIMESTAMP ) || ( ( binaryFileTime != FILE_NOT_FOUND_TIMESTAMP )
...
#ifdef __ANDROID__ //karin: GLES not support glBindMultiTexture
			glActiveTexture(GL_TEXTURE0 + texUnit);
			// printf( "idImage::Bind2D( %s )\n", GetName() );
...
#ifdef __ANDROID__ //karin: GLES only default framebuffer support GL_BACK
    if( idRenderTexture::IsDefaultFramebufferActive() )
#endif
