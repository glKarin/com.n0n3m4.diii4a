...
idCVar r_useIndexBuffers( "r_useIndexBuffers", "1", CVAR_RENDERER | CVAR_ARCHIVE | CVAR_INTEGER, "use ARB_vertex_buffer_object for indexes", 0, 1, idCmdSystem::ArgCompletion_Integer<0,1>  ); //karin: enable index buffer
...
idCVar r_useLightCulling( "r_useLightCulling", "0", CVAR_RENDERER | CVAR_INTEGER, "0 = none, 1 = box, 2 = exact clip of polyhedron faces, 3 = also areas", 0, 3, idCmdSystem::ArgCompletion_Integer<0,3> );
...
idCVar r_useScissor( "r_useScissor", "0", CVAR_RENDERER | CVAR_BOOL, "scissor clip as portals and lights are processed" );
...
static void R_CheckPortableExtensions( void ) {
#ifdef __ANDROID__ //karin: force using GLES3.2 on Android
	glConfig.glVersion = 3.2;

	if(!glConfig.version_string)
		common->FatalError("Failed to init gles!\n");
#else
	...
	// GL_ARB_multitexture
#ifdef __ANDROID__ //karin: GLES2+ support multitexture?
...
#ifdef __ANDROID__ //karin: GLES2+ support cubemap texture
...
#ifdef __ANDROID__ //karin: GLES2+ support NPOT texture
...
#ifdef __ANDROID__ //karin: GLES2+ support GL_INCR_WRAP and GL_DECR_WRAP 
	if(1)
#else
...
#ifdef __ANDROID__ //karin: GLES2+ support glStencilXXXSeparate 
...
#ifdef __ANDROID__ //karin: GLES2+ support VBO
...
#ifdef __ANDROID__ //karin: GLES3 support uniform buffer
...
#ifdef __ANDROID__ //karin: force using screen resolution
	...
	for ( i = 0 ; i < 10 ; i++ ) {
		...
		switch( err ) {
			...
			case GL_INVALID_OPERATION:
				...
				break;
#if !defined(__ANDROID__) //karin: GLES not suppor these error
...
#ifdef __ANDROID__ //karin: GLES glReadPixels only support GL_RGBA
...
#if !defined(__ANDROID__) //karin: GLES not support read stencil pixels
	byte		*buffer;
	int			i, c;
