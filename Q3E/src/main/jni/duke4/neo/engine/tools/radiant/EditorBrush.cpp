...
void Face_MoveTexture(face_t *f, idVec3 delta) {
	...
	if (g_qeglobals.m_bBrushPrimitMode) {
		...
		// clamp the shifts
		Clamp(f->texdef.shift[0], f->d_texture->GetEditorImage()->GetOpts().width);
		Clamp(f->texdef.shift[1], f->d_texture->GetEditorImage()->GetOpts().height);
	}
}
...
void Face_TextureVectors(face_t *f, float STfromXYZ[2][4]) {
	...
	for (j = 0; j < 4; j++) {
		STfromXYZ[0][j] /= q->GetEditorImage()->GetOpts().width;
		STfromXYZ[1][j] /= q->GetEditorImage()->GetOpts().height;
	}
}
...
{ 
	...
	while (ix <= iy) {
		...
		idgr -= 4;
		ix++;
		glVertex3f(x + ix, y + iy, z);
		glVertex3f(x - ix, y + iy, z);
		glVertex3f(x + ix, y - iy, z);
		glVertex3f(x - ix, y - iy, z);
		glVertex3f(x + iy, y + ix, z);
		glVertex3f(x - iy, y + ix, z);
		glVertex3f(x + iy, y - ix, z);
		glVertex3f(x - iy, y - ix, z);
	}
...
void DrawSpeaker(brush_t *b, bool bSelected, bool twoD) {
	...
	if (twoD) {
		...
		if (bSelected) {
			glColor4f(g_qeglobals.d_savedinfo.colors[COLOR_SELBRUSHES].x, g_qeglobals.d_savedinfo.colors[COLOR_SELBRUSHES].y, g_qeglobals.d_savedinfo.colors[COLOR_SELBRUSHES].z, 1);
		} else {
			glColor4f(b->owner->eclass->color.x, b->owner->eclass->color.y, b->owner->eclass->color.z, 1);
		}
...
void Face_FitTexture(face_t *face, float nHeight, float nWidth) {
	if (g_qeglobals.m_bBrushPrimitMode) {
		 ...
		 * td->scale[0] =
		 * -(rot_width/((float)(face->d_texture->GetEditorImage()->GetOpts().width*nWidth)));
		 * td->scale[1] =
		 * -(rot_height/((float)(face->d_texture->GetEditorImage()->GetOpts().height*nHeight)));
		 * td->shift[0] = min_s/td->scale[0]; temp = (int)(td->shift[0] /
		 * (face->d_texture->GetEditorImage()->GetOpts().width*nWidth)); temp =
		 * (temp+1)*face->d_texture->GetEditorImage()->GetOpts().width*nWidth; td->shift[0] =
		 * (int)(temp -
		 * td->shift[0])%(face->d_texture->GetEditorImage()->GetOpts().width*nWidth);
		 * td->shift[1] = min_t/td->scale[1]; temp = (int)(td->shift[1] /
		 * (face->d_texture->GetEditorImage()->GetOpts().height*nHeight)); temp =
		 * (temp+1)*(face->d_texture->GetEditorImage()->GetOpts().height*nHeight);
		 * td->shift[1] = (int)(temp -
		 * td->shift[1])%(face->d_texture->GetEditorImage()->GetOpts().height*nHeight);
		 */
	}
}
