...
void idCommonLocal::PrintLoadingMessage( const char *msg ) {
	...
	renderSystem->DrawStretchPic( 0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, 0, 0, 1, 1, declManager->FindMaterial( "guis/splash/splash" ) );
	int len = strlen( msg );
...
static idCVar	harm_fs_gameLibPath("harm_fs_gameLibPath", "", CVAR_SYSTEM | CVAR_INIT | CVAR_SERVERINFO, "[Harmattan]: Special game dynamic library. e.g. "
		"`<harm_fs_gameLibPath>/libgame.so`, "
...
void idCommonLocal::LoadGameDLL( void ) {
	...
	if(fsgame.Length())
	{
		...
		idStr::snPrintf( dllPath, MAX_OSPATH, fsgame.c_str() );
	}
	else
	{
		common->Printf("[Harmattan]: cvar `harm_fs_gameLibPath` is unset.\n");
		extern const char * Sys_DLLDefaultPath(void);
		const char *dir = Sys_DLLDefaultPath();
		idStr dllFile(dir);
		dllFile.AppendPath("libduke4game.so");
		idStr::snPrintf( dllPath, MAX_OSPATH, dllFile.c_str() );
	}
#else
