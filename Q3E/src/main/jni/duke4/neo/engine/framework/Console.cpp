...
void idConsoleLocal::DrawNotify() {
	...
	for (i = current - NUM_CON_TIMES + 1; i <= current; i++) {
		...
		deviceContext->DrawText(0, v + SMALLCHAR_HEIGHT, CONSOLE_FONT_SCALE, idStr::ColorForIndex(currentColor), temp, CONSOLE_FONT_SCALE, 0, true);
...
void idConsoleLocal::DrawSolidConsole(float frac) {
	...
	deviceContext->DrawText(SCREEN_WIDTH - (i) * SMALLCHAR_WIDTH, (lines - (SMALLCHAR_HEIGHT + SMALLCHAR_HEIGHT / 2)), CONSOLE_FONT_SCALE, colorGold,
		version, 1.0, 0, true);
	...
	deviceContext->DrawText(SCREEN_WIDTH - (i) * SMALLCHAR_WIDTH, (lines - (SMALLCHAR_HEIGHT + SMALLCHAR_HEIGHT / 2)) + SMALLCHAR_HEIGHT, CONSOLE_FONT_SCALE, colorGold,
	...
	for (i = 0; i < rows; i++, y -= SMALLCHAR_HEIGHT, row--) {
		...
		textPtr[0] = 0;
		deviceContext->SetColor(idStr::ColorForIndex(currentColor));
		deviceContext->DrawText(0, idMath::FtoiFast(y) + SMALLCHAR_HEIGHT, CONSOLE_FONT_SCALE, idStr::ColorForIndex(currentColor), temp, CONSOLE_FONT_SCALE, 0, true);
		currentColor = idStr::ColorIndex(C_COLOR_WHITE);
		deviceContext->SetColor(idStr::ColorForIndex(currentColor));
	}
...
void	idConsoleLocal::Draw(bool forceFullScreen) {
	...
	if (com_showFPS.GetInteger() > 0) {
		...
		deviceContext->DrawText(SCREEN_WIDTH - (txt.Length() * SMALLCHAR_WIDTH) - SMALLCHAR_WIDTH, y + SMALLCHAR_HEIGHT, CONSOLE_FONT_SCALE, colorWhite, txt, CONSOLE_FONT_SCALE, 0, true);
		y += SMALLCHAR_HEIGHT;
#ifdef _DEBUG
		txt = "Debug Build";
#else
