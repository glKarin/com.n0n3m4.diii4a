...
int				Sys_ListFiles( const char *directory, const char *extension, idStrList &list )
{
	...
	printf("%s|%s|%s\n", directory, extension, wd);
	sprintf( search, "%s/%s", wd, directory );
	findhandle = opendir(search);
	while(file = readdir(findhandle))
	{
		if(!strcmp(".", file->d_name) || !strcmp("..", file->d_name))
			continue;
		if(strlen(file->d_name) < strlen(extension))
			continue;
		if(!strcmp(file->d_name + strlen(file->d_name) + 1 - strlen(extension), extension))
			continue;
		printf("%s\n", file->d_name);
...
const char *Sys_EXEPath(void) { return ""; }
...
int main( int argc, char** argv ) {
	...
	chdir("/storage/emulated/0/idtech4amm/lib");
