...
const char* Sys_DefaultSavePath()
{
	...
	{
		sprintf( savepath, "%s/dnf", xdg_data_home );
	}
	else
	{
#ifdef __ANDROID__ //karin: getenv( "HOME" ) = NULL on Android
		extern const char * Sys_GameDataDefaultPath(void);
		sprintf( savepath, "%s/.local/share/dnf", Sys_GameDataDefaultPath() );
#else
		sprintf( savepath, "%s/.local/share/dnf", getenv( "HOME" ) );
#endif
	}
#endif
...
#define DEFAULT_BASEPATH				"/sdcard/diii4a/dnf"
...
int Sys_ListFiles( const char* directory, const char* extension, idStrList& list )
{
	...
	struct dirent* entry = ( struct dirent* )Mem_Alloc( direntLen );
