...
#if 0
#define QGLCALL(name, rettype, args, retvar, argvars, ret) rettype q##name args { \
	retvar __##name argvars; \
	GLenum err = __glGetError(); \
	if(err != GL_NO_ERROR) \
		printf("GLERROR: " #name " -> %X\n", err); \
	ret; \
}
#include "../../renderer/qgl_call.h"
#else
#include "../../renderer/qgl_debug.hpp"
#endif
...
#define QGLPROC(name, rettype, args) \
	__##name = (rettype(GL_APIENTRYP)args)GLimp_ExtensionPointer(#name); \
	if (!__##name) \
...
int GLES_Init(glimpParms_t ap)
{
	...
	actualWidth = ap.width;
	actualHeight = ap.height;
	eglConfig = configs[0];
