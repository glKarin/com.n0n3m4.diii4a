...
void idPlayer::UpdateFocus( void ) {
	...
	for ( i = 0; i < listedClipModels; i++ ) {
		...
		//		if ( body && body->IsType( DnAI::Type ) && ( static_cast<idAI *>( body )->GetTalkState() >= TALK_OK ) ) {
...
void idPlayer::Move( void ) {
	...
	if ( noclip || gameLocal.inCinematic || ( influenceActive == INFLUENCE_LEVEL2 ) ) {
		...
		if ( groundEnt && groundEnt->IsType( DnAI::Type ) ) {
...
// jmarshall
	if (gunpos.Length() == 0)
	{
		gunpos = weapon.GetEntity()->GetGunDeclOffset();
	}
// jmarshall end
