...
static void GLimp_HandleError(const char *func, qboolean exit)
{
	...
	if(exit)
		ri.Error(ERR_FATAL, "[Harmattan]: EGL error %s: 0x%04x: %s\n", func, err, GLimp_StringErrors[err - EGL_SUCCESS]);
	else
		ri.Printf(PRINT_ALL, "[Harmattan]: EGL error %s: 0x%04x: %s\n", func, err, GLimp_StringErrors[err - EGL_SUCCESS]);
}
...
qboolean GLimp_OpenDisplay(void)
{
	...
	ri.Printf(PRINT_ALL, "Setup EGL display connection\n" );
 
	if ( !( eglDisplay = eglGetDisplay(EGL_DEFAULT_DISPLAY) ) ) {
		ri.Printf(PRINT_ALL, "Couldn't open the EGL display\n" );
		return false;
	}
	return true;
}
...
static bool GLES_Init_special(void)
{
	...
	ri.Printf(PRINT_ALL, "[Harmattan]: Request special EGL context: %d/%d/%d Color bits, %d Alpha bits, %d depth, %d stencil display. samples %d sample buffers %d.\n",
			red_bits, green_bits,
	...
	while(1)
	{
		...
		{
			ri.Printf(PRINT_ALL, "[Harmattan]: Get EGL context num -> %d.\n", config_count);
			for(int i = 0; i < config_count; i++)
			{
				EGLConfigInfo_t cinfo = GLimp_GetConfigInfo(eglConfigs[i]);
				ri.Printf(PRINT_ALL, "\t%d EGL context: %d/%d/%d Color bits, %d Alpha bits, %d depth, %d stencil display. samples %d sample buffers %d.\n",
								i + 1,
...
static bool GLES_Init_prefer(void)
{
	...
	for (int i = 0; i < 16; i++) {
		...
		ri.Printf(PRINT_ALL, "[Harmattan]: Request EGL context: %d/%d/%d Color bits, %d Alpha bits, %d depth, %d stencil display. samples %d, sample buffers %d.\n",
...
int GLES_Init(qboolean fullscreen)
{
	...
	if(!GLES_Init_special())
	{
		if(!GLES_Init_prefer())
		{
			ri.Error(ERR_FATAL, "Initializing EGL error");
			return false;
		}
	}
	...
	ri.Printf(PRINT_ALL, "[Harmattan]: EGL context: %d/%d/%d Color bits, %d Alpha bits, %d depth, %d stencil display. samples %d, sample buffers %d.\n",
					info.red, info.green,
