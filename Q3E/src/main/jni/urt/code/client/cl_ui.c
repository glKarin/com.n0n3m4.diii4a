...
intptr_t CL_UISystemCalls( intptr_t *args ) {
  ...
  case UI_R_MODELBOUNDS:
	...
	case UI_Q_VSNPRINTF:
#ifdef __ANDROID__ //karin: va_list and intptr
		fprintf(stderr, "UI_Q_VSNPRINTF(%p, %zd, %s, %p) not support!\n", VMA(1), *((size_t *)VMA(2)), (const char *)VMA(3), VMA(4));
		int _ret = snprintf( VMA(1), *((size_t *)VMA(2)), "%s", VMA(3));
		return _ret;
#else
