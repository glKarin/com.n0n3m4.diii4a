...
void Cmd_CompleteTxtName( char *args, int argNum ) {
	if( argNum == 2 ) {
		Field_CompleteFilename( "", "txt", qfalse, qtrue, qtrue );
	}
}

void Con_PrevTab() {
	int currentConsoleNum = currentCon - consoles;
	currentConsoleNum--;
...
void writeTextToConsole(console_t *console, char *txt, qboolean skipnotify) {
	...
	if (console->current >= 0) {
		// NERVE - SMF
		if ( skipnotify ) {
			prev = mainCon->current % NUM_CON_TIMES - 1;
			if ( prev < 0 )
				prev = NUM_CON_TIMES - 1;
			mainCon->times[prev] = 0;
		}
		else
		// -NERVE - SMF
			console->times[console->current % NUM_CON_TIMES] = cls.realtime;
...
void Con_DrawNotify (void)
{
	...
	for (i= currentCon->current-NUM_CON_TIMES+1 ; i<=currentCon->current ; i++)
	{
		...
		for (x = 0 ; x < currentCon->linewidth ; x++) {
			...
			SCR_DrawSmallChar( cl_conXOffset->integer + mainCon->xadjust + (x+1)*SMALLCHAR_WIDTH, v, text[x] & 0xff );
		}
...
void Con_DrawSolidConsole( float frac ) {
	...
	i = strlen( FULL_VERSION );

	for (x=0 ; x<i ; x++) {
		SCR_DrawSmallChar( cls.glconfig.vidWidth - ( i - x + 1 ) * SMALLCHAR_WIDTH,
			lines - SMALLCHAR_HEIGHT, FULL_VERSION[x] );
	}
