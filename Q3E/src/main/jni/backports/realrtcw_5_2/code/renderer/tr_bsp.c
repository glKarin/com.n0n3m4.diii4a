...
static void ParseFoliage(dsurface_t *ds, drawVert_t *verts, msurface_t *surf, int *indexes)
{
	...
	foliage->indexes      = ( glIndex_t * )(foliage->lmTexCoords + foliage->numVerts); // ( unsigned int * ) //karin:  warning: incompatible pointer types assigning to 'glIndex_t *' (aka 'unsigned short *') from 'unsigned int *' [-Wincompatible-pointer-types]
