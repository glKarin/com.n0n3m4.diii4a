...
#define FBD(x) {\
	...
	x; \
	printf("%s -> %x | %s %d | %d %d | %x %x %x\n", #x, glGetError(), fboName.c_str(), frameBuffer, width, height, colorFormat, depthFormat, stencilFormat); \
	}
#else
#define FBD(x) x
#endif
...
void Framebuffer::Init()
{
...
#endif
	{
		...
		globalFramebuffers.hdrFBO->AddColorBuffer( GL_RGBA8, 0 );
#else
...
#ifdef _GLES //karin: framebuffer using GL_RGBA8 instead of GL_RGBA16F
	globalFramebuffers.envprobeFBO->AddColorBuffer( GL_RGBA8, 0 );
#else
...
#ifdef _GLES //karin: framebuffer using GL_RGBA8 instead of GL_RGBA16F
	globalFramebuffers.hdr64FBO->AddColorBuffer( GL_RGBA8, 0 );
#else
...
void Framebuffer::AddColorBuffer( int format, int index, int multiSamples )
{
	...
	if( multiSamples > 0 )
	{
        FBD(glRenderbufferStorageMultisample( GL_RENDERBUFFER, multiSamples, format, width, height ));

		msaaSamples = true;
	}
	else
	{
		FBD(glRenderbufferStorage( GL_RENDERBUFFER, format, width, height ));
	}

	if( notCreatedYet )
	{
		FBD(glFramebufferRenderbuffer( GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + index, GL_RENDERBUFFER, colorBuffers[index] ));
	}
...
void Framebuffer::AddDepthBuffer( int format, int multiSamples )
{
	...
	if( notCreatedYet )
	{
		FBD(glFramebufferRenderbuffer( GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, GL_RENDERBUFFER, depthBuffer ));
	}
...
void Framebuffer::AddStencilBuffer( int format, int multiSamples )
{
	...
	if( notCreatedYet )
	{
		FBD(glFramebufferRenderbuffer( GL_FRAMEBUFFER, GL_STENCIL_ATTACHMENT, GL_RENDERBUFFER, stencilBuffer ));
	}
...
void Framebuffer::AttachImage2D( int target, idImage* image, int index, int mipmapLod )
{
	...
	FBD(glFramebufferTexture2D( GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + index, target, image->texnum, mipmapLod ));
...
void Framebuffer::AttachImageDepth( int target, idImage* image )
{
	...
	FBD(glFramebufferTexture2D( GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, target, image->texnum, 0 ));
...
{
	FBD(glFramebufferTextureLayer( GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, image->texnum, 0, layer ));
