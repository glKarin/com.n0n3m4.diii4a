...
static void R_CheckPortableExtensions()
{
	...
	// RB end
#ifdef _GLES //karin: force setup OpenGL driver
	glConfig.driverType = GLDRV_OPENGL_MESA;
#endif
	...
	{
#ifdef _GLES //karin: DXT1/DXT5 not support on OpenGLES
		idLib::Warning( "GL_ARB_texture_compression or GL_EXT_texture_compression_s3tc not available" );
#else
...
#ifdef _GLES //karin: force setup OpenGLES3.2
...
void idRenderBackend::DrawElementsWithCounters( const drawSurf_t* surf )
{
	...
	RENDERLOG_PRINTF( "Binding Buffers: %p:%i %p:%ld\n", vertexBuffer, vertOffset, indexBuffer, indexOffset );
...
void idRenderBackend::GL_StartFrame()
{
	...
	// performance penalty.
#if !defined(_GLES) //karin: glDrawBuffers on OpenGLES3.0
...
#ifdef _GLES //karin: glClearDepthf
	glClearDepthf( 1.0f );
#else
	glClearDepth( 1.0f );
#endif
...
#if !defined(_GLES) //karin: glPolygonMode not on OpenGLES
	//
	// fill/line mode
	//
