...
vec3 computeParallaxOffset(
	...
	for (int s = linearSteps - 1; s >= 1; s--) {
		float rayH = mix(hgtRange.x, hgtRange.y, float(s) / float(linearSteps));
...
float computeParallaxShadow(
	...
	float step = (hgtRange.y - hgtRange.x) / float(shadowSteps);
	float maxDiff = -1.0;
	...
	return 1.0 - clamp(maxDiff / shadowSoftness, 0.0, 1.0);
}
