...
{
	char vertag[128], lastlog[128], lasthash[128], *hash = NULL, *unixtime = NULL;
	FILE *stream = NULL;
	...
	// Shouldn't be able to get one without the other
	if (hash == NULL || unixtime == NULL)
	{
...
"#define GIT_HASH \"%s\"\n"
"#define GIT_TIME \"%s\"\n"
"#define GIT_DATE_INT %sull\n",
			hash, vertag, hash, lastlog, unixtime);
		fclose(stream);
