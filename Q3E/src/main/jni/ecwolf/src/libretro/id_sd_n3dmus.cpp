...
void
Mix_Chunk_N3D::MIDI_ProgramChange(int channel, int id)
{
...
void
Mix_Chunk_N3D::MIDI_IRQService(void)
{
	int	maxevent = 0;
...
void Mix_Chunk_N3D::EnsureSynthesis(int maxTics)
{
	while (midiData < midiDataStart + midiLength && sample_count < maxTics * rate / TICRATE) {
	    MIDI_IRQService();
	    EnsureSpace(sample_count + samplesPerMusicTick);
	    YM3812UpdateOneMono(*midiOpl, (int16_t *)chunk_samples + sample_count, samplesPerMusicTick);
	    sample_count += samplesPerMusicTick;
	}
}
			     ...
			     bool isLooping)
{
    ...
    N3DTempoEmulation = (midiDivision <= 0);
    seqPtr += ReadBigLong(seqPtr + 4) + 8;
