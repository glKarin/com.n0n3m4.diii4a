...
bool rvStateThread::CurrentStateIs( const char* name ) const {
	return ( !IsIdle() ) ? FindState(name) == GetState()->state : false;
}
...
void rvStateThread::Clear ( bool ignoreStateCalls ) {
	...
	for( call = states.Next(); call != NULL; call = states.Next() ) {
		if ( !ignoreStateCalls && (call->flags & (SFLAG_ONCLEAR|SFLAG_ONCLEARONLY) ) ) {
			ProcessState ( call->state, call->parms );
		}
		call->node.Remove();
		delete call;
	}		
...
stateResult_t rvStateThread::Execute ( void ) {
	...
	for( call = states.Next(); call && count < HISTORY_COUNT; call = states.Next(), ++count ) {
		...
		lastResult = ProcessState ( call->state, call->parms );		
		switch ( lastResult ) {
			case SRESULT_WAIT:
				fl.executing = false;
				return SRESULT_WAIT;
...
================
*/
stateResult_t rvStateThread::ProcessState ( const rvStateFunc<idClass>* state, const stateParms_t& parms ) {
	return (owner->*(state->function)) ( parms );
}

stateResult_t rvStateThread::ProcessState ( const char* name, const stateParms_t& parms ) {
	int				i;
	
	if(!stateCallbacks)
		return SRESULT_ERROR;

	for ( i = 0; stateCallbacks[i].function; i ++ ) {
		if ( !idStr::Icmp ( stateCallbacks[i].name, name ) ) {
			return (owner->*(stateCallbacks[i].function)) ( parms );
		}
	}
...
================
*/
const rvStateFunc<idClass>* rvStateThread::FindState ( const char* name ) const {
	int				i;
...
void rvStateThread::RegisterStateCallbacks(const rvStateFunc<idClass> *arr)
{
	if(stateCallbacks)
	{
		free(stateCallbacks);
	...
	int num = 1;
	while ( arr[num].function ) {
		num++;
	}
	stateCallbacks = (rvStateFunc<idClass> *)malloc(sizeof(*stateCallbacks) * num);
	memcpy(stateCallbacks, arr, sizeof(*stateCallbacks) * num);
}
