...
// sikk---> Item Management: Random Item Value
    if (spawnArgs.GetInt( "ammoRequired"))
    {
        ...
        {
            int bulval = GetRandomValue( "inv_ammo_bullets" );
            bulval += 3;
            if(bulval > 18)
            {
                bulval = 18;
            }
            spawnArgs.SetInt( "inv_ammo_bullets", bulval );
        }
        //Plasmagun should have full ammo since you can't re-use the clip
        if ( spawnArgs.GetInt( "inv_ammo_cells" ) )
            spawnArgs.SetInt( "inv_ammo_cells", spawnArgs.GetInt( "inv_ammo_cells" ));
        if ( spawnArgs.GetInt( "inv_ammo_clip" ) )
        {
            int clipval =  GetRandomValue( "inv_ammo_clip" );
            clipval += 3;
            if(clipval > 30)
            {
                clipval = 30;
            ...
            spawnArgs.SetInt( "inv_ammo_clip", clipval );
        }
        if ( spawnArgs.GetInt( "inv_ammo_belt" ) )
        {
            int beltval =  GetRandomValue( "inv_ammo_belt" );
            beltval += 22;
            if(beltval > 99)
            {
                beltval = 99;
            }
            spawnArgs.SetInt( "inv_ammo_belt", beltval );
