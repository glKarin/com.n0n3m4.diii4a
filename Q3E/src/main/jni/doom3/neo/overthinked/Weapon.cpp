...
const idEventDef EV_Weapon_UpdateWallHide( "updateWallHide" );
const idEventDef EV_Weapon_State("weaponState", "sd");
const idEventDef EV_Weapon_UseAmmo("useAmmo", "d");
const idEventDef EV_Weapon_UseBullet( "useBullet", NULL );
const idEventDef EV_Weapon_AddToClip( "addToClip", "ddd" );
...
const idEventDef EV_Weapon_LaunchProjectiles( "launchProjectiles", "dffffd" );
...
const idEventDef EV_Weapon_WallHide("wallHide", NULL, 'f');
...
EVENT( EV_Weapon_UseBullet,	    			idWeapon::Event_UseBullet )
EVENT(EV_Weapon_AddToClip,					idWeapon::Event_AddToClip)
EVENT(EV_Weapon_AmmoInClip,				idWeapon::Event_AmmoInClip)
EVENT( EV_Weapon_UpdateWallHide,			idWeapon::Event_UpdateWallHide)
...
EVENT( EV_Weapon_WallHide,				    idWeapon::Event_WallHide )
...
void idWeapon::Restore(idRestoreGame *savefile)
{
    ...
    WEAPON_MELEE.LinkTo(			scriptObject, "WEAPON_MELEE" );		// New
...
//		WeaponJoint_t *customJoint;
        ...
        if ( !newParticle.particleFlags.isOnWorldModel )  	// Smoke is not differentiated as world and view particles
        {
            ...
            /*			if( weaponJoints.Get(jointName.c_str(), &customJoint) ) {
            				newParticle.joint = customJoint->joint;
            			}
            			else {*/
...
void idWeapon::UpdateGUI(void)
{
    ...
    {
        // show remaining ammo
        // if( weaponDef->GetName() == idStr("weapon_chaingun") || weaponDef->GetName() == idStr("weapon_pistol") || weaponDef->GetName() == idStr("weapon_machinegun")){
        //    renderEntity.gui[ 0 ]->SetStateString( "player_totalammo", va( "%i", ammoamount + inclip) );//new
        //  }else{
...
/*
================
idWeapon::BeginMeleeFunction
================
*/
void idWeapon::BeginMeleeFunction( bool keyTapped )  	// new
{
    if ( isLinked )
    {
        if (keyTapped)
...
void idWeapon::PresentWeapon(bool showViewModel)
{
    ...
    if ( showViewModel && !disabled )   //disabled flag ensures that fx wont be played when entering cinematic
    {
        UpdateWeaponFx();
    }
#endif //_DENTONMOD
...
void idWeapon::Event_UseBullet()
{
    owner->inventory.UseAmmo(ammoType, 1);
}

void idWeapon::Event_AddToClip( int amount, int secondAmount, int reuse )
{
    ...
    if (reuse == 1)
    {
        ...
        if(secondAmount < 0)
        {
            owner->inventory.UseAmmo(ammoType, -secondAmount, true);
        }
        return;
    }
    ...
    //Reload logic changed by 0x29a
    if (secondAmount == 1)
    {
        if(ammoAvail >= secondAmount)
        {
            ammoClip += secondAmount;
    if (ammoClip > clipSize)
    {
        ammoClip = clipSize;
    }
            owner->inventory.UseAmmo(ammoType,secondAmount );
        }
        else
        {
            ammoClip += ammoAvail;
            ...
            owner->inventory.UseAmmo(ammoType,ammoAvail );
        }
    }
    if (secondAmount == 0)
    {
        if(ammoAvail >= clipSize)
        {
            ammoClip = clipSize;
            owner->inventory.UseAmmo(ammoType,clipSize );
        }
        else
    {
        ammoClip = ammoAvail;
...
void idWeapon::Event_UpdateWallHide( void )
{
    if(gameLocal.time > lastWallCheckTime)
    {
        lastWallCheckTime = gameLocal.time + wallCheckInterval;

        //WEAPON HIDING WHEN APPROACHING A WALL
        // put the world muzzle flash on the end of the joint, no matter what
        //GetGlobalJointTransform( false, flashJointWorld, worldMuzzleFlash.origin, worldMuzzleFlash.axis );
        if ( barrelJointView != INVALID_JOINT )
        {
            ...
            GetGlobalJointTransform( true, barrelJointView, muzzleOrigin, muzzleAxis );
            float ang = 0 ;//idMath::Sin( 0 * gameLocal.random.RandomFloat() );
            float spin = 0 ;// (float)DEG2RAD( 360.0f ) * gameLocal.random.RandomFloat();
            idVec3 dir = playerViewAxis[ 0 ]; // + playerViewAxis[ 2 ] * ( ang * idMath::Sin( spin ) ) - playerViewAxis[ 1 ] * ( ang * idMath::Cos( spin ) );
            dir.Normalize();
            idVec3 view_pos = playerViewOrigin + playerViewAxis[ 0 ] * 1.0f; // Muzzle pos for translation clip model only
            idVec3 muzzle_pos = muzzleOrigin;// + playerViewAxis[ 0 ] * 2.0f;	// muzzle_pos for multiplayer prediction
            // as well as for launching the projectiles
            float muzzleDistFromView = (muzzle_pos - view_pos).LengthSqr( ) * 3.5f; // This is faster, was 3.5

            trace_t			tr;
            gameLocal.clip.Translation( tr, view_pos, view_pos + dir * 4096.0f, NULL, mat3_identity, MASK_SHOT_RENDERMODEL, owner );

            float traceDist = (tr.endpos - view_pos).LengthSqr(); // This is faster

            if ( traceDist < muzzleDistFromView*1.0 )
            {
                //hide weapon when close to wall
                hideOnWall = true;
            }
            else
            {
                //show weapon if not close to wall and hidden
                hideOnWall = false;
            }
        }
        //WEAPON HIDING WHEN APPROACHING A WALL
...
/*
===============
idWeapon::Event_WallHide
===============
*/
void idWeapon::Event_WallHide( void )
{
    if(hideOnWall)
    {
...
void idWeapon::Event_LaunchProjectiles( int num_projectiles, float spread, float fuseOffset, float launchPower, float dmgPower,int melee)
{
    ...
    if (!gameLocal.isClient)
    {
        ...
        int ammoAvail = owner->inventory.HasAmmo(ammoType, ammoRequired);
        if ( ( clipSize != 0 ) && ( ammoClip <= 0 ) && (melee <= 0) )
        {
            return;
        }
    ...
    if ( worldModel.GetEntity() && (melee <= 0) )
    {
...
//			}
    ...
    {
        // toss the brass
        if (brassDelay >= 0 && (melee <= 0) )  // eject brass behaviour can be disabled by simply setting the delay to 0
