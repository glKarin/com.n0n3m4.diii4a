...
=====================
*/
int idActor::GetDamageForLocation( int damage, int location,  const idDict *damageDef )
{
    ...
    int dmg = (int)ceil( damage * damageScale[ location ] );
    if(damageDef->GetBool( "heavy" ) )
    {
        if(damageScale[location]<1)
        {
            dmg = damage;
        }
    }
    if(damageDef->GetBool( "light" ) )
    {
        if(damageScale[location]<=0.5)
        {
            dmg = damage*0.1;
        }
    }
