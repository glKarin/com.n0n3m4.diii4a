...
/*
================
sdAFPhysicsNetworkData::MakeDefault
================
*/
void sdAFPhysicsNetworkData::MakeDefault( void ) {
	atRest			= 0;
	noMoveTime		= 0.f;
	activateTime	= 0.f;
...
/*
================
sdAFPhysicsNetworkData::Write
================
*/
void sdAFPhysicsNetworkData::Write( idFile* file ) const {
	file->WriteInt( atRest );
	file->WriteFloat( noMoveTime );
	file->WriteFloat( activateTime );

	file->WriteInt( bodies.Num() );
	for ( int i = 0; i < bodies.Num(); i++ ) {
		file->WriteVec3( bodies[ i ].origin );
		file->WriteCQuat( bodies[ i ].orientation );
	}
}

/*
================
sdAFPhysicsNetworkData::Read
================
*/
void sdAFPhysicsNetworkData::Read( idFile* file ) {
	file->ReadInt( atRest );
	file->ReadFloat( noMoveTime );
	file->ReadFloat( activateTime );
	...
	for ( int i = 0; i < count; i++ ) {
		...
		file->ReadCQuat( bodies[ i ].orientation );
	}
}
...
/*
================
idAFConstraint_HingeFriction::GetCenter
================
*/
void idAFConstraint_HingeFriction::GetCenter( idVec3& org ) {
	hinge->GetCenter( org );
}
...
void idAFConstraint_Spring::SetLimit( const float minLength, const float maxLength ) {
	assert( ( minLength >= 0.f && ( maxLength == 0.f ) || ( maxLength >= minLength ) ) || ( maxLength >= 0.f && ( minLength == 0.f ) || ( maxLength >= minLength ) ));
...
void idAFConstraint_Suspension::Evaluate( float invTimeStep ) {
	...
	gameLocal.clip.Translation( CLIP_DEBUG_PARMS trace, start, end, wheelModel, axis, CONTENTS_SOLID, NULL );
...
================
*/
idAFBody::idAFBody( const char* name, idClipModel *clipModel, float density ) {
...
void idAFBody::SetClipModel( idClipModel *clipModel ) {
	if ( this->clipModel != NULL && this->clipModel != clipModel ) {
...
================
*/
void idPhysics_AF::ApplyFriction( float timeStep, int endTimeMSec ) {
	int i;
	float invTimeStep;
...
bool idPhysics_AF::CollisionImpulse( float timeStep, idAFBody *body, trace_t &collision ) {
	...
	return self->Collide( collision, velocity, GetBodyId( body ) );
}
...
idEntity *idPhysics_AF::SetupCollisionForBody( idAFBody *body ) const {
	...
	if ( !selfCollision || !body->fl.selfCollision || af_skipSelfCollision.GetBool() ) {
		...
		// disable all bodies
		for ( i = 0; i < bodies.Num(); i++ ) {
			bodies[ i ]->clipModel->Unlink( gameLocal.clip );
		}
		...
		for ( i = 0; i < bodies.Num(); i++ ) {
			if ( bodies[ i ]->fl.selfCollision ) {
				bodies[ i ]->clipModel->Link( gameLocal.clip );
			} else {
				bodies[ i ]->clipModel->Unlink( gameLocal.clip );
			}
		}
		...
		body->clipModel->Unlink( gameLocal.clip );
		...
		for ( i = 0; i < body->constraints.Num(); i++ ) {
			...
			if ( body->constraints[i]->body2 == NULL ) {
				...
				b->clipModel->Unlink( gameLocal.clip );
			}
		}
	}
...
//	#define TEST_COLLISION_DETECTION
	...
	for ( i = 0; i < bodies.Num(); i++ ) {
		...
		if ( body->clipMask != 0 ) {
			...
			if ( gameLocal.clip.Motion( CLIP_DEBUG_PARMS collision, body->current->worldOrigin, body->next->worldOrigin, rotation,
...
================
*/
bool idPhysics_AF::EvaluateContacts( CLIP_DEBUG_PARMS_DECLARATION_ONLY ) {
	int i, j, k, numContacts, numBodyContacts;
	idAFBody *body;
	contactInfo_t contactInfo[10];
	idEntity *passEntity;
	idVecX force( 6, VECX_ALLOCA( 6 ) );
	idVec3 dir;
	...
	for ( i = 0; i < bodies.Num(); i++ ) {
		...
		body->InverseWorldSpatialInertiaMultiply( force, body->current->externalForce.ToFloatPtr() );
		dir = body->current->spatialVelocity.SubVec3( 0 ) + current.lastTimeStep * force.SubVec3( 0 );
		dir.Normalize();

		numContacts = gameLocal.clip.Contacts( CLIP_DEBUG_PARMS contactInfo, 10, body->current->worldOrigin, &dir, 2.0f, //CONTACT_EPSILON,
						body->clipModel, body->current->worldAxis, body->clipMask, passEntity );
...
void idPhysics_AF::SetupContactConstraints( void ) {
	...
	if ( contacts.Num() > contactConstraints.Num() ) {
		int old = contactConstraints.Num();
		contactConstraints.SetNum( contacts.Num() );
		for ( i = old; i < contacts.Num(); i++ ) {
			contactConstraints[ i ] = new idAFConstraint_Contact();
		}
	} else {
		for ( i = contacts.Num(); i < contactConstraints.Num(); i++ ) {
			delete contactConstraints[ i ];
			contactConstraints[ i ] = NULL;
		}
		contactConstraints.SetNum( contacts.Num() );
	}
...
/*
================
idPhysics_AF::GetInertiaTensor
================
*/
const idMat3& idPhysics_AF::GetInertiaTensor( int id ) const {
	if ( id >= 0 && id < bodies.Num() ) {
		return bodies[id]->inertiaTensor;
	}

	gameLocal.Warning( "idPhysics_AF::GetInertiaTensor doesn't yet return a valid result for id < 0" );
	return mat3_zero;
}
...
/*
================
idAF::CalculateCenterOfMass
================
*/
const idVec3& idPhysics_AF::CalculateCenterOfMass( void ) {
	idVec3 moments( 0.f, 0.f, 0.f );
	float totalmass = 0.f;
	idAFBody* mainbody = bodies[ 0 ];
	idMat3 transpose = mainbody->current->worldAxis.Transpose();
	for( int i = 0; i < bodies.Num(); i++ ) {
		idAFBody* body = bodies[ i ];
		idVec3 com = body->current->worldOrigin - mainbody->current->worldOrigin;
		com *= transpose;

		totalmass += body->mass;

		moments += com * body->mass;
	}
	centerOfMass = moments / totalmass;
...
void idPhysics_AF::DebugDraw( void ) {
	...
	if ( af_showMass.GetBool() ) {
		...
		gameRenderWorld->DebugCircle( colorYellow, bodies[0]->GetWorldOrigin() + ( centerOfMass * bodies[0]->GetWorldAxis() ), idVec3( 0.f, 0.f, 1.f ), 16, 16 );
		gameRenderWorld->DebugCircle( colorYellow, bodies[0]->GetWorldOrigin() + ( centerOfMass * bodies[0]->GetWorldAxis() ), idVec3( 0.f, 1.f, 0.f ), 16, 16 );
		gameRenderWorld->DebugCircle( colorYellow, bodies[0]->GetWorldOrigin() + ( centerOfMass * bodies[0]->GetWorldAxis() ), idVec3( 1.f, 0.f, 0.f ), 16, 16 );
	}
...
idPhysics_AF::~idPhysics_AF( void ) {
	...
	contactConstraints.DeleteContents( true );
...
void idPhysics_AF::BuildTrees( void ) {
	...
	if ( af_useLinearTime.GetBool() ) {
		...
		// Gordon: FIXME: Might want this warning later.
/*		if ( trees.Num() > 1 ) {
...
void idPhysics_AF::AddConstraint( idAFConstraint *constraint ) {
	...
	if ( constraints.FindElement( constraint ) ) {
	...
	if ( !bodies.FindElement( constraint->body1 ) ) {
	...
	if ( constraint->body2 && !bodies.FindElement( constraint->body2 ) ) {
...
void idPhysics_AF::ForceBodyId( idAFBody *body, int newId ) {
	...
	if ( id == -1 ) {
		gameLocal.Error( "ForceBodyId: body '%s' is not part of the articulated figure.", body->name.c_str() );
	}
	if ( id != newId ) {
...
int idPhysics_AF::GetBodyId( idAFBody *body ) const {
	...
	id = bodies.FindIndex( body );
	if ( id == -1 && body ) {
		gameLocal.Error( "GetBodyId: body '%s' is not part of the articulated figure.", body->name.c_str() );
	}
	return id;
}
...
int idPhysics_AF::GetBodyId( const char *bodyName ) const {
	...
	gameLocal.Error( "GetBodyId: no body with the name '%s' is not part of the articulated figure.", bodyName );
	return 0;
}
...
int idPhysics_AF::GetConstraintId( idAFConstraint *constraint ) const {
	...
	if ( id == -1 && constraint ) {
		gameLocal.Error( "GetConstraintId: constraint '%s' is not part of the articulated figure.", constraint->name.c_str() );
	}
	return id;
}
...
int idPhysics_AF::GetConstraintId( const char *constraintName ) const {
	...
	gameLocal.Error( "GetConstraintId: no constraint with the name '%s' is not part of the articulated figure.", constraintName );
	return 0;
}
...
idAFConstraint *idPhysics_AF::GetConstraint( const int id ) const {
	if ( id < 0 || id >= constraints.Num() ) {
		gameLocal.Error( "GetConstraint: no constraint with id %d exists", id );
		return NULL;
	}
...
void idPhysics_AF::ClipTranslation( trace_t &results, const idVec3 &translation, const idClipModel *model ) const {
	...
	for ( i = 0; i < bodies.Num(); i++ ) {
		...
		if ( body->clipModel->IsTraceModel() ) {
			if ( model ) {
				gameLocal.clip.TranslationModel( CLIP_DEBUG_PARMS bodyResults, body->current->worldOrigin, body->current->worldOrigin + translation,
									body->clipModel, body->current->worldAxis, body->clipMask,
										model, model->GetOrigin(), model->GetAxis() );
			}
			else {
				gameLocal.clip.Translation( CLIP_DEBUG_PARMS bodyResults, body->current->worldOrigin, body->current->worldOrigin + translation,
...
void idPhysics_AF::ClipRotation( trace_t &results, const idRotation &rotation, const idClipModel *model ) const {
	...
	for ( i = 0; i < bodies.Num(); i++ ) {
		...
		if ( body->clipModel->IsTraceModel() ) {
			if ( model ) {
				gameLocal.clip.RotationModel( CLIP_DEBUG_PARMS bodyResults, body->current->worldOrigin, rotation,
									body->clipModel, body->current->worldAxis, body->clipMask,
										model, model->GetOrigin(), model->GetAxis() );
			}
			else {
				gameLocal.clip.Rotation( CLIP_DEBUG_PARMS bodyResults, body->current->worldOrigin, rotation,
									body->clipModel, body->current->worldAxis, body->clipMask, self );
			}
			if ( bodyResults.fraction < results.fraction ) {
				results = bodyResults;
...
int idPhysics_AF::ClipContents( const idClipModel *model ) const {
	...
	for ( i = 0; i < bodies.Num(); i++ ) {
		...
		if ( body->clipModel->IsTraceModel() ) {
			if ( model ) {
				contents |= gameLocal.clip.ContentsModel( CLIP_DEBUG_PARMS body->current->worldOrigin,
									body->clipModel, body->current->worldAxis, -1,
										model, model->GetOrigin(), model->GetAxis() );
			}
			else {
				contents |= gameLocal.clip.Contents( CLIP_DEBUG_PARMS body->current->worldOrigin,
...
================
*/
void idPhysics_AF::DisableClip( bool activateContacting ) {
	int i;
	for ( i = 0; i < bodies.Num(); i++ ) {
...
const float	AF_FORCE_MAX				= 1e10f;
...
const float	AF_OFFSET_MAX				= 50000;
const int	AF_OFFSET_TOTAL_BITS		= 16;
const int	AF_OFFSET_EXPONENT_BITS		= idMath::BitsForInteger( idMath::BitsForFloat( AF_OFFSET_MAX ) ) + 1;
const int	AF_OFFSET_MANTISSA_BITS		= AF_OFFSET_TOTAL_BITS - 1 - AF_OFFSET_EXPONENT_BITS;

/*
================
idPhysics_AF::GetClipMask
================
*/
int idPhysics_AF::GetClipMask( int id ) const {
	if ( id < 0 || id >= bodies.Num() ) {
		return clipMask;
	} else {
		return bodies[ id ]->GetClipMask();
	}
}

/*
================
idPhysics_AF::SetClipMask
================
*/
void idPhysics_AF::SetClipMask( int mask, int id ) {
	if ( id < 0 || id >= bodies.Num() ) {
		clipMask = mask;
	} else {
...
/*
================
idPhysics_AF::CheckNetworkStateChanges
================
*/
bool idPhysics_AF::CheckNetworkStateChanges( networkStateMode_t mode, const sdEntityStateNetworkData& baseState ) const {
	if ( mode == NSM_VISIBLE ) {
		const sdAFPhysicsNetworkData& baseData = baseState.GetData< sdAFPhysicsNetworkData >();

		if ( baseData.activateTime != current.activateTime ) {
			return true;
		}
		...
		if ( baseData.noMoveTime != current.noMoveTime ) {
			return true;
		}

		if ( baseData.bodies.Num() != bodies.Num() ) {
			return true;
		}

		int i;
		for ( i = 0; i < baseData.bodies.Num(); i++ ) {
			if ( !baseData.bodies[ i ].origin.Compare( bodies[ i ]->current->worldOrigin, 0.01f ) ) {
				return true;
			}

			idCQuat newOrientation = bodies[ i ]->current->worldAxis.ToCQuat();
			if ( !baseData.bodies[ i ].orientation.Compare( newOrientation, 0.0001f ) ) {
				return true;
			}
		}
		return false;
...
/*
================
idPhysics_AF::WriteNetworkState
================
*/
void idPhysics_AF::WriteNetworkState( networkStateMode_t mode, const sdEntityStateNetworkData& baseState, sdEntityStateNetworkData& newState, idBitMsg& msg ) const {
	if ( mode == NSM_VISIBLE ) {
		NET_GET_STATES( sdAFPhysicsNetworkData );

		newData.atRest			= current.atRest;
		newData.activateTime	= current.activateTime;
		newData.noMoveTime		= current.noMoveTime;

		newData.bodies.SetNum( bodies.Num() );
		int i;
		for ( i = 0; i < newData.bodies.Num(); i++ ) {
			newData.bodies[ i ].origin		= bodies[ i ]->current->worldOrigin;
			newData.bodies[ i ].orientation	= bodies[ i ]->current->worldAxis.ToCQuat();
		}
		...
		for ( i = 0; i < newData.bodies.Num(); i++ ) {
			if ( i < baseData.bodies.Num() ) {
				msg.WriteDeltaVector( baseData.bodies[ i ].origin, newData.bodies[ i ].origin );
				msg.WriteDeltaCQuat( baseData.bodies[ i ].orientation, newData.bodies[ i ].orientation );
			} else {
				msg.WriteVector( newData.bodies[ i ].origin );
				msg.WriteCQuat( newData.bodies[ i ].orientation );
			}
		}
		return;
	}
...
/*
================
idPhysics_AF::ApplyNetworkState
================
*/
void idPhysics_AF::ApplyNetworkState( networkStateMode_t mode, const sdEntityStateNetworkData& newState ) {
	traceCollection.ForceNextUpdate();
	if ( mode == NSM_VISIBLE ) {
		NET_GET_NEW( sdAFPhysicsNetworkData );

		//
		current.atRest			= newData.atRest;
		current.activateTime	= newData.activateTime;
		current.noMoveTime		= newData.noMoveTime;

		for ( int i = 0; i < newData.bodies.Num() && i < bodies.Num(); i++ ) {
			bodies[ i ]->current->worldOrigin	= newData.bodies[ i ].origin;
			bodies[ i ]->current->worldAxis		= newData.bodies[ i ].orientation.ToMat3();
		}
...
/*
================
idPhysics_AF::ReadNetworkState
================
*/
void idPhysics_AF::ReadNetworkState( networkStateMode_t mode, const sdEntityStateNetworkData& baseState, sdEntityStateNetworkData& newState, const idBitMsg& msg ) const {
	if ( mode == NSM_VISIBLE ) {
		NET_GET_STATES( sdAFPhysicsNetworkData );

		//
		newData.atRest			= msg.ReadDeltaLong( baseData.atRest );
		newData.activateTime	= msg.ReadDeltaFloat( baseData.activateTime );
		newData.noMoveTime		= msg.ReadDeltaLong( baseData.noMoveTime );

		int count				= msg.ReadDeltaLong( baseData.bodies.Num() );
		newData.bodies.SetNum( count );

		for ( int i = 0; i < newData.bodies.Num(); i++ ) {
			if ( i < baseData.bodies.Num() ) {
				newData.bodies[ i ].origin		= msg.ReadDeltaVector( baseData.bodies[ i ].origin );
				newData.bodies[ i ].orientation = msg.ReadDeltaCQuat( baseData.bodies[ i ].orientation );
			} else {
				newData.bodies[ i ].origin		= msg.ReadVector();
				newData.bodies[ i ].orientation	= msg.ReadCQuat();
			}
		}
...
/*
================
idPhysics_AF::CreateNetworkStructure
================
*/
sdEntityStateNetworkData* idPhysics_AF::CreateNetworkStructure( networkStateMode_t mode ) const {
	if ( mode == NSM_VISIBLE ) {
		return new sdAFPhysicsNetworkData();
	}
	return NULL;
}
