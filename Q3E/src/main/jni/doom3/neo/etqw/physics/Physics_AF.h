...
public:
	...
	virtual void			Translate( const idVec3 &translation );
	virtual void			Rotate( const idRotation &rotation );
	virtual void			GetCenter( idVec3& org );
...
public:
	...
	const idVec3 &			GetLinearExternalForce( void ) const { return current->externalForce.SubVec3(0); }
	const idVec3 &			GetAngularExternalForce( void ) const { return current->externalForce.SubVec3(1); }
...
class sdAFPhysicsNetworkData : public sdEntityStateNetworkData {
public:
...
public:
	...
	idAFBody*				GetBody( const int id ) const { return bodies[ id ]; }
	...
	virtual void			SetComeToRest( bool enable ) { comeToRest = enable; }
...
public:	// common physics interface
	...
	const idVec3&			CalculateCenterOfMass( void );							// update and return
	virtual const idVec3&	GetCenterOfMass( void ) const { return centerOfMass; }	// just return

	virtual int				GetClipMask( int id = -1 ) const;
	virtual void			SetClipMask( int mask, int id = -1 );
...
private:
	idVec3					centerOfMass;		// american spelling :| center of mass of the whole body, calculated at some point in time
...
private:
	...
	void					ApplyFriction( float timeStep, int endTimeMSec );
