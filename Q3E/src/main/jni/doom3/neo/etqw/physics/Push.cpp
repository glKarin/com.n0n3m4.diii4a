...
int idPush::TryTranslatePushEntity( trace_t &results, idEntity *check, idClipModel *clipModel, const int flags,
	...
	if ( physics->IsGroundClipModel( clipModel->GetEntity()->entityNumber, clipModel->GetId() ) ) {
		...
		if ( trace.fraction < 1.0f ) {
			// Arnout:: FIXME: TEMP TEMP DEBUG
			// somehow checkMove becomes a 3 element -1.INF, likely cause trace.fraction is something odd
			float oldTraceFraction = trace.fraction;
...
============
*/
int idPush::DiscardEntities( idEntity *entityList[], int numEntities, int flags, idEntity *pusher, int contents ) {
	int i, num;
	idEntity *check;

	int clipMask = pusher->GetPhysics()->GetClipMask();
	...
	for ( num = i = 0; i < numEntities; i++ ) {
		...
		// if the entity doesn't clip with this pusher
		if ( !( check->GetPhysics()->GetClipMask() & contents ) ) {
			continue;
		}

		// if the pusher doesn't clip with this entity
		if ( !( clipMask & check->GetPhysics()->GetContents() ) ) {
			continue;
		}

		// don't push players in noclip mode
		idPlayer* player = check->Cast< idPlayer >();
		if ( player && player->GetNoClip() ) {
			continue;
		}
...
float idPush::ClipPush( trace_t &results, idEntity *pusher, const int flags,
								const idVec3 &oldOrigin, const idMat3 &oldAxis,
									idVec3 &newOrigin, idMat3 &newAxis, idClipModel * clipModel ) {
	idVec3 translation;
	idRotation rotation;
	float mass;
	...
	if ( translation != vec3_origin ) {
		...
		mass += ClipTranslationalPush( results, pusher, flags, newOrigin, translation, clipModel );
		if ( results.fraction < 1.0f ) {
			newOrigin = oldOrigin;
			newAxis = oldAxis;
			return mass;
	...
	if ( rotation.GetAngle() != 0.0f ) {
		...
		clipModel->SetPosition( newOrigin, oldAxis, gameLocal.clip );

		mass += ClipRotationalPush( results, pusher, flags, newAxis, rotation, clipModel );
		if ( results.fraction < 1.0f ) {
			newOrigin = oldOrigin;
			newAxis = oldAxis;
			return mass;
