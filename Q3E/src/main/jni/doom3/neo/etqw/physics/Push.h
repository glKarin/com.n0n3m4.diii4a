...
public:
	...
	float			ClipTranslationalPush( trace_t &results, idEntity *pusher, const int flags,
											const idVec3 &newOrigin, const idVec3 &move, idClipModel * clipModel );
...
private:
	...
	int				DiscardEntities( idEntity *entityList[], int numEntities, int flags, idEntity *pusher, int contents );
#endif
};
