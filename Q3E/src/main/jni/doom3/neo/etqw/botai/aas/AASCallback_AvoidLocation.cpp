...
void idAASCallback_AvoidLocation::SetObstacles( const idAAS *aas, const idAASObstacle *obstacles, int numObstacles ) {
	...
	for ( int i = 0; i < numObstacles; i++ ) {
		obstacles[i].expAbsBounds[0] = obstacles[i].absBounds[0] - aas->GetSettings()->boundingBox[1];
		obstacles[i].expAbsBounds[1] = obstacles[i].absBounds[1] - aas->GetSettings()->boundingBox[0];
	}
}
