...
int idAF::EntitiesTouchingAF( afTouch_t touchList[ MAX_GENTITIES ] ) const {
	...
	numTouching = 0;
	numClipModels = gameLocal.clip.ClipModelsTouchingBounds( CLIP_DEBUG_PARMS physicsObj.GetAbsBounds(), -1, clipModels, MAX_GENTITIES, self );
	...
	for ( i = 0; i < jointMods.Num(); i++ ) {
		...
		for ( j = 0; j < numClipModels; j++ ) {
			...
			if ( gameLocal.clip.ContentsModel( CLIP_DEBUG_PARMS body->GetWorldOrigin(), body->GetClipModel(), body->GetWorldAxis(), -1, cm, cm->GetOrigin(), cm->GetAxis() ) ) {
...
bool idAF::LoadBody( const idDeclAF_Body *fb, const idJointMat *joints ) {
	...
	switch( fb->modelType ) {
		...
		case TRM_BONE: {
			...
			axis[1] = -axis[1];
			// create trace model to cover the bone
			idVec3 halfBounds( fb->width * 0.5f, fb->width * 0.5f, length * 0.5f );
			trm.SetupBox( idBounds( -halfBounds, halfBounds ) );

//			old way uses a tetrahedron kind of thing, but it doesn't even come close to covering the render model :(
//			trm.SetupBone( length, fb->width );
	...
	if ( body ) {
		...
		if ( !clip->IsEqual( trm ) ) {
			clip = new idClipModel( trm, false );
...
bool idAF::Load( idEntity* ent, const char* fileName ) {
	...
	const idDeclAF* file = declHolder.declAFType.LocalFind( name, false );
	if ( !file ) {
	...
	idRenderModel* model = animator->ModelHandle();
	...
	physicsObj.CalculateCenterOfMass();
...
bool idAF::TestSolid( void ) const {
	...
	for ( i = 0; i < physicsObj.GetNumBodies(); i++ ) {
		...
		if ( gameLocal.clip.Translation( CLIP_DEBUG_PARMS trace, body->GetWorldOrigin(), body->GetWorldOrigin(), body->GetClipModel(), body->GetWorldAxis(), body->GetClipMask(), self ) ) {
