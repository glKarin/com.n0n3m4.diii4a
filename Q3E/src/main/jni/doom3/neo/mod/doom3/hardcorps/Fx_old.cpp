...
void idTeleporter::Event_DoAction( idEntity *activator ) {
	...
	if ( forceChangeXpos && activator->IsType( idPlayer::Type ) ) {
		...
		if( validtarget ){
			...
			player->Teleport( GetPhysics()->GetOrigin(), a, NULL ); //this will set a new lockedXpos 
			player->StartForcedMov( validtarget, false, 300, spawnArgs.GetBool( "totalForce", "0" ) ); //never abort this, wait 0.3s
		}else{
