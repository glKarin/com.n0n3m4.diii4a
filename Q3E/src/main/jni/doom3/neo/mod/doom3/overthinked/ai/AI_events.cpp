...
const idEventDef AI_EjectBrass( "EjectBrassM" );
...
const idEventDef AI_HitByMelee( "hitByMelee",NULL,'d');
...
const idEventDef AI_ChangeProjectileDef("ChangeProjectileDef","d");

CLASS_DECLARATION(idActor, idAI)
EVENT(EV_Activate,							idAI::Event_Activate)
EVENT(EV_Touch,							idAI::Event_Touch)
EVENT( AI_HitByMelee,                       idAI::Event_HitByMelee)
...
EVENT( AI_ChangeProjectileDef,				idAI::Event_ChangeProjectileDef)
...
EVENT( AI_EjectBrass,						idAI::Event_EjectBrass )
...
=====================
*/
void idAI::Event_EjectBrass( void )
{
    int backup = currentProjectile;
    Event_ChangeProjectileDef(666);
    Event_LaunchMissile(this->GetPhysics()->GetOrigin(), this->GetPhysics()->GetAxis().ToAngles() );
    Event_ChangeProjectileDef(backup);
}
...
void idAI::Event_ChangeProjectileDef( int number )
{
    ...
    projectileDef = gameLocal.FindEntityDefDict( kv->GetValue() );
}
...
/*
=====================
idAI::Event_HitByMelee
=====================
*/
bool idAI::Event_HitByMelee(  )
{
    idThread::ReturnInt( hitByMelee );
    return this->hitByMelee;
}
