...
//+auLucifer
    //Disable the cursor as we don't really want it atm
    if(idPlayer::cursor) idPlayer::cursor = 0;
    //-auLucifer
	idUserInterface * cursor = idPlayer::cursor;
...
/*  
	//+auLucifer
    // Force the player to see either left or right. viewPos decides which way to face
    if(usercmd.mx < 0){
        if(viewPos < 0.2) viewPos += 20;
    }else if(usercmd.mx > 0){
        if(viewPos > -0.2) viewPos -= 20;
    }
*/
// PHIL BEGIN
...
void idPlayer::Move( void ) {
	...
	physicsObj.SetDebugLevel( g_debugMove.GetBool() );
	 //+auLucifer
    // Stop it so the play can't move anywhere but forward and backwards
    if(usercmd.rightmove) usercmd.rightmove = 0;

    // Force the player to move along one plane
    if(viewAngles.yaw > 0) viewAngles.yaw = 90;
    else if(viewAngles.yaw < 0) viewAngles.yaw = -90;
// PHIL BEGIN
	//if(usercmd.forwardmove) viewAngles.yaw = 90;
	//else if(-usercmd.forwardmove) viewAngles.yaw = -90;
// PHIL END
    // Force the forward key to travel in one direction as well as the backpedal key
    // Foward travels right and backward travels left
    if(usercmd.forwardmove)
        usercmd.forwardmove = (viewPos < 0 ? usercmd.forwardmove : -usercmd.forwardmove);
    //-auLucifer
