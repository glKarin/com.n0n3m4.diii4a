...
struct ImVec2
{
    ...
    {
        IM_ASSERT(idx == 0 || idx == 1);    // We very rarely use this [] operator, so the assert overhead is fine.
...
struct ImVector
{
    ...
    inline ImVector<T>& operator=(const ImVector<T>& src)
    {
        clear();
        resize(src.Size);
        if (src.Data) memcpy(Data, src.Data, (size_t)Size * sizeof(T));
        return *this;
    }
    inline ~ImVector()
    {
    ...
    inline void         clear()
    {
        if (Data)
        {
            Size = Capacity = 0;    // Important: does not destruct anything
            IM_FREE(Data);
            Data = NULL;
        }
    }
    ...
    {
        for (int n = 0; n < Size; n++) IM_DELETE(Data[n]);    // Important: never called automatically! always explicit.
        clear();
    }
    inline void         clear_destruct()
    {
        for (int n = 0; n < Size; n++) Data[n].~T();    // Important: never called automatically! always explicit.
        clear();
    }
    ...
    {
        IM_ASSERT(new_size <= Size);    // Resize a vector to a smaller size, guaranteed not to cause a reallocation
        Size = new_size;
    }
...
struct ImGuiTextBuffer
{
    ...
    {
        return Buf.Data ? &Buf.back() : EmptyString;    // Buf is zero-terminated, so end() will point on the zero-terminator
    }
    int                 size() const
    {
...
struct ImGuiListClipper
{
    ...
    {
        IncludeItemsByIndex(item_begin, item_end);    // [renamed in 1.89.9]
    }
    inline void ForceDisplayRangeByIndices(int item_begin, int item_end)
    {
        IncludeItemsByIndex(item_begin, item_end);    // [renamed in 1.89.6]
    }
...
struct ImColor
{
    ...
    constexpr 
#endif
		ImColor(int r, int g, int b, int a = 255)             : Value((float)r * (1.0f / 255.0f), (float)g * (1.0f / 255.0f), (float)b * (1.0f / 255.0f), (float)a* (1.0f / 255.0f)) {}
#if 0 // __cplusplus >= 201103L
    constexpr 
#endif
		ImColor(ImU32 rgba)                                   : Value((float)((rgba >> IM_COL32_R_SHIFT) & 0xFF) * (1.0f / 255.0f), (float)((rgba >> IM_COL32_G_SHIFT) & 0xFF) * (1.0f / 255.0f), (float)((rgba >> IM_COL32_B_SHIFT) & 0xFF) * (1.0f / 255.0f), (float)((rgba >> IM_COL32_A_SHIFT) & 0xFF) * (1.0f / 255.0f)) {}
...
struct ImGuiSelectionBasicStorage
{
    ...
    {
        return AdapterIndexToStorageId(this, idx);    // Convert index to item id based on provided adapter.
    }
};
...
struct ImDrawListSplitter
{
    ...
    inline void                 Clear()
    {
        _Current = 0;    // Do not clear Channels[] so our allocations are reused next frame
        _Count = 1;
    }
...
struct ImDrawList
{
    ...
    {
        PrimWriteIdx((ImDrawIdx)_VtxCurrentIdx);    // Write vertex with unique index
...
struct ImFontGlyphRangesBuilder
{
    ...
    {
        int off = (int)(n >> 5);    // Get bit n in the array
        ImU32 mask = 1u << (n & 31);
        return (UsedChars[off] & mask) != 0;
    }
    inline void     SetBit(size_t n)
    {
        int off = (int)(n >> 5);    // Set bit n in the array
        ImU32 mask = 1u << (n & 31);
        UsedChars[off] |= mask;
    }
...
struct ImFontAtlas
{
    ...
    bool                        IsBuilt() const
    {
        return Fonts.Size > 0 && TexReady;    // Bit ambiguous: used to detect when user didn't build texture but effectively we should check TexID != 0 except that would be backend dependent...
    }
