...
template< typename T > ID_INLINE T Lerp( const T from, const T to, float f ) { return from + ( ( to - from ) * f ); }

template< typename T > ID_INLINE T InchesToMetres( const T value ) { return static_cast< T >( value * 0.0254f ); }
template< typename T > ID_INLINE T MetresToInches( const T value ) { return static_cast< T >( value * 39.37f ); }
template< typename T > ID_INLINE T InchesToFeet( const T value ) { return static_cast< T >( value / 12.f ); }
template< typename T > ID_INLINE T FeetToMiles( const T value ) { return static_cast< T >( value / 5280.f ); }
template< typename T > ID_INLINE T FeetToInches( const T value ) { return static_cast< T >( value * 12.f ); }
template< typename T > ID_INLINE T MetresToFeet( const T value ) { return InchesToFeet( MetresToInches( value ) ); }
template< typename T > ID_INLINE T FeetToMetres( const T value ) { return FeetToInches( InchesToMetres( value ) ); }

template< typename T > ID_INLINE float UPSToKPH( const T value ) { return InchesToMetres( value ) * 0.001f * 60.f * 60.f; }
template< typename T > ID_INLINE float KPHToUPS( const T value ) { return MetresToInches( value ) / ( 0.001f * 60.f * 60.f ); }

template< typename T > ID_INLINE float UPSToMPH( const T value ) { return FeetToMiles( InchesToFeet( value ) ) * 60.f * 60.f; }
...
template< typename T > ID_INLINE int MINS2MS( const T value ) { return idMath::FtoiFast( value * idMath::M_SEC2MS * 60.f ); }
template< typename T > ID_INLINE int SEC2MS( const T value ) { return idMath::FtoiFast( value * idMath::M_SEC2MS ); }
template< typename T > ID_INLINE float MS2SEC( const T value ) { return static_cast< float >( value * idMath::M_MS2SEC ); }
