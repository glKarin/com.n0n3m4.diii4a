...
public:
	...
	bool					IsDefaultValue( void ) { return internalVar->InternalIsDefaultValue(); }
	...
	void					SetString( const char *value ) { internalVar->InternalSetString( value, true, true ); }
	void					SetBool( const bool value ) { internalVar->InternalSetBool( value, true, true ); }
	void					SetInteger( const int value ) { internalVar->InternalSetInteger( value, true, true ); }
	void					SetFloat( const float value ) { internalVar->InternalSetFloat( value, true, true ); }

	void					RegisterCallback( idCVarCallback* callback ) { internalVar->InternalRegisterCallback( callback ); }
	void					UnRegisterCallback( idCVarCallback* callback ) { internalVar->InternalUnRegisterCallback( callback ); }
...
private:
	...
	virtual bool			InternalIsDefaultValue() const { return false; }
