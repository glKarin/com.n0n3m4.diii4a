...
static bool GLES_Init_prefer(const glimpParms_t &ap)
{
	...
	for (int i = 0; i < 16; i++) {
		...
		int multisamples = ap.multiSamples;
		suc = false;
        ...
        EGLint HARM_EGL_OPENGL_ES_BIT;
        if(gl_version == GL_VERSION_GL_CORE)
        {
            HARM_EGL_OPENGL_ES_BIT = EGL_OPENGL_BIT;
        }
        ...
        {
#ifdef _OPENGLES3
            HARM_EGL_OPENGL_ES_BIT = (gl_version != GL_VERSION_GL_ES2 ? EGL_OPENGL_ES3_BIT : EGL_OPENGL_ES2_BIT);
#else
            HARM_EGL_OPENGL_ES_BIT = EGL_OPENGL_ES2_BIT
#endif
        }
...
int GLES_Init(glimpParms_t ap)
{
	...
	EGLint ctxAttrib[9] = { 0 };
	int attrIndex = 0;
#define EGL_SET_ATTR(n, v) { ctxAttrib[attrIndex++] = n; ctxAttrib[attrIndex++] = v; }
    if(gl_version == GL_VERSION_GL_CORE)
    {
		EGL_SET_ATTR(EGL_CONTEXT_MAJOR_VERSION, 3);
		//EGL_SET_ATTR(EGL_CONTEXT_MINOR_VERSION, 2);
		//EGL_SET_ATTR(EGL_CONTEXT_OPENGL_PROFILE_MASK, EGL_CONTEXT_OPENGL_CORE_PROFILE_BIT);
    }
    else if(gl_version == GL_VERSION_GL_COMPATIBILITY)
    {
		EGL_SET_ATTR(EGL_CONTEXT_MAJOR_VERSION, 3);
		//EGL_SET_ATTR(EGL_CONTEXT_MINOR_VERSION, 2);
		//EGL_SET_ATTR(EGL_CONTEXT_OPENGL_PROFILE_MASK, EGL_CONTEXT_OPENGL_COMPATIBILITY_PROFILE_BIT);
    }
    else
    {
		EGLint HARM_EGL_CONTEXT_CLIENT_VERSION;
#ifdef _OPENGLES3
        HARM_EGL_CONTEXT_CLIENT_VERSION = (gl_version != GL_VERSION_GL_ES2 ? 3 : 2);
#else
        HARM_EGL_CONTEXT_CLIENT_VERSION = 2
#endif
		EGL_SET_ATTR(EGL_CONTEXT_CLIENT_VERSION, HARM_EGL_CONTEXT_CLIENT_VERSION);
    }

    EGL_SET_ATTR(HARM_EGL_CONTEXT_OPENGL_DEBUG, harm_r_debugOpenGL.GetBool() ? 1 : 0);
	ctxAttrib[attrIndex] = EGL_NONE;
