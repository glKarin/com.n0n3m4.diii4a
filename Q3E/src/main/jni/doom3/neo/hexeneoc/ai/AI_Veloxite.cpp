...
void idAI_Veloxite::Restore( idRestoreGame *savefile )
{
    ...
    bool b;
    savefile->ReadBool( b );
    b ? AI_ALIGNING = true : AI_ALIGNING = false;
    savefile->ReadBool( b );
    b ? AI_WALLING = true : AI_WALLING = false;
    savefile->ReadBool( b );
    b ? AI_LEDGING = true : AI_LEDGING = false;
    savefile->ReadBool( b );
    b ? AI_SLOPING = true : AI_SLOPING = false;
    savefile->ReadBool( b );
    b ? AI_LEAPING = true : AI_LEAPING = false;
    savefile->ReadBool( b );
    b ? AI_FALLING = true : AI_FALLING = false;
    savefile->ReadBool( b );
    b ? AI_DROPATTACK = true : AI_DROPATTACK = false;
    savefile->ReadBool( b );
    b ? AI_ONWALL = true : AI_ONWALL = false;
...
bool idAI_Veloxite::checkHovering( void )
{
    if ( AI_ONGROUND )
    {
        ...
        if ( wall && !wallIsWalkable( wall ) )
        {
            idVec3 vecUp = physicsObj.GetGravity();
            vecUp.Normalize();
            idVec3 dir = FacingNormal() + vecUp * 0.5;
            dir.Normalize();
...
void idAI_Veloxite::getOnSurface( const idVec3 &newNorm, int numt )
{
    ...
    if (!AI_LEAPING)   // if we're AI_LEAPING, don't screw with the origin
    {
        ...
        {
            upMax = 1.66f;    // AI_FALLING, AI_WALLING
        }
...
void idAI_Veloxite::Event_getVeloxJumpVelocity( float speed, float max_height, float channel, const char *animname )
{
    ...
    if ( AI_ENEMY_IN_FOV )   //todo: no?
    {
        ...
        if ( AI_ONWALL )
        {
            ...
            if ( zDist > 0 )
            {
                jumpVelocity.z += zDist * 2;
            }
            jumpVelocity.Normalize();
            jumpVelocity *= VELOX_LEAP_SPEED;
            idThread::ReturnVector( jumpVelocity );
            return;
        }
        else
        {
            ...
            idThread::ReturnVector( GetJumpVelocity( PredictEnemyPos( GetAnimLength( (int)channel, animname ) ), VELOX_LEAP_SPEED, VELOX_LEAP_MAXHEIGHT ) );
            return;
        }
    }
