...
const idEventDef BOT_FindInRadius( "findInRadius", "vf", 'f' );
...
const idEventDef BOT_IsTeamGame( "isTeamGame", NULL, 'f' );
const idEventDef BOT_IsPickedUp( "isPickedUp", "E", 'f' );
const idEventDef BOT_SetAimRate( "setAimRate", "f", NULL );
const idEventDef BOT_GetAimDir( "getAimDir", "ef", 'v' );
const idEventDef BOT_IsMoveDone( "isMoveDone", NULL, 'f' );
const idEventDef BOT_IsMoveUnreachable( "isMoveUnreachable", NULL, 'f' );
const idEventDef BOT_TestMoveToPosition( "testMoveToPosition", "v", 'f' );
const idEventDef BOT_GetNumSpawnedEntities( "getNumSpawnedEntities", NULL, 'f' );
const idEventDef BOT_PowerUpActive( "powerUpActive", NULL, 'f' );
...
EVENT( BOT_IsTeamGame,						botAi::Event_IsTeamGame )
EVENT( BOT_IsPickedUp,						botAi::Event_IsPickedUp )
EVENT( BOT_SetAimRate,						botAi::Event_SetAimRate )
...
void botAi::ReadUserCmdsFromSnapshot( const idBitMsg &msg )
{
    ...
    for ( i = 0; i < numBots; i++ )
    {
        ...
        usercmd_t &cmd = (usercmd_t &)gameLocal.usercmds[clientNum]; // TinMan: Overwrite the usrcmds, as bot isn't propper client this will be null anyway.
...
void botAi::LinkScriptVariables( void )
{
    ...
    AI_STRAFE_LEFT.LinkTo(		scriptObject, "AI_LEFT" );
    AI_STRAFE_RIGHT.LinkTo(		scriptObject, "AI_RIGHT" );
    ...
    AI_ENEMY_VISIBLE.LinkTo(	scriptObject, "ENEMY_VISIBLE" );
    AI_ENEMY_IN_FOV.LinkTo(		scriptObject, "ENEMY_IN_FOV" );
    AI_ENEMY_DEAD.LinkTo(		scriptObject, "ENEMY_DEAD" );
    ...
    AI_WEAPON_FIRE.LinkTo(		scriptObject, "AI_FIRE" );

    //BOT_COMMAND.LinkTo(			scriptObject, "BOT_COMMAND" );

    AI_RESPAWN.LinkTo(				scriptObject, "AI_RESPAWN" );
}
...
void botAi::GetBodyState( void )
{
    ...
    AI_RESPAWN = playerEnt->IsRespawning();
}
...
void botAi::KickObstacles( const idVec3 &dir, float force, idEntity *alwaysKick )
{
    ...
    numListedClipModels = gameLocal.ClipModelsTouchingBounds( NULL, clipBounds, clipmask, clipModelList, MAX_GENTITIES );
...
=====================
*/
bool botAi::ReachedPos( const idVec3 &pos, const aiMoveCommand_t moveCommand ) const
{
...
bool botAi::SetMoveOutOfRange( idEntity *ent, float range )
{
    ...
    if ( !aas->FindNearestGoal( goal, areaNum, org, pos, travelFlags, 0.0f, 0.0f, &obstacle, 1, findGoal ) )
    {
        StopMove( MOVE_STATUS_DEST_UNREACHABLE );
        AI_DEST_UNREACHABLE = true;
        return false;
...
bool botAi::SetMoveToAttackPosition( idEntity *ent )
{
    ...
    botAASFindAttackPosition findGoal( playerEnt, physicsObject->GetGravityAxis(), ent, pos, EyeOffset(playerEnt) );
    if ( !aas->FindNearestGoal( goal, areaNum, org, pos, travelFlags, 0.0f, 0.0f, &obstacle, 1, findGoal ) )
    {
        StopMove( MOVE_STATUS_DEST_UNREACHABLE );
        AI_DEST_UNREACHABLE = true;
...
bool botAi::SetMoveToCover( idEntity *entity, const idVec3 &hideFromPos )
{
    ...
    if ( !aas->FindNearestGoal( hideGoal, areaNum, org, hideFromPos, travelFlags, 0.0f, 0.0f, &obstacle, 1, findCover ) )
    {
        StopMove( MOVE_STATUS_DEST_UNREACHABLE );
        AI_DEST_UNREACHABLE = true;
        return false;
...
bool botAi::EntityCanSeePos( idActor *actor, const idVec3 &actorOrigin, const idVec3 &pos )
{
    ...
    eye = actorOrigin + EyeOffset(actor);
    ...
    gameLocal.TracePoint( NULL, results, eye, point, MASK_SOLID, actor );
...
idVec3 botAi::GetMovePosition( void )
{
    ...
    if ( ai_debugMove.GetBool() )
    {
        ...
        gameRenderWorld->DebugLine( colorYellow, physicsObject->GetOrigin() + EyeOffset(playerEnt), physicsObject->GetOrigin() + EyeOffset(playerEnt) + playerEnt->viewAxis[ 0 ] * physicsObject->GetGravityAxis() * 16.0f, gameLocal.msec, true );
        DrawRoute();
    }
...
bool botAi::EnemyPositionValid( void ) const
{
    ...
    gameLocal.TracePoint( NULL, tr, playerEnt->GetEyePosition(), lastVisibleEnemyPos + lastVisibleEnemyEyeOffset, MASK_OPAQUE, playerEnt );
    if ( tr.fraction < 1.0f )
    {
...
void botAi::SetEnemyPosition( void )
{
    ...
    lastVisibleEnemyEyeOffset = EyeOffset(enemyEnt);
...
bool botAi::CanSee( idEntity *ent, bool useFov ) const
{
    ...
    gameLocal.TracePoint( NULL, tr, eye, toPos, CONTENTS_OPAQUE | CONTENTS_RENDERMODEL, this );
    // TinMan: *debug
    float t = 1;
...
trace_t botAi::GetPlayerTrace( idPlayer * player )
{
    ...
    gameLocal.TracePoint( NULL, tr, eye, eye + ( dir * maxRange ), CONTENTS_OPAQUE | CONTENTS_RENDERMODEL, player );
...
void botAi::Event_FindInRadius( const idVec3 &origin, float radius/*, const char *classname*/ )
{
    ...
    numListedEntities = gameLocal.EntitiesTouchingBounds( this, bounds, -1, entityList, MAX_GENTITIES );
    ...
    for ( e = 0; e < numListedEntities; e++ )
    {
        ...
        {
			if(ent->GetPhysics()->GetClipModel() && !ent->GetPhysics()->GetClipModel()->IsTraceModel())
				continue;
            ...
            if ( pushOrigin != entityOrigin )
            {
                ...
                gameLocal.Translation( NULL, result, pushOrigin, pushOrigin + ent->GetPhysics()->GetGravityNormal() * max_dist,	ent->GetPhysics()->GetClipModel(), ent->GetPhysics()->GetAxis(), MASK_SOLID, ent );
...
void botAi::Event_CanHitEnemy( void )
{
    ...
    gameLocal.TracePoint( NULL, tr, eye, toPos, MASK_SHOT_BOUNDINGBOX, playerEnt );
...
void botAi::GetAIAimTargets( idActor *actor, const idVec3 &lastSightPos, idVec3 &headPos, idVec3 &chestPos )
{
    headPos = lastSightPos + botAi::EyeOffset(actor);
    chestPos = ( headPos + lastSightPos + actor->GetPhysics()->GetBounds().GetCenter() ) * 0.5f;
}

/*
=====================
botAi::Event_IsTeamGame
=====================
*/
void botAi::Event_IsTeamGame( void )
{
    idThread::ReturnFloat( gameLocal.IsTeamGame() );
}

/*
=====================
botAi::Event_IsPickedUp
=====================
*/
void botAi::Event_IsPickedUp( idEntity *entity )
{
    bool b = false;
    if(entity && entity->IsType(idItem::Type))
    {
        b = (static_cast<idItem *>(entity))->pickedUp;
    }
    idThread::ReturnFloat( b );
}

/*
=====================
botAi::Event_SetAimRate
=====================
*/
void botAi::Event_SetAimRate( float f )
{
    if ( f > 1.0f )
    {
        aimRate = 1.0f;
...
/*
=====================
botAi::Event_IsMoveDone
=====================
*/
void botAi::Event_IsMoveDone( void )
{
    idThread::ReturnFloat( MoveDone() );
}
...
=====================
*/
void botAi::Event_IsMoveUnreachable( void )
{
    idThread::ReturnFloat( AI_DEST_UNREACHABLE );
}
...
/*
=====================
botAi::Event_GetNumSpawnedEntities
=====================
*/
void botAi::Event_GetNumSpawnedEntities( void )
{
    idThread::ReturnFloat( gameLocal.spawnedEntities.Num() );
}

/*
=====================
botAi::Event_PowerUpActive
=====================
*/
