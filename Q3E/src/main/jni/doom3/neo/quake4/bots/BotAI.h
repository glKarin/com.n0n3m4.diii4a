...
protected:
    ...
    bool					ReachedPos( const idVec3 &pos, const aiMoveCommand_t moveCommand ) const;
...
private:
    ...
    void					Event_IsTeamGame( void );
    void					Event_IsPickedUp( idEntity *entity ); // TinMan: Item has been picked up
    void					Event_SetAimRate( float f );
    void					Event_GetAimDir( idEntity *aimAtEnt, float prefered ); // TinMan: wizzbang all in one aim prediction
    void					Event_IsMoveDone( void );
    void					Event_IsMoveUnreachable( void );
    void					Event_TestMoveToPosition( const idVec3 &pos );
    void					Event_GetNumSpawnedEntities( void );
