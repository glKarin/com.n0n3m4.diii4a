...
static idVec3 operator/(float a, const idVec3 &b)
{
	return idVec3(a / b[0], a / b[1], a / b[2]);
}
	...
	for( int x = 0; x < 2; x++ )
	{
		...
		for( int y = 0; y < 2; y++ )
		{
			...
			for( int z = 0; z < 2; z++ )
			{
				...
				if( tw <= FLT_SMALLEST_NON_DENORMAL )
				{
...
void idRenderMatrix::ProjectedNearClippedBounds( idBounds& projected, const idRenderMatrix& mvp, const idBounds& bounds, bool windowSpace )
{
	...
	*/
	...
	
	{
		...
		const float fractionA = ( fabs( deltaA ) > FLT_SMALLEST_NON_DENORMAL ) ? ( d0 / deltaA ) : 0.0f;
		const float fractionB = ( fabs( deltaB ) > FLT_SMALLEST_NON_DENORMAL ) ? ( d1 / deltaB ) : 0.0f;
		const float fractionC = ( fabs( deltaC ) > FLT_SMALLEST_NON_DENORMAL ) ? ( d2 / deltaC ) : 0.0f;
		const float fractionD = ( fabs( deltaD ) > FLT_SMALLEST_NON_DENORMAL ) ? ( d3 / deltaD ) : 0.0f;
	
		const float fractionE = ( fabs( deltaE ) > FLT_SMALLEST_NON_DENORMAL ) ? ( d4 / deltaE ) : 0.0f;
		const float fractionF = ( fabs( deltaF ) > FLT_SMALLEST_NON_DENORMAL ) ? ( d5 / deltaF ) : 0.0f;
		const float fractionG = ( fabs( deltaG ) > FLT_SMALLEST_NON_DENORMAL ) ? ( d6 / deltaG ) : 0.0f;
		const float fractionH = ( fabs( deltaH ) > FLT_SMALLEST_NON_DENORMAL ) ? ( d7 / deltaH ) : 0.0f;
	
		const float fractionI = ( fabs( deltaI ) > FLT_SMALLEST_NON_DENORMAL ) ? ( d0 / deltaI ) : 0.0f;
		const float fractionJ = ( fabs( deltaJ ) > FLT_SMALLEST_NON_DENORMAL ) ? ( d1 / deltaJ ) : 0.0f;
		const float fractionK = ( fabs( deltaK ) > FLT_SMALLEST_NON_DENORMAL ) ? ( d2 / deltaK ) : 0.0f;
		const float fractionL = ( fabs( deltaL ) > FLT_SMALLEST_NON_DENORMAL ) ? ( d3 / deltaL ) : 0.0f;
...
static idVec4 ClipHomogeneousLineToSide( const idVec4& p0, const idVec4& p1, int axis, float side )
{
	...
	const float delta = d0 - d1;
	const float f = idMath::Fabs( delta ) > FLT_SMALLEST_NON_DENORMAL ? ( d0 / delta ) : 1.0f;
