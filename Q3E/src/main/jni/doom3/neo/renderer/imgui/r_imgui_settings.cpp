...
enum cvarFlag
{
    IMGUI_CVAR_COMPONENT_SPACE = BIT(0),
    IMGUI_CVAR_COMPONENT_CHECKBOX = BIT(1),
    IMGUI_CVAR_COMPONENT_COMBO = BIT(2),
    IMGUI_CVAR_COMPONENT_INPUT_FLOAT = BIT(3),
    IMGUI_CVAR_COMPONENT_INT_SLIDER = BIT(4),
    IMGUI_CVAR_COMPONENT_INPUT_INT = BIT(5),
    IMGUI_CVAR_COMPONENT_INPUT_VECTOR = BIT(6),
    IMGUI_CVAR_COMPONENT_INPUT = BIT(7),
    IMGUI_CVAR_COMPONENT_BUTTON = BIT(8),

    IMGUI_CVAR_GROUP_RENDERER = BIT(11),
    IMGUI_CVAR_GROUP_FRAMEWORK = BIT(12),
    IMGUI_CVAR_GROUP_GUI = BIT(13),
    IMGUI_CVAR_GROUP_GAME = BIT(14),
    IMGUI_CVAR_GROUP_OTHER = BIT(15),
};
...
void idImGuiSettings::AddOption(const char *name = NULL, const char *displayName, int flag, const char *str)
{
	idCVar *cvar = NULL;
	bool isCmd = false;
	if(name && name[0])
	{
		isCmd = flag & IMGUI_CVAR_COMPONENT_BUTTON;
		if(!isCmd)
		{
    ...
    const char *gname;
    if(flag & IMGUI_CVAR_GROUP_RENDERER) {
        gname = "Renderer";
    }
    else if(flag & IMGUI_CVAR_GROUP_FRAMEWORK) {
        gname = "Framework";
    }
    else if(flag & IMGUI_CVAR_GROUP_GUI) {
        gname = "GUI";
    }
    else if(flag & IMGUI_CVAR_GROUP_GAME) {
        gname = "Game";
    }
    else if(flag & IMGUI_CVAR_GROUP_OTHER) {
        gname = "Other";
    }
    ...
    int comp = IMGUI_CVAR_COMPONENT_SPACE;
    if(cvar)
    {
        if(item.flag & IMGUI_CVAR_COMPONENT_CHECKBOX)
            comp = IMGUI_CVAR_COMPONENT_CHECKBOX;
        else if(item.flag & IMGUI_CVAR_COMPONENT_COMBO)
            comp = IMGUI_CVAR_COMPONENT_COMBO;
        else if(item.flag & IMGUI_CVAR_COMPONENT_INPUT_FLOAT)
            comp = IMGUI_CVAR_COMPONENT_INPUT_FLOAT;
        else if(item.flag & IMGUI_CVAR_COMPONENT_INT_SLIDER)
            comp = IMGUI_CVAR_COMPONENT_INT_SLIDER;
        else if(item.flag & IMGUI_CVAR_COMPONENT_INPUT_INT)
            comp = IMGUI_CVAR_COMPONENT_INPUT_INT;
        else if(item.flag & IMGUI_CVAR_COMPONENT_INPUT_VECTOR)
            comp = IMGUI_CVAR_COMPONENT_INPUT_VECTOR;
        else if(item.flag & IMGUI_CVAR_COMPONENT_INPUT)
            comp = IMGUI_CVAR_COMPONENT_INPUT;
        else
        {
		...
		comp = IMGUI_CVAR_COMPONENT_BUTTON;
            ...
            if (ImGui::BeginTabItem(group.name.c_str()))
            {
                for(int m = 0; m < group.list.Num(); m++)
                {
                    ...
                    switch (item.component) {
                        case IMGUI_CVAR_COMPONENT_CHECKBOX:
                            RenderCheckboxCVar(item);
                            break;
                        case IMGUI_CVAR_COMPONENT_COMBO:
                            RenderComboCVar(item);
                            break;
                        case IMGUI_CVAR_COMPONENT_INPUT_FLOAT:
                            RenderFloatCVar(item);
                            break;
                        case IMGUI_CVAR_COMPONENT_INT_SLIDER:
                            RenderIntRangeCVar(item);
                            break;
                        case IMGUI_CVAR_COMPONENT_INPUT_INT:
                            RenderIntCVar(item);
                            break;
                        case IMGUI_CVAR_COMPONENT_INPUT_VECTOR:
                            RenderVectorCVar(item);
                            break;
                        case IMGUI_CVAR_COMPONENT_INPUT:
                            RenderInputCVar(item);
                            break;
                        case IMGUI_CVAR_COMPONENT_BUTTON:
                            RenderButtonCVar(item);
                            break;
                        case IMGUI_CVAR_COMPONENT_SPACE:
                            RenderNullCVar(item);
                            break;
                        default:
                            break;
...
void ImGui_RegisterCvar(const char *name = NULL, const char *displayName = NULL, int flag = 0, const char *options = NULL)
{
...
void ImGui_RegisterOptions(void)
{
    ...
    // framework
    ImGui_RegisterCvar(NULL, "Generic", IMGUI_CVAR_GROUP_FRAMEWORK);
    ImGui_RegisterCvar("harm_com_consoleHistory", NULL, IMGUI_CVAR_COMPONENT_COMBO, "0=Don't save;1=Save on exit;2=Save on every command");
    ImGui_RegisterCvar("harm_r_maxAllocStackMemory", "Control memory allocation on heap or stack", IMGUI_CVAR_GROUP_FRAMEWORK);
    ImGui_RegisterCvar("com_disableAutoSaves", "Don't create Autosaves on new map");
    ImGui_RegisterCvar(NULL, NULL, IMGUI_CVAR_GROUP_FRAMEWORK);

    // renderer
	ImGui_RegisterCvar(NULL, "Lighting", IMGUI_CVAR_GROUP_RENDERER);
    ImGui_RegisterCvar("harm_r_lightingModel", "Lighting model", IMGUI_CVAR_COMPONENT_COMBO, "0=No light;1=Phong;2=Blinn-Phong;3=PBR;4=Ambient");
    ...
    ImGui_RegisterCvar("harm_r_normalCorrectionPBR", "Vertex normal correction in PBR");
    ImGui_RegisterCvar("harm_r_ambientLightingBrightness", "Lighting brightness in ambient");
	...
	ImGui_RegisterCvar(NULL, "Shadow mapping", IMGUI_CVAR_GROUP_RENDERER);
    ImGui_RegisterCvar("r_useShadowMapping", "Soft shadow mapping", IMGUI_CVAR_COMPONENT_CHECKBOX);
    ...
    ImGui_RegisterCvar("r_shadowMapSplits", "Cascaded shadow mapping with parallel lights", IMGUI_CVAR_COMPONENT_INT_SLIDER);
    ImGui_RegisterCvar("harm_r_shadowMapNonParallelLightUltra", "Non-parallel light allow ultra quality shadow mapping");
#endif
    ImGui_RegisterCvar(NULL, NULL, IMGUI_CVAR_GROUP_RENDERER);

	ImGui_RegisterCvar(NULL, "Stencil shadow", IMGUI_CVAR_GROUP_RENDERER);
    ...
    ImGui_RegisterCvar("harm_r_stencilShadowSoftCopyStencilBuffer", "Copy stencil buffer directly for soft stencil shadow", IMGUI_CVAR_COMPONENT_COMBO, "0=Copy depth buffer and bind and renderer stencil buffer to texture directly;1=Copy stencil buffer to texture directly");
#endif
#endif
    ImGui_RegisterCvar(NULL, NULL, IMGUI_CVAR_GROUP_RENDERER);

	ImGui_RegisterCvar(NULL, "Renderer", IMGUI_CVAR_GROUP_RENDERER);
#ifdef _D3BFG_CULLING
    ImGui_RegisterCvar("harm_r_occlusionCulling", "DOOM3-BFG style occlusion culling", IMGUI_CVAR_COMPONENT_CHECKBOX);
#endif
    ImGui_RegisterCvar("r_maxFps", "Max FPS", IMGUI_CVAR_COMPONENT_INT_SLIDER);
    ImGui_RegisterCvar("r_renderMode", "Retro postprocess", IMGUI_CVAR_COMPONENT_COMBO, "0=Doom;1=CGA;2=CGA Highres;3=Commodore 64;4=Commodore 64 Highres;5=Amstrad CPC 6128;6=Amstrad CPC 6128 Highres;7=Sega Genesis;8=Sega Genesis Highres;9=Sony PSX");
#ifdef _MULTITHREAD
    ...
    ImGui_RegisterCvar(NULL, "Image", IMGUI_CVAR_GROUP_RENDERER);
    ...
    ImGui_RegisterCvar(NULL, "Shader", IMGUI_CVAR_GROUP_RENDERER);
    ImGui_RegisterCvar("harm_r_useHighPrecision", "Use high precision float on GLSL shader", IMGUI_CVAR_COMPONENT_CHECKBOX);
...
#endif
    ImGui_RegisterCvar("reloadGLSLprograms", "Reload loaded GLSL shaders", IMGUI_CVAR_COMPONENT_BUTTON | IMGUI_CVAR_GROUP_RENDERER);
    ImGui_RegisterCvar("exportGLSLShaderSource", "Export built-in GLSL shaders", IMGUI_CVAR_COMPONENT_BUTTON | IMGUI_CVAR_GROUP_RENDERER);
    ImGui_RegisterCvar("printGLSLShaderSource", "Print built-in GLSL shaders", IMGUI_CVAR_COMPONENT_BUTTON | IMGUI_CVAR_GROUP_RENDERER);
    ImGui_RegisterCvar("exportDevShaderSource", "Export built-in GLSL shaders for developer", IMGUI_CVAR_COMPONENT_BUTTON | IMGUI_CVAR_GROUP_RENDERER);
    ImGui_RegisterCvar(NULL, NULL, IMGUI_CVAR_GROUP_RENDERER);
    ...
    ImGui_RegisterCvar(NULL, "Other", IMGUI_CVAR_GROUP_RENDERER);
    ImGui_RegisterCvar("r_screenshotFormat", "Screenshot image format", IMGUI_CVAR_COMPONENT_COMBO, "0=TGA;1=BMP;2=PNG;3=JPG;4=DDS");
    ImGui_RegisterCvar("r_screenshotJpgQuality", "Screenshot quality for JPG", IMGUI_CVAR_COMPONENT_INT_SLIDER);
    ImGui_RegisterCvar("r_screenshotPngCompression", "Screenshot compression level for PNG", IMGUI_CVAR_COMPONENT_INT_SLIDER);
    ImGui_RegisterCvar("harm_r_debugOpenGL", "Enable OpenGL debug");
    ImGui_RegisterCvar("harm_r_autoAspectRatio", "Setup view aspect ratio automatic", IMGUI_CVAR_COMPONENT_COMBO, "0=Manual;1=Setup r_aspectRatio to -1;2=Setup r_aspectRatio to 0,1,2 by screen size");
    ImGui_RegisterCvar("glConfig", "Print OpenGL config", IMGUI_CVAR_COMPONENT_BUTTON | IMGUI_CVAR_GROUP_RENDERER);
    ImGui_RegisterCvar(NULL, NULL, IMGUI_CVAR_GROUP_RENDERER);

    // GUI
    ImGui_RegisterCvar(NULL, "Generic", IMGUI_CVAR_GROUP_GUI);
    ImGui_RegisterCvar("r_scaleMenusTo43", "4:3 aspect ratio GUI", IMGUI_CVAR_GROUP_GUI | IMGUI_CVAR_COMPONENT_COMBO, "0=Disable;1=Only main menu;2=All");
    ImGui_RegisterCvar(NULL, NULL, IMGUI_CVAR_GROUP_GUI);

    ImGui_RegisterCvar(NULL, "Font", IMGUI_CVAR_GROUP_GUI);
...
#ifdef _RAVEN
    ImGui_RegisterCvar("harm_gui_defaultFont", "Default GUI font", IMGUI_CVAR_COMPONENT_COMBO, "chain;lowpixel;marine;profont;r_strogg;strogg");
#endif
#ifdef _HUMANHEAD
    ImGui_RegisterCvar("harm_ui_translateAlienFont", "Translate alien font", IMGUI_CVAR_COMPONENT_COMBO, "fonts;fonts/menu;");
	...
	// game
    ImGui_RegisterCvar(NULL, "Generic", IMGUI_CVAR_GROUP_GAME);
    ImGui_RegisterCvar("harm_g_skipHitEffect", "Skip hit effect in game when starting engine");
    ImGui_RegisterCvar("skipHitEffect", "Skip hit effect in game", IMGUI_CVAR_COMPONENT_BUTTON | IMGUI_CVAR_GROUP_GAME);
#ifdef _RAVEN
    ...
    ImGui_RegisterCvar(NULL, NULL, IMGUI_CVAR_GROUP_GAME);

    ImGui_RegisterCvar(NULL, "Full-body awareness", IMGUI_CVAR_GROUP_GAME);
    ImGui_RegisterCvar("harm_pm_fullBodyAwareness", "Enable full-body awareness");
    ImGui_RegisterCvar("harm_pm_fullBodyAwarenessOffset", "View offset in full-body awareness", IMGUI_CVAR_COMPONENT_INPUT_VECTOR);
    ...
    ImGui_RegisterCvar(NULL, "Bot", IMGUI_CVAR_GROUP_GAME);
    ...
    ImGui_RegisterCvar(NULL, "Flashlight", IMGUI_CVAR_GROUP_GAME);
    ...
    ImGui_RegisterCvar("harm_ui_viewLightRadius", "Player flashlight radius", IMGUI_CVAR_COMPONENT_INPUT_VECTOR);
    ImGui_RegisterCvar("harm_ui_viewLightOffset", "Player flashlight offset", IMGUI_CVAR_COMPONENT_INPUT_VECTOR);
    ImGui_RegisterCvar("harm_ui_viewLightType", "Player flashlight type", IMGUI_CVAR_COMPONENT_COMBO, "0=Spot light;1=Point light");
    ImGui_RegisterCvar(NULL, NULL, IMGUI_CVAR_GROUP_GAME);
    ...
    ImGui_RegisterCvar(NULL, "Rivensin Mod", IMGUI_CVAR_GROUP_GAME);
	...
	// other
    ImGui_RegisterCvar("imgui_scale", "ImGui scale", IMGUI_CVAR_GROUP_OTHER);
    ImGui_RegisterCvar("imgui_fontScale", "ImGui font scale", IMGUI_CVAR_GROUP_OTHER);
