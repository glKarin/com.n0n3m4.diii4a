...
bool idModelIqm::ToMd5Mesh(idMd5MeshFile &md5mesh, float scale, bool addOrigin) const
{
    ...
    md5mesh.Commandline() = "Convert from iqm file";
	if(addOrigin)
		numBones++;
    ...
    for(j = 0; j < meshes.Num(); j++)
    {
        ...
        for(unsigned int u = 0; u < iqmMesh->num_triangles; u++)
        {
            ...
            for(int k = 0; k < 3; k++)
            {
                ...
                if(w < 1.0f)
                {
                    common->Warning("Vertex '%d' weight sum is less than 1.0", vertexIndex);
                }
            }
            md5meshTri_t md5Tri;
...
bool idModelIqm::ToMd5Anim(md5model::idMd5AnimFile &md5anim, md5model::idMd5MeshFile &md5mesh, const char *animName, float scale) const
{
	for(int i = 0; i < anims.Num(); i++)
	{
		...
		{
			return ToMd5Anim(md5anim, md5mesh, i, scale);
		}
	}
...
bool idModelIqm::ToMd5Anim(md5model::idMd5AnimFile &md5anim, md5model::idMd5MeshFile &md5mesh, int animIndex, float scale) const
{
    ...
    md5anim.Commandline() = "Convert from iqm file";
...
static int R_ConvertIqmToMd5(const char *iqmMesh, bool doMesh = true, const idStrList *animList = NULL, float scale = -1.0f, bool addOrigin = false)
{
    ...
    if(iqm.Parse(iqmMesh, animList ? idModelIqm::PARSE_FRAME : idModelIqm::PARSE_JOINT))
    {
		...
		if(iqm.ToMd5Mesh(md5MeshFile, scale, addOrigin))
		{
			if(doMesh)
			{
				md5MeshFile.Commandline().Append(va(" '%s': scale=%f, addOrigin=%d", iqmMesh, scale > 0.0f ? scale : 1.0, addOrigin));
    ...
    for(int i = 0; i < list.Num(); i++)
    {
		...
		if(isNumber)
		{
			...
			ok = iqm.ToMd5Anim(md5AnimFile, md5MeshFile, index, scale);
            md5animPath = iqmMesh;
		...
		else
		{
			...
			ok = iqm.ToMd5Anim(md5AnimFile, md5MeshFile, name.c_str(), scale);
			animName = anim;
            md5animPath = anim;
            md5animPath.SetFileExtension(".md5anim");
		}
        if(ok)
        {
            md5AnimFile.Commandline().Append(va(" '%s': scale=%f, addOrigin=%d", animName, scale > 0.0f ? scale : 1.0, addOrigin));
