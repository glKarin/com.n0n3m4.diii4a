...
int NormalDistanceDXT1( const int* vector, const int* normalized )
{
	...
	intNormal[0] = Ftob( ( floatNormal[0] + 1.0f ) * ( 255.0f / 2.0f ) + 0.5f );
	intNormal[1] = Ftob( ( floatNormal[1] + 1.0f ) * ( 255.0f / 2.0f ) + 0.5f );
	intNormal[2] = Ftob( ( floatNormal[2] + 1.0f ) * ( 255.0f / 2.0f ) + 0.5f );
...
int NormalDistanceDXT5( const int* vector, const int* normalized )
{
	...
	floatNormal[1] *= rcplen;
	floatNormal[2] *= rcplen;
	intNormal[c0] = Ftob( ( floatNormal[0] + 1.0f ) / 2.0f * 255.0f + 0.5f );
	intNormal[c1] = Ftob( ( floatNormal[1] + 1.0f ) / 2.0f * 255.0f + 0.5f );
	intNormal[c2] = Ftob( ( floatNormal[2] + 1.0f ) / 2.0f * 255.0f + 0.5f );
...
void idDxtEncoder::RotateNormalsDXT1( byte* block ) const
{
	...
	for( int i = 0; i < 32; i += 1 )
	{
		...
		for( int j = 0; j < 16; j++ )
		{
			...
			float rx = c * x - s * y;
			float ry = s * x + c * y;
			rotatedBlock[j * 4 + 0] = Ftob( ( rx + 1.0f ) / 2.0f * 255.0f );
			rotatedBlock[j * 4 + 1] = Ftob( ( ry + 1.0f ) / 2.0f * 255.0f );
		}
	...
	for( int j = 0; j < 16; j++ )
	{
		...
		float rx = c * x - s * y;
		float ry = s * x + c * y;
		block[j * 4 + 0] = Ftob( ( rx + 1.0f ) / 2.0f * 255.0f );
		block[j * 4 + 1] = Ftob( ( ry + 1.0f ) / 2.0f * 255.0f );
...
void idDxtEncoder::RotateNormalsDXT5( byte* block ) const
{
	...
	for( int i = 0; i < 32; i += 1 )
	{
		...
		for( int j = 0; j < 16; j++ )
		{
			...
			float rx = c * x - s * y;
			float ry = s * x + c * y;
			rotatedBlock[j * 4 + 3] = Ftob( ( rx + 1.0f ) / 2.0f * 255.0f );
			rotatedBlock[j * 4 + 1] = Ftob( ( ry + 1.0f ) / 2.0f * 255.0f );
		}
	...
	for( int j = 0; j < 16; j++ )
	{
		...
		block[j * 4 + 1] = Ftob( ( ry + 1.0f ) / 2.0f * 255.0f );
		block[j * 4 + 3] = Ftob( ( rx + 1.0f ) / 2.0f * 255.0f );
