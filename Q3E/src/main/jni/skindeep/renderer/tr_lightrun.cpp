...
void R_SetLightProject( idPlane lightProject[4], const idVec3 origin, const idVec3 target,
					   const idVec3 rightVector, const idVec3 upVector, const idVec3 start, const idVec3 stop, const bool isParallel ) {
	float		dist;
	float		scale;
	float		rLen, uLen;
	idVec3		normal;
...
//normal = right.Cross( up );
	...
	{
		// BLENDO HACK:
		// Lets us treat a spotlight as a rectangular volume of parallel light all shining towards the base.
		// Used to fake sunlight for interiors.
		lightProject[0][0] = 0.5f / rLen;
		lightProject[1][1] = 0.5f / uLen;
...
void R_DeriveLightData( idRenderLightLocal *light ) {
	...
	if ( !light->parms.pointLight ) {
		...
		R_SetLightProject( light->lightProject, vec3_origin /* light->parms.origin */, light->parms.target,
			light->parms.right, light->parms.up, light->parms.start, light->parms.end, light->parms.parallel);
	} else {
		// point light
	...
	if ( light->parms.parallel ) {
		...
		{
			// BLENDO HACK:
			// To simulate indoor sunlight, we need support for a projected light with an infinitely far away global light origin.
			// Spotlights don't have a lightCenter, so instead we use the vector between the 'true' origin and the target to determine the direction
			dir = -light->parms.target * light->parms.axis;
		}
		else
		{
