...
bool idRenderWorldLocal::InitFromMap( const char *name ) {
	...
	if ( name == mapName ) {
		if ( currentTimeStamp != FILE_NOT_FOUND_TIMESTAMP && currentTimeStamp == mapTimeStamp ) {
			common->DPrintf( "idRenderWorldLocal::InitFromMap: retaining existing map\n" );
			FreeDefs();
			TouchWorldModels();
...
void idRenderWorldLocal::AddWorldModelEntities() {
	...
	for ( i = 0 ; i < numPortalAreas ; i++ ) {
		...
		for ( int j = 0; j < hModel->NumSurfaces(); j++ ) {
			...
			// SM: Enable the portal sky if the portal sky flag is set, rather than a hardcoded name
