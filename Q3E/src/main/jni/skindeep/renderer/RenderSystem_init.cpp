...
#ifdef _DIII4A //karin: 8 is too large on Android
idCVar r_multiSamples( "r_multiSamples", "0", CVAR_RENDERER | CVAR_ARCHIVE | CVAR_INTEGER, "number of antialiasing samples. 0 = off. Options: 2,4,8", 0, 8 );//BC anti-aliasing
#else
...
static void R_CheckPortableExtensions( void ) {
#ifdef _GLES //karin: using OpenGLES 3.2
	...
	// GL_ARB_multitexture
#ifdef _GLES //karin: support programming-pipeline multitexture on GLES
...
#ifdef _GLES //karin: support cubemap on GLES
...
#ifdef _GLES //karin: support npot on GLES
	if(!glConfig.textureNonPowerOfTwoAvailable) glConfig.textureNonPowerOfTwoAvailable = R_CheckExtension( "GL_OES_texture_npot" );
#endif
...
#ifdef _GLES //karin: check extension on GLES
	if ( R_CheckExtension( "GL_EXT_texture_lod" ) ) 
#else
...
#ifdef _GLES //karin: image3D support on GLES
	if(!glConfig.texture3DAvailable) glConfig.texture3DAvailable = R_CheckExtension( "GL_OES_texture_3D" );
#endif
...
#ifdef _GLES //karin: support stencil index wrap on GLES
	if(true)
#else
	...
	// ARB_vertex_buffer_object
#ifdef _GLES //karin: support VBO on GLES
...
#ifdef _GLES //karin: cubemap and multitexture support on GLES
...
#ifdef _GLES //karin: support float texture on GLES
	if (!(R_CheckExtension("GL_OES_texture_float") || R_CheckExtension("GL_ARB_texture_float")) || !glConfig.GLSLAvailable)
#else
...
void R_ReadTiledPixels( int width, int height, byte *buffer, renderView_t *ref = NULL ) {
...
#ifdef _GLES //karin: using GL_RGBA with glReadPixels
	...
	for ( int xo = 0 ; xo < width ; xo += oldWidth ) {
		for ( int yo = 0 ; yo < height ; yo += oldHeight ) {
			...
			int	row = ( w * 4 );		// OpenGL pads to dword boundaries
