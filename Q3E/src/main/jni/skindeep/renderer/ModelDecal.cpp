...
=================
*/
void idRenderModelDecal::AddWinding( const idWinding &w, const idMaterial *decalMaterial, const idPlane fadePlanes[2], float fadeDepth, int startTime, int seekDepth) {
	int i;
	...
	if ( !nextDecal ) {
		...
		nextDecal->previousDecal = this;
	}

	// How long have been searching for a decal for this winding?
	// If we exceed the max decal count per model, don't create it (we'll just have to wait for some old ones to fade)
	seekDepth++;
	if (seekDepth < MAX_DECALS_PER_MODEL)
	{
		// let the next decal on the chain take a look
		nextDecal->AddWinding(w, decalMaterial, fadePlanes, fadeDepth, startTime, seekDepth);
	}
	// Else fail silently (don't want to spam the console)
}
...
/*
=================
idRenderModelDecal::DeleteAndJoinNeighbours
=================
*/
void idRenderModelDecal::DeleteAndJoinNeighbours( idRenderModelDecal* decal, idRenderModelDecal *&next, idRenderModelDecal *&previous)
{

	if (decal != NULL)
	{
		next = decal->nextDecal;
		previous = decal->previousDecal;
		Free(decal);
		if (next != NULL)
		{
			next->previousDecal = previous;
		}
		if (previous != NULL)
		{
			previous->nextDecal = next;
		}
	}
}
...
idRenderModelDecal *idRenderModelDecal::RemoveFadedDecals( idRenderModelDecal *decals, int time ) {
	...
	decalInfo_t	decalInfo;
	idRenderModelDecal *nextDecal = NULL;
	idRenderModelDecal *previousDecal = NULL;

	while (decals != NULL)
	{
		// free the decal if no material set
		if (decals->material == NULL) {
			DeleteAndJoinNeighbours(decals, nextDecal, previousDecal);
			decals = nextDecal;
			continue;
		}
		...
		previousDecal = decals;
		decals = decals->nextDecal;
	}

	// Seek back to the start of the chain, and then return that so that the model can hold onto it
	while (previousDecal != NULL)
	{
		decals = previousDecal;
		previousDecal = decals->previousDecal;
	}
