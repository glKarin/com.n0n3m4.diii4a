...
private:
	...
	static void				ListModelsSimple_f(const idCmdArgs& args);
...
void idRenderModelManagerLocal::ListModelsSimple_f(const idCmdArgs& args) {
	int inUse = 0;
...
void idRenderModelManagerLocal::Init() {
	...
	cmdSystem->AddCommand("listModelsSimple", ListModelsSimple_f, CMD_FL_RENDERER, "lists all models");
...
void idRenderModelManagerLocal::EndLevelLoad() {
	common->DPrintf( "----- idRenderModelManagerLocal::EndLevelLoad -----\n" );
...
//			common->Printf( "keeping %s\n", model->Name() );
	...
	common->DPrintf( "%5i models purged from previous level, ", purgeCount );
	common->DPrintf( "%5i models kept.\n", keepCount );
	if ( loadCount ) {
		common->DPrintf( "%5i new models loaded in %5.1f seconds\n", loadCount, (end-start) * 0.001 );
	}
}
