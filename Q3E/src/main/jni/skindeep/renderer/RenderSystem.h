...
typedef struct glconfig_s {
	...
	const char			*renderer_short_string; // blendo eric: for display in options
	...
	int					displayWidth; // blendo eric: from SDL_GetCurrentDisplayMode
	int					displayHeight; // blendo eric: from SDL_GetCurrentDisplayMode
	...
	bool				khrDebugAvailable; // SM: To help with debugging frame captures
	bool				GLSLAvailable;
...
// SM: Removed this because we're using the BFG version now
// font support
...
const int MICROCHAR_WIDTH = 5; //BC
const int MICROCHAR_HEIGHT = 8; //BC
...
public:
	...
	virtual void			DrawMicroStringExt(float x, float y, const char *string, const idVec4 &setColor, const idMaterial *material) = 0;
	...
	virtual void			DrawSmallChar( float x, float y, int ch, const idMaterial *material ) = 0;
	virtual void			DrawSmallStringExt( float x, float y, const char *string, const idVec4 &setColor, bool forceColor, const idMaterial *material ) = 0;
	...
	virtual void			EndFrame( double *frontEndMsec, double *backEndMsec ) = 0;
	...
	virtual void			PrepareForPostProcessPass() = 0;
	...
	virtual int				GetImageBytes(idImage* img, byte** bytes) = 0;
	...
	virtual bool			PrimaryViewExists() = 0;

	// SM: Function to tell us whether to use Blendo ambience or not
...
void R_ScreenshotFilename( int &lastNumber, const char *base, idStr &fileName, const char* extensionType = "tga");
