...
idCVar idVertexCache::r_vertexBufferMegs( "r_vertexBufferMegs", "64" /*BC was 32*/, CVAR_INTEGER|CVAR_RENDERER, "" );
#ifdef _GLES //karin: clear VBO on GLES
enum {
    VBO_CLEAR_NONE = 0,
    VBO_CLEAR_ZERO = 1,
    VBO_CLEAR_DELETE = 2,
...
void idVertexCache::ActuallyFree( vertCache_t *block ) {
	...
	if ( block->tag != TAG_TEMP ) {
		...
		if ( block->vbo ) {
#ifdef _GLES //karin: clear VBO on GLES
			if(harm_r_clearVertexBuffer.GetInteger() == 2)
			{
				qglDeleteBuffers(1, &block->vbo);
				qglGenBuffers( 1, & block->vbo );
			}
			else if(harm_r_clearVertexBuffer.GetInteger() == 1)
			{
...
vertCache_t	*idVertexCache::AllocFrameTemp( void *data, int size ) {
	...
	if ( block->vbo ) {
		qglBindBuffer( GL_ARRAY_BUFFER, block->vbo );
		qglBufferSubData( GL_ARRAY_BUFFER, block->offset, (GLsizeiptrARB)size, data );
	} else {
