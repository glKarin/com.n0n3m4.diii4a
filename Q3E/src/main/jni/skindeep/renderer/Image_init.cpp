...
idCVar idImageManager::image_anisotropy( "image_anisotropy", "8", CVAR_RENDERER | CVAR_ARCHIVE, "set the maximum texture anisotropy if available. Options: 1,2,4,8" ); //BC how good textures look far away.
...
idCVar idImageManager::image_useCompression( "image_useCompression", "0", CVAR_RENDERER | CVAR_ARCHIVE | CVAR_BOOL, "0 = force everything to high quality" ); //BC default to zero. Was 1.
...
idCVar idImageManager::image_useNormalCompression( "image_useNormalCompression", "0", CVAR_RENDERER | CVAR_ARCHIVE | CVAR_INTEGER, "2 = use rxgb compression for normal maps, 1 = use 256 color compression for normal maps if available" ); //BC default to zero. Was 2.
...
static void R_DepthImage( idImage *image ) {
	...
	image->GenerateImage( ( byte * )data, DEFAULT_SIZE, DEFAULT_SIZE,
		TF_NEAREST, false, TR_CLAMP, TD_DEPTH );
}
...
void R_ReloadImages_f( const idCmdArgs &args ) {
	...
	// Force a frame capture to resize the _currentRender correctly
...
static void R_FBOImage(idImage* image)
{
	image->type = TT_2D;
}

idImage* idImageManager::ImageFromFBO(const char* _name, int internalFormat)
{
	idStr name;
	idImage* image;
	int	hash;
	...
	image = AllocImage(name);
	image->generatorFunction = R_FBOImage;
...
void idImageManager::Init() {
	...
	currentDepthImage = ImageFromFBO( "_currentDepth", GL_DEPTH24_STENCIL8 );
	currentPostProcessImage = ImageFromFBO( "_currentPostProcess", GL_RGBA );
	currentCustomImage = ImageFromFBO("_currentCustom", GL_R8UI );
	currentLightImage = ImageFromFBO("_currentLight", GL_RGB );
...
void idImageManager::EndLevelLoad() {
	...
	common->DPrintf( "----- idImageManager::EndLevelLoad -----\n" );
...
//			common->Printf( "Loading %s\n", image->imgName.c_str() );
	...
	common->DPrintf( "%5i purged from previous\n", purgeCount );
	common->DPrintf( "%5i kept from previous\n", keepCount );
	common->DPrintf( "%5i new loaded\n", loadCount );
	common->DPrintf( "all images loaded in %5.1f seconds\n", (end-start) * 0.001 );
}
