...
===============
*/
void R_SetupProjection(viewDef_t * viewDef ) { //r_lock dhewm3
...
=================
*/
void R_SetupViewFrustum(viewDef_t* viewDef) { //r_lock dhewm3
	int		i;
	float	xs, xc;
	float	ang;
...
/*
=================
R_AddGlobalViewEntities

	SW: Sometimes we want to override all the rules of visibility and draw an object in the world -- regardless of whether the normal rules of culling would cause it to be discarded.
	A common use-case for this is in-world UI elements like the LKP (Last Known Position) marker, which we want to keep rendering even when the player isn't in the same room.

	Normally, entities are considered for drawing by flooding through the visible portals and finding what's visible in the rendered areas.
	This method explicitly inserts entities to be drawn, independent of the flooding process -- and ensures they aren't culled via other means (i.e. scissors)

	NOTES:
	- This does NOT force entities to be drawn on top of everything else. For that, make sure that renderentity.weaponDepthHack is true
	- This currently only works on ambient draw calls (unlit surfaces, glowing stuff, etc). Lighting interactions are a different kettle of fish, and we don't need them right now
=================
*/
static void R_AddGlobalViewEntities(idRenderWorldLocal* renderWorld)
{
...
void R_RenderView( viewDef_t *parms ) {
	...
	R_SetupViewFrustum(tr.viewDef); //r_lock dhewm3

	// we need to set the projection matrix before doing
	// portal-to-screen scissor box calculations
	R_SetupProjection(tr.viewDef); //r_lock dhewm3

	// identify all the visible portalAreas, and the entityDefs and
	// lightDefs that are in them and pass culling.
	static_cast<idRenderWorldLocal *>(parms->renderWorld)->FindViewLightsAndEntities();

	// Add entities that need to be drawn globally (e.g. LKP marker)
	R_AddGlobalViewEntities(parms->renderWorld);
