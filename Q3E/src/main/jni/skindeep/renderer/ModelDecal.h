...
public:
	...
	void						AddDecalDrawSurf( struct viewEntity_s *space );
								...
								// Returns the next/previous decals in the chain.
	idRenderModelDecal *		Next( void ) const { return nextDecal; }
	idRenderModelDecal*			Previous(void) const { return previousDecal; }
...
private:
	...
	static const int			MAX_DECALS_PER_MODEL = 256; // Picked arbitrarily, should be fine to change (be wary of stack overflows though!)
	...
	idRenderModelDecal *		previousDecal;

								// Adds the winding triangles to the appropriate decal in the
								// chain, creating a new one if necessary.
	void						AddWinding( const idWinding &w, const idMaterial *decalMaterial, const idPlane fadePlanes[2], float fadeDepth, int startTime, int seekDepth = 0);
	...
	void						AddDepthFadedWinding( const idWinding &w, const idMaterial *decalMaterial, const idPlane fadePlanes[2], float fadeDepth, int startTime);
								...
								// Removes a decal from the middle of the chain and joins its previous/next neighbours.
								// Pointers to the neighbours are returned via the parameters
	static void					DeleteAndJoinNeighbours(idRenderModelDecal* decal, idRenderModelDecal *&next, idRenderModelDecal *&previous);
};
