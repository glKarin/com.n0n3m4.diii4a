...
#define CUSTOMFONTSHADER // blendo eric: gather verts created on gui draw into surfaces
#ifdef CUSTOMFONTSHADER
	// blendo eric: similar to R_CreateAmbientCache, but frame alloc
	// the verts have already been produced by this point, but not tangents
	if (surf->material->TestMaterialFlag(MF_FONT)) {
		if (!tri->facePlanes) {
			tri->facePlanes = (idPlane*)R_FrameAlloc(sizeof(idPlane) * (tri->numIndexes / 3));
		}
...
void idGuiModel::DrawStretchPicRotated(float x, float y, float w, float h, float s1, float t1, float s2, float t2, const idMaterial *shader, float angle)
{
	idDrawVert verts[4];
	glIndex_t indexes[6];
	indexes[0] = 3;
