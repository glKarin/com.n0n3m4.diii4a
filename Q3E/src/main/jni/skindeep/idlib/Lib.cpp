...
// SM: Added to fix crash if asserts are hit after unhandled exceptions
bool disableAssertPrintf = false;
...
//BC
idVec4	colorSonar =  idVec4(.42f, .737f, .816f, 1.00f);
...
void AssertFailed( const char *file, int line, const char *expression ) {
	if ( !disableAssertPrintf ) // SM: Added to fix crash if asserts are hit after unhandled exceptions
	{
		common->Warning( "ASSERTION FAILED!\n%s(%d): '%s'\n", file, line, expression );
	}
#ifdef _MSC_VER
...
// blendo eric: debug break
void ForceDebuggerBreak()
{
#ifdef _MSC_VER
	__debugbreak();
#elif defined(__unix__)
