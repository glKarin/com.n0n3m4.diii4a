...
void idTraceModel::SetupCylinder( const idBounds &cylBounds, const int numSides ) {
	...
	for ( i = 0; i < n; i++ ) {
		...
		// blendo eric: fixed this bounds code from original
		polys[i].bounds.AddPoint(verts[i]);
		polys[i].bounds.AddPoint(verts[(n + i) % numVerts]);
		polys[i].bounds.AddPoint(verts[(i+1) % numVerts]);
		polys[i].bounds.AddPoint(verts[(n + i+1) % numVerts]);
	}
