...
typedef enum {
	...
	LEXFL_NOCOMMENTSAREWHITESPACE		= BIT(14)	// SM: If true, don't treat comments as whitespace and instead create a token
} lexerFlags_t;
...
public:
	...
	void			SetOffset( int start );  // blendo eric: set reader offset
...
private:
	...
	int				ReadComment( idToken* token ); // SM: Added for optional comment tokens
...
ID_INLINE void idLexer::SetOffset( int start ) { // blendo eric
	idLexer::lastScript_p = idLexer::script_p;
	idLexer::script_p = start + idLexer::buffer;
}
