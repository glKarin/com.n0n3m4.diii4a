...
bool idMatX::SVD_Factor( idVecX &w, idMatX &V ) {
	...
	for ( k = numColumns - 1; k >= 0; k-- ) {
		for ( its = 1; its <= 30; its++ ) {
			...
			for ( l = k; l >= 0; l-- ) {
				nm = l - 1;
				if ( ( idMath::Fabs( rv1[l] ) + anorm ) == anorm /* idMath::Fabs( rv1[l] ) < FLT_EPSILON */ ) {
					flag = 0;
					break;
				}
				if ( ( idMath::Fabs( w[nm] ) + anorm ) == anorm /* idMath::Fabs( w[nm] ) < FLT_EPSILON */ ) {
					break;
				}
			}
			if ( flag ) {
				...
				for ( i = l; i <= k; i++ ) {
					...
					if ( ( idMath::Fabs( f ) + anorm ) != anorm /* idMath::Fabs( f ) > FLT_EPSILON */ ) {
						g = w[i];
						h = Pythag( f, g );
						w[i] = h;
						h = 1.0f / h;
