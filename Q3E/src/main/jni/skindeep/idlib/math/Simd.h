...
//#define __BLENDO_SIMD__
#endif
#if !defined(__BLENDO_SIMD_INLINE__)
//#define __BLENDO_SIMD_INLINE__
#endif
...
// blendo eric:
BLENDO SIMD changes
	All normal simd is off by default, since VS inline x86 assembly is not working.

	Add preprocessor macro defines:
	__BLENDO_SIMD__
		Adds some converted simd to modern VS, including some RB doom ones like joint transforms
	__BLENDO_SIMD_INLINE__ 
		Changes the old virtual simd classes (Simd.h idSIMDProcessor) to be a single blendo simd class
		(see Simd_Blendo.h idSIMDProcessor).
		Assumes SIMD exists on modern processors, removing SIMD ver specific virtual calls,
		allowing inline calls on tight loops like interaction geo processing.
...
public:
	...
	static void			GetInfo(const class idCmdArgs &args);
private:
	static idStr SimdInfo;
};
...
// blendo eric: allow swap to blendo inline simd processor here
#if defined(__BLENDO_SIMD_INLINE__)
