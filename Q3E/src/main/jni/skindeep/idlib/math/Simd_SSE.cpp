...
// blendo eric: intitialize required to remove warning, but not necessary to work (vs might produce init code anyways, so speed might be negligible)
	xmm1 = _mm_setzero_ps();
	xmm2 = _mm_setzero_ps();
	xmm3 = _mm_setzero_ps();
	xmm4 = _mm_setzero_ps();
...
#if defined(__BLENDO_SIMD_AVX__) // blendo eric: added simd instr, portions inc from rbdoom --- START ---

#define SwapValues( a, b ) auto c = a; a = b; b = c;
#ifndef _mm_madd_ps
...
#endif // blendo eric: added rbdoom simd --- END ---
