...
//#include "idlib/math/Random.h" // SM: Moved to .cpp
...
public:
	...
	float			LengthFast( void ) const;
	float			Normalize( void );				// returns length
	idVec3			Normalized(void);				// returns new vector
	float			NormalizeFast( void );			// returns length
...
ID_INLINE idVec3 idVec3::Normalized(void) {
	float sqrLength, invLength;
...
public:
	...
	float			w;
					...
					explicit idVec4( const idVec3& vec3, const float w = 0.0f );
...
ID_INLINE idVec4::idVec4(const idVec3& vec3, const float w) {
	this->x = vec3.x;
	this->y = vec3.y;
	this->z = vec3.z;
	this->w = w;
