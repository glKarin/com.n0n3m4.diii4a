...
public:
	...
	static float				LerpShort(float value1, float value2, float amount);
	static float				CubicEaseOut(float lerpValue);
	static float				CubicEaseIn(float lerpValue);
	static float				CubicEaseInOut(float lerpValue);
	static float				PopLerp(float minSize, float maxSize, float normalSize, float transition);
    static bool                 DistanceBoxCheck(float boxPositionX, float boxPositionY, float boxPositionZ, int boxSize, float targetPositionX, float targetPositionY, float targetPositionZ);
...
//BC a nice lerp that does a little overshoot and settle.
ID_INLINE float idMath::PopLerp(float minSize, float maxSize, float normalSize, float transition)
{
	if (transition >= 1)
		return normalSize;
	...
	{
		transition /= popPoint;
		return idMath::Lerp(minSize, maxSize, transition);
	}

	transition -= popPoint;
	transition /= popRemnant;

	return idMath::Lerp(maxSize, normalSize, transition);
}



//BC a nice lerp that eases out. No ease-in.
ID_INLINE float idMath::CubicEaseOut(float lerpValue)
{
	float f = (lerpValue - 1);
	return f * f * f + 1;
}

ID_INLINE float idMath::CubicEaseIn(float lerpValue)
{
	return lerpValue * lerpValue;
}

ID_INLINE float idMath::CubicEaseInOut(float lerpValue)
{
	float value0 = 0.0f;
	float value1 = 1.0f;
	lerpValue = idMath::ClampFloat(0.0f, 1.0f, (lerpValue - value0) / (value1 - value0));
	return lerpValue * lerpValue * (3 - 2 * lerpValue);
}
...
//BC quick n dirty distance check; makes a box around a location and then sees if a point is inside it.
ID_INLINE bool idMath::DistanceBoxCheck(float boxPositionX, float boxPositionY, float boxPositionZ, int boxSize, float targetPositionX, float targetPositionY, float targetPositionZ)
{
	float boxRadius = boxSize / 2;
	return (targetPositionX > boxPositionX - boxRadius
		&& targetPositionX < boxPositionX + boxRadius
		&& targetPositionY > boxPositionY - boxRadius
		&& targetPositionY < boxPositionY + boxRadius
		&& targetPositionZ > boxPositionZ - boxRadius
		&& targetPositionZ < boxPositionZ + boxRadius);
