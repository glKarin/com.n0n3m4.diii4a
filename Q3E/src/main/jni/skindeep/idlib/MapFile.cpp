...
bool idMapEntity::NeedsReload(const idMapEntity *oldEntity) const {
	...
	if (m != n)
		return true; //amount of keys changed.
	...
	for (int i = 0; i < n; i++)
	{
		...
		return true; //a value changed.
	}

	//if (GetGeometryCRC() != oldEntity->GetGeometryCRC())
	//	return true;

	return false; //everything is same.
}
...
bool idMapFile::Write( const char *fileName, const char *ext, bool fromBasePath ) {
	...
	idLib::common->Printf( "writing %s. Please **RELOAD** map in Radiant.\n", qpath.c_str() );
...
//Hotreload from DARKMOD. For various reasons I had to make a much lesser version than darkmod's hot reload :(
idMapReloadInfo idMapFile::TryReload() {

	idMapReloadInfo res;
	idList<idStr> oldEntities, newEntities;
	
	//Save a snapshot of all the "old" entities.
	idList<idMapEntity *> oldMapEnts;
	for (int i = 0; i < entities.Num(); i++)
	{
		idMapEntity *ent = new idMapEntity();
		ent->epairs = entities[i]->epairs;
		oldMapEnts.AddUnique(ent);
	}
	

	auto PrepareListOfEntityNameAndIds = [](const idList<idMapEntity*> &entities, idList<idStr> &result) -> bool {
		for (int i = 0; i < entities.Num(); i++)
		{
			idMapEntity *ent = entities[i];
			if (const idKeyValue *kv = ent->epairs.FindKey("name"))
				result.AddUnique(kv->GetValue().c_str());
			else {
				if (idStr::Cmp(ent->epairs.GetString("classname", ""), "worldspawn") == 0)
				{
					//result.AddUnique("worldspawn");
				}
				else {
	...
	PrepareListOfEntityNameAndIds(this->entities, oldEntities);
	...
	{
		common->Warning("Load failed.\n");
		res.mapInvalid = true;
		return res;
	}

	PrepareListOfEntityNameAndIds(this->entities, newEntities);
	
	//Create lists for ModifiedEntities, AddedEntities, and RemovedEntities.

	//Create list for AddedEntities.
	for (int i = 0; i < newEntities.Num(); i++)
	{
		if (oldEntities.Find(newEntities[i]) == NULL)
		{
			//Is new entity.
			res.addedEntities.AddUnique(newEntities[i]);
		}
	}

	//Create list for RemovedEntities.
	for (int i = 0; i < oldEntities.Num(); i++)
	{
		if (newEntities.Find(oldEntities[i]) == NULL)
		{
			//Is a removed entity.
			//idStr entname = oldEntities[i];
			res.removedEntities.AddUnique(oldEntities[i]);
		}
	}

	//Create list for ModifiedEntities.
	for (int i = 0; i < newEntities.Num(); i++)
	{
		if (oldEntities.Find(newEntities[i]) == NULL)
			continue; //Cannot find it in old entity array. Skip it.

		//Ok, entity exists in both old and new entity arrays.
		//See if its values differ.

		const idMapEntity *newEnt = this->FindEntity(newEntities[i]);
		const idMapEntity *oldEnt = FindEntityByName(newEntities[i], oldMapEnts);
		
		if (newEnt == NULL || oldEnt == NULL)
			continue;

		if (newEnt->NeedsReload(oldEnt)) //TODO: FIX THIS!!!!!!!!!!!!!!
			res.modifiedEntities.AddUnique(newEntities[i]);
	}
...
idMapEntity * idMapFile::FindEntityByName(idStr entName, idList<idMapEntity *> entList)
{
	for (int i = 0; i < entList.Num(); i++)
	{
		if (idStr::Cmp(entName, entList[i]->epairs.GetString("name")) == 0)
		{
			return entList[i];
		}
	}
