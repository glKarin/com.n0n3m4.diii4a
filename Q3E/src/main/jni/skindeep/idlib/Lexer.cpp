...
int idLexer::ReadWhiteSpace( void ) {
	while(1) {
		...
		// skip comments
		// SM: If we are treating them as whitespace
		if (*idLexer::script_p == '/' && (idLexer::flags & LEXFL_NOCOMMENTSAREWHITESPACE) == 0 ) {
			// comments //
	...
	switch(*idLexer::script_p) {
		...
		default: //NOTE: decimal ASCII code, NOT octal
		{
			if (*idLexer::script_p < '0' || *idLexer::script_p > '9') {
				idLexer::Error("Unknown escape char. Check your backslashes.");
			}
			for (i = 0, val = 0; ; i++, idLexer::script_p++) {
				c = *idLexer::script_p;
				if (c >= '0' && c <= '9')
...
SM: Optional support for parsing comments as tokens
================
*/
int idLexer::ReadComment( idToken* token ) {
	// Skip initial /
	idLexer::script_p++;
...
int idLexer::ReadName( idToken *token ) {
	...
	do {
		...
		c = *idLexer::script_p;
				...
				(c >= '0' && c <= '9') ||
				c == '_' ||
				c < 0 || // SM: Also allow negative chars as names because of choices UTF-8 parsing
...
int idLexer::ReadToken( idToken *token ) {
	...
	// SM: Added optional support for parsing comments as tokens
	else if ( idLexer::flags & LEXFL_NOCOMMENTSAREWHITESPACE &&
			  c == '/' && ( *( idLexer::script_p + 1 ) == '/' || *( idLexer::script_p + 1 ) == '*' )) {
		if ( !idLexer::ReadComment( token ) ) {
			return 0;
		}
	}
	...
	// if there is a name
	// SM: Also allow negative characters here because choices use token parsing
