...
public:
	...
	void			Shuffle( int seed = (int)time( NULL ) ); // SM: Added a random shuffle of the list
...
template< class type >
void idList<type>::Shuffle( int seed /* = (int)time( NULL ) */ ) {
	idRandom random( seed );
	for ( int i = num - 1; i > 0; --i ) {
		idSwap( list[i], list[random.RandomInt() % ( i + 1 )] );
	}
}
