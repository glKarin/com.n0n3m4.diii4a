...
const float EFFECT_FADE_RATE		= 0.1f; // SW: How quickly to fade out old effects when switching to a new one
...
public:
	...
	// SM: track which sound world this channel is in
	idSoundWorldLocal*	soundWorld;
	bool				addedAutoDuck;

	// blendo eric: debug time info for sounds
	int lastGameLocalTimeStart;
	int estimatedTime;
};
...
public:
	...
	virtual int			StartSound( const idSoundShader *shader, const s_channelType channel, float diversity = 0, int shaderFlags = 0, bool allowSlow = true /* D3XP */, int gameTime = -1/*blendo eric: debug sound time info*/, idStr* outSampleName = nullptr);
	...
	// blendo eric: get [0,1] estimated intensity of complete sound
	virtual	float		OverallIntensity(s_channelType channel);
	...
	// SM: Make soundWorld protected and add a function to set it
	void				SetSoundWorld( idSoundWorldLocal * newSoundWorld );

	virtual bool		CurrentlyPlayingChannel( const s_channelType channel ) const;
protected:
...
public:
	...
	virtual void			FadeSoundClasses( const int soundClass, const float to, const float over, bool forAutoDuck = false );
	...
	virtual void			FadeOutMusic( float fadeToDb, float seconds );
	virtual void			SetMusicMultiplier( float multVar );
	...
	// SaveGame Support
	virtual void			WriteToSaveGame( idSaveGame *savefile );
	virtual void			ReadFromSaveGame( idRestoreGame *savefile );

	virtual void			ReadFromSaveGameSoundChannel( idRestoreGame *saveGame, idSoundChannel *ch );
	virtual void			ReadFromSaveGameSoundShaderParams( idRestoreGame *saveGame, soundShaderParms_t *params );
	virtual void			WriteToSaveGameSoundChannel( idSaveGame *saveGame, idSoundChannel *ch );
	virtual void			WriteToSaveGameSoundShaderParams( idSaveGame *saveGame, soundShaderParms_t *params );

	virtual int				EnterSlowmo(void);
	virtual void			SetSlowmoSpeed(float speed, int handle);
	virtual void			ExitSlowmo(int handle);
	virtual void			ClearSlowmoClients(void);
	virtual void			SetEnviroSuit( bool active );

	virtual void			SetOverrideEfx(bool active, idStr name);
	...
	void					MixLoop( int current44kHz, int numSpeakers, float *finalMixBuffer, bool muteInBackground );
	void					AVIUpdate( void );
	...
	// blendo eric: [0,1] sound intensity at position
	float					FindIntensityArtificial(idSoundEmitterLocal* sound, const idVec3* listenerPosition, const s_channelType channel);
	...
	bool					overrideEfx;
	idStr					overrideEfxName;

	ALuint					effectLerpSlot;		// SW: Holds the identifier for a secondary effect slot, akin to listenerSlot
	ALuint					lerpEffect;			// SW: Holds the secondary effect itself, akin to listenerEffect

	float					lerpBlendFactor;	// SW: Controls how much to blend our lerp effect into the main effect. Should be clamped 0.0 -- 1.0
	...
	// SM
	idSoundFade				autoDuckClassFade[SOUND_MAX_CLASSES]; // for autoducking fade
	...
	bool					slowmoActive;
	float					slowmoSpeed;
	idList<slowmoClient_t>	slowmoClients;
	int						nextSlowmoClientHandle;
	bool					enviroSuitActive;

	//BC
	virtual bool			ReverbCheck(const char * reverbName);

	// SM for autoducking
	void AddAutoDuck();
	void RemoveAutoDuck();

	float					multiplierMusic; // blendo eric: alters music volume
private:
	int						autoDuckCount;
};
...
public:
	...
	static idCVar			s_volumeCurve;
	static idCVar			s_volumeBoost;
	static idCVar			s_volumeAll;
	static idCVar			s_volumeEffects;
	static idCVar			s_volumeMusic;
	static idCVar			s_volumeVoice;
	...
	//BC
	static idCVar			s_debug;
	static idCVar			s_drawoffset;
	static idCVar			s_voDebug;

	// SW: 
	static idCVar			s_showReverbBlend;
	//static idCVar			s_enviroSuitLPFOcclusion;
	//static idCVar			s_enviroSuitLPFOcclusionRatio;

	static idCVar			s_enviroSuitHighpass;

	static idCVar			s_muteInBackground;

	static idCVar			s_debugSlowmo;

	//SM
	static idCVar			s_autoduck_db;
	static idCVar			s_autoduck_fadeTime;

	// blendo eric
	static idCVar			s_aiMuffleDoor;
	static idCVar			s_aiMuffleCorner;
