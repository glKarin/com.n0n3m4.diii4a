...
bool idSoundShader::ParseShader( idLexer &src ) {
	...
	parms.minDistance = 1;		//BC default sound mindistance = 1
	parms.maxDistance = 10;		//BC default sound maxdistance = 10
	parms.volume = 1;
	parms.shakes = 0;
	...
	while ( 1 ) {
		...
		else if (!token.Icmp("no_slow")) {
			parms.soundShaderFlags |= SSF_NO_SLOW;
		}
		// plain
		...
		// SM: autoduck
		else if ( !token.Icmp( "autoduck" ) ) {
			parms.soundShaderFlags |= SSF_AUTODUCK;
		}
		else if ( !token.Icmp( "synchronized" ) ) {
			parms.soundShaderFlags |= SSF_SYNCHRONIZED;
		}
