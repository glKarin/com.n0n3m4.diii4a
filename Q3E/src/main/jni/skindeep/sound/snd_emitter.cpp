...
void idSoundChannel::Start( void ) {
	...
	// SM: Auto ducking
	if ( parms.soundShaderFlags & SSF_AUTODUCK ) {
		soundWorld->AddAutoDuck();
		addedAutoDuck = true;
	}
}
...
// SM
void idSoundEmitterLocal::SetSoundWorld( idSoundWorldLocal * newSoundWorld ) {
	soundWorld = newSoundWorld;
	for ( int i = 0; i < SOUND_MAX_CHANNELS; i++ ) {
		channels[i].soundWorld = newSoundWorld;
	}
}

bool idSoundEmitterLocal::CurrentlyPlayingChannel( const s_channelType channel ) const {
	if ( playing ) {
		if ( channel != SCHANNEL_ANY ) {
			for ( int i = 0; i < SOUND_MAX_CHANNELS; i++ ) {
				if ( channels[i].triggerState && channels[i].triggerChannel == channel ) {
					return true;
				}
			}
		} else {
...
void idSoundEmitterLocal::Spatialize( idVec3 listenerPos, int listenerArea, idRenderWorld *rw ) {
	...
	if ( rw ) {
		...
		if ( soundInArea == -1 )
		{
			...
			{
				common->Warning("Sound: sound origin in wall? %.1f %.1f %.1f\n", origin.x, origin.y, origin.z);
			}
#endif
...
int idSoundEmitterLocal::StartSound(const idSoundShader *shader, const s_channelType channel, float diversity, int soundShaderFlags, bool allowSlow, int gameTime
	...
	else if (idSoundSystemLocal::s_showStartSound.GetInteger() == 1)
	{
		common->Printf("StartSound '%s'\n", shader->GetName());
	}
	...
	if ( channel != SCHANNEL_ANY ) {
		for( i = 0; i < SOUND_MAX_CHANNELS; i++ ) {
			...
			if ( chan->triggerState && chan->soundShader && chan->triggerChannel == channel ) {
				...
				// blendo eric: cutoff time
				if (chan->lastGameLocalTimeStart >= 0 && cvarSystem->GetCVarBool("s_debug"))
				{
					int timeElapsed = gameTime - chan->lastGameLocalTimeStart;
					int earlyCuttoffTime = timeElapsed - chan->estimatedTime;
					if (earlyCuttoffTime > 1)
					{
						common->Printf("Sound '%s' was cut off (%dms)\n", chan->soundShader->base->GetName(), earlyCuttoffTime);
					}
				}
	...
	if ( idSoundSystemLocal::s_skipHelltimeFX.GetBool() || chanParms.soundShaderFlags & SSF_NO_SLOW) {
	...
	chan->lastGameLocalTimeStart = gameTime;
	chan->estimatedTime = length;
...
/* blendo eric:
===================
idSoundEmitterLocal::OverallIntensity
