...
//			common->Printf( "Purging %s\n", sample->name.c_str() );
	...
	common->DPrintf( "%5ik referenced\n", useCount / 1024 );
	common->DPrintf( "%5ik purged\n", purgeCount / 1024 );
}
...
void idSoundSample::MakeDefault(void) {
	...
	if (alGetError() != AL_NO_ERROR) {
		openalBuffer = 0;
		common->Warning("idSoundCache: error generating OpenAL hardware buffer");
		hardwareBuffer = false;
	}
	else
	{
...
void idSoundSample::Load( void ) {
	...
	if ( objectInfo.wFormatTag == WAVE_FORMAT_TAG_PCM ) {
		...
		if (alGetError() != AL_NO_ERROR)
		{
			openalBuffer = 0;
			hardwareBuffer = false;
			common->Warning("idSoundCache: error generating OpenAL hardware buffer. NO SOUND PLAYBACK DEVICE FOUND."); //BC this is what gets called when all Windows audio playback devices are disabled.
		}
		else
		{
