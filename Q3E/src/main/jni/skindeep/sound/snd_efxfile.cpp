...
#define efxi(param, value)													\
	do {																	\
		...
		err = alGetError();													\
		if (err != AL_NO_ERROR)												\
			common->Error("alEffecti(" # param ", %d) "					\
							"failed: 0x%x", _v, err);						\
	} while (false)
...
#define efxf(param, value)													\
	do {																	\
		...
		err = alGetError();													\
		if (err != AL_NO_ERROR)												\
			common->Error("alEffectf(" # param ", %.3f) "					\
							"failed: 0x%x", _v, err);						\
	} while (false)
...
#define efxfv(param, value0, value1, value2)								\
	do {																	\
		...
		err = alGetError();													\
		if (err != AL_NO_ERROR)												\
			common->Error("alEffectfv(" # param ", %.3f, %.3f, %.3f) "	\
...
bool idEFXFile::LoadFile( const char *filename, bool OSPath ) {
	...
	while ( !src.EndOfFile() ) {
		...
		{
			//BC don't load efx with same name twice.
			bool alreadyExists = false;
			for (int i = 0; i < effects.Num(); i++)
			{
				//idStr z1 = effects[i]->name;
				//idStr z2 = effect->name;
				//int zz = idStr::Cmp(effects[i]->name.c_str(), effect->name.c_str());
				
				if (!idStr::Icmp(effects[i]->name.c_str(), effect->name.c_str()) )
				{
					alreadyExists = true;
					i = effects.Num() + 1; //break out of loop.
				}
			}

			if (alreadyExists) //entry already exists. don't load it again.
			{
				delete effect;
				continue;
			}
