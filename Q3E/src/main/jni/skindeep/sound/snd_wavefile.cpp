...
int idWaveFile::Open( const char* strFileName, waveformatex_t* pwfx ) {
	...
	if ( mhmmio->Length() <= 0 ) {
		common->Warning("Sound is empty? '%s'", strFileName);
		mhmmio = NULL;
		return -1;
	}
	if ( ReadMMIO() != 0 ) {
		// ReadMMIO will fail if its an not a wave file
		common->Error("Not valid wav file? '%s' (try re-exporting the wav file)", strFileName);
		Close();
		return -1;
	}
...
int idWaveFile::ReadMMIO( void ) {
	...
	{
		common->Warning("Error: wav file needs to be Signed 16-bit PCM.");
