...
static const int	SSF_NO_EFX =			BIT(10); // Skips EFX effects such as echo, reverb, etc. Should be good for UI stuff.
static const int	SSF_NO_SLOW =			BIT(11); // Does not slow down when the sound world is in slow-motion
static const int	SSF_AUTODUCK =			BIT(12); // Reduces volumes of all other non-autoduck sounds
static const int	SSF_SYNCHRONIZED =		BIT(13); // always update the sound shader even if inaudible (for cases where sounds are synchronized across multiple speakers)
...
// blendo eric: voice 2 and music channel
static const int SCHANNEL_TWO = 2;	// any following integer can be used as a channel number
static const int SCHANNEL_MUSIC = 12;	// any following integer can be used as a channel number
...
public:
	...
	// SM
	virtual bool			CurrentlyPlayingChannel( const s_channelType channel ) const = 0;
};
...
public:
	...
	// to is in Db (sigh), over is in seconds
	virtual void			FadeSoundClasses( const int soundClass, const float to, const float over, bool forAutoDuck = false ) = 0;

	// background music
	virtual	void			PlayShaderDirectly( const char *name, int channel = -1 ) = 0;
	virtual void			FadeOutMusic( float fadeToDb, float seconds ) = 0;
	virtual void			SetMusicMultiplier( float multVar ) = 0;
	...
	virtual void			ExitSlowmo(int handle) = 0;
	virtual void			ClearSlowmoClients(void) = 0;
	virtual void			SetEnviroSuit( bool active ) = 0;

	virtual void			SetOverrideEfx(bool active, idStr name) = 0;

	//BC
	virtual bool			ReverbCheck(const char * reverbName) = 0;
};
...
public:
	...
	//BC
	idStr					soundcardname; //for displaying soundcard info in the options menu
};
