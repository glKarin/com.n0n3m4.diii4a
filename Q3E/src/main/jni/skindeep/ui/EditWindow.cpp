...
// blendo eric
int	idEditWindow::ActualLineCount() {
	// drawtext records an extra break at first and last char, regardless if last char is newline
	// and there is always one more line than breaks
	if( text.Length() > 0) {
		int lineCount = breaks.Num();
		lineCount -= 2; // subtract 2 added breaks in the array
		if(text[text.Length()-1] == '\n') { // add one if it actually did end in a newline
			lineCount += 1;
		}
		lineCount += 1; // add 1, since there's 1 more line than breaks
		return lineCount;
	}
	return 0;
}
...
// blendo eric
float idEditWindow::TextHeight()
{
	float lineCount = ActualLineCount();
	float lineBreaks = Max(0.0f,lineCount-1.0f);
	
	// lines * char height
	float textHeight = lineCount * GetMaxCharHeight();
	//  + line breaks * line spacing
	textHeight = textHeight + (lineBreaks * 5.0f);
	return textHeight;
}
...
void idEditWindow::Draw( int time, float x, float y ) {
	...
	if ( wrap && !scroller->SliderHidden() ) {
		float lineHeight = GetMaxCharHeight( ) + 5;
		rect.y -= scroller->GetValue() * lineHeight;
		rect.w -= sizeBias;
		rect.h = TextHeight(); // blendo eric: use true text height
	}
...
const char *idEditWindow::HandleEvent(const sysEvent_t *event, bool *updateVisuals) {
	...
	if ( event->evType == SE_CHAR ) {
		...
		if ( key == K_DOWNARROW ) {
			if ( idKeyInput::IsDown( K_CTRL ) || readonly) {
...
{
	const char *thumbImage = "guis/assets/scrollbar_thumb";
	const char *barImage = "guis/assets/scrollbarv";
	const char *scrollerName = "_scrollerWinV";

	if (horizontal) {
		barImage = "guis/assets/scrollbarh";
...
void idEditWindow::EnsureCursorVisible()
{
	...
	if ( !wrap ) {
		...
		// blendo eric: fix for line count
		int fit = 0;

		float lineHeight = GetMaxCharHeight();
		float rectHeight = rect.h;
		if(rectHeight > lineHeight) // does one line fit
		{
			rectHeight = rectHeight - lineHeight; // first line doesn't have padding before
			fit = 1;
			fit += rectHeight / (5+GetMaxCharHeight()); // other lines have padding
		}

		// drawtext always records a break at first and last char
		// so actual lines is = array count - 2 + 1
		if ( fit < ActualLineCount() ) {
			scroller->SetRange(0, breaks.Num() + 1 - fit, 1);
			scroller->SetVisiblePercent( (float)fit / (ActualLineCount()+0.00001f) );
		} else {
