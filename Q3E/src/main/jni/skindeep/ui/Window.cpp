...
const idRegEntry idWindow::RegisterVars[] = {
	...
	//BC
	{ "textalign", idRegister::INT },
	{ "model", idRegister::STRING },		// allow updating of renderdef model
	{ "modelFov", idRegister::FLOAT },		// allow updating of renderdef fov

	// blendo eric
	{ "letterspacing", idRegister::FLOAT },
	{ "fontmaterial", idRegister::STRING },
	{ "cloneChildTemplate", idRegister::INT },

};

const int idWindow::NumRegisterVars = sizeof(RegisterVars) / sizeof(idRegEntry);

//BC note: this list refers to the enum in the Window.h file.  i.e. onMouseEnter = ON_MOUSEENTER
const char *idWindow::ScriptNames[] = {
	"onMouseEnter",		//When mouse enters gui element.
	"onMouseExit",		//When mouse leaves gui element.
	"onAction",			//When LMB
	"onActivate",		//When gui is opened/started
	"onDeactivate",		//When gui is closed
	"onESC",			//When press ESC
	"onEvent",
	"onTrigger",
	"onActionRelease",	//When LMB is released (I think? need to check this)
...
void idWindow::CommonInit() {
	...
	lastTimeRun = 0;
	origin.Zero();
	font = renderSystem->RegisterFont( "" ); // SM: From BFG
	timeLine = -1;
	xOffset = yOffset = 0.0;
	cursor = 0;
...
void idWindow::SetFont() {
	// SM: From BFG
	// blendo eric: supply font mat
	dc->SetFont( font, fontMaterial );
}
...
void idWindow::Draw( int time, float x, float y ) {
	...
	if ( textShadow ) {
		...
		shadowRect.x += .6f; //BC was 1. NOTE: also change the 'shadowRect.x +=' line in simplewindow.cpp
		shadowRect.y += .6f; //BC was 1
		dc->DrawText( shadowText, textScale, textAlign, textShadow == 1 ? colorBlack : idVec4(0, 0, 0, .5f) /*bc light shadow*/, shadowRect, !( flags & WIN_NOWRAP ), -1, false, 0, NULL, letterSpacing /* blendo eric: letter spacing added */);
	}
	dc->DrawText( text, textScale, textAlign, foreColor, textRect, !( flags & WIN_NOWRAP ), -1, false, NULL, 0, letterSpacing /* blendo eric: letter spacing added */);
...
/* // blendo eric alt hover state
================
idWindow::OverClientRect
================
*/
bool idWindow::OverClientRect() {
	if (Contains(clientRect, gui->CursorX(), gui->CursorY()) && !noEvents && visible)
	{
		// blendo eric: trying to apply the rules of RouteMouseCoords() here, but not sure of use
		if (parent
			&& parent->GetCaptureChild()
			&& parent->GetCaptureChild() != this) {
				return false;
		}
		return true;
	}
...
const char *idWindow::RouteMouseCoords(float xd, float yd) {
	...
	if (overChild) {
		...
		// SM: This seems to fix problem where mouse exit doesn't trigger on children when appropriate
		if ( !( overChild->flags & WIN_HOLDCAPTURE ) ) {
			overChild->RouteMouseCoords( xd, yd );
		}
		overChild = NULL;
	}
	return "";
...
void idWindow::RunNamedEvent ( const char* eventName )
{
	...
	//BC todo: do some error checking to see if it can't find a namedEvent that matches the eventName
...
const char *idWindow::HandleEvent(const sysEvent_t *event, bool *updateVisuals) {
	...
	if ( flags & WIN_DESKTOP ) {
		...
		CalcRects(0,0);
		// SM: This seems unnecessary and causes weird jittering when fullscreening some menus
	...
	if (visible && !noEvents) {
		...
		if (event->evType == SE_KEY) {
			...
			if (event->evValue == K_MOUSE1 || JoyEmulateMouseClick(event)) {
			...
			else if (event->evValue == K_MOUSE2)
			{
				...
				if (!event->evValue2 && !contextUpRun)
				{
					contextUpRun = RunScript(ON_CONTEXTRELEASE); //BC
				}

			}
			else if ((event->evValue == K_BACKSPACE || event->evValue == K_DEL) && forceDeleteKeys) //BC
			{
				//BC propagate this event to the children.
				int c = children.Num();
				for (int i = 0; i < c; i++) {
					idWindow *child = children[i];
					if (child->Contains(gui->CursorX(), gui->CursorY())) {
						const char *childRet = child->HandleEvent(event, updateVisuals);
						if (childRet && *childRet) {
							return childRet;
						}
			...
			} else if ((event->evValue == K_ESCAPE || JoyEmulateEscape(event)) && event->evValue2) {
				if (GetFocusedChild()) {
					const char *childRet = GetFocusedChild()->HandleEvent(event, updateVisuals);
					if (childRet && *childRet) {
						return childRet;
		...
		} else if (event->evType == SE_MOUSE || event->evType == SE_MOUSEGAMEPAD) {
			if (updateVisuals) {
				*updateVisuals = true;
			}
...
void idWindow::SetupTransforms(float x, float y) {
	...
	// blendo eric: use updated winvars
	if ( shear.x() || shear.y() ) {
		static idMat3 smat;
		smat.Identity();
		smat[0][1] = shear.x();
...
idWindow* idWindow::GetOverChild()
{
	if (flags & WIN_DESKTOP) {
		return gui->GetDesktop()->overChild;
	}
	return NULL;
}
...
idWindow *idWindow::SetFocus(idWindow *w, bool scripts) {
	// blendo eric : allow unfocus with null window ptr

	// only one child can have the focus
	idWindow *lastFocus = NULL;
	if (!w || (w->flags & WIN_CANFOCUS)) {
...
idWinVar *idWindow::GetWinVarByName(const char *_name, bool fixup, drawWin_t** owner) {
	...
	if (idStr::Icmp(_name, "controllerbinding") == 0) {
		retVar = &controllerBinding;
	}
	if (idStr::Icmp(_name, "cursorBound") == 0) {
		retVar = &cursorBound;
	}
...
bool idWindow::ParseInternalVar(const char *_name, idParser *src) {
	...
	if (idStr::Icmp(_name, "shear") == 0) {
		...
		float tempY = src->ParseFloat();
		shear = idVec2(tempX, tempY);
		return true;
	}
	...
	// blendo eric
	if (idStr::Icmp(_name, "letterspacing") == 0) {
		letterSpacing = src->ParseFloat();
		return true;
	}
	if (idStr::Icmp(_name, "fontmaterial") == 0) {
		idStr fontStr;
		ParseString(src, fontStr);
		fontMaterialName = fontStr;
		fontMaterial = fontMaterialName.Length() ? declManager->FindMaterial(fontMaterialName,false) : NULL;
		return true;
	}
	if (idStr::Icmp(_name, "controllerbinding") == 0) {
		controllerBinding = src->ParseBool();
		return true;
	}
	if (idStr::Icmp(_name, "cursorBound") == 0) {
		cursorBound = src->ParseBool();
		return true;
	}
	if (idStr::Icmp(_name, "cloneChildTemplate") == 0) {
...
bool idWindow::Parse( idParser *src, bool rebuild) {
	...
	while( token != "}" ) {
		...
		if ( token == "windowDef" || token == "animationDef" ) //BC normal rect.
		{
		...
		else if ( token == "editDef" ) //BC textbox entry.
		{
		...
		else if ( token == "choiceDef" ) //BC multi-choice option
		{
		...
		else if ( token == "sliderDef" ) //BC sliderbar
		{
		...
		else if ( token == "bindDef" ) //BC keybind
		{
		...
		else if ( token == "listDef" ) //BC listbox.
		{
		...
		else if ( token == "renderDef" ) //BC displays a 3D model.
		{
		...
		else if ( token == "gameSSDDef" ) //BC one of the minigames
		{
		...
		else if ( token == "gameBearShootDef" ) //BC one of the minigames
		{
		...
		else if ( token == "gameBustOutDef" ) //BC one of the minigames
		{
		...
		else if (token == "bindDisplayDef") //SM display keybind
		{
			//These are the key prompts that appear on the hud during gameplay.
			idBindDisplayWindow* win = new idBindDisplayWindow(dc, gui);
			SaveExpressionParseState();
			win->Parse(src, rebuild);
			RestoreExpressionParseState();
			AddChild(win);
		...
		else if (token == "feedListDef") // blendo eric: event log feed alerts
		{
			idFeedAlertWindow* win = new idFeedAlertWindow(dc, gui);
			SaveExpressionParseState();
			win->Parse(src, rebuild);
			RestoreExpressionParseState();
			AddChild(win);
		...
		else if (token == "templateDef") // blendo eric: cloneable window
		{
			int windowParseStart  = src->GetFileOffset(); // save offset to reparse multiple times
			int cloneAmt = (int)cloneChildTemplate > 0 ? (int)cloneChildTemplate : 1; 
			for(int cloneIdx = 0; cloneIdx < cloneAmt; cloneIdx++ )
			{
				src->SetOffset(windowParseStart);
				idWindow* win = new idWindow(dc, gui);
				SaveExpressionParseState();
				win->Parse(src, rebuild);
				RestoreExpressionParseState();
				AddChild(win);
				win->name = win->name + idStr(cloneIdx);
				win->SetParent(this);
				dwt.simp = NULL;
				dwt.win = win;
				drawWindows.Append(dwt);
...
SW: Used to force a window back into a noTime state mid-lifetime. To resume, use ResetTime
================
*/
void idWindow::PauseTime() {
	noTime = true;
}
...
intptr_t idWindow::ParseTerm( idParser *src,	idWinVar *var, intptr_t component ) {
	...
	if (!token.Icmp("isgamepad")) {
		return WEXP_REG_IS_GAMEPAD;
	}
...
bool idWindow::OverchildInteractive()
{
	if (!overChild) {
		return scripts[ON_ACTION];
	}

	return overChild->OverchildInteractive();
}
...
bool idWindow::JoyEmulateMouseClick(const sysEvent_t* event) const
{
	bool isFullscreen = !gui->GetDesktop() || (gui->GetDesktop()->GetFlags() & WIN_MENUGUI);
	return event->evValue == K_JOY1 && isFullscreen;
}

bool idWindow::JoyEmulateEscape(const sysEvent_t* event) const
{
	bool isFullscreen = !gui->GetDesktop() || (gui->GetDesktop()->GetFlags() & WIN_MENUGUI);
	return event->evValue == K_JOY2 && isFullscreen;
}
