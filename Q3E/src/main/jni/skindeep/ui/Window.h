...
public:
	...
	bool Interactive();
	virtual bool OverchildInteractive(); // SM: Like interactive but only returns for self or the overchild
	...
	bool		JoyEmulateMouseClick(const sysEvent_t* event) const;
	bool		JoyEmulateEscape(const sysEvent_t* event) const;
