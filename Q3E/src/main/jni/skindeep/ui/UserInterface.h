...
public:
	...
	virtual idRectangle			DrawArbitraryText(const idStr& text, float scale, idVec4 colour, float x, float y, idStr fontName, idVec4 shadowColor, int textAlign = 0, float letterSpace = 0.0f, idVec4 boxColor = vec4_zero) = 0;
	virtual idRectangle			DrawSubtitleText( const idStr& text, float scale, idVec4 colour, float x, float y, float maxWidth, float maxY, float bgOpacity, idStr fontName, bool noDraw = false ) = 0; // blendo eric: noDraw, allow drawrect size calc without drawing
	...
	virtual void				SetStateVec4( const char* varName, const idVec4& value ) = 0;
