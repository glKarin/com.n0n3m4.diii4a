...
public:
	...
	void				DrawMaterial(float x, float y, float w, float h, const idMaterial *mat, const idVec4 &color, float scalex = 1.0, float scaley = 1.0, float offsetx = 0.0f, float offsety = 0.0f); // blendo eric: allow texture coord offset, i.e allow centering
	...
	int					DrawText(const char *text, float textScale, int textAlign, idVec4 color, idRectangle rectDraw, bool wrap, int cursor = -1, bool calcOnly = false, idList<int> *breaks = NULL, int limit = 0, float letterSpace = 0.0f);
	...
	int					TextWidth(const char *text, float scale, int limit, float letterSpace = 0.0f);
	...
	// SM: Replaced with verison from BFG
	// blendo eric: added active font mat
	void				SetFont(idFont * font, const idMaterial * fontMat = NULL) { activeFont = font; activeFontMaterial = fontMat = fontMat; }
...
private:
	...
	idFont *			activeFont;
	const idMaterial	*activeFontMaterial; // blendo eric: override code gen font material
	idStr				fontName;
	float				xScale;
	float				yScale;
