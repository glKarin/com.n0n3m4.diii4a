...
const char *idChoiceWindow::HandleEvent(const sysEvent_t *event, bool *updateVisuals) {
	...
	if ( event->evType == SE_KEY ) {
		...
		if ( key == K_RIGHTARROW || key == K_KP_RIGHTARROW || key == K_MOUSE1 || JoyEmulateMouseClick(event))  {
...
void idChoiceWindow::UpdateChoicesAndVals( void ) {
	...
	if ( choiceVals.Length() && latchedVals.Icmp( choiceVals ) ) {
		...
		if ( choices.Num() != values.Num() ) {
			common->Warning( "idChoiceWindow:: gui '%s' window '%s' has value count (%d) unequal to choices count (%d)", gui->GetSourceFile(), name.c_str(), values.Num(), choices.Num());
		}
...
// blendo eric : default to losing focus on mouse exit (and potentially gaining on enter)
void idChoiceWindow::MouseEnter() {
	idWindow::MouseEnter();
}
void idChoiceWindow::MouseExit() {
	if (flags & WIN_FOCUS)
	{
		SetFocus(NULL);
	}
