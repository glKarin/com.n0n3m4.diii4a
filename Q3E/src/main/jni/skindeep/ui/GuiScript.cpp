...
void Script_PauseTime(idWindow* window, idList<idGSWinVar>* src) {
	idWinStr* parm = dynamic_cast<idWinStr*>((*src)[0].var);
	drawWin_t* win = NULL;
	if (parm && src->Num()) {
		win = window->GetGui()->GetDesktop()->FindChildByName(*parm);
...
//BC run a namedEvent from within the script.
...
guiCommandDef_t commandList[] = {
	...
	// SW:
	// Forces a window into the noTime state (which can normally only be set on initialisation)
	// The first parameter is the name of the window. If no parameter is provided, the current window is used.
	{ "pauseTime", Script_PauseTime, 0, 1}
};
