...
const idEventDef EV_Camera_SetPath("SetPath", "efdff");
const idEventDef EV_Camera_SetFOVLerp("setCameraFOVLerp", "ffdd");
const idEventDef EV_CAMERA_StartDollyZoom("startDollyZoom", "ef");
const idEventDef EV_Camera_SetOverrideRoll("setOverrideRoll", "f");
const idEventDef EV_Camera_SetRollLerp("setRollLerp", "ffdd");
const idEventDef EV_Camera_GetCameraAngles("getCameraAngles", NULL, 'v');
...
CLASS_DECLARATION( idCamera, idCameraView )
	...
	EVENT( EV_Camera_SetPath,		idCameraView::SetPath)
	EVENT( EV_Camera_SetFOVLerp,	idCameraView::SetFOVLerp)
	EVENT( EV_CAMERA_StartDollyZoom,idCameraView::StartDollyZoom)
	EVENT( EV_Camera_SetOverrideRoll, idCameraView::Event_SetOverrideRoll)
	EVENT( EV_Camera_SetRollLerp, idCameraView::Event_SetRollLerp)
	EVENT( EV_Camera_GetCameraAngles, idCameraView::Event_GetCameraAngles)
END_CLASS
...
void idCameraView::Event_SetOverrideRoll(float roll)
{
	overrideRoll = roll;
}

// SW: Cameras behave a little differently to other entities when it comes to angles,
// so we have a special event that handles it here.
void idCameraView::Event_GetCameraAngles(void)
{
	idAngles angles = CalcViewAxis().ToAngles();
...
//BC manual way to activate/deactivate cinema cam.
void idCameraView::SetActive(bool value)
{
	if (value)
	{
		gameLocal.SetCamera(this);
...
//TODO: make the bind angle orientation settable via script boolean argument
void idCameraView::SetPath(idEntity *pathEnt, float movetime, int angleRotate, float accelerationTime, float decelerationTime)
{
	idDict args;
	idEntity *mover;
	...
	if (!pathEnt)
	{
		common->Error("idCameraView %s failed to find spline path.\n", GetName());
		return;
	}

	if (this->IsBound())
	{
		this->Unbind(); // Remove ourselves from any other bind master (such as e.g. another path mover)
	}

	args.Clear();
	args.Set("classname", "func_mover");
	args.SetFloat("move_time", movetime);
	...
	mover->SetOrigin(pathEnt->GetPhysics()->GetOrigin());
	this->SetOrigin(pathEnt->GetPhysics()->GetOrigin());
	this->Bind(mover, (angleRotate >= 1) ? true : false);
	((idMover *)mover)->Event_SetAccellerationTime(accelerationTime);
	((idMover *)mover)->Event_SetDecelerationTime(decelerationTime);
	((idMover *)mover)->Event_StartSpline(pathEnt);
...
void idCameraView::Think()
{
	idEntity::Think();
}

//Sets up a FOV lerp for this camera. Lerp type is an enum (see camera.h header LERPTYPE_*)
void idCameraView::SetFOVLerp(float start, float end, int timeMS, int lerpType)
{
	fovLerpStart = start;
	fovLerpEnd = end;
...
// Rolls the camera (uses same enums as FOV lerp)
void idCameraView::Event_SetRollLerp(float startAngle, float endAngle, int timeMS, int lerpType)
{
	rollLerpStart = startAngle;
	rollLerpEnd = endAngle;
	rollLerpStartTime = gameLocal.time;
	rollLerpEndTime = gameLocal.time + timeMS;
	rollLerpType = lerpType;
}

// Start a dolly zoom effect where we dynamically adjust the FOV to maintain the same width for the target.
void idCameraView::StartDollyZoom(idEntity* target, float targetWidth)
{
	if (target)
	{
		dollyZoomActive = true;
		dollyZoomTarget = target;
		dollyZoomWidth = targetWidth;
	}
}
...
void idCameraView::Spawn( void ) {
	...
	// Optionally specify a manual vertical fov. This will skip FOV calculation and use the FOV values as provided.
	fov_v = spawnArgs.GetFloat("fov_v", "0"); 

	// Sets the roll value for a camera that is using attachedView to set its view axes
	overrideRoll = spawnArgs.GetFloat("override_roll", "0");

	fovLerpStart = fovLerpEnd = fov;
	fovLerpStartTime = fovLerpEndTime = 0;
	rollLerpStart = rollLerpEnd = overrideRoll;
	rollLerpStartTime = rollLerpEndTime = 0;

	PostEventMS( &EV_Camera_SetAttachments, 0 );
	UpdateChangeableSpawnArgs(NULL);
}

idMat3 idCameraView::CalcViewAxis(void) 
{
	idVec3 dir;
	idAngles viewAngles;
	idMat3 viewAxis;
	...
	if (attachedView) {
		dir = attachedView->GetPhysics()->GetOrigin() - origin;
		dir.Normalize();
		viewAngles = dir.ToAngles();
		if (gameLocal.time > rollLerpStartTime && gameLocal.time < rollLerpEndTime)
		{
			switch (rollLerpType)
			{
			case LERPTYPE_LINEAR:
			{
				overrideRoll = idMath::Lerp(rollLerpStart, rollLerpEnd, (float)(gameLocal.time - rollLerpStartTime) / (float)(rollLerpEndTime - rollLerpStartTime));
				break;
			}
			case LERPTYPE_CUBIC_EASE_IN:
			{
				overrideRoll = idMath::Lerp(rollLerpStart, rollLerpEnd, idMath::CubicEaseIn((float)(gameLocal.time - rollLerpStartTime) / (float)(rollLerpEndTime - rollLerpStartTime)));
				break;
			}
			case LERPTYPE_CUBIC_EASE_OUT:
			{
				overrideRoll = idMath::Lerp(rollLerpStart, rollLerpEnd, idMath::CubicEaseOut((float)(gameLocal.time - rollLerpStartTime) / (float)(rollLerpEndTime - rollLerpStartTime)));
				break;
			}
			case LERPTYPE_CUBIC_EASE_INOUT:
			{
				overrideRoll = idMath::Lerp(rollLerpStart, rollLerpEnd, idMath::CubicEaseInOut((float)(gameLocal.time - rollLerpStartTime) / (float)(rollLerpEndTime - rollLerpStartTime)));
				break;
			}
			}
		}
		if (overrideRoll)
		{
			viewAngles.roll = overrideRoll;
		}
		viewAxis = viewAngles.ToMat3();
	}
	else {
		viewAxis = ent->GetPhysics()->GetAxis();
	}
...
void idCameraView::GetViewParms( renderView_t *view ) {
	...
	view->viewaxis = CalcViewAxis();

	if (fov_v > 0)
	{
		// if vertical FOV is specified, ignore the FOV calculations below and set the FOV exactly as specified in the spawnargs (e.g. for monitors)
		view->fov_x = fov;
		view->fov_y = fov_v;
	}
	else
	{
		if (gameLocal.time > fovLerpStartTime && gameLocal.time < fovLerpEndTime)
		{
			switch (fovLerpType)
			{
			case LERPTYPE_LINEAR:
			{
				fov = idMath::Lerp(fovLerpStart, fovLerpEnd, (float)(gameLocal.time - fovLerpStartTime) / (float)(fovLerpEndTime - fovLerpStartTime));
				break;
			}
			case LERPTYPE_CUBIC_EASE_IN:
			{
				fov = idMath::Lerp(fovLerpStart, fovLerpEnd, idMath::CubicEaseIn((float)(gameLocal.time - fovLerpStartTime) / (float)(fovLerpEndTime - fovLerpStartTime)));
				break;
			}
			case LERPTYPE_CUBIC_EASE_OUT:
			{
				fov = idMath::Lerp(fovLerpStart, fovLerpEnd, idMath::CubicEaseOut((float)(gameLocal.time - fovLerpStartTime) / (float)(fovLerpEndTime - fovLerpStartTime)));
				break;
			}
			case LERPTYPE_CUBIC_EASE_INOUT:
			{
				fov = idMath::Lerp(fovLerpStart, fovLerpEnd, idMath::CubicEaseInOut((float)(gameLocal.time - fovLerpStartTime) / (float)(fovLerpEndTime - fovLerpStartTime)));
				break;
			}
			}
		}
		else if (dollyZoomActive)
		{
			float width = dollyZoomWidth;
			float dist = (this->GetPhysics()->GetOrigin() - dollyZoomTarget->GetPhysics()->GetOrigin()).Length();
			fov = 2.0f * RAD2DEG(idMath::ATan(width / (dist * 2.0f))); // Formula derived from the dolly zoom effect formula, rearranging for FOV
			if (developer.GetBool())
				gameLocal.Printf("%f\n", fov);
		}

		// Will always calculate the correct FOV for fullscreen views
