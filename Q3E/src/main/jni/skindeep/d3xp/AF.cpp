...
idAF::idAF( void ) {
	self = NULL;
	animator = NULL;
	overrideCurrent = NULL; // SM: To allow setting up pose from another actor
	modifiedAnim = 0;
	baseOrigin.Zero();
	baseAxis.Identity();
	poseTime = -1;
...
bool idAF::LoadBody( const idDeclAF_Body *fb, const idJointMat *joints ) {
	...
	body->SetNeverBlock(fb->neverBlock);
...
bool idAF::Load( idEntity *ent, const char *fileName ) {
	...
	physicsObj.SetNeverBlock(file->neverBlock);
...
void idAF::StartFromCurrentPose( int inheritVelocityTime ) {
	...
	if ( inheritVelocityTime > 0 ) {
		...
		// SM: Use overrideCurrent if set
		SetupPose( overrideCurrent ? overrideCurrent : self, gameLocal.time );
	}
