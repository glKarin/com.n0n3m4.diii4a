...
//BC
const float RICOCHET_SPEED_THRESHOLD = 2000;
const float PENETRATION_SPEED_THRESHOLD = 3000;
const float PENETRATION_MAXDISTANCE = 14;

const float BULLET_PLAYERLENIENCY_DOTPRODUCT = -.6f;	//Dotproduct threshold of off-screen bullet leniency.
const float BULLET_PLAYERLENIENCY_DAMAGEMODIFIER = .4f; //Allow leniency to player if they're hit by off-screen bullets. Reduce damage by multiplying it by this number.


const idEventDef EV_Explode( "<explode>", NULL );
const idEventDef EV_ScriptExplode("explode", NULL); // accessible by map scripts
...
CLASS_DECLARATION( idEntity, idProjectile )
	...
	EVENT( EV_ScriptExplode,		idProjectile::Event_Explode )
...
void idProjectile::Create( idEntity *owner, const idVec3 &start, const idVec3 &dir ) {
	...
	displayDelaytime = gameLocal.time + spawnArgs.GetInt("displayDelaytime", "0");

	//doPenetration = penetrate wood. We don't want certain projectiles to penetrate, such as explosive things, rockets, mech cannon.
	doPenetration = this->spawnArgs.GetBool("doPenetration", "1");
}
...
void idProjectile::Think( void ) {
	...
	if (gameLocal.time >= displayDelaytime)
		Present();
...
bool idProjectile::Collide( const trace_t &collision, const idVec3 &velocity ) {
	...
	//DEBUG
	//gameRenderWorld->DebugArrow(colorGreen, collision.endpos, collision.endpos + (collision.c.normal * 32), 8, 20000);
	//gameRenderWorld->DebugArrowSimple(collision.endpos);
	...
	{
		//assert( 0 );
		gameRenderWorld->DebugArrow(colorOrange, collision.endpos, collision.endpos + (collision.c.normal * 32), 8, 20000);
		PostEventMS(&EV_Remove, 0);
		return true;
	}
	...
	if ( spawnArgs.GetFloat( "push", "0", push ) && push > 0.0f && !ent->IsType(idPlayer::Type))
	{
		//TODO: Make shotgun NOT blow away the mech into the sky.
		float targetMass = gameLocal.entities[collision.c.entityNum]->GetPhysics()->GetMass();
		ent->ApplyImpulse( this, collision.c.id, collision.c.point, (push * dir) * targetMass );
	}
	...
	//Activate levers/buttons.
	if (ent->IsType(idLever::Type))
	{
		ent->ProcessEvent(&EV_Activate, this);

		idEntityFx::StartFx("fx/frob_lines", &ent->GetPhysics()->GetOrigin(), &mat3_identity, NULL, false); //make a special particle fx.
	}
	...
	if ( ent->IsType( idActor::Type ) || ( ent->IsType( idAFAttachment::Type ) && static_cast<const idAFAttachment*>(ent)->GetBody()->IsType( idActor::Type ) ) )
	{
		...
		else
		{
			if (ent->IsType(idAI::Type))
			{
				...
				{
					//projectile has hit an actor.
					idVec3 noisepos = owner.GetEntity()->GetPhysics()->GetOrigin();
				}
			}
		}
	}
	...
	isBrittleGlass = gameLocal.entities[collision.c.entityNum]->IsType(idBrittleFracture::Type) && (gameLocal.entities[collision.c.entityNum]->health <= 0);

	//BC Handle ricochets. Bullets ricochet off metal surfaces.
	...
	if (surfaceType == SURFTYPE_METAL && velocity.Length() >= RICOCHET_SPEED_THRESHOLD && spawnArgs.GetBool("ricochets", "1") /*  && !(ent->fl.takedamage)*/ )
	{
		//A fast bullet projectile hit a metal surface.
		idVec3 incidentVec = velocity;
		idVec3 normal = collision.c.normal;
		idVec3 reflectionAngle;
		float currentSpeed = velocity.Length();
		idMat3 axis;
		idVec3 tmp;
		const char *fxname, *decal, *typeName = "";

		//common->Printf("bullet vel %f\n", velocity.Length());

		incidentVec.Normalize();
		reflectionAngle = (incidentVec - 2.0f * DotProduct(incidentVec, normal) * normal);
		...
		//Since we're bypassing the Explode funciton, do the particle fx.
		fxname = spawnArgs.GetString("model_metal");

		if (fxname && *fxname)
		{
			const idDeclParticle *sparkFx = static_cast<const idDeclParticle *>(declManager->FindType(DECL_PARTICLE, fxname));

			if (sparkFx)
			{
				gameLocal.smokeParticles->EmitSmoke(sparkFx, gameLocal.time, 0, collision.endpos, collision.endAxis, timeGroup);
			}
		}


		//do a decal.
		decal = spawnArgs.GetString(va("mtr_detonate_metal", typeName));
		if (*decal != '\0')
		{
			gameLocal.ProjectDecal(collision.c.point, -collision.c.normal, 8.0f, true, 6.0f, decal);
		}

		StartSound("snd_ricochet", SND_CHANNEL_ITEM, 0, true, NULL);

		//After a ricochet, the shooter can get hit by their own bullet.
		owner = NULL;
		ignore = NULL;
		physicsObj.GetClipModel()->SetOwner(NULL);
		...
		ignore = DoEntDamage(ent, damageScale, collision, damageDefName, dir); //If damageable, then apply damage to ent.
	...
	else if ((surfaceType == SURFTYPE_WOOD || isBrittleGlass) && velocity.Length() >= PENETRATION_SPEED_THRESHOLD && doPenetration)
	{
		//HANDLE WOOD PENETRATION.
		idVec3 penetrationPos;
		int penetrationContents;
		
		//if glass, then shatter.
		if (isBrittleGlass)
		{
			idVec3 pushDir = velocity;
			pushDir.Normalize();
			gameLocal.entities[collision.c.entityNum]->AddForce(this, this->entityNumber, collision.endpos, pushDir * 16384.0f);
		}
		
		//Attempt to penetrate this surface.
		penetrationPos = collision.endpos + (collision.c.normal * -PENETRATION_MAXDISTANCE);		
		penetrationContents = gameLocal.clip.Contents(penetrationPos, this->GetPhysics()->GetClipModel(), this->GetPhysics()->GetClipModel()->GetAxis(), -1, this);

		if (penetrationContents & MASK_SOLID)
		{
			//Projectile is inside a brush. Bullet stops here.
			//gameRenderWorld->DebugCircle(colorRed, penetrationPos, collision.c.normal, 4, 9, 3000);
		}
		else
		{
			trace_t penetrationTr;
			idMat3 axis;
			idVec3 tmp;
			const char *decal;
			const char *fxname;
			...
			//Bullet can penetrate here. Do bullet penetration.
			SetOrigin(penetrationPos);
			physicsObj.SetLinearVelocity(moveDir * (velocity.Length() / 2.0f)); //slow down bullets so it doesn't penetrate forever.
			...
			//gameRenderWorld->DebugArrow(colorCyan, penetrationPos, penetrationPos + moveDir * 32, 8, 3000);


			if (isBrittleGlass)
			{
				//Glass.
				fxname = spawnArgs.GetString("model_glass_penetrate");
				if (fxname && *fxname)
				{
					const idDeclParticle *sparkFx = static_cast<const idDeclParticle *>(declManager->FindType(DECL_PARTICLE, fxname));
			...
			{
				//Wood.
				gameLocal.clip.TracePoint(exitTr, penetrationPos, penetrationPos + (moveDir * -PENETRATION_MAXDISTANCE), MASK_SOLID, this);

				decal = spawnArgs.GetString("mtr_detonate_wood_penetrate");
				if (*decal != '\0')
				{
					//Apply a decal on the side that the bullet enters.
					gameLocal.ProjectDecal(collision.c.point, -collision.c.normal, 8.0f, true, 10.0f, decal);

					if (exitTr.fraction < 1)
					{
						//Apply a decal on the bullet exit.
						gameLocal.ProjectDecal(exitTr.c.point, -exitTr.c.normal, 8.0f, true, 7.0f, decal);
					}
				}

				fxname = spawnArgs.GetString("model_wood_penetrate");
				if (fxname && *fxname)
				{
					const idDeclParticle *sparkFx = static_cast<const idDeclParticle *>(declManager->FindType(DECL_PARTICLE, fxname));
					...
					if (sparkFx)
					{
						...
						if (exitTr.fraction < 1)
						{
							gameLocal.smokeParticles->EmitSmoke(sparkFx, gameLocal.time, 0, exitTr.endpos, exitTr.endAxis, timeGroup);
						}
					}
				}
				...
				{
					//Apply damage to wood.
					gameLocal.entities[collision.c.entityNum]->Damage(this, this, moveDir, spawnArgs.GetString("def_damage"), 1.0f, 0);
				}
			}
	...
	else if (ent->fl.takedamage)
	{
		if (ent->spawnArgs.GetBool("bulletghost")) //This is for the kittycat. So the cat doesn't get hit by bullets.
		{
			damageDefName = spawnArgs.GetString("def_damage");
			damageScale = 1.0f;
	...
	//BC if target is player, be lenient. If bullet originated out of my FOV, then do less damage.
	...
	//		if (owner.GetEntity())
	//		{
	//			if (owner.GetEntity() != ent) //If we somehow got hit by our own projectile, then take 100% damage.
	//			{
	//				//Do a check to compare player facing to the bullet origin position
	//				float facingResult;
	//				idVec3 playerDir, dirToPlayer;
	//
	//				playerDir = gameLocal.GetLocalPlayer()->viewAngles.ToForward();
	//				//playerDir.x = 0;
	//				//playerDir.z = 0;
	//
	//				dirToPlayer = ent->GetPhysics()->GetOrigin() -  owner.GetEntity()->GetPhysics()->GetOrigin();
	//				dirToPlayer.NormalizeFast();
	//
	//				facingResult = DotProduct(dirToPlayer, playerDir);
	//
	//				if (facingResult > BULLET_PLAYERLENIENCY_DOTPRODUCT)
	//				{
	//					//Take reduced damage.
	//					damageScale *= BULLET_PLAYERLENIENCY_DAMAGEMODIFIER;
	//				}
	//			}
	//		}
	//	}
	...
	ignore = DoEntDamage(ent, damageScale, collision, damageDefName, dir); //Do the damage.
	...
	if (!neutralized)
	{
		...
		else if (spawnArgs.GetBool("impact_decal_effect"))
		{
			if (ent != NULL && ent->IsType(idActor::Type))
			{
				const char *decal, *typeName;
				...
				if (*decal != '\0')
				{
					idVec3 decalDir = collision.c.normal;
					//decalDir.Normalize();
					decalDir = -decalDir;

					//TODO: make decalsize data-driven.
					ent->ProjectOverlay(collision.endpos , decalDir, 5, decal);
					//gameRenderWorld->DebugSphere(colorGreen, idSphere(collision.c.point, .5f), 10000, true);

					//gameRenderWorld->DebugArrow(colorGreen, collision.endpos, collision.endpos + (decalDir * 8), 1, 5000);
				}
			}
			else
			{
...
idEntity * idProjectile::DoEntDamage(idEntity	*ent, float damageScale, const trace_t &collision, const char	*damageDefName, idVec3 dir)
{
	if (!ent->fl.takedamage)
		return NULL;
	...
	if ((owner.GetEntity() && owner.GetEntity()->IsType(idPlayer::Type))
		|| (originalOwner.GetEntity() && originalOwner.GetEntity()->IsType(idPlayer::Type)))
	{
		...
		if (ent->health > 0)
		{
			...
			if (owner.IsValid())
				player = static_cast<idPlayer *>(owner.GetEntity());
			else if (originalOwner.IsValid())
				player = static_cast<idPlayer *>(originalOwner.GetEntity());
	...
	if (damageDefName[0] != '\0' && !this->fl.hidden) //we set it to be fl.hidden when the autoaim'd bullet hits something. This is so that the same autoaim'd bullet doesn't inflict damage multiple times.
	{
		int			surfaceType;
		surfaceType = collision.c.material != NULL ? collision.c.material->GetSurfaceType() : SURFTYPE_NONE;

		ent->Damage(this, originalOwner.GetEntity(), dir, damageDefName, damageScale, CLIPMODEL_ID_TO_JOINT_HANDLE(collision.c.id), surfaceType);
		return ent;
	}	
...
void idProjectile::Explode( const trace_t &collision, idEntity *ignore ) {
	...
	idVec3		normal;
	int			removeTime;
	bool		doSuspiciousNoise = true;
	const char  *spawnOnExplode, *surfaceValue;
	...
	//BC surface-specific explosion sounds.
	sndExplode = "snd_explode"; //Default explosion sound.
	if (surfaceType == SURFTYPE_FLESH)
	{
		if (spawnArgs.GetString("snd_explode_flesh", "", &surfaceValue))
		{
			sndExplode = "snd_explode_flesh";
		}
	}
	...
	if ( !( fxname && *fxname ) )
	{
		if (surfaceType == SURFTYPE_FLESH && g_bloodEffects.GetBool() && g_hiteffects.GetBool())
		{
			fxname = spawnArgs.GetString("model_flesh");
		}
		else if ( ( surfaceType == SURFTYPE_NONE ) || ( surfaceType == SURFTYPE_STONE ) )
		{			
			fxname = spawnArgs.GetString( "model_smokespark" ); //Default particle hit effect.
		}
		else if ( surfaceType == SURFTYPE_RICOCHET || surfaceType == SURFTYPE_METAL)
		{
			fxname = spawnArgs.GetString( "model_metal" );
		}
		else if (surfaceType == SURFTYPE_FABRIC )
		{
			fxname = spawnArgs.GetString("model_fabric");
		}
		else if (surfaceType == SURFTYPE_TILE)
		{
			fxname = spawnArgs.GetString("model_tile");
		}
		else if (surfaceType == SURFTYPE_GLASS)
		{
			fxname = spawnArgs.GetString("model_glass");
		}
		else
		{
	...
	if ( fxname && *fxname  && !neutralized)
	{
		//BC display bullet hit particles
		idAngles sprayAngle;
		sprayAngle = (collision.c.normal).ToAngles();
		sprayAngle.pitch += 90;
		SetAxis(sprayAngle.ToMat3());
	...
	//When projectile hits a surface, alert nearby ai.
	if (collision.c.entityNum >= 0 && collision.c.entityNum < MAX_GENTITIES - 2)
	{
		if (gameLocal.entities[collision.c.entityNum])
		{
			...
			{
				//Don't do suspicious noise if projectile hits an actor.
				doSuspiciousNoise = false;
			}
		}
	}

	if (doSuspiciousNoise)
	{
		//projectile hit a surface.
		gameLocal.SpawnInterestPoint(this, this->GetPhysics()->GetOrigin(), this->spawnArgs.GetString("def_bulletinterestnoise", "interest_wallbullet_noise"));
	}

	// bind the projectile to the impact entity if necesary
	if ( spawnArgs.GetBool( "bindOnImpact" ) )
	{
		if (gameLocal.entities[collision.c.entityNum])
		{
	...
	spawnOnExplode = spawnArgs.GetString("spawn_on_explode", "");

	if (spawnOnExplode && *spawnOnExplode)
	{
		idEntity *spawnedEnt;
		const idDict *spawnedObject = gameLocal.FindEntityDefDict(spawnOnExplode, false);		
		gameLocal.SpawnEntityDef(*spawnedObject, &spawnedEnt, false);
		spawnedEnt->SetOrigin(this->GetPhysics()->GetOrigin());
		spawnedEnt->GetPhysics()->SetAngularVelocity(idVec3(30, 30, 30));
		spawnedEnt->GetPhysics()->SetLinearVelocity(idVec3(0, 0, 64));
	}
	...
	if ( fxdebris )
	{
		const idDict *debris = gameLocal.FindEntityDefDict( spawnArgs.GetString("def_debris", "debris_small"), false );

		//BC
		if (debris)
		{
			//int amount = max(1, gameLocal.random.RandomInt(fxdebris) );
			int amount = fxdebris;
...
void idGuidedProjectile::Launch( const idVec3 &start, const idVec3 &dir, const idVec3 &pushVelocity, const float timeSinceFire, const float launchPower, float dmgPower ) {
	...
	if ( owner.GetEntity() )
	{
		if (owner.GetEntity()->IsType(idGunnerMonster::Type)) //BC
		{
			if (static_cast<idGunnerMonster *>(owner.GetEntity())->lastEnemySeen.IsValid())
			{
				enemy = static_cast<idGunnerMonster *>(owner.GetEntity())->lastEnemySeen.GetEntity();
			}
		}
		else if ( owner.GetEntity()->IsType( idAI::Type ) )
		{
		...
		else if ( owner.GetEntity()->IsType( idPlayer::Type ) )
		{
			...
			//gameRenderWorld->DebugArrow(colorYellow, start, end, 8, 10000);
			
			//check if looking DIRECTLY at an enemy.
			if ( tr.fraction < 1.0f )
			{
				...
				if (enemy.GetEntity())
				{
					if (!enemy.GetEntity()->IsType(idActor::Type))
					{
						enemy = NULL;
					}
				}
			...
			//if (enemy.GetEntity())
			//{
			//	gameRenderWorld->DebugArrow(colorWhite, enemy.GetEntity()->GetPhysics()->GetOrigin() + idVec3(0, 0, 64), enemy.GetEntity()->GetPhysics()->GetOrigin(), 4, 10000);
			//}

			if (enemy.GetEntity() == NULL)
			{
				//Use the player's autoaim function.
				player->UpdateAutoAim();
				if (player->autoaimEnt.GetEntity())
				{
					enemy = player->autoaimEnt.GetEntity();
				}
			}

			// if all else fails, fallback to the player's EnemyWithMostHealth function.
			...
			//if (enemy.GetEntity())
			//{
			//	gameRenderWorld->DebugArrow(colorRed, enemy.GetEntity()->GetPhysics()->GetOrigin() + idVec3(0, 0, 128), enemy.GetEntity()->GetPhysics()->GetOrigin(), 8, 10000);
			//}
		}
	}
	...
	if (!enemy.IsValid())
	{
		//no enemy found. So, try to find one.
		enemy = AcquireEnemy();
	}

	//if (enemy.GetEntity())
	//{
	//	gameRenderWorld->DebugArrow(colorRed, enemy.GetEntity()->GetPhysics()->GetOrigin() + idVec3(0, 0, 32), enemy.GetEntity()->GetPhysics()->GetOrigin(), 4, 5000);
	//}
	...
	burstMode = spawnArgs.GetBool( "burstMode" ); //makes it burst when it's close to target?
	unGuided = false;
...
idEntity *idGuidedProjectile::AcquireEnemy()
{	
	...
	{
		//Note: by default this doesn't target the player..... change this if we need to later
		...
		gameLocal.clip.TracePoint(losTr, GetPhysics()->GetOrigin(), candidatePosition, MASK_SOLID, this);
		//gameRenderWorld->DebugArrow(colorCyan, this->GetPhysics()->GetOrigin(), candidatePosition, 4, 10000);
		if (losTr.fraction >= 1)
		{
			float currentDistance = (GetPhysics()->GetOrigin() - candidatePosition).LengthFast();
...
void idDebris::Spawn( void ) {
	...
	// SW: allows us to place debris manually without it being ejected from a weapon/projectile/etc
	if (spawnArgs.GetBool("selfLaunch", "0"))
	{
		this->Create(gameLocal.GetLocalPlayer(), spawnArgs.GetVector("origin", "0 0 0"), mat3_identity);
		this->Launch();
...
bool idDebris::Collide( const trace_t &collision, const idVec3 &velocity ) {
	...
	if ( sndBounce != NULL && gameLocal.time >= nextBounceSnd)
	{
		nextBounceSnd = gameLocal.time + 500;
		StartSoundShader( sndBounce, SND_CHANNEL_BODY, 0, false, NULL );
	}

	if (gameLocal.time > suspiciousNoiseTimer)
	{
		//TODO? currently all debris makes people suspicious, make this a flag that is enabled/disabled per item.
		suspiciousNoiseTimer = gameLocal.time + 500;
		gameLocal.SetSuspiciousNoise(this, this->GetPhysics()->GetOrigin(), 128, NOISE_LOWPRIORITY);
	}
	...
	if (splat.Length() > 0)
	{
		if (*splat != '\0')
		{
			gameLocal.ProjectDecal(collision.endpos, -collision.c.normal, 8.0f, true, 4.0 + gameLocal.random.RandomFloat() * 12.0f, splat);
			splat = "";
		}
	}

	if (gameLocal.time >= nextBounceFX)
	{
		const char *fxBounce;
		...
		nextBounceFX = gameLocal.time + 1000;
	}
