...
public:
	...
	//BC
	idList<idStr>           nodeAnimList; //BC idle animations
	int						lastTimeUsed; //keep track of last time the searchnode was used, so people aren't repeating searchnodes immediately.
...
private:
	...
	//BC private start
	void					Event_PropAnim(const char *animName, int loop);	
	void					Event_PropAnimDone(int blendFrames);	
	void					Event_SetPropBlendFrames(int framecount);
	void					Event_StopAnim();
	//BC private end
};
...
private:
	void				Event_Activate( idEntity *activator );
	void				Event_FadeColor(float r, float g, float b, float timeSeconds);
...
public:
	...
	void				Spawn( void );
	virtual void		Think(); //SM
	void				Event_Activate( idEntity *activator );
	void				Event_HasInfiniteStage();

	void				SetActive(bool value); //BC
	bool				GetParticleActive();//BC
...
private:
	bool				hidden;
	bool				airlessGravity; // If true, airlessness affects gravity of the emitter
	bool				hasInfiniteStage;
};
...
public:
	...
	void				SetBlocked(bool value, bool blockView); //BC
...
protected:
	void				Event_MatchTarget(void); //BC moving to protected so that elevatorcableSpace can access this.
...
private:
	...
	void				Event_Activate( idEntity *activator );
	void				Event_PortalToggle(int value); //bc
};
...
private:
	...
	void				Event_Activate( idEntity *activator );
	void				Event_ObstacleToggle(int value); //bc
};
