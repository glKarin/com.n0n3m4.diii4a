...
============
*/
void idAFAttachment::Damage( idEntity *inflictor, idEntity *attacker, const idVec3 &dir, const char *damageDefName, const float damageScale, const int location, const int materialType ) {
...
static const float BOUNCE_SOUND_LOUDIMPACT_THRESHOLD = 400.0f; //BC velocity needed to make corpse impact loud be fleshy.
...
bool idAFEntity_Base::Collide( const trace_t &collision, const idVec3 &velocity ) {
	...
	if ( af.IsActive() ) {
		...
		if ( v > BOUNCE_SOUND_MIN_VELOCITY && gameLocal.time > nextSoundTime )
		{
			...
			if ( StartSound( v > BOUNCE_SOUND_LOUDIMPACT_THRESHOLD ? "snd_bigbounce" : "snd_bounce", SND_CHANNEL_ANY, 0, false, NULL ) )
			{
				// don't set the volume unless there is a bounce sound as it overrides the entire channel
				// which causes footsteps on ai's to not honor their shader parms
				SetSoundVolume( f );

				gameLocal.DoParticle(spawnArgs.GetString("smoke_drag", ""), collision.endpos); //BC
			}

			nextSoundTime = gameLocal.time + 500;

			//BC frob or damage things.
			if (collision.c.entityNum > 0 && collision.c.entityNum < MAX_GENTITIES - 2)
			{
				...
				if (gameLocal.entities[collision.c.entityNum]->isFrobbable && isValid)
				{
					//Frob it.
					if (gameLocal.entities[collision.c.entityNum]->DoFrob(0, this))
					{
						idEntityFx::StartFx("fx/frob_lines", &gameLocal.entities[collision.c.entityNum]->GetPhysics()->GetOrigin(), &mat3_identity, NULL, false);
					}
				}
				else if (gameLocal.entities[collision.c.entityNum]->fl.takedamage && isValid)
				{
					//inflict damage on thing it hit.
					...
					if (damageDefName[0] != '\0')
					{
						...
						damageDir.Normalize();
						gameLocal.entities[collision.c.entityNum]->Damage(this, this, damageDir, damageDefName, 1.0f, CLIPMODEL_ID_TO_JOINT_HANDLE(collision.c.id));
						...
						if (gameLocal.entities[collision.c.entityNum]->spawnArgs.GetBool("bleed"))
						{
							...
							if (damageDef)
							{
								if (damageDef->GetInt("damage") > 0) //BC only apply damage decal if damage is inflicted.
								{
									gameLocal.entities[collision.c.entityNum]->AddDamageEffect(collision, velocity, damageDefName);
								}
							}
						}
					}
...
================
*/
void idAFEntity_Gibbable::Damage( idEntity *inflictor, idEntity *attacker, const idVec3 &dir, const char *damageDefName, const float damageScale, const int location, const int materialType ) {
	if ( !fl.takedamage ) {
		return;
	}
...
bool idAFEntity_Gibbable::Collide( const trace_t &collision, const idVec3 &velocity )
{
	...
	//BC allow AFEntity_Gibbable things to trigger ev_touch events. This is for storing bodies in the lifeboat/lifepod.
...
void idAFEntity_Gibbable::SpawnGibs( const idVec3 &dir, const char *damageDefName ) {
	...
	const idDeclEntityDef *minigibDef;
	...
	for ( i = 0; i < list.Num(); i++ )
	{
		...
		else
		{
			...
			list[i]->GetPhysics()->SetLinearVelocity( velocity * GIB_SPEED);
		}
#ifdef _D3XP
		...
		list[i]->PostEventSec( &EV_Remove, 600.0f ); //BC REMOVAL TIME, was 4
	}



	//BC spawn a bunch of debris at the center of the actor.
	minigibDef = gameLocal.FindEntityDef(spawnArgs.GetString("def_minigib", ""));
	if (minigibDef)
	{
		int k;
		idVec3 actorCenter = GetPhysics()->GetAbsBounds().GetCenter();
		
		#define MINIGIB_AMOUNT 24
		#define MINIGIB_SPEED 160

		for (k = 0; k < MINIGIB_AMOUNT; k++)
		{
			idDebris *minigibDebris;
			idEntity *minigibEnt;
			idVec3 spawnPos;
			idVec3 minigibDir;

			gameLocal.SpawnEntityDef(minigibDef->dict, &minigibEnt, false);
			if (!minigibEnt || !minigibEnt->IsType(idDebris::Type))
			{
				return;
			}

			//TODO: CHeck if spawnpos is in world before spawning.

			spawnPos = actorCenter + idVec3(gameLocal.random.CRandomFloat() * 16, gameLocal.random.CRandomFloat() * 16, gameLocal.random.CRandomFloat() * 16);
			//gameRenderWorld->DebugLine(colorYellow, spawnPos + idVec3(0, 0, -16), spawnPos + idVec3(0, 0, 16), 30000);
			minigibDir = spawnPos - actorCenter;
			minigibDir.Normalize();

			minigibDebris = static_cast<idDebris *>(minigibEnt);
			minigibDebris->Create(NULL, spawnPos, mat3_identity);
			minigibDebris->Launch();
			minigibDebris->GetPhysics()->SetLinearVelocity(minigibDir * MINIGIB_SPEED);
			minigibDebris->GetPhysics()->SetAngularVelocity(idVec3(gameLocal.random.CRandomFloat() * 128, gameLocal.random.CRandomFloat() * 128, gameLocal.random.CRandomFloat() * 128));
		}
...
void idAFEntity_Gibbable::Gib( const idVec3 &dir, const char *damageDefName ) {
	...
	//BC do not remove gibbed enemies... let them respawn back in.
	//PostEventSec( &EV_Gibbed, 2.0f );
	PostEventSec(&EV_Hide, 2.0f);
}
...
void idHarvestable::Think() {
	...
	if(startTime && gameLocal.slow.time - startTime > giveDelay && ! given) {
		...
		thePlayer->Give(spawnArgs.GetString("give_item"), spawnArgs.GetString("give_value"), 0);
		given = true;
	}
