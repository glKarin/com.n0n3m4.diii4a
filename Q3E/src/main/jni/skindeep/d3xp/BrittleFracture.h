...
#define SUCTION_TIME 15000 //how long until window auto-seals itself.
...
public:
	...
	//BC public
	void						SetRoomAssignment();
	idEntityPtr<idEntity>		assignedRoom;
	//idList<int>					vacuumDoorsToClose;
...
private:
	...
	const idMaterial *			crackedMaterial;
	float						decalSize;
	float						maxShardArea;
	...
	//BC private
	bool						hasActivatedTargets;
	idDict						glasspieceDict;
	void						Event_SpectatorTouch(idEntity *other, trace_t *trace);
	void						SetDoorVacuumLock();
	
	qhandle_t					portal; // SW: Keep a reference to the portal so we can close it when the player is far enough away
	int							portalFadeState; // Used to efficiently control lerping between static image and the real thing
	void						FindPortalHandle(void);
	void						UpdatePortalVisibility(void);

	enum {
		PORTAL_FADE_OPAQUE,
		PORTAL_FADE_FADING,
		PORTAL_FADE_TRANSPARENT
	};

	void						DoSplinePull();
	idEntity *					GetVacuumSeparator();

	void						PrintEventMessage(idEntity *inflictor);
};
