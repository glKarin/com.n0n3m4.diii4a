...
void idPVS::CreatePassages( void ) const {
	...
	if ( passageMemory < 1024 ) {
		gameLocal.DPrintf( "%5d bytes passage memory used to build PVS\n", passageMemory );
	}
	else {
		gameLocal.DPrintf( "%5d KB passage memory used to build PVS\n", passageMemory>>10 );
	}
}
...
void idPVS::Init( void ) {
	...
	gameLocal.DPrintf( "%5u msec to calculate PVS\n", timer.Milliseconds() );
	gameLocal.DPrintf( "%5d areas\n", numAreas );
	gameLocal.DPrintf( "%5d portals\n", numPortals );
	gameLocal.DPrintf( "%5d areas visible on average\n", totalVisibleAreas / numAreas );
	if ( numAreas * areaVisBytes < 1024 ) {
		gameLocal.DPrintf( "%5d bytes PVS data\n", numAreas * areaVisBytes );
	}
	else {
		gameLocal.DPrintf( "%5d KB PVS data\n", (numAreas * areaVisBytes) >> 10 );
	}
}
