...
public:
	...
	bool					UpdateDrag( idPlayer *player, const idVec3 &origin, const idVec3 &dir );

	//BC
	void					SetRGBSliders(idVec3 color);
...
private:
	...
	float					selectionOffset; //BC for dragging mode.
	
	int						moveState;
	enum					{ EDITENT_IDLE, EDITENT_MOVING };

	bool					IsLookingAtEnt(idPlayer *player, idEntity *skip2, const idVec3 &origin, const idVec3 &dir);
};
...
public:
	...
	bool					StartDrag(idEntity *ent, jointHandle_t dragJoint, idVec3 dragPos, int _id); //Start dragging a thing.
	bool					isDragging;
	void					StopDrag(bool isThrow);
...
public:
	...
	void					SetStartingPoint(idPlayer *player);
