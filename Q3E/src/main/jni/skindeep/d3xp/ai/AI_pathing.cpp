...
============
*/
int GetObstacles(const idAI* ai, const idPhysics *physics, const idAAS *aas, const idEntity *ignore, int areaNum, const idVec3 &startPos, const idVec3 &seekPos, obstacle_t *obstacles, int maxObstacles, idBounds &clipBounds ) {
	...
	for ( i = 0; i < numListedClipModels && numObstacles < MAX_OBSTACLES; i++ ) {
		...
		if ( obEnt->IsType( idActor::Type ) ) {
			...
			// SW: Check if the obstacle is a friendly pirate, and prepare to give way to them
			if (ai != NULL && obEnt->team == ai->team && obEnt->IsType( idGunnerMonster::Type ))
			{
				idAI* otherAI = static_cast<idAI*>(obEnt);

				// Are they standing still? If so, it's our responsibility to steer around them
				if (obPhys->GetLinearVelocity().LengthFast() > 0)
				{
					// They're not standing still. We're both moving. Now: do we give way to them, or do they give way to us?
					if (ai->GetYieldPriority() > otherAI->GetYieldPriority())
					{
						// We have a higher priority than them, so we ignore them and expect them to get out of our way
						continue;
					}
				}
...
============
*/
bool idAI::FindPathAroundObstacles(const idAI* ai, const idPhysics *physics, const idAAS *aas, const idEntity *ignore, const idVec3 &startPos, const idVec3 &seekPos, obstaclePath_t &path ) {
	...
	numObstacles = GetObstacles(ai, physics, aas, ignore, areaNum, path.startPosOutsideObstacles, path.seekPosOutsideObstacles, obstacles, MAX_OBSTACLES, clipBounds );
...
bool PathTrace( const idEntity *ent, const idAAS *aas, const idVec3 &start, const idVec3 &end, int stopEvent, struct pathTrace_s &trace, predictedPath_t &path ) {
	...
	if ( !aas || !aas->GetSettings() ) {
		...
		aasTrace.getOutOfLedge = true;
		if ( stopEvent & SE_ENTER_LEDGE_AREA ) {
			aasTrace.flags |= (AREA_LEDGE | AREA_NOWALK);
			aasTrace.requireFlags |= AREA_FLOOR;
		}
		...
		if ( clipTrace.fraction >= 1.0f ) {
			...
			if ( aasTrace.fraction < 1.0f ) {
				if ( stopEvent & SE_ENTER_LEDGE_AREA ) {
					if ( aas->AreaFlags( aasTrace.blockingAreaNum ) & (AREA_LEDGE | AREA_NOWALK)) {
...
=====================
*/
bool idAI::TestTrajectory( const idVec3 &start, const idVec3 &end, float zVel, float gravity, float time, float max_height, const idClipModel *clip, int clipmask, const idEntity *ignore, const idEntity *targetEntity, int drawtime, bool hasSolidTarget ) {
	int i, numSegments;
	...
	for ( i = 0; i < numSegments; i++ ) {
		...
		if ( trace.fraction < 1.0f ) {
			...
			{
				if (hasSolidTarget) //BC
				{
					result = false;
				}
				else
				{
					//BC If we're throwing at a position, not a target, then check to see if obstacle position is close enough to target destination.
					float distToTarget = (trace.endpos - end).LengthSqr();
...
=====================
*/
bool idAI::PredictTrajectory( const idVec3 &firePos, const idVec3 &target, float projectileSpeed, const idVec3 &projGravity, const idClipModel *clip, int clipmask, float max_height, const idEntity *ignore, const idEntity *targetEntity, int drawtime, idVec3 &aimDir, bool hasSolidTarget) {
	int n, i, j;
	...
	for ( i = 0; i < n; i++ ) {
		...
		if ( TestTrajectory( firePos, target, zVel, projGravity[2], ballistics[i].time, firePos.z + max_height, clip, clipmask, ignore, targetEntity, drawtime, hasSolidTarget) ) {
			aimDir = dir[i];
			return true;
		}
	}
