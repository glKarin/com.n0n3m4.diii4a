...
void idAASLocal::CalculateAreaTravelTimes(void) {
	...
	for ( n = 0; n < file->GetNumAreas(); n++ ) {
		...
		for ( maxt = i = 0, reach = file->GetArea( n ).reach; reach; reach = reach->next, i++ ) {
			...
			if ( i >= MAX_REACH_PER_AREA ) {
				gameLocal.Warning( "PATHFINDING: i >= MAX_REACH_PER_AREA" );
			}
			reach->number = i;
	...
	assert(  (unsigned long long)( ( ptrdiff_t ) bytePtr - ( ptrdiff_t ) areaTravelTimes ) <= numAreaTravelTimes * sizeof( unsigned short ) );
}
...
bool idAASLocal::FindNearestGoal( aasGoal_t &goal, int areaNum, const idVec3 origin, const idVec3 &target, int travelFlags, aasObstacle_t *obstacles, int numObstacles, idAASCallback &callback ) const {
	...
	while ( updateListStart )
	{
		...
		for ( i = 0, reach = file->GetArea( curUpdate->areaNum ).reach; reach; reach = reach->next, i++ )
		{
			...
			if ( !( nextArea->flags & AREA_LEDGE ) )
			{
				...
				{
					//BC
					idVec3 candidatePos = AreaCenter(reach->toAreaNum);
					//gameRenderWorld->DebugArrow(colorRed, candidatePos + idVec3(0, 0, 32), candidatePos, 8, 5000);
	...
	if ( bestAreaNum )
	{
		...
		//BC debug.
		//common->Printf("AI: Found a MoveToCover location!\n");
		//gameRenderWorld->DebugArrow(colorCyan, goal.origin + idVec3(0, 0, 256), goal.origin, 8, 5000);
