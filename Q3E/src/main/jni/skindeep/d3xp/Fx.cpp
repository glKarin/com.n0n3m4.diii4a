...
//BC amount of time for force-fadeout.
const int FORCEFADETIME = 2000;
...
void idEntityFx::FadeOut(void)
{
	forcefadeStartTime = gameLocal.time;
}
...
//Force the FX to fade out.
void idEntityFx::ForceFade(const idFXSingleAction& fxaction, idFXLocalAction& laction, const int time)
{
	float fadePct = FORCEFADETIME - (time - forcefadeStartTime);

	if (fadePct < 0)
		fadePct = 0;

	fadePct = fadePct / (float)FORCEFADETIME;

	if (laction.lightDefHandle != -1)
	{
		//LIGHTS.
		laction.renderLight.shaderParms[SHADERPARM_RED] = fxaction.lightColor.x * fadePct;
		laction.renderLight.shaderParms[SHADERPARM_GREEN] = fxaction.lightColor.y * fadePct;
		laction.renderLight.shaderParms[SHADERPARM_BLUE] = fxaction.lightColor.z * fadePct;
		gameRenderWorld->UpdateLightDef(laction.lightDefHandle, &laction.renderLight);
	}
	if (laction.modelDefHandle != -1)
	{
		//PARTICLES.
		laction.renderEntity.shaderParms[SHADERPARM_ALPHA] =   fadePct;
		gameRenderWorld->UpdateEntityDef(laction.modelDefHandle, &laction.renderEntity);
	}

	if (fadePct <= 0)
	{
		//If fade done, then stop this FX.
		forcefadeStartTime = 0;
		Stop();
	}
}
	...
	for( ieff = 0; ieff < fxEffect->events.Num(); ieff++ )
	{
		...
		if ( pct >= 1.0f )
		{
			...
			if ( fxaction.restart )
			{
				...
				//Delay is called here. Restart the effect.
				//common->Printf("Looping %d\n", gameLocal.time);
				if (fxaction.fire.Length())
				{
					for (j = 0; j < fxEffect->events.Num(); j++)
					{
						...
						{
							actions[j].soundStarted = false; //BC add this so fx can loop?????
							actions[j].start = time; //BC add this so fx can loop?????
		...
		switch( fxaction.type ) {
			...
			case FX_LIGHT: {
				if ( useAction->lightDefHandle == -1 ) {
					if ( fxaction.type == FX_LIGHT ) {
						...
						if (fxaction.relativeoffset != vec3_zero)
						{
							idVec3 forward, up, right;
							GetPhysics()->GetAxis().ToAngles().ToVectors(&forward, &right, &up);
							useAction->renderLight.origin +=
								(forward * fxaction.relativeoffset.x) + (right * fxaction.relativeoffset.y) + (up * fxaction.relativeoffset.z);
						}

						//gameRenderWorld->DebugArrow(colorGreen, GetPhysics()->GetOrigin(), useAction->renderLight.origin, 2, 10000);
				...
				{
					//BC attach light FXs to moving objects.
					if (fxaction.trackOrigin)
					{
						useAction->renderLight.origin = GetPhysics()->GetOrigin() + fxaction.offset;
						//useAction->renderLight.axis = fxaction.explicitAxis ? fxaction.axis : GetPhysics()->GetAxis();
						useAction->renderLight.axis = fxaction.axis ;
				...
				if (forcefadeStartTime > 0)
				{
					ForceFade(fxaction, *useAction, time);
				}

				break;
			}
			case FX_SOUND: {
				//common->Printf("%d attempt sound\n", gameLocal.time);
			...
			case FX_DECAL: {
				if ( !useAction->decalDropped )
				{
					...
					//BC do a traceline down
					if (fxaction.explicitAxis)
					{
						//Arg is 1. Spawn decal based on angle.
						idAngles traceDir = this->GetPhysics()->GetAxis().ToAngles();
						traceDir.pitch += 90;

						idVec3 startPos = this->GetPhysics()->GetOrigin() + (traceDir.ToForward() * -0.1f);
						idVec3 endPos = startPos + (traceDir.ToForward() * 48);						

						gameLocal.clip.TracePoint(floorPos, startPos, endPos, MASK_SOLID, this);
						//gameRenderWorld->DebugArrow(colorGreen, this->GetPhysics()->GetOrigin(), floorPos.endpos, 1, 10000);
					}
					else
					{
						//Arg is 0. Spawn decal on ground.
						gameLocal.clip.TracePoint(floorPos, this->GetPhysics()->GetOrigin(), this->GetPhysics()->GetOrigin() + idVec3(0, 0, -64), MASK_SOLID, this);
					}
					
					//Uncomment this to see where the decal is attempting to spawn.
					//gameRenderWorld->DebugArrow(colorGreen, this->GetPhysics()->GetOrigin(), floorPos.endpos, 4, 10000);

					if (floorPos.fraction < 1)
					{
						gameLocal.ProjectDecal(floorPos.endpos, -floorPos.c.normal, 8.0f, true, fxaction.size, fxaction.data);
					}
					else
					{
			...
			case FX_MODEL: {
				if ( useAction->modelDefHandle == -1 ) {
					...
					useAction->renderEntity.shaderParms[ SHADERPARM_PARTICLE_GRAVITYSCALE ] = 1.0f; // SM
					...
					//BC allow the FX to be angled relative to the bone direction.
					if (fxaction.relativeAngle.x != 0 || fxaction.relativeAngle.y != 0 || fxaction.relativeAngle.z != 0)
					{
						idStr jointname = Joint();
						if (jointname && jointname.Length() > 0)
						{
							jointHandle_t bindJoint;
							idVec3 bindPos;
							idMat3 bindMat3;
							bindJoint = GetBindMaster()->GetAnimator()->GetJointHandle(jointname);
							((idAnimatedEntity *)GetBindMaster())->GetJointWorldTransform(bindJoint, gameLocal.time, bindPos, bindMat3);

							idAngles particleAngle = bindMat3.ToAngles();
							particleAngle.pitch += fxaction.relativeAngle.x;
							particleAngle.yaw += fxaction.relativeAngle.y;
							particleAngle.roll += fxaction.relativeAngle.z;

							useAction->renderEntity.axis = particleAngle.ToMat3();
						}
					}
				...
				else if ( fxaction.trackOrigin )
				{
					...
					if (jointname && jointname.Length() > 0)
					{
						...
						useAction->renderEntity.origin = bindPos;
						useAction->renderEntity.axis = bindMat3;
...
idEntityFx::~idEntityFx() {
	...
	// SM: Just in case, make sure any scheduled events for this entity are canceled,
	// as this may be causing an occasional crash bug
...
idEntityFx *idEntityFx::StartFx( const char *fx, const idVec3 *useOrigin, const idMat3 *useAxis, idEntity *ent, bool bind ) {
	...
	if (fx[0] == '\0') //if fx string is empty...
		return NULL;
...
================
idEntityFx::StartFx
SM: Added version that doesn't take in an entity and takes in an origin/axis
================
*/
idEntityFx *idEntityFx::StartFx( const char *fx, const idVec3 &useOrigin, const idMat3 &useAxis, idEntity *ent /*= NULL*/, bool bind /*= false*/ )
{
	return StartFx( fx, &useOrigin, &useAxis, ent, bind );
}
