...
extern const idEventDef EV_Start_Vent;
extern const idEventDef EV_Stop_Vent;
...
public:
	...
	idMat3					GetWorldModelAxis(void); // DW
	...
	idEntity *				DropItem( const idVec3 &velocity, int activateDelay, int removeDelay, bool died, bool combatThrow );
	...
	idVec3					GetMuzzlePosition(bool viewModel);
	idVec3					GetJointPosition(jointHandle_t jointName);

	void					RackTheSlide();
	void					InspectMagazine(bool active);
	bool					IsInspectingMagazine();

	void					InspectChamber(bool active);
	bool					IsInspectingChamber();
	...
	int						Event_RoundIsChambered(void);

	bool					IsReadyToBash(void) const;
	bool					IsRacking(void) const;

	void					SetAimMode(bool value);

	bool					IsAttacking(); //is currently in the middle of an attack.

    idVec3                  GetThrowSpawnPosition();
...
private:
	...
	idScriptBool			WEAPON_RACKTHESLIDE;
	idScriptBool			WEAPON_INSPECTMAGAZINE;
	idScriptBool			WEAPON_INSPECTCHAMBER;
	idScriptBool			WEAPON_INSPECTWEAPON;

	idScriptBool			WEAPON_AIMMODE;
	...
	void					Event_LaunchProjectiles( int num_projectiles, float spread, float fuseOffset, float launchPower, float dmgPower, float chamberNewRound );
	...
	void					Event_SetChambered(float value);
	void					Event_EjectLiveRound(void);
	void					Event_GetJiggleState(void);
	void					Event_StartVent(void);
	void					Event_StopVent(void);

	void					Event_SetNozzleRadius(float radius);
	bool					nozzleLerping;
	int						nozzleLerpTimer;
	float					nozzleLerpValueStart;
	float					nozzleLerpValueEnd;
	bool					nozzleGlowAffectLightMeter;

	
	idDict					singleAmmoDict;

	void					Event_SetClipAmount(int amount);

	int						lastEndattack; //time since last attack ended.

	bool					Event_UnloadMag();
	
	void					CheckPlayerFootShot();
