...
void idTestModel::TestModel_f( const idCmdArgs &args ) {
	...
	//BC do traceline to place model on ground.
	trace_t trForward;
	gameLocal.clip.TracePoint(trForward, player->firstPersonViewOrigin, player->firstPersonViewOrigin + player->viewAngles.ToForward() * 100, MASK_SOLID, NULL);

	trace_t trDown;
	gameLocal.clip.TracePoint(trDown, trForward.endpos, trForward.endpos + idVec3(0,0,-300), MASK_SOLID, NULL);

	offset = trDown.endpos;
