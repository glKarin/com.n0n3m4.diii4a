...
// blendo eric: moved channel names to anim.h
...
=====================
*/
const char *idAnim::AddFrameCommand( const idDeclModelDef *modelDef, int framenum, idLexer &src, const idDict *def, int animChannel /* blendo eric: 0 = all channels */) {
	int					i;
	int					index;
	idStr				text;
	idStr				funcname;
	...
	fc.channel = animChannel; // blendo eric
	...
	else if (token == "sound_vo") //BC vomanager system. Use this to hook up an animation event to a sound cue.
	{
		if (!src.ReadTokenOnLine(&token))
		{
			return "Unexpected end of line";
		...
		{			
			gameLocal.Error("AnimEvent: sound_vo '%s' needs to be a 'snd_' sound cue.", token.c_str());
		}

		if (!src.ReadTokenOnLine(&token))
		{
			return "AnimEvent: unexpected end of line (sound_vo)";
		}
		
		fc.index = atoi(token); //BC this isn't great, but we're just using the integer of the VO_CATEGORY_* values.
	}
	else if ( token == "skin" ) {
		if( !src.ReadTokenOnLine( &token ) ) {
			return "Unexpected end of line";
	...
	else if (token == "particle") //BC
	{
		...
		str += token;
		fc.type = FC_PARTICLE;
		fc.string = new idStr(str); //particle name.
		fc.index = jointInfo->num;  //joint to fire the effect.
	...
	else if (token == "launch_atlaser") //BC fire projectile at the laser position.
	{
		if (!src.ReadTokenOnLine(&token)) {
			return "Unexpected end of line";
		}
		...
		fc.type = FC_LAUNCHMISSILEATLASER;
		fc.string = new idStr(token);
	}
	else if (token == "jointmarker")
	{
		if (!src.ReadTokenOnLine(&token)) {
			return "Unexpected end of line";
		}
		...
		fc.type = FC_JOINTMARKER;
		fc.string = new idStr(token);
	}
	else if (token == "ejectbrass") //BC
	{
		fc.type = FC_EJECTBRASS;
	}
	...
	else if (token == "glassyank") //BC
	{
		//yank a piece of glass out of foot.
		fc.type = FC_GLASSYANK;
	}
	//else if (token == "bulletpluck")
	//{
	//	fc.type = FC_BULLETPLUCK; //BC this is now consolidated into the healwound call.
	//}	
	else if (token == "healwound")
	{
		//BC
		if (!src.ReadTokenOnLine(&token))
		{
		...
		fc.type = FC_HEALWOUND;
		fc.string = new idStr(token);
	}
	else if (token == "camspurt") //BC spurt something out of player camera direction. i.e. blood spurt when healing wound
	{
		if (!src.ReadTokenOnLine(&token))
		{
			return "Unexpected end of line";
	...
	else if (token == "leftfootrun")
	{
		fc.type = FC_LEFTFOOTRUN;
	}
	else if (token == "rightfootrun")
	{
		fc.type = FC_RIGHTFOOTRUN;
	}
	else if (token == "stopvent")
	{
		fc.type = FC_STOP_VENT;
	}
	else if (token == "startvent")
	{
		fc.type = FC_START_VENT;
	}
	else if (token == "dodamage")
	{
		if (!src.ReadTokenOnLine(&token))
		{
			return "Unexpected end of line";
	...
	else if (token == "hide") //BC
	{
		fc.type = FC_HIDE;
	}
	else if (token == "show") //BC
	{
		fc.type = FC_SHOW;
	}
	else if (token == "jockeyslam") //BC
	{
		fc.type = FC_JOCKEYSLAM;
	}
	else if (token == "worlddamage") //BC
	{
		fc.type = FC_WORLDDAMAGE;
	}
	else if (token == "interestpoint") // SW
	{
		if (!src.ReadTokenOnLine(&token))
		{
			return "Unexpected end of line";
		}
		fc.type = FC_INTERESTPOINT;
		fc.string = new idStr(token);
	}
	else
	{
		//BC bad animevent is a pretty nasty bug. So just completely stop the game when this happens.
#ifdef _DEBUG
		common->Error("Unknown command '%s'", token.c_str());
#endif
...
=====================
*/
void idAnim::CallFrameCommands( idEntity *ent, int from, int to, int animChannel /* blendo eric: added channel option, 0 = all channels */) const {
	int index;
	int end;
	int frame;
	int numframes;
	...
	while( frame != to ) {
		...
		while( index < end ) {
			...
			// blendo eric: check that the frame command is a matching animation channel, 0 = all channels
			if (animChannel && command.channel && (command.channel != animChannel)) {
				continue;
			}
			...
			switch( command.type ) {
				...
				case FC_PARTICLE:
				{
					...
					//This particle code only spawns on the first frame.
					//const idDeclParticle *smokeFly = NULL;
					//smokeFly = static_cast<const idDeclParticle *>(declManager->FindType(DECL_PARTICLE, command.string->c_str()));
					//if (smokeFly)
					//{
					//	jointHandle_t jointNum;
					//	jointNum = ent->GetAnimator()->GetJointHandle(modelDef->GetJointName(command.index));
					//
					//	idVec3 offset;
					//	idMat3 axis;
					//
					//	//ent->GetAnimator()->GetJointTransform( jointNum, gameLocal.time, offset, axis );
					//
					//	ent->GetAnimator()->GetJointTransform(jointNum, gameLocal.time, offset, axis);
					//	offset = ent->GetPhysics()->GetOrigin() + offset * ent->GetPhysics()->GetAxis();
					//	axis = axis * ent->GetPhysics()->GetAxis();
					//
					...
					if (smokeFly)
					{
						...
						if (!ent->GetAnimator()->GetJointTransform(jointNum, gameLocal.time, offset, axis))
							return;
				...
				case FC_HEALWOUND:
				{
					if (command.string)
					{
						...
						{
							static_cast<idPlayer*>(ent)->HealWound(command.string->c_str());
						}
					}
					else
					{
						gameLocal.Error("Healwound animation event is missing an argument.");
					}
				...
				case FC_CAMSPURT:
				{
					if (command.string)
					{
						gameLocal.GetLocalPlayer()->SetCamSpurt(command.string->c_str());						
					}
					else
					{
						gameLocal.Error("Camspurt animation event is missing an argument.");
					}
				...
				//	if (ent->IsType(idPlayer::Type))
				//	{
				//		static_cast<idPlayer*>(ent)->DoBulletpluck();
				//	}
				//	break;
				//}
				...
				case FC_GLASSYANK:
				{
					if (ent->IsType(idPlayer::Type))
					{
						static_cast<idPlayer*>(ent)->DoGlassYankShard();
					}
					break;
				}
				...
				case FC_LEFTFOOTRUN:
				{
					ent->ProcessEvent(&EV_FootstepLeftRun);
					break;
				}
				case FC_RIGHTFOOTRUN:
				{
					ent->ProcessEvent(&EV_FootstepRightRun);
					break;
				}
				case FC_START_VENT:
				{
					ent->ProcessEvent(&EV_Start_Vent);
					break;
				}
				case FC_STOP_VENT:
				{
					ent->ProcessEvent(&EV_Stop_Vent);
					break;
				}

				case FC_LAUNCHMISSILEATLASER: {
					ent->ProcessEvent(&AI_LaunchMissileAtLaser, command.string->c_str());
					break;
				}

				case FC_DODAMAGE:
				{
					ent->ProcessEvent(&AI_DoDamage, command.string->c_str());
					break;
				}

				case FC_EJECTBRASS: {
					ent->ProcessEvent(&AI_EjectBrass);
					break;
				}
				...
				case FC_JOCKEYSLAM:
				{
					...
					{
						static_cast<idAI *>(ent)->DoJockeySlamDamage();
					}
					break;
				}
				...
				case FC_WORLDDAMAGE:
				{
					if (ent->IsType(idAI::Type))
					{
						static_cast<idAI *>(ent)->DoWorldDamage();
					}
					break;
				}
				...
				case FC_SOUND_VO:
				{
					...
					{
						//command.index = vo category, defined in VO_CATEGORY_* in bc_vomanager.h
						gameLocal.voManager.SayVO(ent, command.string->c_str(), command.index);					
						//gameLocal.Warning("Framecommand 'sound_voice' on entity '%s', anim '%s', frame %d: Could not find sound '%s'", ent->name.c_str(), FullName(), frame + 1, command.string->c_str());						
					}					
					break;
				}

				case FC_INTERESTPOINT:
				{
					if (command.string->Length() > 0)
					{
						gameLocal.SpawnInterestPoint(ent, ent->GetPhysics()->GetOrigin() + idVec3(0, 0, 1), command.string->c_str());
					}
					else
					{
						gameLocal.Error("Frame command FC_INTERESTPOINT has empty parameter.");
					}
					break;
				}
			}
...
=====================
*/
void idAnimBlend::CallFrameCommands( idEntity *ent, int fromtime, int totime, int channel /* blendo eric: added channel for frame commands (0=all) */ ) const {
	const idMD5Anim	*md5anim;
	frameBlend_t	frame1;
	frameBlend_t	frame2;
	int				fromFrameTime;
	...
	// blendo eric: added channel for frame commands
	if ( fromFrameTime <= 0 ) {
		// make sure first frame is called
		anim->CallFrameCommands( ent, -1, frame2.frame1, channel ); 
	} else {
		anim->CallFrameCommands( ent, frame1.frame1, frame2.frame1, channel );
	}
}
...
=====================
*/ // blendo eric: original signature
bool idAnimBlend::BlendAnim( int currentTime, int channel, int numJoints,
							idJointQuat *blendFrame, float &blendWeight,
							bool removeOriginOffset, bool overrideBlend, bool printInfo ) const {

	const int *jointIndices = modelDef->GetChannelJoints( channel );
	const int jointIndicesNum = modelDef->NumJointsOnChannel( channel );

	return BlendAnim( currentTime, channel, numJoints,
					jointIndices, jointIndicesNum,
					blendFrame, blendWeight, removeOriginOffset, overrideBlend, printInfo );
}
...
idAnimBlend::BlendAnim
=====================
*/ // blendo eric: overriden signature, not req atm, but might be used later
bool idAnimBlend::BlendAnim( int currentTime, int channel, int numJoints,
							const int *jointIndices, const int jointIndicesNum,
							idJointQuat *blendFrame, float &blendWeight, 
							bool removeOriginOffset, bool overrideBlend, bool printInfo ) const {
	int				i;
	float			lerp;
	...
	if ( numAnims == 1 ) {
		md5anim = anim->MD5Anim( 0 );
		if ( frame ) {
			md5anim->GetSingleFrame( frame - 1, jointFrame, jointIndices, jointIndicesNum );
		} else {
			md5anim->ConvertTimeToFrame( time, cycle, frametime );
			md5anim->GetInterpolatedFrame( frametime, jointFrame, jointIndices, jointIndicesNum );
		}
	} else {
		//
		...
		for( i = 0; i < numAnims; i++ ) {
			if ( animWeights[ i ] > 0.0f ) {
				...
				if ( frame ) {
					md5anim->GetSingleFrame( frame - 1, ptr, jointIndices, jointIndicesNum );
				} else {
					md5anim->GetInterpolatedFrame( frametime, ptr, jointIndices, jointIndicesNum );
				}

				// only blend after the first anim is mixed in
				if ( ptr != jointFrame ) {
					SIMDProcessor->BlendJoints( jointFrame, ptr, lerp, jointIndices, jointIndicesNum );
				}
	...
	if ( !blendWeight ) {
		...
		SIMDProcessor->BlendJoints( blendFrame, jointFrame, lerp, jointIndices, jointIndicesNum );
	}

	if ( printInfo ) {
		if ( frame ) {
			gameLocal.Printf( "  %s: '%s', %d, %.2f%%\n", ANIMCHANNEL_Names[ channel ], anim->FullName(), frame, weight * 100.0f );
		} else {
			gameLocal.Printf( "  %s: '%s', %.3f, %.2f%%\n", ANIMCHANNEL_Names[ channel ], anim->FullName(), ( float )frametime.frame1 + frametime.backlerp, weight * 100.0f );
		}
	}
...
#if defined(_DEBUG)
	// blendo eric: check if the move delta is faster than an arbitrary max
	// which may indicate that the model's origin joint is not starting from 0 when walking
	static idList<const char *> DEBUG_animsChecked = idList<const char *>();
	if( gameLocal.msec < 100 && DEBUG_animsChecked.Num() < 4 && (!AnimName() || DEBUG_animsChecked.FindIndex(AnimName()) >= 0) ) {
		if(AnimName()){
			DEBUG_animsChecked.Append(AnimName());
		}
		const float DEBUG_ANIM_MOVE_MAX =  (10.0f * METERS_TO_DOOM);
...
void idAnimBlend::BlendDeltaRotation( int fromtime, int totime, idQuat &blendDelta, float &blendWeight ) const {
	...
	for( i = 0; i < num; i++ ) {
		if ( animWeights[ i ] > 0.0f ) {
			...
			if ( animWeights[ i ] == mixWeight ) {
				...
				ANIMSLERP( q1, q1, q3, lerp );
				anim->GetOriginRotation( q3, i, time2, cycle );
				ANIMSLERP( q2, q1, q3, lerp );
			}
		}
	}
	...
	if ( !blendWeight ) {
		...
		lerp = weight / ( blendWeight + weight );
		ANIMSLERP( blendDelta, blendDelta, q3, lerp );
		blendWeight += weight;
	}
}
...
void idDeclModelDef::CopyDecl( const idDeclModelDef *decl ) {
	...
	skin = decl->skin;
	overrideSkinInSubview = decl->overrideSkinInSubview;
...
void idDeclModelDef::GetJointList( const char *jointnames, idList<jointHandle_t> &jointList ) const {
	...
	while( *pos ) {
		...
		if ( !joint ) {
			gameLocal.Error( "Unknown joint '%s' in '%s' for model '%s'", jointname.c_str(), jointnames, GetName() ); //BC was warning, now error
			continue;
		}
...
/*
=====================
idDeclModelDef::GetOverrideSkinInSubview
=====================
*/
const idDeclSkin* idDeclModelDef::GetOverrideSkinInSubview(void) const {
	return overrideSkinInSubview;
}
...
bool idDeclModelDef::ParseAnim( idLexer &src, int numDefaultAnims ) {
	...
	int selectedAnimChannel = 0; // blendo eric, default 0 = all anim channels
	...
	if ( src.CheckTokenString( "{" ) ) {
		while( 1 ) {
			...
			if ( token == "}" ) {
			...
			} else if (token == "framechannel") {
				// blendo eric: anim channel option, reuse channel name finding code
				...
				for (newChannel = ANIMCHANNEL_ALL + 1; newChannel < ANIM_NumAnimChannels; newChannel++) {
					if (!idStr::Icmp(ANIMCHANNEL_Names[newChannel], token)) {
						break;
					}
				}

				if (newChannel >= ANIM_NumAnimChannels) {
					src.Warning("def 'channel' Unknown channel '%s'", token.c_str());
					MakeDefault();
					gameLocal.Error("");
					return false;
				}

				selectedAnimChannel = newChannel;
				...
				err = anim->AddFrameCommand( this, framenum, src, NULL, selectedAnimChannel); // blendo eric: added anim channel option
				if ( err ) {
					src.Error( "%s", err );					
					MakeDefault();
					gameLocal.Error(""); //BC error out.
...
bool idDeclModelDef::Parse( const char *text, const int textLength ) {
	...
	while( 1 ) {
		...
		if ( token == "inherit" ) {
		...
		} else if ( token == "overrideSkinInSubview" ) {
			if( !src.ReadToken( &token2 ) ) {
				src.Warning( "Unexpected end of file" );
				MakeDefault();
				return false;
			}
			overrideSkinInSubview = declManager->FindSkin( token2 );
			if ( !overrideSkinInSubview ) {
				src.Warning( "OverrideSkinInSubview '%s' not found", token2.c_str() );
				MakeDefault();
				return false;
			}
		} else if ( token == "mesh" ) {
			...
			for( i = ANIMCHANNEL_ALL + 1; i < ANIM_NumAnimChannels; i++ ) {
				if ( !idStr::Icmp( ANIMCHANNEL_Names[ i ], token2 ) ) {
					break;
				}
			}
...
void idAnimator::ServiceAnims( int fromtime, int totime ) {
	...
	if ( modelDef->ModelHandle() ) {
		...
		for( i = 0; i < ANIM_NumAnimChannels; i++ ) {
			for( j = 0; j < ANIM_MaxAnimsPerChannel; j++, blend++ ) {
				blend->CallFrameCommands( entity, fromtime, totime, i ); // blendo eric: added channel for frame commands
			}
		}
	}
...
bool idAnimator::CreateFrame( int currentTime, bool force ) {
	...
	// blendo eric: alter calls to BlendAnim to exclude bridge joints from being blended (twice)
	...
	if ( baseBlend < 1.0f ) {
		for( i = ANIMCHANNEL_ALL + 1; i < ANIM_NumAnimChannels; i++ ) {
			...
			blend = channels[ i ];
			for( j = 0; j < ANIM_MaxAnimsPerChannel; j++, blend++ ) {
				if ( blend->BlendAnim( currentTime, i, numJoints, jointFrame, blendWeight,
										removeOriginOffset, false, debugInfo ) ) {
					hasAnim = true;
					if ( blendWeight >= 1.0f ) {
						// fully blended
						break;
			...
			if ( debugInfo && !AFPoseJoints.Num() && !blendWeight ) {
				gameLocal.Printf( "%d: %s using default pose in model '%s'\n", gameLocal.time, ANIMCHANNEL_Names[ i ], modelDef->GetModelName() );
			}
		}
	}
	...
	if ( modelDef->NumJointsOnChannel( ANIMCHANNEL_EYELIDS ) ) {
		...
		blendWeight = baseBlend;
		for( j = 0; j < ANIM_MaxAnimsPerChannel; j++, blend++ ) {
			if ( blend->BlendAnim( currentTime, ANIMCHANNEL_EYELIDS, numJoints, jointFrame, blendWeight,
									removeOriginOffset, true, debugInfo ) ) {
				hasAnim = true;
				if ( blendWeight >= 1.0f ) {
					// fully blended
					break;
	...
	#if defined(_DEBUG) // blendo eric: check for unblended joints (except origin), which can cause subtle glitches
		static int DEBUG_unblended_count = 0;
		if(DEBUG_unblended_count < 3 && entity->IsType( idActor::Type ) && currentTime > 0) {
			for( int defJointIndex = 1; defJointIndex < numJoints && DEBUG_unblended_count < 3 ; ++defJointIndex ) {
				if( static_cast<const idJointQuat>(jointFrame[defJointIndex]) == defaultPose[defJointIndex] ) {
					int chan = modelDef->GetJoint(defJointIndex)->channel;
					blend = channels[chan];
					gameLocal.Warning( "Model '%s' joint '%s' chan '%s'  has unblended animation '%s' (default pose)",
						modelDef->GetName(), modelDef->GetJointName(defJointIndex), ANIMCHANNEL_Names[chan], blend->AnimName());
					DEBUG_unblended_count++;
				}
			}
		}
	#endif
	...
	if ( !hasAnim && !jointMods.Num() ) {
		...
		#if defined(_DEBUG) // blendo eric: report missing anims
			if(entity->IsType( idActor::Type ) && currentTime > 0) {
				gameLocal.Warning( "Ent '%s' Model '%s' had no animation (%dt)", 
					modelDef->GetName(), entity ? entity->GetName() : "", currentTime );
			}
		#endif
