...
static const char* ANIMCHANNEL_Names[ANIM_NumAnimChannels] = {
...
// blendo eric: fast quaternion slerp, just a nlerp, normalized linear interp
#define _FAST_QUAT_SLERP_
#if !defined( ANIMSLERP )
#if defined( _FAST_QUAT_SLERP_ )
	#include "idlib/math/Quat.h"

	#if defined(_DEBUG)
		static int ANIM_SLERP_COUNTER = 0;
		static void RESET_SLERP_COUNTER(){ ANIM_SLERP_COUNTER = 0; }
	#endif

	ID_INLINE void ANIM_NLERP( float * out, const float * from, const float * to, float t ) {
		if ( t < 0.0f ) {
			...
			float tinv = (1-t);
			out[0] = from[0]*tinv + to[0]*t;
			out[1] = from[1]*tinv + to[1]*t;
			out[2] = from[2]*tinv + to[2]*t;
			out[3] = from[3]*tinv + to[3]*t;
		}
		#if defined(_DEBUG)
			ANIM_SLERP_COUNTER++;
		#endif
	...
	#define ANIMSLERP( out, from, to, t ) ANIM_NLERP(out.ToFloatPtr(), from.ToFloatPtr(), to.ToFloatPtr(), t)
#else
	#define ANIMSLERP( out, from, to, t ) out.Slerp(from,to,t)
#endif
#endif
...
typedef enum {
	...
	FC_STOP_VENT,
	FC_PARTICLE,
	FC_LAUNCHMISSILEATLASER,
	FC_DODAMAGE,
	FC_EJECTBRASS,
	FC_JOINTMARKER,				//for getting CanHitFromAnim info from an animation.
	FC_HIDE,
	FC_SHOW,
	FC_JOCKEYSLAM,
	FC_WORLDDAMAGE,
...
} frameCommandType_t;		//BC note: the last item in the list always needs a comma.
...
typedef struct {
	...
	idStr					*string;
	int						channel; // blendo eric: allow command to only be called on one animation channel
...
public:
	...
	const char					*AddFrameCommand( const class idDeclModelDef *modelDef, int framenum, idLexer &src, const idDict *def, int animChannel ); // blendo eric: added frame anim channel option
	void						CallFrameCommands( idEntity *ent, int from, int to, int animChannel ) const; // blendo eric: added frame anim channel option
...
public:
	...
	const idDeclSkin*			GetOverrideSkinInSubview(void) const;
...
private:
	...
	idList<idAnim *>			anims;
	const idDeclSkin *			skin;
	const idDeclSkin*			overrideSkinInSubview;
};
...
private:
	...
	void						CallFrameCommands( idEntity *ent, int fromtime, int totime, int channel /* blendo eric: added channel for frame commands, 0 = all */) const;
	...
	// blendo eric: added override for blendanim to fix bug, never actually used
	bool						BlendAnim( int currentTime, int channel, int numJoints,
									idJointQuat *blendFrame, float &blendWeight,
									bool removeOrigin, bool overrideBlend, bool printInfo ) const;
	bool						BlendAnim( int currentTime, int channel, int numJoints,
