...
#ifdef FAST_QUAT_SLERP
	FastSlerp( q1, q2, frame.backlerp, rotation);
#else
...
void idMD5Anim::GetInterpolatedFrame( frameBlend_t &frame, idJointQuat *joints, const int *index, int numIndexes ) const {
	...
	// copy the baseframe
	// blendo eric: todo double copy in idAnimator::CreateFrame( int currentTime, bool force )?
...
void idMD5Anim::CheckModelHierarchy( const idRenderModel *model ) const {
	...
	for( i = 0; i < jointInfo.Num(); i++ ) {
		...
		if ( modelJoints[ i ].name != animationLib.JointName( jointNum ) ) {
			gameLocal.Error( "Model '%s' joint names don't match anim '%s' ('%s', '%s')", model->Name(), name.c_str(),
				modelJoints[i].name.c_str(), animationLib.JointName(jointNum));
		}
