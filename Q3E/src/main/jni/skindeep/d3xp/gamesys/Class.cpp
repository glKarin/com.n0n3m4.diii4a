...
classSpawnFunc_t idClass::CallSpawnFunc( idTypeInfo *cls ) {
	...
	assert(cls->Spawn); // blendo eric: trying to catch mystery crash
...
bool idClass::PostEventArgs( const idEventDef *ev, int time, int numargs, ... ) {
	...
	if ( isRemoved ) {
		gameLocal.Warning( "Skipping post event on removed object." );
		return false;
	}
...
void idClass::WarningMsg( const char* msg )
{
	gameLocal.Warning( msg );
}
