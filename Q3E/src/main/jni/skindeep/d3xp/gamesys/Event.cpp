...
#define MAX_EVENTSPERFRAME			4096 //BC default = 4096
...
void idEvent::ServiceEvents( void ) {
	...
	while( !EventQueue.IsListEmpty() ) {
		...
		if ( !event->object->IsRemoved() ) {
			...
			gameLocal.Warning( "idEvent::ServiceEvents: Skipping event on removed object." );
		}
		...
		if ( num > MAX_EVENTSPERFRAME ) {
			gameLocal.Error( "Event overflow.  Possible infinite loop in script.\nIf this occurs on map load, you may have too many entities emitting postspawn events." );
		}
	}
}
...
void idEvent::Shutdown( void ) {
	gameLocal.DPrintf( "Shutdown event system\n" );

	if ( !initialized ) {
		gameLocal.DPrintf( "...not started\n" );
		return;
	}
