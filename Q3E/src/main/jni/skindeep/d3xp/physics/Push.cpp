...
int idPush::TryTranslatePushEntity( trace_t &results, idEntity *check, idClipModel *clipModel, const int flags,
	...
	if ( physics->IsGroundClipModel( clipModel->GetEntity()->entityNumber, clipModel->GetId() ) ) {
		...
		if ( trace.fraction < 1.0f ) {
			...
			// Certain complex objects (e.g. ragdolls) can be a problem for pushers (e.g. doors) because of how they get wedged.
			// We override the blocking behaviour here and count on other systems to bail them out (see idDoor::DisplacePotentialBlockage)
			if (physics->GetNeverBlock())
			{
				return PUSH_OK;
			}
...
int idPush::DiscardEntities( idEntity *entityList[], int numEntities, int flags, idEntity *pusher )
{
	...
	for ( num = i = 0; i < numEntities; i++ )
	{
		...
		//BC CRASH. See also: idEntity::TouchTriggers()
		if (check->IsHidden() || check->entityNumber < 0 || check->entityNumber >= MAX_GENTITIES - 1 || check->entityDefNumber < -1 /* || check->GetPhysics() == NULL || check->team >= TEAM_LAST*/)
		{
			continue;
		}
