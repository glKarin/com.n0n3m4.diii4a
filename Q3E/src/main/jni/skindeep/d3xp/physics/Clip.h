...
public:
	...
	// SM: Added ignoreContentMask to ALL of these
	// clip versus the rest of the world
	bool					Translation( trace_t &results, const idVec3 &start, const idVec3 &end,
								const idClipModel *mdl, const idMat3 &trmAxis, int contentMask, const idEntity *passEntity, int ignoreContentMask = 0 );
	...
	bool					TracePoint( trace_t &results, const idVec3 &start, const idVec3 &end,
								int contentMask, const idEntity *passEntity, int ignoreContentMask = 0 );
	bool					TraceBounds( trace_t &results, const idVec3 &start, const idVec3 &end, const idBounds &bounds,
								int contentMask, const idEntity *passEntity, int ignoreContentMask = 0, const idMat3 &trmAxis = mat3_identity );
	...
	void					TranslationModel( trace_t &results, const idVec3 &start, const idVec3 &end,
								...
								cmHandle_t model, const idVec3 &modelOrigin, const idMat3 &modelAxis, int ignoreContentMask = 0 );
	...
	int						EntitiesTouchingBounds( const idBounds &bounds, int contentMask, idEntity **entityList, int maxCount, int ignoreContentMask = 0 ) const;
	int						ClipModelsTouchingBounds( const idBounds &bounds, int contentMask, idClipModel **clipModelList, int maxCount, int ignoreContentMask = 0 ) const;
...
private:
	...
	int						GetTraceClipModels( const idBounds &bounds, int contentMask, const idEntity *passEntity, idClipModel **clipModelList, int ignoreContentMask ) const;
...
ID_INLINE bool idClip::TracePoint( trace_t &results, const idVec3 &start, const idVec3 &end, int contentMask, const idEntity *passEntity, int ignoreContentMask ) {
	Translation( results, start, end, NULL, mat3_identity, contentMask, passEntity, ignoreContentMask );
	return ( results.fraction < 1.0f );
}

ID_INLINE bool idClip::TraceBounds( trace_t &results, const idVec3 &start, const idVec3 &end, const idBounds &bounds, int contentMask, const idEntity *passEntity, int ignoreContentMask, const idMat3 &trmAxis ) {
	temporaryClipModel.LoadModel( idTraceModel( bounds ) );
	Translation( results, start, end, &temporaryClipModel, trmAxis, contentMask, passEntity, ignoreContentMask );
	return ( results.fraction < 1.0f );
}
