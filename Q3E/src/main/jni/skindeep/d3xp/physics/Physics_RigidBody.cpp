...
/*
================
idPhysics_RigidBody::GetCurrentState

	Cheat method to extract the physics object's state, allowing us to extrapolate it in the throwing arc prediction code
================
*/
rigidBodyPState_t idPhysics_RigidBody::GetCurrentState()
{
	return current;
}
...
/*
================
idPhysics_RigidBody::GetCenterOfMass

	Cheat method to extract the physics object's center of mass, allowing us to extrapolate it in the throwing arc prediction code
================
*/
idVec3 idPhysics_RigidBody::GetCenterOfMass()
{
	return centerOfMass;
}
...
bool idPhysics_RigidBody::CollisionImpulse( const trace_t &collision, idVec3 &impulse ) {
	...
	// blendo eric: angular dampening on impact per impact
	if (rb_angularDampen.GetBool()) {
		current.i.angularMomentum *= idMath::Fabs(1.0f - angularFriction);
	}
...
//#define TEST_COLLISION_DETECTION
	...
	if ( gameLocal.clip.Motion( collision, current.i.position, next.i.position, rotation, clipModel, current.i.orientation, clipMask, self ) ) {
		// HACK: if the surface normal *and* collision point are the same as they were on the last collision check, 
		// we probably have an object stuck in a ragdoll (usually in a limb joint). For some reason, the game gets the normal backwards.
		if (lastThinkCollision && 
			collision.c.point == lastThinkCollisionPoint && collision.c.normal == lastThinkCollisionNormal)
		{
			// Flip our momentum to try to extricate ourselves, burn off most of our energy
			current.i.linearMomentum *= -0.2;
			current.i.angularMomentum *= -0.2;
		}
		...
		lastThinkCollision = true;
		lastThinkCollisionNormal = collision.c.normal;
		lastThinkCollisionPoint = collision.c.point;
...
idPhysics_RigidBody::idPhysics_RigidBody( void ) {
	...
	// blendo eric: for out of bounds ents
	lastValid = current;
	lastValidPreCollision = current;
	lastValidActivated = false;
	lastValidActivatedPreCollision = false;
...
void idPhysics_RigidBody::Rest( void ) {
	...
	// SM: Added event for physics rest
	self->ProcessEvent( &EV_PostPhysicsRest );
}
...
//	current.i.angularMomentum -= current.pushVelocity.SubVec3( 1 ) * inertiaTensor;
	...
	if ( collided ) {
		...
		if ( ent && ( !cameToRest || !ent->IsAtRest() ) ) {
			...
			if ((!self->spawnArgs.GetBool("actor_knockback", "0") && !ent->IsType(idActor::Type)) || (self->spawnArgs.GetBool("actor_knockback", "0") && ent->IsType(idActor::Type))) //BC control what can and can't do knockback on actors.
			{
...
//	current.i.angularMomentum += current.pushVelocity.SubVec3( 1 ) * inertiaTensor;
	...
	if ( IsOutsideWorld() )
	{
		gameLocal.Warning( "rigid body moved outside world bounds for entity '%s' type '%s' at (%s)", self->name.c_str(), self->GetType()->classname, current.i.position.ToString(0) );

		//BC
		if (developer.GetBool())
		{
			gameRenderWorld->DebugLine(colorPink, current.i.position + idVec3(0, 0, -32), current.i.position + idVec3(0, 0, 32), 30000);
		}

		// blendo eric: try to fix out of worldbounds rigid body using last good known position
		if (lastValidActivatedPreCollision)
		{
			current = lastValidPreCollision;
			current.i.linearMomentum.Zero();
			current.i.angularMomentum.Zero();
			// update the position of the clip model
			clipModel->Link(gameLocal.clip, self, clipModel->GetId(), current.i.position, current.i.orientation);
	...
	{
		// blendo eric: save in worldbounds and uncollided state
		if (!collided)
		{
			lastValid = current;
			lastValidActivated = true;
		}
		else if(lastValidActivated)
		{
			lastValidPreCollision = lastValid;
			lastValidActivatedPreCollision = true;
		}
	}
