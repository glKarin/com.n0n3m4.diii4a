...
/*
================
idPhysics_AF::AddSqueezeGraceEnt

  SW: Add an entity to our 'squeeze grace' list, which tracks entities we need to very temporarily ignore collisions with.
  The intended application of this is to extricate ragdolls from doors that they have clipped into.
================
*/

void idPhysics_AF::AddSqueezeGraceEnt(idEntity* ent, int timer)
{
	// Remove any old references to the same entity
	for (int i = 0; i < squeezeGraceList.Num(); i++)
	{
		if (squeezeGraceList[i].ent == ent)
		{
			squeezeGraceList.RemoveIndex(i);
		}
	}

	squeezeGraceRef_t ref;
	ref.ent = ent;
	ref.timer = (timer < 0) ? 0 : timer;
	squeezeGraceList.Append(ref);
}
	...
	for ( i = 0; i < bodies.Num(); i++ ) {
		...
		if ( body->clipMask != 0 ) {
			...
			if ( gameLocal.clip.Motion( collision, body->current->worldOrigin, body->next->worldOrigin, rotation,
				...
				// Check our squeeze grace list for objects that we are very temporarily passing through
				bool skipThisCollision = false;
				for (int i = 0; i < this->squeezeGraceList.Num(); i++)
				{
					if (gameLocal.entities[collision.c.entityNum] == squeezeGraceList[i].ent)
					{
						skipThisCollision = true;
					}
				}
				
				if (!skipThisCollision)
				{
...
bool idPhysics_AF::EvaluateContacts( void ) {
	...
	for ( i = 0; i < bodies.Num(); i++ ) {
		...
		{
			// Skip contact if the entity is a door in our 'squeeze grace' list (we are probably being shoved out of it)
			idEntity* ent = gameLocal.entities[contactInfo[j].entityNum];
			bool skipThisCollision = false;
			for (k = 0; k < this->squeezeGraceList.Num(); k++)
			{
				if (ent == squeezeGraceList[k].ent)
				{
					skipThisCollision = true;
				}
			}
			if (skipThisCollision)
			{
				// Delete this entry and shuffle everything forward
				for (k = j + 1; k < numContacts; k++)
				{
					contactInfo[k - 1] = contactInfo[k];
				}
				numContacts--;
			}
...
bool idPhysics_AF::Evaluate( int timeStepMSec, int endTimeMSec ) {
	...
	for (int i = 0; i < squeezeGraceList.Num(); i++)
	{
		squeezeGraceList[i].timer -= timeStepMSec;
		if (squeezeGraceList[i].timer <= 0)
		{
			squeezeGraceList.RemoveIndex(i);
		}
	}
