...
public:	// common physics interface
	...
	rigidBodyPState_t		GetCurrentState(void);
	idVec3					GetCenterOfMass(void);

private:
	// state of the rigid body
	rigidBodyPState_t		current;
	rigidBodyPState_t		saved;
	rigidBodyPState_t		lastValidPreCollision; // blendo eric: saved state before move outside worldbounds
	rigidBodyPState_t		lastValid; // blendo eric: saved state before move outside worldbounds
	bool					lastValidActivated; // blendo eric: was state saved before outside worldbounds
	bool					lastValidActivatedPreCollision; // blendo eric: was state saved before outside worldbounds
	...
	// tracking last collision for hacky impact spam fix
	bool					lastThinkCollision;			// did we collide with something on the last think?
	idVec3					lastThinkCollisionPoint;	// If there was a collision on the last think, where was it?
	idVec3					lastThinkCollisionNormal;	// If there was a collision on the last think, what was the normal?
