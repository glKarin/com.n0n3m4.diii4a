...
#include "gamesys/SysCvar.h" // blendo eric
...
/* blendo eric: clip model debug
================
idPhysics_Base::DebugDraw
================
*/
void idPhysics_Base::DebugDraw( void ) {
	if (g_showClipBodies.GetBool()) {
		collisionModelManager->DrawModel(GetClipModel()->Handle(), GetClipModel()->GetOrigin(),
			GetClipModel()->GetAxis(), vec3_origin, 0.0f);
	}
}