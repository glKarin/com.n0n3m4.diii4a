...
void idClip::Init( void ) {
	...
	gameLocal.DPrintf( "map bounds are (%1.1f, %1.1f, %1.1f)\n", size[0], size[1], size[2] );
	gameLocal.DPrintf( "max clip sector is (%1.1f, %1.1f, %1.1f)\n", maxSector[0], maxSector[1], maxSector[2] );
...
void idClip::ClipModelsTouchingBounds_r( const struct clipSector_s *node, listParms_t &parms ) const {
	...
	for ( clipLink_t *link = node->clipLinks; link && parms.count < parms.maxCount; link = link->nextInSector ) {
		...
		if ( !( check->contents & parms.contentMask ) || check->contents & parms.ignoreContentMask ) {
			continue;
		}
...
================
*/
int idClip::ClipModelsTouchingBounds( const idBounds &bounds, int contentMask, idClipModel **clipModelList, int maxCount, int ignoreContentMask ) const {
	listParms_t parms;
	...
	parms.ignoreContentMask = ignoreContentMask;
...
================
*/
int idClip::EntitiesTouchingBounds( const idBounds &bounds, int contentMask, idEntity **entityList, int maxCount, int ignoreContentMask ) const {
	idClipModel *clipModelList[MAX_GENTITIES];
	int i, j, count, entCount;

	count = idClip::ClipModelsTouchingBounds( bounds, contentMask, clipModelList, MAX_GENTITIES, ignoreContentMask );
	entCount = 0;
	for ( i = 0; i < count; i++ ) {
...
====================
*/
int idClip::GetTraceClipModels( const idBounds &bounds, int contentMask, const idEntity *passEntity, idClipModel **clipModelList, int ignoreContentMask ) const {
	int i, num;
	idClipModel	*cm;
	idEntity *passOwner;

	num = ClipModelsTouchingBounds( bounds, contentMask, clipModelList, MAX_GENTITIES, ignoreContentMask );
...
void idClip::TranslationEntities( trace_t &results, const idVec3 &start, const idVec3 &end,
						const idClipModel *mdl, const idMat3 &trmAxis, int contentMask, const idEntity *passEntity, int ignoreContentMask ) {
	int i, num;
	...
	num = GetTraceClipModels( traceBounds, contentMask, passEntity, clipModelList, ignoreContentMask );
...
bool idClip::Translation( trace_t &results, const idVec3 &start, const idVec3 &end,
	...
	if ( !passEntity || passEntity->entityNumber != ENTITYNUM_WORLD ) {
		// test world
		idClip::numTranslations++;
		collisionModelManager->Translation( &results, start, end, trm, trmAxis, contentMask, 0, vec3_origin, mat3_default, ignoreContentMask );
...
bool idClip::Rotation( trace_t &results, const idVec3 &start, const idRotation &rotation,
	...
	if ( !passEntity || passEntity->entityNumber != ENTITYNUM_WORLD ) {
		// test world
		idClip::numRotations++;
		collisionModelManager->Rotation( &results, start, rotation, trm, trmAxis, contentMask, 0, vec3_origin, mat3_default, ignoreContentMask );
		results.c.entityNum = results.fraction != 1.0f ? ENTITYNUM_WORLD : ENTITYNUM_NONE;
		if ( results.fraction == 0.0f ) {
			return true;		// blocked immediately by the world
		}
...
bool idClip::Motion( trace_t &results, const idVec3 &start, const idVec3 &end, const idRotation &rotation,
	...
	if ( mdl != NULL && rotation.GetAngle() != 0.0f && rotation.GetVec() != vec3_origin ) {
		...
		if ( start == end ) {
			// pure rotation
			return Rotation( results, start, rotation, mdl, trmAxis, contentMask, passEntity, ignoreContentMask );
		}
	} else if ( start != end ) {
		// pure translation
		return Translation( results, start, end, mdl, trmAxis, contentMask, passEntity, ignoreContentMask );
	} else {
		// no motion
		results.fraction = 1.0f;
		results.endpos = start;
	...
	if ( !passEntity || passEntity->entityNumber != ENTITYNUM_WORLD ) {
		...
		idClip::numTranslations++;
		collisionModelManager->Translation( &translationalTrace, start, end, trm, trmAxis, contentMask, 0, vec3_origin, mat3_default, ignoreContentMask );
	...
	if ( !passEntity || passEntity->entityNumber != ENTITYNUM_WORLD ) {
		...
		idClip::numRotations++;
		collisionModelManager->Rotation( &rotationalTrace, endPosition, endRotation, trm, trmAxis, contentMask, 0, vec3_origin, mat3_default, ignoreContentMask );
...
int idClip::Contacts( contactInfo_t *contacts, const int maxContacts, const idVec3 &start, const idVec6 &dir, const float depth,
	...
	if ( !passEntity || passEntity->entityNumber != ENTITYNUM_WORLD ) {
		// test world
		idClip::numContacts++;
		numContacts = collisionModelManager->Contacts( contacts, maxContacts, start, dir, depth, trm, trmAxis, contentMask, 0, vec3_origin, mat3_default, ignoreContentMask );
	} else {
		numContacts = 0;
	}
...
============
*/
int idClip::Contents( const idVec3 &start, const idClipModel *mdl, const idMat3 &trmAxis, int contentMask, const idEntity *passEntity, int ignoreContentMask ) {
	int i, num, contents;
	idClipModel *touch, *clipModelList[MAX_GENTITIES];
	idBounds traceBounds;
	const idTraceModel *trm;
	...
	if ( !passEntity || passEntity->entityNumber != ENTITYNUM_WORLD ) {
		// test world
		idClip::numContents++;
		contents = collisionModelManager->Contents( start, trm, trmAxis, contentMask, 0, vec3_origin, mat3_default, ignoreContentMask );
	} else {
		contents = 0;
	}
	...
	num = GetTraceClipModels( traceBounds, -1, passEntity, clipModelList, ignoreContentMask );
	...
	for ( i = 0; i < num; i++ ) {
		...
		idClip::numContents++;
		if ( collisionModelManager->Contents( start, trm, trmAxis, contentMask, touch->Handle(), touch->origin, touch->axis, ignoreContentMask ) ) {
...
void idClip::TranslationModel( trace_t &results, const idVec3 &start, const idVec3 &end,
					...
					cmHandle_t model, const idVec3 &modelOrigin, const idMat3 &modelAxis, int ignoreContentMask ) {
	const idTraceModel *trm = TraceModelForClipModel( mdl );
	idClip::numTranslations++;
	collisionModelManager->Translation( &results, start, end, trm, trmAxis, contentMask, model, modelOrigin, modelAxis, ignoreContentMask );
}
...
void idClip::RotationModel( trace_t &results, const idVec3 &start, const idRotation &rotation,
	...
	const idTraceModel *trm = TraceModelForClipModel( mdl );
	idClip::numRotations++;
	collisionModelManager->Rotation( &results, start, rotation, trm, trmAxis, contentMask, model, modelOrigin, modelAxis, ignoreContentMask );
}
...
int idClip::ContactsModel( contactInfo_t *contacts, const int maxContacts, const idVec3 &start, const idVec6 &dir, const float depth,
	...
	const idTraceModel *trm = TraceModelForClipModel( mdl );
	idClip::numContacts++;
	return collisionModelManager->Contacts( contacts, maxContacts, start, dir, depth, trm, trmAxis, contentMask, model, modelOrigin, modelAxis, ignoreContentMask );
}
...
int idClip::ContentsModel( const idVec3 &start,
	...
	const idTraceModel *trm = TraceModelForClipModel( mdl );
	idClip::numContents++;
	return collisionModelManager->Contents( start, trm, trmAxis, contentMask, model, modelOrigin, modelAxis, ignoreContentMask );
}
