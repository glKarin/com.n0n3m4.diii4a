...
public:
	...
	void					SetNeverBlock(const bool neverBlock) { fl.neverBlock = neverBlock; }
...
private:
	...
	struct bodyFlags_s {
		...
		bool				isZero				: 1;	// true if 's' is zero during calculations
		bool				neverBlock			: 1;    // SW: true if this body should never block pushers
	} fl;
};
...
// SW: Holds information about an entity that we are attempting to 'squeeze past'. 
// We do this by ignoring collisions between the AF and the entity for a short 'grace period' of time
// (represented by the timer, which ticks down every time we think).
// When the timer hits zero, this struct should be removed from the list.
typedef struct squeezeGraceRef_s {
	idEntity*	ent;
	int			timer;
} squeezeGraceRef_t;
...
public:
	...
	void					AddSqueezeGraceEnt(idEntity* ent, int timer);
...
private:
	...
	idList<squeezeGraceRef_t> squeezeGraceList;			// SW: List of objects which we have entered a short 'grace period' of not colliding with.
															// This is mainly necessary for extricating ragdolls from doors which they have become hopelessly embedded in.
