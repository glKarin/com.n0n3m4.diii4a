...
//BC
const float AI_BLINDSPOT_DOTPRODUCT = .5f;

const int VERTICALHEIGHT_SIGHTTHRESHOLD = 96;

const int DARKNESS_VIEWRANGE = 112; //guards can see you when you're this close to them, regardless if you're concealed in darkness
...
extern const idEventDef EV_FootstepLeftRun;
extern const idEventDef EV_FootstepRightRun;
...
public:
	...
	void					SetState( const function_t *newState, bool updateScriptImmediate = false); // blendo eric: added updateScriptImmediate to consolidate for debugging
	void					SetState( const char *statename, bool updateScriptImmediate = false ); // blendo eric: added updateScriptImmediate to consolidate for debugging
	...
	const char*				GetAnimName(int channel); // blendo eric
	...
	//BC PUBLIC
	idVec3					GetEyeBonePosition(void);
	
	int						energyShieldCurrent;
	int						energyShieldMax;
	bool					GetEnergyshieldActive();


	idVec3					lastDamageOrigin;			//what was last position of the DAMAGE that damaged me. Example: if hit by turret bullet, this gives position of the bullet (not the turret).
	idVec3					lastAttackerDamageOrigin;	//what was the last position of the ATTACKER that damaged me. Example: if hit by turret bullet, this gives position of the turret (not the bullet).
	idEntityPtr<idEntity>	lastAttacker;
	int						lastDamagedTime; //When was I last damaged.

	
	
	int						stunStartTime;
	int						stunTime;
	idStr					stunAnimationName;
	virtual void			StartStunState(const char *damageDefName);

	void					SetMaxHealthDelta(int delta);

	
	int						vo_lastSpeakTime;
	int						vo_lastCategory;


    void					AttachItem(idStr itemDefName, idStr jointName, idVec3 originOffset, idAngles angleOffset);


	int						GetPointdefenseStatus(void);
	int						pointdefenseAmount;
	void					PlayerTookCarryable( idEntity* item );

	idList<idAttachInfo>	attachmentsToDrop; //These are attachments that drop off the body when the actor dies. Items are attached via AttachItem()
...
protected:
	...
	idList<int>				damageBonus;		// BC damage bonus per damage group
	...
	idStr					customIdleAnim;
	void					Event_GetCustomIdleAnim();

	void					Event_GetStunAnim();

	bool					IsTargetImmediatelyFrontOfMe(const idVec3 &pos) const;
	...
	void					AttachBeltattachments();
	bool					hasAttachedBeltItems;
	idEntity *				FindBeltattachEnt(idStr _name, idVec3 _itemPos, idMat3 _itemAxis);
	idVec3					GetBeltAttachmentVec3(idStr entityName, idStr keyname);
...
private:
	...
	void					Event_FootstepGeneric(int side, bool isRunning);
	...
	//BC private
	void					Event_FootstepLeft(void);
	void					Event_FootstepRight(void);
	void					Event_FootstepLeftRun(void);
	void					Event_FootstepRightRun(void);	
	void					SetupVisionbox();
	void					Event_getEyePos();
	void					Event_GetActorCenter();	
	
	void					DropBeltAttachments(); //this interacts with attachmentsToDrop
	
	bool					IsStundamageValid(idVec3 _damageDir);

	void					Event_ActorVO(const char* stringName, int voCategory);
