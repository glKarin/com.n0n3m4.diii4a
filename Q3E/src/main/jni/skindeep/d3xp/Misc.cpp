...
void idPathCorner::Spawn( void )
{
	...
	idAAS* aas;
	idStr aasName;
	if (spawnArgs.GetString("aas", "", aasName)) // Let path_corners override the AAS to test reachability from
	{
		aas = gameLocal.GetAAS(aasName);
	}
	else
	{
		aas = gameLocal.GetAAS(0); // Default AAS -- most path_corners won't need to override this
	}
...
void idDamagable::BecomeBroken( idEntity *activator ) {
	...
	idStr scriptCall;
	if (spawnArgs.GetString("callOnBreak", "", scriptCall))
	{
		gameLocal.RunMapScript(scriptCall);
	}
	...
	if (spawnArgs.GetBool("removeWhenBroken")) {
		Hide();
...
//BC
const idEventDef EV_PropAnim("propanim", "sd");
const idEventDef EV_TurnJointToward("turnjointtoward", "sv");
const idEventDef EV_SetPropBlendFrames("setPropBlendFrames", "d");
const idEventDef EV_StopAnim("stopanim");
const idEventDef EV_PropAnimDone("propAnimDone", "d", 'd');
...
CLASS_DECLARATION( idAFEntity_Gibbable, idAnimated )
	...
	EVENT(EV_PropAnim,				idAnimated::Event_PropAnim)
	EVENT(EV_TurnJointToward,		idAnimated::Event_TurnJointToward)
	EVENT(EV_SetPropBlendFrames,	idAnimated::Event_SetPropBlendFrames)
	EVENT(EV_StopAnim,				idAnimated::Event_StopAnim)
	EVENT(EV_PropAnimDone,			idAnimated::Event_PropAnimDone)
...
bool idAnimated::GetPhysicsToSoundTransform( idVec3 &origin, idMat3 &axis ) {
	// SM: If GetJointTransform returns false, that means the joint is invalid
	// and therefore origin/axis are not set by GetJointTransform.
	// We have to return false in this case or UpdateSound will use garbage data for the origin/axis
	if ( animator.GetJointTransform( soundJoint, gameLocal.time, origin, axis ) ) {
...
void idAnimated::Event_Start( void )
{
	...
	// offset the start time of the shader to sync it to the game time
	//BC I commented this out.... do we need this?
	//renderEntity.shaderParms[ SHADERPARM_TIMEOFFSET ] = -MS2SEC( gameLocal.time );
...
//BC just play an animation.
void idAnimated::Event_PropAnim(const char *animName, int loop)
{
	anim = animator.GetAnim(animName);
	if (!anim) { gameLocal.Error("idAnimated '%s' at (%s): cannot find anim '%s'", name.c_str(), GetPhysics()->GetOrigin().ToString(0), animName); }

	//bc force the next animation to trigger.
	activated = false;

	spawnArgs.SetInt("cycle", (loop > 0) ? -1 : 1); //does it loop

	PostEventMS(&EV_Activate, 0, this); //play the animation.
}
...
void idAnimated::Event_TurnJointToward(const char *jointName, const idVec3 &lookPoint)
{
	...
	{
		gameLocal.Warning("Event_TurnJointToward: can't find joint '%s'\n", jointName);
		return;
	}

	animator.GetJointTransform(joint, gameLocal.time, jointPos, jointAxis);
	...
	animator.SetJointAxis(joint, JOINTMOD_WORLD, lookVec.ToMat3() * mat);
}

void idAnimated::Event_SetPropBlendFrames(int framecount)
{
	blendFrames = framecount;
}

void idAnimated::Event_StopAnim()
{
	animator.Clear(ANIMCHANNEL_ALL, gameLocal.time, 4);
}

// SW 13th Feb 2025: Animated prop equivalent to actor's Event_AnimDone
void idAnimated::Event_PropAnimDone(int blendFrames)
{
	bool result;
	int animDoneTime = animator.CurrentAnim(ANIMCHANNEL_ALL)->GetEndTime();
	if (animDoneTime < 0) {
		// playing a cycle
		result = false;
	}
...
const idEventDef EV_FadeColor("fadeColor", "ffff");

CLASS_DECLARATION( idEntity, idStaticEntity )
	EVENT( EV_Activate,				idStaticEntity::Event_Activate )
	EVENT( EV_FadeColor,			idStaticEntity::Event_FadeColor)
END_CLASS
...
void idStaticEntity::Spawn( void ) {
	...
	//bc
	spin = spawnArgs.GetBool("spin", "0");

	// SW: Make static entity frobbable if it hosts a GUI
	if (renderEntity.gui[0])
	{
		if (renderEntity.gui[0]->IsInteractive())
		{
			isFrobbable = true;
		}
	}
...
void idStaticEntity::Event_FadeColor(float r, float g, float b, float timeSeconds)
{
	this->Fade(idVec4(r, g, b, 1), timeSeconds);
}
...
const idEventDef EV_HasInfiniteStage( "hasInfiniteStage", NULL, 'f' );

CLASS_DECLARATION( idStaticEntity, idFuncEmitter )
	EVENT( EV_Activate,				idFuncEmitter::Event_Activate )
	EVENT( EV_emitter_enable, idFuncEmitter::SetActive )//BC
	EVENT( EV_HasInfiniteStage, idFuncEmitter::Event_HasInfiniteStage )
END_CLASS
...
void idFuncEmitter::Spawn( void ) {
	...
	// SM
	spawnArgs.GetBool( "airlessGravity", "0", airlessGravity );
	if ( airlessGravity ) {
		BecomeActive( TH_THINK );
		renderEntity.shaderParms[SHADERPARM_PARTICLE_GRAVITYSCALE] = 0.0f;
	} else {
		renderEntity.shaderParms[SHADERPARM_PARTICLE_GRAVITYSCALE] = 1.0f;
	}

	//BC optional sound effect.
	idStr soundName = spawnArgs.GetString("snd_ambient");
	if (soundName.Length() > 1 && soundName[0] != '\0')
	{
		StartSound("snd_ambient", SND_CHANNEL_BODY);
	}

	hasInfiniteStage = false;
	const idDeclParticle *modelDef = static_cast<const idDeclParticle *>( declManager->FindType( DECL_PARTICLE, spawnArgs.GetString( "model" ), false ) );
	if ( modelDef )
	{
		for ( int i = 0; i < modelDef->stages.Num(); i++ )
		{
			if ( modelDef->stages[i]->cycles == 0 )
			{
				hasInfiniteStage = true;
			}
		}
	}
...
void idFuncEmitter::Think() {
	idStaticEntity::Think();

	// SM
	if ( airlessGravity ) {
		float oldValue = renderEntity.shaderParms[SHADERPARM_PARTICLE_GRAVITYSCALE];
		renderEntity.shaderParms[SHADERPARM_PARTICLE_GRAVITYSCALE] = gameLocal.GetAirlessAtPoint( GetPhysics()->GetOrigin() ) ? 0.0f : 1.0f;
		if ( oldValue != renderEntity.shaderParms[SHADERPARM_PARTICLE_GRAVITYSCALE] ) {
			UpdateVisuals();
			BecomeInactive( TH_THINK );
		}
	}
...
void idFuncEmitter::Event_HasInfiniteStage()
{
	idThread::ReturnInt( hasInfiniteStage );
}

void idFuncEmitter::SetActive(bool value)
{
	if (value)
	{
		renderEntity.shaderParms[SHADERPARM_PARTICLE_STOPTIME] = 0;
...
bool idFuncEmitter::GetParticleActive()
{
	return (!hidden);
}
...
void idTextEntity::Spawn( void ) {
	...
	textsize = spawnArgs.GetFloat("textsize", ".25");
	depthTest = spawnArgs.GetBool("depthtest", "1");

	//BC allow changing text color
	idVec3 color = spawnArgs.GetVector("_color", "1 1 1");
	textColor = idVec4(color.x, color.y, color.z, 1);
}
...
void idTextEntity::Think( void ) {
	if (thinkFlags & TH_THINK) {
		...
		if (!IsHidden())
		{
			gameRenderWorld->DrawText(textFormatted, GetPhysics()->GetOrigin(), textsize, textColor, playerOriented ? gameLocal.GetLocalPlayer()->viewAngles.ToMat3() : GetPhysics()->GetAxis().Transpose(), 1, 0, depthTest);
...
CLASS_DECLARATION( idEntity, idVacuumSeparatorEntity )
	...
	EVENT( EV_PostSpawn,	idVacuumSeparatorEntity::Event_PostSpawn )
END_CLASS
...
void idVacuumSeparatorEntity::Spawn() {
	...
	b = idBounds( spawnArgs.GetVector( "origin" ) ).Expand( spawnArgs.GetInt("radius", "16"));
	...
	//gameRenderWorld->DebugArrow(colorGreen, this->GetPhysics()->GetOrigin(), this->GetPhysics()->GetOrigin() + forward * 64, 4, 500000);

	PostEventMS(&EV_PostSpawn, 0);
}

void idVacuumSeparatorEntity::Event_PostSpawn()
{
	//Verify it's pointing toward space.
	#if _DEBUG
	if (spawnArgs.GetBool("airlock") == 0)
	{
		idVec3 forward;
		this->GetPhysics()->GetAxis().ToAngles().ToVectors(&forward, NULL, NULL);
		#define BOUNDSIZE 24
		trace_t tr;
		idVec3 trPoint = GetPhysics()->GetOrigin() + forward * 48;

		bool isError = false;

		if (!gameLocal.GetAirlessAtPoint(trPoint))
		{
			isError = true;
		}
		...
		if (!isError)
		{
			gameLocal.clip.TraceBounds(tr, trPoint, trPoint, idBounds(idVec3(-BOUNDSIZE, -BOUNDSIZE, -BOUNDSIZE), idVec3(BOUNDSIZE, BOUNDSIZE, BOUNDSIZE)), MASK_SOLID, NULL);
			if (tr.fraction < 1)
			{
				isError = true;
			}
		...
		if (isError)
		{
			gameRenderWorld->DebugTextSimple("BAD VACUUM DIRECTION?", this->GetPhysics()->GetOrigin(), 500000);
			gameRenderWorld->DebugArrow(colorYellow, this->GetPhysics()->GetOrigin(), this->GetPhysics()->GetOrigin() + forward * 64, 12, 500000);
			common->Warning("Possible bad vacuumseparator direction: '%s'\n", GetName());
		}
	}
#endif
}
...
void idVacuumSeparatorEntity::Event_Activate( idEntity *activator )
{
	...
	//Determine whether the vacuum state has changed.
	/*
	idVec3 pointInside, pointOutside;
	idVec3 forward;
	this->GetPhysics()->GetAxis().ToAngles().ToVectors(&forward, NULL, NULL);
	pointInside = this->GetPhysics()->GetOrigin() + forward * -4;
	pointOutside = this->GetPhysics()->GetOrigin() + forward * 4;
	if (!gameLocal.GetAirlessAtPoint(pointInside) || !gameLocal.GetAirlessAtPoint(pointOutside))
	{
		//The vacuum has changed. Do the air suck stuff.
		idStr			fxBreak;
		int listedEntities, i;
		idEntity *entityList[MAX_GENTITIES];
	
		listedEntities = gameLocal.EntitiesWithinRadius(GetPhysics()->GetOrigin(), 1024, entityList, MAX_GENTITIES);
		...
		for (i = 0; i < listedEntities; i++)
		{
			...
			if (ent)
			{
				if (ent->IsType(idMoveable::Type) || ent->IsType(idMoveableItem::Type) || ent->IsType(idActor::Type) || ent->GetPhysics()->IsType(idPhysics_RigidBody::Type))
				{
					//have an object.
					//throw it.
					...
					//Determine whether ent is in front or behind the window.
					this->GetPhysics()->GetAxis().ToAngles().ToVectors(&forward, NULL, NULL);
					dirToEntity = frontOfMachinePos - ent->GetPhysics()->GetOrigin();
					...
					//TODO make it behave differentl ydepending on whether its dotproduct to suction point
					idVec3 suctionPoint, throwDir;
					float pullStrength;
					trace_t losTr;
					idVec3 entCenter;


					//Suctionpoint: where to pull the object.
					suctionPoint = GetPhysics()->GetOrigin() + idVec3(gameLocal.random.CRandomFloat() * 32, gameLocal.random.CRandomFloat() * 32, gameLocal.random.CRandomFloat() * 32);

					//Only pull if suctionpoint has LOS to the object.
					gameLocal.clip.TracePoint(losTr, suctionPoint, ent->GetPhysics()->GetOrigin(), MASK_SOLID, ent);
					if (losTr.fraction < 1)
						continue;

					//Get the CENTER of the object.
					entCenter = idVec3(ent->GetPhysics()->GetOrigin().x, ent->GetPhysics()->GetOrigin().y, ent->GetPhysics()->GetAbsBounds().GetCenter().z);

					throwDir = GetPhysics()->GetOrigin() - entCenter;
					throwDir.Normalize();
					...
					else
					{
						...
						{
							static_cast<idAI *>(ent)->EnterVacuum();
						}

						pullStrength = 512 + gameLocal.random.RandomInt(128);
						ent->GetPhysics()->SetGravity(vec3_zero);
					}

					//TODO: chagne this to applyimpulse.
					ent->GetPhysics()->SetLinearVelocity(throwDir * pullStrength);

					ent->GetPhysics()->SetAngularVelocity(idVec3(gameLocal.random.CRandomFloat() * 32, gameLocal.random.CRandomFloat() * 32, gameLocal.random.CRandomFloat() * 32));
				}
			}
		}
		...
		if (fxBreak.Length())
		{
			...
			fxPos += forward * -64; //move it INTO the interior a little.


			idAngles fxAngle = GetPhysics()->GetAxis().ToAngles();
			fxAngle.pitch += 90;
			idMat3 fxMat = fxAngle.ToMat3();

			idEntityFx::StartFx(fxBreak, &fxPos, &fxMat, this, true);
		}
	}*/
	...
	//BC when a vacuum separator is disabled, then yank nearby objects toward it.

	//TODO: make this work more similar to the airlock suction.
...
void idVacuumSeparatorEntity::SetBlocked(bool value, bool blockView)
{
	if (!portal) {
		return;
	}
	...
	{
		gameLocal.SetPortalState(portal, blockView ?  PS_BLOCK_ALL : (PS_BLOCK_AIR | PS_BLOCK_LOCATION));
	}
}
...
void idBeam::Event_MatchTarget( void ) {
	...
	if ( !targetBeam ) {
		gameLocal.Error( "Could not find valid beam target for '%s'. Note: you must make this target a func_beam", name.c_str() );
	}
...
const idEventDef EV_Portal_Toggle("portaltoggle", "d");

CLASS_DECLARATION( idEntity, idFuncAASPortal )
	EVENT( EV_Activate,				idFuncAASPortal::Event_Activate )
	EVENT( EV_Portal_Toggle,		idFuncAASPortal::Event_PortalToggle)
END_CLASS
...
void idFuncAASPortal::Event_PortalToggle(int value)
{
	state = value;
	gameLocal.SetAASAreaState(GetPhysics()->GetAbsBounds(), AREACONTENTS_CLUSTERPORTAL, state);
}
...
const idEventDef EV_Obstacle_Toggle("obstacletoggle", "d");

CLASS_DECLARATION( idEntity, idFuncAASObstacle )
	EVENT( EV_Activate,				idFuncAASObstacle::Event_Activate )
	EVENT(EV_Obstacle_Toggle,		idFuncAASObstacle::Event_ObstacleToggle)
END_CLASS
...
//BC
void idFuncAASObstacle::Event_ObstacleToggle(int value)
{
	state = value;
	gameLocal.SetAASAreaState(GetPhysics()->GetAbsBounds(), AREACONTENTS_OBSTACLE, state);
}
