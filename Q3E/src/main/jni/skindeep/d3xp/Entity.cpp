...
const idEventDef EV_DistanceToPoint2D("distanceToPoint2D", "v", 'f');
...
const idEventDef EV_SetGuiUnique ( "setGuiUnique", "ds" );
...
const idEventDef EV_GetRepairPosition("getRepairPosition", NULL, 'v');
const idEventDef EV_GetCurrentHealth("getCurrentHealth", NULL, 'd');
const idEventDef EV_GetMaxHealth("getMaxHealth", NULL, 'd');
const idEventDef EV_SetCurrentHealth("setCurrentHealth", "d");
const idEventDef EV_SetMaxHealth("setMaxHealth", "d");
const idEventDef EV_SetCinematic("setCinematic", "d");
const idEventDef EV_PostPhysicsRest("postPhysicsRest");
const idEventDef EV_ApplyDamage("applyDamage", "s");
const idEventDef EV_SetFrobbable("setFrobbable", "d");
const idEventDef EV_SetSolid("setSolid", "d");
const idEventDef EV_SetFrozen( "setFreezeframe", "d" ); //for freezeframe 
const idEventDef EV_UpdateDisplayName("updateDisplayName", "s");
const idEventDef EV_RepairEntity("setRepair", NULL);
...
EVENT(EV_SetGuiUnique, idEntity::Event_SetGuiUnique)
...
//BC
EVENT(EV_DistanceToPoint2D,		idEntity::Event_DistanceToPoint2D)
EVENT(EV_GetIndex,				idEntity::Event_GetIndex)
EVENT(EV_GetRepairPosition,		idEntity::Event_GetRepairPosition)
EVENT(EV_GetCurrentHealth,		idEntity::Event_GetCurrentHealth)
EVENT(EV_GetMaxHealth,			idEntity::Event_GetMaxHealth)
EVENT(EV_SetCurrentHealth,		idEntity::Event_SetCurrentHealth)
EVENT(EV_SetMaxHealth,			idEntity::Event_SetMaxHealth)
EVENT(EV_SetCinematic,			idEntity::Event_SetCinematic)
EVENT(EV_PostPhysicsRest,		idEntity::Event_PostPhysicsRest)
EVENT(EV_PostSpawn,				idEntity::Event_PostSpawn)
EVENT(EV_ApplyDamage,			idEntity::ApplyDamage)
EVENT(EV_SetFrobbable,			idEntity::Event_SetFrobbable)
EVENT(EV_SetSolid,              idEntity::Event_SetSolid)
EVENT(EV_SetFrozen,				idEntity::Event_SetFrozen)
EVENT(EV_UpdateDisplayName,		idEntity::Event_UpdateDisplayName)
EVENT(EV_RepairEntity,			idEntity::Event_RepairEntity)
...
void UpdateGuiParms( idUserInterface *gui, const idDict *args ) {
	...
	while( kv ) {
		if (kv->GetValue().Find("\\n")) { //blendo eric: map strings don't properly parse newlines
			idStr valReplacedChars = kv->GetValue();
			valReplacedChars.Replace("\\n", "\n");
			gui->SetStateString(kv->GetKey(), valReplacedChars);
		} else {
...
================
*/
void AddRenderGui( const char *name, idUserInterface **gui, const idDict *args, bool forceNOTunique = false ) {
	const idKeyValue *kv = args->MatchPrefix( "gui_parm", NULL );
	*gui = uiManager->FindGui( name, true, ( kv != NULL ), forceNOTunique );
...
void idGameEdit::ParseSpawnArgsToRenderEntity( const idDict *args, renderEntity_t *renderEntity ) {
	...
	// SW: Check for an 'override' skin that only shows up in subviews (e.g. mirrors, remote cameras).
	// The main use case for this is drawing the player's arms in mirrors, no matter what skin they currently have
	temp = args->GetString("overrideSkinInSubview");
	if (temp[0] != '\0') {
		renderEntity->overrideSkinInSubview = declManager->FindSkin(temp);
	}
	else if (modelDef) {
		renderEntity->overrideSkinInSubview = modelDef->GetOverrideSkinInSubview();
	}

	// SW: Check if this entity needs to know about its own light interactions.
	// This is usually because we want to detect the amount of light falling on it for gameplay reasons
	renderEntity->trackInteractions = args->GetBool("trackInteractions");
	if (renderEntity->trackInteractions && g_luminance_enabled.GetBool())
		...
		renderEntity->interactionCallback = NULL;

	// SW: Allow entities outside of the view frustum to still be 'drawn'
	// This is wasteful, but necessary for some cases (e.g. we always want to draw the player in order to get interaction info out
	renderEntity->noFrustumCull = args->GetBool("noFrustumCull");
	...
	// SW: If no shaderparm5 is specified, treat it as diversity (this is similar to how projectiles treat it)
	if (!args->GetFloat("shaderParm5", "0", renderEntity->shaderParms[5]))
	{
		renderEntity->shaderParms[5] = gameLocal.random.CRandomFloat();
	}
	...
	renderEntity->shaderParms[SHADERPARM_PARTICLE_GRAVITYSCALE] = 1.0f;
	...
	// Some entities can make use of this at runtime, but there's nothing to parse from spawnargs right now.
	renderEntity->fragmentMapOverride = NULL;
	...
	for( i = 0; i < MAX_RENDERENTITY_GUI; i++ ) {
		...
		if ( temp[ 0 ] != '\0' ) {
			...
			temp = args->GetString( i == 0 ? "gui_synced" : va( "gui_synced%d", i + 1 ) );
			if ( temp[0] != '\0') {
				AddRenderGui( temp, &renderEntity->gui[i], args, true );
			}
		}
	}
}
...
idEntity::idEntity() {
	...
	thinkFlags		= 0;
	dormantStart	= 0;
	cinematic		= true; //BC was defaulting to false.
	renderView		= NULL;
	cameraTarget	= NULL;
	health			= 0;
	...
	//bc
	isFrobbable = false;
	isFrobHoldable = false;
	needsRepair = false;
	healthbarDisplaytime = 0;
	cameraiconDisplaytime = 0;
	repairRequestVerified = false;
	jockeyFrobDebounceMax = 0;
	jockeyFrobDebounceTimer = 0;

	spectrum = SPECTRUM_UNSPECIFIED;
	canClamber = true;
	isFrozen = false;
...
void idEntity::FixupLocalizedStrings() {
	for ( int i = 0; i < spawnArgs.GetNumKeyVals(); i++ ) {
		...
		{
			//BC store the localized string so we can refer to it for debug reasons.

			//This is a BAD hack :(
			//Ignore the loc string for "NOTE" , so that we instead grab the note's actual text
			idStr keyvalue1 = kv->GetValue();
			if (idStr::Cmp(keyvalue1.c_str(), "#str_def_gameplay_900083") != 0)
			{
				locID = kv->GetValue();
			}
...
void idEntity::Spawn( void ) {
	...
	renderEntity.isPlayer = false; // SM: Option for lights to not cast a player shadow
	...
	fl.drawGlobally = spawnArgs.GetBool( "drawGlobally", "0" );
	if (fl.drawGlobally)
	{
		// Force our globally drawn entity to render on top of everything
	...
	cinematic = spawnArgs.GetBool( "cinematic", "1" ); //BC was defaulting to off.
	...
	else if (spawnArgs.GetString("model1"))
	{
		//BC choose a random model. To use this, don't include a "model" key and instead do "model1", "model2", ...
		
		const idKeyValue *kv = spawnArgs.MatchPrefix("model", NULL);
		...
		if (kv != NULL)
		{
			...
			randIndex = gameLocal.random.RandomInt(totalModels);
	...
	isFrobHoldable = spawnArgs.GetBool( "frobholdable", "0" );
	displayName = spawnArgs.GetString("displayname");
	displayNameHold = spawnArgs.GetString("displaynamehold");

	//BC precache the broken model.
	spawnArgs.GetString("brokenmodel", "", brokenmodel);
	if (brokenmodel.Length() && !renderModelManager->CheckModel(brokenmodel))
	{
		gameLocal.Error("idEntity '%s' at (%s): cannot load brokenmodel variation '%s'", name.c_str(), GetPhysics()->GetOrigin().ToString(0), brokenmodel.c_str());
	}

	//Set the repairnode offset. The way this works: XYZ represents the FORWARD, RIGHT, UP relative offset of repair position.
	if (spawnArgs.GetVector("repair_offset", "0 0 0", repairOffset))
	{
		idVec3 forwardDir, rightDir ,upDir;

		this->GetPhysics()->GetAxis().ToAngles().ToVectors(&forwardDir, &rightDir, &upDir);
		repairWorldposition = this->GetPhysics()->GetOrigin() + (forwardDir * repairOffset.x) + (rightDir * repairOffset.y) + (upDir * repairOffset.z);
	}
	else
	{
		repairWorldposition = this->GetPhysics()->GetAbsBounds().GetCenter();
	}

	//TODO: make this handle an arbitrary amount of precaches
	//precache stuff.
	particle01Cache = spawnArgs.GetString("cache_particle01");
	if (particle01Cache[0])
	{
		declManager->FindType(DECL_PARTICLE, particle01Cache);
	}

	fx01Cache = spawnArgs.GetString("cache_fx01");
	if (fx01Cache[0])
	{
		declManager->FindType(DECL_FX, fx01Cache);
	}

	fx02Cache = spawnArgs.GetString("cache_fx02");
	if (fx02Cache[0])
	{
	...
	if (spawnArgs.GetBool("depthhack", "0"))
	{
		SetDepthHack(true);
	}
	...
	// Should this entity dynamically update its spectrum as it moves around?
	dynamicSpectrum = spawnArgs.GetBool("dynamicSpectrum", "0");
	...
	{
		renderEntity.spectrum = SPECTRUM_UNSPECIFIED;
	}

	lastFireattachmentDamagetime = 0;

	itemDamageVelocityMin = spawnArgs.GetFloat( "itemDamageVelocityMin", "100" );

	jockeyFrobDebounceMax = spawnArgs.GetInt("jockeyfrobdebouncetime", "500");

	forceNoGravity = spawnArgs.GetFloat( "gravity", "1" ) == 0.0f;
	zeroGravityNudge = spawnArgs.GetBool("zerog_nudge", "1");
	if ( forceNoGravity )
	{
		PostEventMS( &EV_PostSpawn, 0 );
	}

	canClamber = spawnArgs.GetBool( "canclamber", "1" );

	renderEntity.isFrozen = false;

	//See if entity has 'speakername' spawnarg for the subtitle.
	temp = spawnArgs.GetString( "speakerName" );
	if ( temp && temp[0] )
	{
		speakerName = common->GetLanguageDict()->GetString( temp );
	}
	else
	{
		//if no speakername, then fall back to displayname.
		temp = spawnArgs.GetString("displayname");
		speakerName = common->GetLanguageDict()->GetString(temp);
	}
	...
	infomapPositionOffset = spawnArgs.GetVec2("infomapoffset", "0 0");

	if (spawnArgs.GetBool("debugbutton") && g_autorunVignettes.GetBool() )
	{
...
idEntity::~idEntity( void ) {
	...
	// SW: If the player has us set as their frob target, null the pointer so that they don't reference garbage during HUD drawing
	if (gameLocal.GetLocalPlayer() && gameLocal.GetLocalPlayer()->GetFrobEnt() == this)
	{
	...
	if ( ( flags & TH_PHYSICS ) ) {
		...
		if ( teamMaster && teamMaster != this ) {
			...
			if ( teamMaster->IsAtRest() ) {
				// blendo eric: only deactivate physics if it was added from binding
				// and there's no other entity on the team, otherwise the leftover team will freeze
				if( (teamMaster->thinkFlags & TH_PHYSICSFROMBIND) ){
					bool otherTeamMemberFound = false;
					for( idEntity* nextTeam = teamMaster->teamChain ; nextTeam ; nextTeam = nextTeam->teamChain ) {
						if( nextTeam != this ) {
							otherTeamMemberFound = true;
						}
					}
					if(!otherTeamMemberFound) {
						teamMaster->thinkFlags &= ~TH_PHYSICSFROMBIND;
...
void idEntity::FreeModelDef( void ) {
	if ( modelDefHandle != -1 ) {
		// SM: Moved this to here to ensure if the model def is freed we also remove the global render ent
		// There was at least one case where the entity def was freed but it was still in the global list
		if ( fl.drawGlobally )
		{
			// Drop ourselves from the list of globally rendered ents
			gameRenderWorld->RemoveGlobalRenderEnt( modelDefHandle );
		}
...
void idEntity::ProjectOverlay( const idVec3 &origin, const idVec3 &dir, float size, const char *material ) {
	...
	//gameRenderWorld->DebugArrow(colorRed, origin, origin + (dir * 8), 1.0f, 10000);
...
void idEntity::Present( void ) {
	...
	// SM: Static entities won't ever set the render view if they aren't within
	// the original player PVS on spawn.
	// This may be a performance bottleneck if we have too many of these, would
	// need to change this to be dynamic in that case.
	...
	if ( modelDefHandle == -1 ) {
		...
		if (fl.drawGlobally)
		{
			// SW: Track this entity independently as something that needs to be drawn at all times, no matter its location.
			// Normally we flow through portals to find what we can draw, so unseen locations wouldn't even be considered.
			gameRenderWorld->AddGlobalRenderEnt(modelDefHandle); 
		}
	} else {
		gameRenderWorld->UpdateEntityDef( modelDefHandle, &renderEntity );
...
void idEntity::UpdateTrackedInteractions(renderEntity_s* renderEnt, idList<trackedInteraction_t>* interactions)
{
}
...
bool idEntity::StartSound( const char *soundName, const s_channelType channel, int soundShaderFlags, bool broadcast, int *length ) {
	...
	{
		gameLocal.Error("StartSound invalid name: '%s'", soundName);
	}
#else
	...
	{
		if (cvarSystem->GetCVarBool("s_debug") && !spawnArgs.GetString(sound)[0] == '\0')
		{
			gameLocal.Warning("StartSound failed to find: %s\n", soundName);
		}
...
void idEntity::UpdateSound( void ) {
	if ( refSound.referenceSound ) {
		if ( refSound.referenceSound == ( idSoundEmitter* )( 1 ) )
		{
			common->Warning( "refSound.referenceSound for %s is 1, setting to NULL", GetName() );
			refSound.referenceSound = NULL;
			return;
		}
		...
		if ( GetPhysicsToSoundTransform( origin, axis ) ) {
			...
			//BC offset the sound origin point. This is needed in case the origin point is buried inside geometry or a caulk brush, as that causes sound occlusion to behave weird.
			idVec3 soundOffset = spawnArgs.GetVector("sound_offset");
			if (soundOffset.x != 0 || soundOffset.y != 0 || soundOffset.z != 0)
			{
				idVec3 forwardDir, rightDir, upDir;
				this->GetPhysics()->GetAxis().ToAngles().ToVectors(&forwardDir, &rightDir, &upDir);
				refSound.origin += (forwardDir * soundOffset.x) + (rightDir * soundOffset.y) + (upDir * soundOffset.z);

				if (cvarSystem->GetCVarBool("s_drawoffset"))
				{
					gameRenderWorld->DebugArrow(colorGreen, refSound.origin + (forwardDir * -soundOffset.x) + (rightDir * -soundOffset.y) + (upDir * -soundOffset.z), refSound.origin, 4, 900000);
				}
			}
...
bool idEntity::IsPlayingSound( const s_channelType channel ) {
	if ( refSound.referenceSound ) {
		return refSound.referenceSound->CurrentlyPlayingChannel( channel );
	}
...
void idEntity::FinishBind( void ) {
	...
	// blendo eric: mark if physics was activated on this bind
	// so unbind doesn't disable already active physics later
	if(!(teamMaster->thinkFlags & TH_PHYSICS))
	{
		teamMaster->thinkFlags |= TH_PHYSICSFROMBIND;
	}
...
void idEntity::SetPhysics( idPhysics *phys ) {
	...
	if (bindMaster == (idEntity*)(1))
	{
		common->Warning("bindMaster for %s is 1, setting to NULL", GetName());
		bindMaster = NULL;
	}
	physics->SetMaster( bindMaster, fl.bindOrientated );
}
...
// SW: Entities dynamically determine what spectrum of light they should respond to, 
// so they can be alternately lit by the interior/exterior sunlight.
void idEntity::UpdateSpectrum(void)
{
	int area;
	if (numPVSAreas > 0)
	{
		area = PVSAreas[0]; // We can't be too clever about entities spanning multiple areas, so just pick one arbitrarily
	}
	else
	{
		// Do a lookup at the origin as a fallback
		area = gameRenderWorld->PointInArea(this->physics->GetOrigin());
	}

	if (area == gameLocal.vacuumAreaNum && this->spectrum != SPECTRUM_EXTERIOR)
	{
		this->spectrum = SPECTRUM_EXTERIOR;
	}
	else if (area != gameLocal.vacuumAreaNum && this->spectrum != SPECTRUM_INTERIOR)
	{
		this->spectrum = SPECTRUM_INTERIOR;
	}
	renderEntity.spectrum = this->spectrum;
}
...
============
*/
void idEntity::Damage( idEntity *inflictor, idEntity *attacker, const idVec3 &dir, const char *damageDefName, const float damageScale, const int location, const int materialType)
{
	if ( !fl.takedamage ) {
		return;
	}
	...
	//BC filter for only accepting specific damage types.
	int filterCount = 0;
	bool acceptDamage = true;
	const idKeyValue *kv;
	kv = this->spawnArgs.MatchPrefix("dmgfilter", NULL);
	while (kv)
	{
		...
		if (candidate.Length() > 0)
		{
			...
			if (!DamageDefHasDmgType(damageDef, candidate))
			{
				acceptDamage = false;
			}			
		}		
	...
	if (!acceptDamage)
		return; //did not pass the damage filter.
...
//For the damage filter. Check if damage definition has the necessary damage type.
bool idEntity::DamageDefHasDmgType(const idDict *damageDef, idStr filtername)
{
	const idKeyValue *dv;
	dv = damageDef->MatchPrefix("dmgtype", NULL);
	while (dv)
	{
		idStr candidate = dv->GetValue();
		if (candidate.Length() > 0)
		{
			if (idStr::Icmp(candidate, filtername) == 0)
				return true;
		}

		dv = this->spawnArgs.MatchPrefix("dmgtype", dv); //Iterate to next entry.
	}

	return false;
}

//BC script call for applying damage directly to an entity.
void idEntity::ApplyDamage(const char *damageDef)
{
	Damage(NULL, NULL, vec3_zero, damageDef, 1.0f, 0);
}
...
void idEntity::AddDamageEffect( const trace_t &collision, const idVec3 &velocity, const char *damageDefName ) {
	...
	if (g_bloodEffects.GetBool())
	{
		...
		bleed = spawnArgs.GetString(va("particle_wound_%s", materialType) );

		if (*bleed == '\0')
		{
			bleed = spawnArgs.GetString("particle_wound");
		}

		if (*bleed != '\0')
		{
			gameLocal.DoParticle(bleed, collision.endpos);
		}
	}
...
bool idEntity::HandleGuiCommands( idEntity *entityGui, const char *cmds ) {
	...
	if ( entityGui && cmds && *cmds ) {
		...
		while( 1 ) {
			...
			//BC gui commands.
			if (!token.CmpPrefix("impulse_"))
			{
				idStr integer = token.Right(token.Length() - 8);
				int impulseNumber = atoi(integer.c_str());

				entityGui->DoGenericImpulse(impulseNumber);
				continue;
			}
...
bool idEntity::TouchTriggers( void ) const {
	...
	for ( i = 0; i < numClipModels; i++ ) {
		...
		//BC CRASH. Sometimes getting garbage/null entities, so try to filter them out here. See also: idPush::DiscardEntities()
		if (ent->IsHidden() || ent->entityNumber <= 0 || ent->entityNumber >= MAX_GENTITIES - 1 || ent->entityDefNumber < -1 /*|| ent->GetPhysics() == NULL || ent->team >= TEAM_LAST*/)
		{
			continue;
		}
		...
		ent->ProcessEvent( &EV_Touch, this, &trace ); //BC this gets called when player collides into any entity (i.e. triggers)
...
/*
================
idEntity::Event_EntityTeleport

Exposes teleport method to scripting.
...
void idEntity::Event_SetOwner( idEntity *owner ) {
	...
	//TODO: figure out what I was trying to achieve here
	//So this is somehow related to making objects go zero-g. I have no idea what this hooks up to. It looks like a nasty hack, so I am going to comment it out.
	//if (owner)
	//{
...
void idEntity::Event_StartSoundShader( const char *soundName, int channel ) {
	...
	bool played = StartSoundShader( declManager->FindSound( soundName ), (s_channelType)channel, 0, false, &length );
	
	if(length == 0 || !played)
	{
		gameLocal.Printf("StartSoundShader did not play: %s\n", soundName);
...
void idEntity::Event_SetGuiParm( const char *key, const char *val ) {
	bool changed = false;
	idStr* lastValPtr = nullptr;
	if (!GuiParamStr.Get(key, &lastValPtr) || (idStr::Cmp(val, *lastValPtr) != 0)) {
		changed = true;
		idStr newVal = val;
		GuiParamStr.Set(key, newVal);
	}
	...
	for ( int i = 0; i < MAX_RENDERENTITY_GUI; i++ ) {
		if (renderEntity.gui[i]) {
			...
			if (changed)
			{
				renderEntity.gui[i]->SetStateString(key, common->GetLanguageDict()->GetString(val));
...
void idEntity::Event_SetGuiFloat( const char *key, float f ) {
	float* lastValPtr = nullptr;
	if (!GuiParamFloat.Get(key, &lastValPtr) || f != *lastValPtr)
	{
		GuiParamFloat.Set(key, f);
		for ( int i = 0; i < MAX_RENDERENTITY_GUI; i++ ) {
			if ( renderEntity.gui[ i ] ) {
...
void idEntity::Event_SetGuiInt(const char *key, int f) {
	int* lastValPtr = nullptr;
	if (!GuiParamInt.Get(key, &lastValPtr) || f != *lastValPtr)
	{
		GuiParamInt.Set(key, f);
		for (int i = 0; i < MAX_RENDERENTITY_GUI; i++) {
			if (renderEntity.gui[i]) {
				renderEntity.gui[i]->SetStateString(key, va("%d", f));
				renderEntity.gui[i]->StateChanged(gameLocal.time);
			}
		}
	}
...
void idEntity::Event_UpdateCameraTarget( void ) {
	...
	if ( cameraTarget ) {
		...
		// SM: If the camera target is invalid, make sure we clear out the remote render view
...
//BC
void idEntity::Event_DistanceToPoint2D(const idVec3 &point)
{
	float dist = (GetPhysics()->GetOrigin().ToVec2() - point.ToVec2()).LengthFast();
	idThread::ReturnFloat(dist);
}
...
void idEntity::Event_SetGui( int guiNum, const char *guiName) {
	SetGui(guiNum, guiName, false);

}

void idEntity::Event_SetGuiUnique(int guiNum, const char* guiName) {
	SetGui(guiNum, guiName, true);
}

void idEntity::SetGui(int guiNum, const char* guiName, bool unique) {
	...
	if (gui) {
		*gui = uiManager->FindGui(guiName, true, unique);
...
void idAnimatedEntity::AddLocalDamageEffect( jointHandle_t jointNum, const idVec3 &localOrigin, const idVec3 &localNormal, const idVec3 &localDir, const idDeclEntityDef *def, const idMaterial *collisionMaterial ) {
	...
	if (this->IsType(idAI::Type))
	{
		if (static_cast<idAI *>(this)->energyShieldCurrent > 0)
		{
			return;
		}
	}
	...
	if ( !gameLocal.isMultiplayer && g_bloodEffects.GetBool() && !(IsType(idPlayer::Type)))
	{
		...
		if ( *decal == '\0' )
		{
			//unable to find the decal.
			decal = def->dict.RandomPrefix( key, gameLocal.random );
		}
		
		if ( *decal != '\0' )
		{
			//BC this is the wound decal that appears on models.
			//apply the decal.
			ProjectOverlay( origin, dir, 15 /*decalsize*/, decal );
		}
	}
...
int idAnimatedEntity::Event_PlayAnim(const char* animname, int animBlendFrames, bool looping)
{
	int anim;
	int channel = 0;
	int animDoneTime = 0;
	...
	if (!anim)
	{
		gameLocal.Warning("missing '%s' animation on '%s' (%s)", animname, name ? name.c_str() : "unknown", GetEntityDefName());
...
void idEntity::ShowHealthbar()
{
	healthbarDisplaytime = gameLocal.time + HEALTHBAR_DISPLAYTIME;
}

void idEntity::ShowCameraicon()
{
	cameraiconDisplaytime = gameLocal.time + SECURITYCAMERA_DISPLAYTIME;
}
...
idEntity::DoFrob
	...
	SW: Because GUIs are not derived from idEntity, we cannot directly call DoFrob on them.
	Instead, the entity on which the GUI is 'hosted' must interpret this,
	and fire off a mouse click event to the GUI.

================
*/
bool idEntity::DoFrob(int index, idEntity* frobber)
{

	//Generic call script when entity is frobbed.
	idStr frobScript = spawnArgs.GetString("call_onfrob");
	if (frobScript.Length() > 0)
	{
		gameLocal.RunMapScriptArgs(frobScript.c_str(), frobber, this );
	}



	// Making the assumption that only one GUI exists per entity? 
	// Not sure if this is future-proof. Or even present-proof.
	// 	   
	// I'm also intentionally excluding thrown objects from triggering the event here,
	// since that seems like the kind of thing that would only ever happen by accident.
	// 
	// SM: Only enable this path if we have gui_inGameLeftClick set to true, which is the old behavior
	if (renderEntity.gui[0] && frobber->IsType(idPlayer::Type) && gui_inGameLeftClick.GetBool())
	{
		const char* command = NULL;
		bool updateVisuals = false;

		sysEvent_t evButtonDown = sys->GenerateMouseButtonEvent(BUTTON_ATTACK, 1);
		command = renderEntity.gui[0]->HandleEvent(&evButtonDown, gameLocal.time, &updateVisuals);
...
bool idEntity::DoFrobHold( int index, idEntity* frobber )
{
	return false;
}

void idEntity::DoRepairTick(int amount) //For the repair bot.
{
}

void idEntity::DoGenericImpulse(int index)
{
	//BC generic way to hook up GUI buttons to code.

	//if we want to re-route the impulse to the owner.
	if (spawnArgs.GetBool("send_owner_impulse"))
	{
		if (GetPhysics()->GetClipModel()->GetOwner() != NULL)
		{
			GetPhysics()->GetClipModel()->GetOwner()->DoGenericImpulse(index);
		}
	}
}

void idEntity::SetDepthHack(bool value)
{
	renderEntity.weaponDepthHack = value;
}
...
void idEntity::Event_GetRepairPosition(void)
{
	idThread::ReturnVector(this->repairWorldposition);
}

void idEntity::Event_GetCurrentHealth(void)
{
	idThread::ReturnInt(this->health);
}
...
//BC
void idEntity::UpdateGravity(bool forceUpdate)
{
	...
	newAirless = gameLocal.GetAirlessAtPoint(this->GetPhysics()->GetOrigin()) || forceNoGravity;


	if ((newAirless != lastGravityState) || forceUpdate)
	{
		//state has changed.
		if (newAirless)
		{
			...
			if (zeroGravityNudge)
			{
				//If object is still, give it a small push upward. Otherwise, inherit current velocity.
				idVec3 curVel = this->GetPhysics()->GetLinearVelocity();
				if (curVel.Length() <= 0)
				{
					//nudge upward when gravity turns off.
					idVec3 upwardsVel = idVec3(0, 0, IsType(idActor::Type) ? this->GetPhysics()->GetMass() : this->GetPhysics()->GetMass() / 2);
					
                    //We give actors a bit more of a nudge because it looks/feels better.
                    const float MAX_UPWARDS = IsType(idActor::Type) ? 30.0f : 5.0f;

					upwardsVel.z = Min(upwardsVel.z, MAX_UPWARDS);
					this->GetPhysics()->SetLinearVelocity(upwardsVel);

					this->GetPhysics()->SetAngularVelocity(idVec3(gameLocal.random.CRandomFloat() * GRAVITYFLOATANGULAR, gameLocal.random.CRandomFloat() * GRAVITYFLOATANGULAR, gameLocal.random.CRandomFloat() * GRAVITYFLOATANGULAR));
				}
			}
		...
		{
			this->GetPhysics()->SetGravity(gameLocal.GetGravity());  //return to normal gravity.

			if (zeroGravityNudge)
			{
				this->GetPhysics()->SetLinearVelocity(idVec3(0, 0, 1)); //nudge it a little to wake it up.
			}
		}
...
void idEntity::Event_SetCinematic(int value)
{
	cinematic = (value > 0) ? true : false;
}



bool idEntity::isInOuterSpace()
{
	...
	if (locationEntity)
	{
		...
		return (!idStr::Cmp(locationEntity->GetLocation(), common->GetLanguageDict()->GetString("#str_00000")));
	}
...
//BC from quadcow
void idEntity::Event_SetCameraTarget(idEntity *ent)
{
	ent->cameraTarget = ent;
	this->cameraTarget = ent;

	UpdateVisuals();  //force camera update.
}

bool idEntity::DoesPlayerHaveLOStoMe()
{
	//Do distance check. If we're right next to the baddie, do slow mo.
	float distToPlayer = (GetPhysics()->GetOrigin() - gameLocal.GetLocalPlayer()->GetPhysics()->GetOrigin()).LengthFast();

	//Do direction check.
	idVec3 dirPlayerToMe = GetPhysics()->GetOrigin() - gameLocal.GetLocalPlayer()->firstPersonViewOrigin;
	float facingResult = DotProduct(dirPlayerToMe, gameLocal.GetLocalPlayer()->viewAngles.ToForward());
	if (facingResult < 0 && distToPlayer > 64)
		return false; //I am not within the 180 degree cone of player. Ignore.


	//Do a series of traces to see if player has LOS to the enemy.
	idBounds actorBounds = GetPhysics()->GetBounds();
	for (int i = 0; i < 3; i++)
	{
		idVec3 targetPoint;
		if (i == 0)
			targetPoint = GetPhysics()->GetOrigin() + idVec3(0, 0, actorBounds[1].z); //check head.
		else if (i == 1)
			targetPoint = GetPhysics()->GetOrigin() + idVec3(0, 0, actorBounds[1].z / 2); //check center of mass.
		else
			targetPoint = GetPhysics()->GetOrigin() + idVec3(0, 0, 1); //check feet.

		trace_t tr;
		gameLocal.clip.TracePoint(tr, gameLocal.GetLocalPlayer()->firstPersonViewOrigin, targetPoint, MASK_SOLID, NULL);
		if (tr.fraction >= 1)
		{
			//has LOS.
			return true;
...
{
	// Don't go through updating visuals if the flag is the same it currently is
	// We also need this to prevent any infinite recursion in the case where a child
	// tells its parent to set the flag (currently this is the case with bc_frobcube)
	if (enable != isCustomPostFlagSet(&renderEntity, flag))
	{
		if (enable)
			setCustomPostFlag(&renderEntity, flag);
		else
			clearCustomPostFlag(&renderEntity, flag);
...
void idEntity::DynamicUpdateDrawGlobally(bool isGlobal)
{
	if (fl.drawGlobally != isGlobal)
	{
		fl.drawGlobally = isGlobal;
		if (modelDefHandle != -1)
		{
			if (fl.drawGlobally)
				gameRenderWorld->AddGlobalRenderEnt(modelDefHandle);
...
void idEntity::Event_SetFrobbable(int value)
{
	isFrobbable = (value > 0) ? true : false;
}
...
void idEntity::Event_SetFrozen( int enable )
{
	isFrozen = ( enable > 0) ? true : false;
	renderEntity.isFrozen = isFrozen;
	for ( idEntity* childEnt = GetNextTeamEntity(); childEnt != nullptr; childEnt = childEnt->GetNextTeamEntity() )
	{
		childEnt->isFrozen = isFrozen;
		childEnt->renderEntity.isFrozen = isFrozen;
	}
...
void idEntity::Event_UpdateDisplayName(char* newName)
{
	displayName = newName;
}
...
//Script call to repair entity.
void idEntity::Event_RepairEntity()
{
	DoRepairTick(1);
}

void idEntity::Event_SetSystemicVoEnabled(int value)
{
	systemicVoEnabled = (value > 0) ? true : false;
}

void idEntity::Event_IsPlayingSound(int channel)
{
	idThread::ReturnFloat( (float) (IsPlayingSound(channel)) );
}