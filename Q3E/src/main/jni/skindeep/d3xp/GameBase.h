...
// SW: We have very tentatively bumped this up from the base value of 12 in order to accommodate larger maps like sh_guppy.
// It looks like the engine is pretty good about not using hardcoded assumptions, and in theory there's no reason why this won't scale safely.
// We should keep an eye out for loops that iterate over arrays of size MAX_GENTITIES without a way to break out, though -- this might degrade their performance noticeably.
// We've also consequentially halved the cap on idGameLocal::spawnCount (see idGameLocal::RegisterEntity for details) but that's probably not an issue either.
