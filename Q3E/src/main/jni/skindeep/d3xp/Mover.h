...
public:
	...
	void					Event_MoveToLocalPos(idVec3& pos);
...
typedef enum {
	MOVER_POS1, //0 fully closed.
	MOVER_POS2, //1 fully open.
	MOVER_1TO2, //2 opening.
	MOVER_2TO1 //3 closing.
} moverState_t;
...
public:
	...
	void					SetPostEvents(bool value);
	void					SetSparksEnabled(bool value) { sparksEnabled = value; }
protected:
	idVec3					pos1;
	idVec3					pos2;
	...
	//BC
	void					DoParticleFX_Open();
	void					DoParticleFX_Close();
	void					DoParticleFX_Sparks();
	void					ClearParticleFX_Sparks();
...
public:
	...
	void					ForceTriggerDelay(int millisec);
	void					SetUnlockDelay(int millisec);
	virtual void			Killed(idEntity *inflictor, idEntity *attacker, int damage, const idVec3 &dir, int location);
	void					SetCrusher(bool value);

	void					AddBarricade();
	void					RemoveBarricade();
	bool					IsBarricaded();
	idVec3					GetBarricadeColor();

	void					Event_SetBarricade(int value);

	//BC public end

protected:
	void					SpawnVentPeek( const char* name, const idVec3& position, float extraYaw = 0.0f );
	class idDoorBarricade*	SpawnBarricade( const char* name, const idVec3& position, float extraYaw = 0.0f );
	virtual void			Event_Reached_BinaryMover(void); //bc added the VIRTUAL

	virtual void			SetAASAreaState(bool closed); // SW: made virtual so that ventdoors can override this.
...
private:
	...
	idVec3					shoveDir;				// SW: Direction (relative to the door model) that blockages should be shoved.
													// This will be inverted by code depending on what side of the door they're closest to.

	

	void					GetLocalTriggerPosition( const idClipModel *trigger );
	void					CalcTriggerBounds( float size, idBounds &bounds );
	void					DisplacePotentialBlockage(idPhysics* blockage);
	...
	void					Event_SpawnDisplaceTrigger(void);
	...
	bool					AllowMonsterOnly(idEntity *ent);
	bool					monster_trigger;
	bool					damage_only_objects; //Don't damage players or AI, only damage things that are static and/or corpses.

	//BC private
	bool					blocked_stayopen;
	int						triggerDelaytime;
	bool					playerBlock; //stays open only if player is the thing in the door trigger.
	bool					AllowPlayerBlock(idEntity *ent);
	bool					blocked_pushaway;

	bool					doorLocked;
	bool					unlockdelayActive;
	int						unlockdelayTime;
	bool					vacuumlock; // SM

	int						airlessCheckTimer;
	idVec3					originalPosition;
	idVec3					originalCenterMass;
