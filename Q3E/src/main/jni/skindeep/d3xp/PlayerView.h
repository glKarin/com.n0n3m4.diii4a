...
private:
	const idMaterial *		ftlVignetteMaterial;	//BC
};



//BC
class FullscreenFX_DownedState : public FullscreenFX
{
	const idMaterial*		material;
...
class FullscreenFX_RoqVideo : public FullscreenFX
{
	const idMaterial* material;
...
class FullscreenFX_ConcussVision : public FullscreenFX {
	const idMaterial*		material;
...
private:
	...
	int						rampTime;
	int						state;
	enum					{CONC_IDLE, CONC_RAMPUP, CONC_DOUBLEVISION, CONC_RAMPDOWN};
	int						stateTimer;

	int						lastRampdownTime;
};


class FullscreenFX_BulletwoundVision : public FullscreenFX {
    const idMaterial*		material;
...
private:
    ...
    void                    DrawHQ(float lerp);
};


// SM
class FullscreenFX_Blur : public FullscreenFX
{
	static const int MAX_BLUR_PASSES = 8;
	const idMaterial*		material;
...
class FullscreenFX_TelescopeBlur : public FullscreenFX
{
	static const int MAX_TELESCOPEBLUR_PASSES = 8;
	const idMaterial*		material;
...
class FullscreenFX_GasVision : public FullscreenFX {
	const idMaterial*		material;
	const idMaterial*		lidMaterial;
	const idMaterial*		blurMaterial;
...
private:
	int						state;
	enum					{GV_IDLE, GV_CLOSING, GV_OPENING};

	int						stateTimer;
	
};

class FullscreenFX_Brightness : public FullscreenFX
{
	const idMaterial*		material;
...
class FullscreenFX_MotionBlur : public FullscreenFX
{
	const idMaterial*		material;
...
class FullscreenFX_ZoomMode : public FullscreenFX
{
	const idMaterial*		material;
...
class FullscreenFX_Outline : public FullscreenFX
{
	const idMaterial* material;
...
private:
	...
	int					hiddenTimer;
	bool				hiddenActive;
	float				hiddenStartAlpha;
	float				hiddenEndAlpha;
	float				hiddenCurrentAlpha;
	void				UpdateHiddenVignette();


	int					confinedTimer;
	bool				confinedActive;
	float				confinedStartAlpha;
	float				confinedEndAlpha;
	float				confinedCurrentAlpha;
	void				UpdateConfinedVignette();

	int					isLightedTimer;
	bool				isLightedActive;
	
	void				UpdateDurabilityFlash();
	int					durabilityflashStartTime;
	bool				durabilityflashActive;
	const idMaterial *	durabilityflashMaterial;


    bool                bloodrageLerping;
    float               bloodrageStartLerpValue;
    float               bloodrageEndLerpValue;
    float               bloodrageCurrentValue;
    int                 bloodrageLerpTimer;
	int					bloodrageTotalLerpTime;
...
public:
	...
	//BC public
	const idMaterial *	bloodedgeMaterial;
	const idMaterial *	bokehMaterial;		//BC	
	void				SetBloodbagOverlay();
	bool				bloodbagOverlayActive;
	int					bloodbagState;
	int					bloodbagTimer;
	...
	void				DoDurabilityFlash(); //when a held item explodes

    void                DoBloodrageLerp(float newValue, int timeMS);
