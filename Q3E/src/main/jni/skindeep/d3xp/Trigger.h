...
extern const idEventDef EV_PlayerLookEnter;
extern const idEventDef EV_PlayerLookExit;
...
protected:
	void				CallScriptArgs( idEntity* activator, idEntity* self, const function_t* functionToCall = nullptr ) const;
...
private:
	...
	void				Event_PlayerLookEnter();
	void				Event_PlayerLookExit();

	const function_t*	playerLookEnterFunc;
	const function_t*	playerLookExitFunc;
	float				playerLookRange;
};
...
//BC Trigger that pushes player

class idTrigger_Push : public idTrigger
{
public:
	CLASS_PROTOTYPE(idTrigger_Push);
...
private:
	...
	void				Event_Touch(idEntity *other, trace_t *trace);
	void				Event_triggerpushactivate(int value);
};


//===============================================================================

//BC Trigger that activates when a classname enters it

class idTrigger_Classname : public idTrigger
{
public:

	CLASS_PROTOTYPE(idTrigger_Classname);

	idTrigger_Classname(void);
...
private:
	...
	int					timer;
	// SM
	idList<idStr>		requiredClassnames;
	bool				Matches(const idStr& className);
...
//BC Trigger that teleports player back to last time they touched solid ground. Used for death pits

class idTrigger_FallTrigger : public idTrigger
{
public:

	CLASS_PROTOTYPE(idTrigger_FallTrigger);

	idTrigger_FallTrigger(void);
