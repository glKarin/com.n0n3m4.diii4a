...
#include "renderer/tr_local.h" // blendo eric: for clip bounds debug
...
// blendo eric: for clip bounds debug
idCVar cm_drawPolyBounds(	"cm_drawPolyBounds",	"0",		CVAR_GAME | CVAR_BOOL,	"blendo eric: draw polygon bounds for collision debug");
...
void idCollisionModelManagerLocal::DrawEdge( cm_model_t *model, int edgeNum, const idVec3 &origin, const idMat3 &axis ) {
	...
	if ( edge->internal ) {
		...
		if ( edge->numUsers > 2 ) {
			...
			session->rw->DebugLine(cm_color, start, end, 1); //BC do line instead of arrow.
		}
	}
...
void idCollisionModelManagerLocal::DrawPolygon( cm_model_t *model, cm_polygon_t *p, const idVec3 &origin, const idMat3 &axis, const idVec3 &viewOrigin ) {
	...
	// blendo eric: clip bounds debug
	if (cm_drawPolyBounds.GetBool()) {
		session->rw->DebugBounds(colorOrange, p->bounds, origin);
	}
}
...
void idCollisionModelManagerLocal::DebugOutput( const idVec3 &origin ) {
	...
	for ( i = 0; i < cm_testTimes.GetInteger(); i++ ) {
		Translation( &trace, start, testend[i], &itm, boxAxis, CONTENTS_SOLID|CONTENTS_PLAYERCLIP, cm_testModel.GetInteger(), vec3_origin, modelAxis, 0 );
	}
	timer.Stop();
	...
	if ( cm_testRotation.GetBool() ) {
		...
		for ( i = 0; i < cm_testTimes.GetInteger(); i++ ) {
			...
			Rotation( &trace, start, rotation, &itm, boxAxis, CONTENTS_SOLID|CONTENTS_PLAYERCLIP, cm_testModel.GetInteger(), vec3_origin, modelAxis, 0 );
		}
		timer.Stop();
		t = timer.Milliseconds();
