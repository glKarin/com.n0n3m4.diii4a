...
cm_model_t *idCollisionModelManagerLocal::LoadRenderModel( const char *fileName ) {
	...
	common->Printf( "Loaded collision %s\n", model->name.c_str() );
...
void idCollisionModelManagerLocal::PrintModelInfo( const cm_model_t *model ) {
	common->DPrintf( "%6i vertices (%zu KB)\n", model->numVertices, (model->numVertices * sizeof(cm_vertex_t))>>10 );
	common->DPrintf( "%6i edges (%zu KB)\n", model->numEdges, (model->numEdges * sizeof(cm_edge_t))>>10 );
	common->DPrintf( "%6i polygons (%i KB)\n", model->numPolygons, model->polygonMemory>>10 );
	common->DPrintf( "%6i brushes (%i KB)\n", model->numBrushes, model->brushMemory>>10 );
	common->DPrintf( "%6i nodes (%zu KB)\n", model->numNodes, (model->numNodes * sizeof(cm_node_t))>>10 );
	common->DPrintf( "%6i polygon refs (%zu KB)\n", model->numPolygonRefs, (model->numPolygonRefs * sizeof(cm_polygonRef_t))>>10 );
	common->DPrintf( "%6i brush refs (%zu KB)\n", model->numBrushRefs, (model->numBrushRefs * sizeof(cm_brushRef_t))>>10 );
	common->DPrintf( "%6i internal edges\n", model->numInternalEdges );
	common->DPrintf( "%6i sharp edges\n", model->numSharpEdges );
	common->DPrintf( "%6i contained polygons removed\n", model->numRemovedPolys );
	common->DPrintf( "%6i polygons merged\n", model->numMergedPolys );
	common->DPrintf( "%6i KB total memory used\n", model->usedMemory>>10 );
}
...
void idCollisionModelManagerLocal::BuildModels( const idMapFile *mapFile ) {
	...
	//BC force reloading the collision model. Changes were sometimes not being propagated.
	...
	common->DPrintf( "collision data:\n" );
	common->DPrintf( "%6i models\n", numModels );
	PrintModelInfo( &model );
	common->DPrintf( "%u msec to load collision data.\n", timer.Milliseconds() );
}
...
bool idCollisionModelManagerLocal::TrmFromModel( const cm_model_t *model, idTraceModel &trm ) {
	...
	{
		//BC make this an error.
		//common->Printf( "idCollisionModelManagerLocal::TrmFromModel: model %s has too many edges.\n", model->name.c_str() );
		common->Error("idCollisionModelManagerLocal::TrmFromModel: model %s has too many edges.\n", model->name.c_str());
		PrintModelInfo( model );
		return false;
	}
	...
	if ( !TrmFromModel_r( trm, model->node ) ) {
		common->Printf( "idCollisionModelManagerLocal::TrmFromModel: clipmodel '%s' has too many polygons (max: %d).\n", model->name.c_str(), MAX_TRACEMODEL_POLYS);
		PrintModelInfo( model );
		return false;
	}
