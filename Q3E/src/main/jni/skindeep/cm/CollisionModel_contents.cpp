...
bool idCollisionModelManagerLocal::TestTrmVertsInBrush( cm_traceWork_t *tw, cm_brush_t *b ) {
	...
	if ( !(b->contents & tw->contents) || (b->contents != -1 && (b->contents & tw->ignoreContents)) ) {
		return false;
	}
...
int idCollisionModelManagerLocal::Contents( const idVec3 &start,
	...
	return ContentsTrm( &results, start, trm, trmAxis, contentMask, model, modelOrigin, modelAxis, ignoreContentMask );
}
