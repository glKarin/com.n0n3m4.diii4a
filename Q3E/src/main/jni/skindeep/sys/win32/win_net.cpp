...
void Sys_InitNetworking( void ) {
	...
	common->DPrintf( "Winsock Initialized\n" );
	...
	if( ( dwRetVal = GetAdaptersInfo( pAdapterInfo, &ulOutBufLen) ) != NO_ERROR ) {
		...
		while( pAdapter ) {
			common->DPrintf( "Found interface: %s %s - ", pAdapter->AdapterName, pAdapter->Description );
	...
	if( !foundloopback && num_interfaces < MAX_INTERFACES ) {
		common->DPrintf( "Sys_InitNetworking: adding loopback interface\n" );
