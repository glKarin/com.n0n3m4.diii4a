...
void idBrushBSP::Build( idBrushList brushList, int skipContents,
	...
	common->DPrintf( "[Brush BSP]\n" );
	common->DPrintf( "%6d brushes\n", brushList.Num() );
	...
	common->DPrintf( "\r%6d grid cells\n", gridCells.Num() );
...
void idBrushBSP::Portalize( void ) {
	common->DPrintf( "[Portalize BSP]\n" );
	common->DPrintf( "%6d nodes\n", (numSplits - numPrunedSplits) * 2 + 1 );
	numPortals = 0;
	MakeOutsidePortals();
	MakeTreePortals_r( root );
	common->DPrintf( "\r%6d nodes portalized\n", numPortals );
}
...
bool idBrushBSP::RemoveOutside( const idMapFile *mapFile, int contents, const idStrList &classNames ) {
	...
	common->DPrintf( "%6d solid leaf nodes\n", solidLeafNodes );
	common->DPrintf( "%6d outside leaf nodes\n", outsideLeafNodes );
	common->DPrintf( "%6d inside leaf nodes\n", insideLeafNodes );
...
void idBrushBSP::MergePortals( int skipContents ) {
	numMergedPortals = 0;
	common->DPrintf( "[Merge Portals]\n" );
	SetPortalPlanes();
	MergePortals_r( root, skipContents );
	common->DPrintf( "%6d portals merged\n", numMergedPortals );
}
