...
void idAASBuild::SetPortalFlags_r( idBrushBSPNode *node ) {
	...
	if ( !node->GetChild(0) && !node->GetChild(1) ) {
		for ( p = node->GetPortals(); p; p = p->Next(s) ) {
			...
			// if solid at the other side of the portal, this is either a floor (FACE_FLOOR) or a non-walkable solid surface (FACE_SOLID)
		...
		if (node->GetContents() & AREACONTENTS_NOWALK)
			node->SetFlag( AREA_NOWALK );
...
void idAASBuild::GravitationalSubdivision( idBrushBSP &bsp ) {
	...
	common->DPrintf( "[Gravitational Subdivision]\n" );

	SetPortalFlags_r( bsp.GetRootNode() );
	GravSubdiv_r( bsp.GetRootNode() );

	common->DPrintf( "\r%6d subdivisions\n", numGravitationalSubdivisions );
}
