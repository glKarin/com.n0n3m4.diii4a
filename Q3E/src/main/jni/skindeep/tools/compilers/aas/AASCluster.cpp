...
bool idAASCluster::Build( idAASFileLocal *file ) {
	...
	while( 1 ) {
		...
		common->DPrintf( "\r%6d", file->portals.Num() );
	...
	common->DPrintf( "\r%6d portals\n", file->portals.Num() );
	common->DPrintf( "%6d clusters\n", file->clusters.Num() );

	for ( int i = 0; i < file->clusters.Num(); i++ ) {
		common->DPrintf( "%6d reachable areas in cluster %d\n", file->clusters[i].numReachableAreas, i );
	}
