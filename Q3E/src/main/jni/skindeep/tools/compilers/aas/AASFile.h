...
#define AREA_NOWALK					BIT(8)		// SW: Area should not be walked through (but still retains reachability/walkability)
...
#define AREACONTENTS_NOWALK			BIT(5)		// SW: Area contains a 'no walk' volume. AI will try to path out of it, but not into it
...
typedef struct aasTrace_s {
	...
	int							getOutOfLedge;		// SW: trace out of ledge/nowalk areas if the trace starts in them

	// SW: Areas MUST HAVE these flags in order to NOT block the trace
	// Please make sure these are mutually exclusive with the flags
	int							requireFlags;		
	int							requireTravelFlags;
	...
	aasTrace_s(void)			{ areas = NULL; points = NULL; getOutOfSolid = false; getOutOfLedge = false; flags = requireFlags = travelFlags = requireTravelFlags = maxAreas = 0; }
} aasTrace_t;
