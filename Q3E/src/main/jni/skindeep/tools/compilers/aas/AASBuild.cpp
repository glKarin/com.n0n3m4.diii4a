...
void idAASBuild::ClipBrushSidesWithProcBSP( idBrushList &brushList ) {
	...
	common->DPrintf( "%6d brush sides clipped\n", clippedSides );
}
...
int idAASBuild::ContentsForAAS( int contents ) {
	...
	if (contents & CONTENTS_NOWALK) {
		c |= AREACONTENTS_NOWALK;
	}
	return c;
}
...
bool idAASBuild::Build( const idStr &fileName, const idAASSettings *settings ) {
	...
	if ( !CheckForEntities( mapFile, entityClassNames ) ) {
		delete mapFile;
		common->DPrintf( "no entities in map that use %s\n", settings->fileExtension.c_str() );
		return true;
	}
	...
	// Counterintuitively, this does not dump all the ledges into a file -- it simply creates the file object for writing to in later methods
	if ( aasSettings->writeBrushMap ) {
	...
	common->DPrintf( "%6d seconds to create AAS\n", (Sys_Milliseconds() - startTime) / 1000 );
