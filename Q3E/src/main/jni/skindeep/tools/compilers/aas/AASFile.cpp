...
bool idAASFileLocal::Load( const idStr &fileName, unsigned int mapFileCRC ) {
	...
	if ( mapFileCRC && c != mapFileCRC ) {
		common->Warning( "AAS file '%s' is out of date. Run DMAP on this map.", name.c_str() );
		return false;
	}
...
void idAASFileLocal::PrintInfo( void ) const {
	common->DPrintf( "%6d KB file size\n", MemorySize() >> 10 );
	common->DPrintf( "%6d areas\n", areas.Num() );
	common->DPrintf( "%6d max tree depth\n", MaxTreeDepth() );
...
void idAASFileLocal::ReportRoutingEfficiency( void ) const {
	...
	common->DPrintf( "%6d reachable areas\n", numReachableAreas );
	common->DPrintf( "%6d reachabilities\n", NumReachabilities() );
	common->DPrintf( "%6d KB max routing cache\n", ( total * 3 ) >> 10 );
}
