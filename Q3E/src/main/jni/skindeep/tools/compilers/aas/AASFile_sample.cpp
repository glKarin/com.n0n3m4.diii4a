...
bool idAASFileLocal::Trace( aasTrace_t &trace, const idVec3 &start, const idVec3 &end ) const {
	...
	while( 1 ) {
		...
		if ( nodeNum < 0) {
			...
			if ( 
					( areas[-nodeNum].flags & trace.flags ) || 
					( areas[-nodeNum].travelFlags & trace.travelFlags ) || 
					( trace.requireFlags && !( areas[-nodeNum].flags & trace.requireFlags )) || 
					( trace.requireTravelFlags && !( areas[-nodeNum].travelFlags & trace.requireTravelFlags )) 
				) {
				if ( !trace.lastAreaNum ) {
					trace.fraction = 0.0f;
					v1 = vec3_origin;
				...
				if ( trace.lastAreaNum || !trace.getOutOfLedge ) {
					return true;
				}	
			}
