...
void idAASReach::Reachability_EqualFloorHeight( int areaNum ) {
	int i, k, l, m, n, faceNum, face1Num, face2Num, otherAreaNum, edge1Num = 0, edge2Num, walkabilityMultiplier;
	...
	// SW: Make areas with 'nowalk' vastly less desirable for pathing
	walkabilityMultiplier = (area->contents & AREACONTENTS_NOWALK) 
		? 1000
		: 1;
	...
	for ( i = 0; i < area->numFaces; i++ ) {
		...
		if ( k < area->numFaces ) {
			...
			reach->travelTime = 1 * walkabilityMultiplier;
				...
				if ( trace.lastAreaNum && trace.fraction >= 1.0f ) {
					...
					if ( trace.lastAreaNum == area2num ) {
						...
						if ( i >= trace.numAreas )
						{
							...
							//common->Printf("\n    Creating TFL_WALKOFFLEDGE from %d to %d.", area1num, area2num);
							return true;
						}
					}
				}
...
bool idAASReach::Build( const idMapFile *mapFile, idAASFileLocal *file ) {
	...
	common->DPrintf( "\r%6d reachabilities\n", numReachabilities );
