...
static void SetBrushContents( uBrush_t *b ) {
	...
	for ( i=1 ; i<b->numsides ; i++, s++ ) {
		...
		if ( !s->material->HasCoverage(MC_OPAQUE) ) {
			b->opaque = false;
		}
	}
...
bool LoadDMapFile( const char *filename ) {
	...
	common->Printf( "----- LoadDMapFile: loading '%s'\n", filename );
	...
	common->DPrintf( "%5i total world brushes\n", brushes );
	common->DPrintf( "%5i total world triSurfs\n", triSurfs );
	common->DPrintf( "%5i patches\n", c_numMapPatches );
	common->DPrintf( "%5i entities\n", dmapGlobals.num_entities );
	common->DPrintf( "%5i planes\n", dmapGlobals.mapPlanes.Num() );
	common->DPrintf( "%5i areaportals\n", c_areaportals );
	common->DPrintf( "size: %5.0f,%5.0f,%5.0f to %5.0f,%5.0f,%5.0f\n", mapBounds[0][0], mapBounds[0][1],mapBounds[0][2],
