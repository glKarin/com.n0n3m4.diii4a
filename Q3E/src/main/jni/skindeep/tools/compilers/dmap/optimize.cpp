...
static	void RemoveIfColinear( optVertex_t *ov, optIsland_t *island ) {
	...
	if ( !e2 ) {
		...
		common->DPrintf( "WARNING: vertex with only one edge\n" );
		return;
	}
...
static void LinkTriToEdge( optTri_t *optTri, optEdge_t *edge ) {
	if ( ( edge->v1 == optTri->v[0] && edge->v2 == optTri->v[1] )
		...
		if ( edge->backTri ) {
			common->DPrintf( "Warning: LinkTriToEdge: already in use\n" );
			return;
		}
		edge->backTri = optTri;
		return;
...
//DrawEdges( island );
	...
	if ( !opposite ) {
		common->DPrintf( "Warning: BuildOptTriangles: couldn't locate opposite\n" );
		return;
	}
...
static	void	RegenerateTriangles( optIsland_t *island ) {
	...
	for ( optTri = island->tris ; optTri ; optTri = optTri->next ) {
		...
		if ( plane.Normal() * dmapGlobals.mapPlanes[ island->group->planeNum ].Normal() <= 0 ) {
			...
			common->DPrintf( "WARNING: backwards triangle generated!\n" ); //BC remove this warning...
			// discard it
			FreeTri( tri );
			continue;
		}
...
static void AddOriginalTriangle( optVertex_t *v[3] ) {
	...
	if ( !IsTriangleValid( v[0], v[1], v[2] ) ) {
		common->DPrintf( "WARNING: backwards triangle in input!\n" ); //BC remove this warning...
		return;
	}
...
void	OptimizeGroupList( optimizeGroup_t *groupList ) {
	...
	common->DPrintf( "----- OptimizeAreaGroups Results -----\n" );
	common->DPrintf( "%6i tris in\n", c_in );
	common->DPrintf( "%6i tris after edge removal optimization\n", c_edge );
	common->DPrintf( "%6i tris after final t junction fixing\n", c_tjunc2 );
}
