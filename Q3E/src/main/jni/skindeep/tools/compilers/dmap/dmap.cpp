...
============
*/
bool Dmap( const idCmdArgs &args ) {
	int			i;
	int			start, end;
	char		path[1024];
	idStr		passedName;
	...
	//BC default to none. The default setting (SO_MERGE_SURFACES) was causing some problems with the huge light that covers the entire map.
	dmapGlobals.shadowOptLevel = SO_NONE;
...
//	dmapGlobals.shadowOptLevel = SO_CULL_OCCLUDED;
	...
	// SM: Set noFloodEntities based on cvar
	dmapGlobals.noFloodEntities.Clear();
	idStr str = cvarSystem->GetCVarString( "dmap_noFloodEntities" );
	idStr entityType;
	while ( str.Length() ) {
		int n = str.Find( ";" );
		if ( n >= 0 ) {
			entityType = str.Left( n );
			str = str.Right( str.Length() - n - 1 );
		} else {
			entityType = str;
			str = "";
		}
		dmapGlobals.noFloodEntities.AddUnique( entityType );
	}
	...
	common->DPrintf( "%i total shadow triangles\n", dmapGlobals.totalShadowTriangles );
	common->DPrintf( "%i total shadow verts\n", dmapGlobals.totalShadowVerts );

	end = Sys_Milliseconds();	
	common->DPrintf( "%5.0f seconds for dmap\n", ( end - start ) * 0.001f );
	...
	if ( !leaked ) {
		...
		if ( !noCM ) {
			...
			common->DPrintf( "%5.0f seconds to create collision map\n", ( end - start ) * 0.001f );
		}
...
============
*/
bool Dmap_f( const idCmdArgs &args ) {
...
//BC We have this because we need a dmap that doesn't return a bool, for "dmap2" to work.
void DmapStandard_f(const idCmdArgs &args) {
	Dmap_f(args);
}
