...
#ifdef DEMO
idCVar	idFileSystemLocal::fs_game("fs_game", "basedemo", CVAR_SYSTEM | CVAR_INIT | CVAR_SERVERINFO | CVAR_ROM, "mod path");
#else
...
idModList *idFileSystemLocal::ListMods( void ) {
	...
	const int	NUM_SEARCH_PATHS = 3;
	const char	*search[ NUM_SEARCH_PATHS ];
	int			isearch;
	...
	for ( isearch = 0; isearch < NUM_SEARCH_PATHS; isearch++ ) {
...
idStr idFileSystemLocal::GetModDescription(idStr descPath)
{
	...
	if (f)
	{
		...
		{
			common->DWarning("error reading mod description '%s'", descPath.c_str());
			fclose(f);
		}
	}
