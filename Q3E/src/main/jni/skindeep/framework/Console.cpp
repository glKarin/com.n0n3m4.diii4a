...
idCVar idConsoleLocal::con_speed( "con_speed", "16", CVAR_SYSTEM, "speed at which the console moves up and down" );
...
#endif
idCVar idConsoleLocal::con_numLines("con_numLines", "5", CVAR_SYSTEM | CVAR_NOCHEAT, "number of lines to show",1,NUM_CON_TIMES);
...
float SCR_DrawFPS( float y ) {
	...
	if ( index > FPS_FRAMES ) {
		...
		double msTime = time_gameFrame+time_gameDraw;

		static double runningMSTime = 0.0f;
		msTime = msTime*0.05f + runningMSTime*0.95f;
		runningMSTime = msTime;
		...
		s = va( "%3ifps %3.1fms", fps, msTime);
		w = strlen( s ) * BIGCHAR_WIDTH;

		idVec4 textColor = (fps >= 40) ? colorWhite : colorRed;
		renderSystem->DrawBigStringExt( 635 - w, idMath::FtoiFast( y ) + 2, s, textColor, true, localConsole.charSetShader);
	}
...
void	idConsoleLocal::Close() {
	...
	historyLine = nextHistoryLine;  //BC if close console, then reset history queue index.
}
...
const char * idConsoleLocal::GetHistoryLine(int line) {
	line++;
	if (line > COMMAND_HISTORY) { return NULL; }
	int lineWrapped = (nextHistoryLine - line) % COMMAND_HISTORY;
...
void idConsoleLocal::KeyDownEvent( int key ) {
	...
	// blendo eric: copy and paste to system clipboard added
	if (key == 'v' && idKeyInput::IsDown(K_CTRL)) {
		consoleField.Paste();
		return;
	}
	if (key == 'c' && idKeyInput::IsDown(K_CTRL)) {
		// copy last console line (this is different from up arrow history!)
		int lineCount = idKeyInput::IsDown(K_SHIFT) ? 100 : 1;
		int lineCountEnd = lineCount + 5;

		idStr lastLines = GetLastLines(lineCount);
		lastLines.StripTrailingWhitespace();

		while (lastLines.IsEmpty() && lineCount++ < lineCountEnd)
		{
			lastLines = GetLastLines(lineCount);
			lastLines.StripTrailingWhitespace();
		}

		if (!lastLines.IsEmpty()) {
			Sys_SetClipboardData(lastLines);
		}
		return;
	}
	...
	if ( key == K_ENTER || key == K_KP_ENTER ) {
		...
		// blendo eric: save history file on every entry, rather than having to properly shutdown
		console->SaveHistory();
...
bool	idConsoleLocal::ProcessEvent( const sysEvent_t *event, bool forceAccept ) {
	...
	if(event->evType == SE_KEY)
	{
		if( event->evValue == Sys_GetConsoleKey( false ) || event->evValue == Sys_GetConsoleKey( true )
		    || (event->evValue == K_ESCAPE && (keyCatching||idKeyInput::IsDown( K_SHIFT ))) ) // shift+esc should also open console
		{
			consoleKey = true;
		}
	}
...
void idConsoleLocal::DrawNotify() {
	...
	int MaxLines = con_numLines.GetInteger();
	
	v = 0;
	for ( i = current - MaxLines; i <= current; i++ ) {
		if ( i < 0 ) {
			continue;
		}
...
void idConsoleLocal::DrawSolidConsole( float frac ) {
	...
	for ( x = 0; x < i; x++ )
	{
		renderSystem->DrawSmallChar( SCREEN_WIDTH - ( i - x ) * SMALLCHAR_WIDTH, (lines-(SMALLCHAR_HEIGHT+SMALLCHAR_HEIGHT/2)) + 7, engineversion[x], localConsole.charSetShader );
	}

	//idStr dllVersion = idStr::Format("DLL: %s", game->GetDLLVersionString().c_str());
	//i = dllVersion.Length();
	//for (x = 0; x < i; x++)
	//{
	//	renderSystem->DrawSmallChar(SCREEN_WIDTH - (i - x) * SMALLCHAR_WIDTH, (lines - (SMALLCHAR_HEIGHT + SMALLCHAR_HEIGHT / 2)) + 7, dllVersion[x], localConsole.charSetShader);
	//}
	...
	//BC draw command history
	if (nextHistoryLine != historyLine)
	{
		float history_y = SCREEN_HEIGHT - (SMALLCHAR_HEIGHT * 1.5f); //where to draw history commands. Start from bottom of screen.
		int historyLinesToShow = ((1.0f-frac) * SCREEN_HEIGHT) / SMALLCHAR_HEIGHT; //determine how many lines we can fit into this space.
		historyLinesToShow -= 1;
		int historyLinesToShowHalf = (historyLinesToShow / 2);

		int historyLineShowStart = historyLine - historyLinesToShowHalf;

		// edge detect
		while (historyLineShowStart < 0 || (nextHistoryLine - historyLineShowStart) > COMMAND_HISTORY)
		{
			historyLineShowStart++;
			historyLinesToShow--;
		}

		int historyLineShowEnd = historyLineShowStart + historyLinesToShow;

		// edge detect
		if (nextHistoryLine <= historyLineShowEnd)
		{
			historyLineShowEnd = nextHistoryLine - 1;
			historyLinesToShow--;
			history_y -= SMALLCHAR_HEIGHT;
		}

		for (int i = 0; i < historyLinesToShow; i++)
		{
			int currentLineShow = (historyLineShowEnd - i);
			currentLineShow = currentLineShow % COMMAND_HISTORY;
			idStr s = historyEditLines[currentLineShow].GetBuffer();
			if (s)
			{
				//draw the command history
				bool selected = currentLineShow == (historyLine % COMMAND_HISTORY);

				renderSystem->SetColor(selected ? colorBlue : colorBlack);
				renderSystem->DrawStretchPic(15, history_y, s.Length() * SMALLCHAR_WIDTH, SMALLCHAR_HEIGHT, 0, 0, 0, 0, whiteShader);

				if (selected)
				{
					renderSystem->DrawSmallStringExt(5, history_y, "*", colorCyan, true, localConsole.charSetShader);
				}

				//renderSystem->DrawSmallStringExt(15 + 1, history_y + 1, s, colorBlack, true, localConsole.charSetShader);
				renderSystem->DrawSmallStringExt(15, history_y, s, selected ? colorCyan : colorLtGrey, true, localConsole.charSetShader);
			}
			history_y -= SMALLCHAR_HEIGHT;
		}
	}
