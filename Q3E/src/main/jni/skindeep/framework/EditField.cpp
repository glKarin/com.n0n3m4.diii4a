...
idStrList globalAutoCompleteMatches;

static void AddMatches( const char *s ) {
	if ( idStr::Icmpn( s, globalAutoComplete.currentMatch, strlen( globalAutoComplete.currentMatch ) ) == 0 ) {
		globalAutoCompleteMatches.Append( idStr::Format( "    %s\n", s ) );
	}
}

static void AddCvarMatches( const char *s ) {
	if ( idStr::Icmpn( s, globalAutoComplete.currentMatch, strlen( globalAutoComplete.currentMatch ) ) == 0 ) {
		globalAutoCompleteMatches.Append( idStr::Format( "    %s" S_COLOR_WHITE " = \"%s\"\n", s, cvarSystem->GetCVarString( s ) ) );
	}
}
...
void idEditField::AutoComplete( void ) {
	...
	if ( !autoComplete.valid ) {
		...
		globalAutoCompleteMatches.Clear();
		cmdSystem->CommandCompletion( AddMatches );
		globalAutoCompleteMatches.Sort();
		for ( int i = 0; i < globalAutoCompleteMatches.Num(); ++i ) {
			common->Printf( globalAutoCompleteMatches[i] );
		}
		cmdSystem->ArgCompletion( autoComplete.completionString, PrintMatches );

		globalAutoCompleteMatches.Clear();
		cvarSystem->CommandCompletion( AddCvarMatches );
		globalAutoCompleteMatches.Sort();
		for ( int i = 0; i < globalAutoCompleteMatches.Num(); ++i ) {
			common->Printf( globalAutoCompleteMatches[i] );
		}
