...
typedef enum {
	...
	CVAR_SCRIPTMODIFIED		= BIT(19)	// set when the variable is modified in script, will be reset on level unload
} cvarFlags_t;
...
public:
	...
	//bc
	virtual const char *	GetCvarByIndex(int index) = 0;
	virtual int				GetCvarTotal(void) = 0;
};
...
ID_INLINE void idCVar::Init( const char *name, const char *value, int flags, const char *description,
	...
	if ( staticVars != (idCVar *)0xFFFFFFFFFFFFFFFF ) {
...
ID_INLINE void idCVar::RegisterStaticVars( void ) {
	if ( staticVars != (idCVar *)0xFFFFFFFFFFFFFFFF ) {
		...
		staticVars = (idCVar *)0xFFFFFFFFFFFFFFFF;
	}
}
