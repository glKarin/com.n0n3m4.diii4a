...
static const keyname_t keynames[] =
{
	...
	{ "JOY5", 			K_JOY5, 			"JOY5" },
	{ "JOY6", 			K_JOY6, 			"JOY6" },
	{ "JOY7", 			K_JOY7, 			"JOY_BTN_LSTICK" },
	{ "JOY8", 			K_JOY8, 			"JOY_BTN_RSTICK" },
	{ "JOY9", 			K_JOY9, 			"JOY_BTN_START" },
	{ "JOY10", 			K_JOY10, 			"JOY_BTN_BACK" },
...
void Key_Unbindall_f( const idCmdArgs &args ) {
	...
	if (args.Argc() == 2)
	{
		unbindType = args.Argv(1);
	}

	for ( i = 0; i < MAX_KEYS; i++ ) {
		
		// If we're only doing keyboard and this is gamepad, ignore it
		if (unbindType == "keyboard" && i >= K_JOY1 && i <= K_JOY_DPAD_RIGHT)
			continue;

		// If we're only doing gamepad and this is keyboard, ignore it
		if (unbindType == "gamepad" && (i < K_JOY1 || i > K_JOY_DPAD_RIGHT))
			continue;
...
============
*/
const char *idKeyInput::KeysFromBinding( const char *bind, bool joystick ) {
	int i;
	static idStr keyName;

	keyName.Clear();
	if ( bind && *bind ) {
		for ( i = MAX_KEYS - 1; i >= 0; i-- ) { // SM go backwards so special keys have priority over letters
			bool isJoystickKey = i >= K_JOY1 && i <= K_JOY_DPAD_RIGHT;
			if ( isJoystickKey == joystick && keys[i].binding.Icmp( bind ) == 0 ) {
				if ( keyName.Length() > 0 )
				{
					keyName += ' ';
					keyName += common->GetLanguageDict()->GetString( "#str_07183" ); //"OR"
					keyName += ' ';
				}
				keyName += KeyNumToString( i, true );
			}
		}
	}
	if ( keyName.Length() == 0 )
	{
		keyName = common->GetLanguageDict()->GetString( "#str_07133" ); //"EMPTY"
	}
	
	//idStr::ToLower( keyName );
	keyName.ToUpper(); //BC this is done via localization english.lang now.
...
bool idKeyInput::UnbindKeyboardBinding(const char* binding) {
	bool unbound = false;
	int i;

	if (binding && *binding) {
		for (i = 0; i < MAX_KEYS; i++) {
			if ((i < K_JOY1 || i > K_JOY_DPAD_RIGHT) && keys[i].binding.Icmp(binding) == 0) {
				SetBinding(i, "");
				unbound = true;
			}
		}
...
bool idKeyInput::UnbindGamepadBinding(const char* binding) {
	bool unbound = false;
	int i;
