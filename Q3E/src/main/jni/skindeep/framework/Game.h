...
{
public:
	LoadingContext( idGame* g );
	~LoadingContext();

	LoadingContext( const LoadingContext& l ) = delete;
	LoadingContext& operator=( const LoadingContext& l ) = delete;
	LoadingContext( LoadingContext&& l )
		: game( l.game )
	{}
	LoadingContext& operator=( LoadingContext&& l ) = delete;
private:
	idGame* game;
};
...
public:
	...
	//BC
	virtual void				OnMapChange() = 0;

	friend class LoadingContext;
	virtual LoadingContext		GetLoadingContext() = 0;
	virtual bool				InLoadingContext() = 0;

	virtual idStr				GetDLLVersionString() = 0;
private:
	virtual void				AddLoadingContext() = 0;
	virtual void				RemoveLoadingContext() = 0;
};

ID_INLINE LoadingContext::LoadingContext( idGame* g )
	: game( g )
{
	game->AddLoadingContext();
}

ID_INLINE LoadingContext::~LoadingContext()
{
	game->RemoveLoadingContext();
}
...
public:
	...
	//BC
	idUserInterface *			lighteditMenu;
	bool						editmenuActive;
	idEntity *					GetFirstSelectedEntity();
