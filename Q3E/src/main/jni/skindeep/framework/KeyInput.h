...
typedef enum {
	...
	K_GRAVE_A = 229,	// lowercase a with grave accent
...
public:
	...
	static bool			UnbindKeyboardBinding(const char* bind);
	static bool			UnbindGamepadBinding(const char* bind);
	static int			NumBinds( const char *binding );
	static bool			ExecKeyBinding( int keyNum );
	static const char *	KeysFromBinding( const char *bind, bool joystick = false );
