...
void idParticleStage::ParticleOrigin( particleGen_t *g, idVec3 &origin ) const {
	...
	if ( worldGravity ) {
		idVec3 gra( 0, 0, -gravity * g->renderEnt->shaderParms[SHADERPARM_PARTICLE_GRAVITYSCALE] );
		gra *= g->renderEnt->axis.Transpose();
		origin += gra * g->age * g->age;
	} else {
		origin[2] -= gravity * g->renderEnt->shaderParms[SHADERPARM_PARTICLE_GRAVITYSCALE] * g->age * g->age;
	}
}
