...
int idDeclFile::LoadAndParse() {
	...
	while( 1 ) {
		...
		if ( newDecl ) {
			...
			{
				//BC this used to be a src.warning but I'm changing it to Error to make it stop the game.
				common->Error( "%s '%s' previously defined at %s:%i", declManagerLocal.GetDeclNameFromType( identifiedType ), name.c_str(), newDecl->sourceFile->fileName.c_str(), newDecl->sourceLine );
				continue;
			}
...
void idDeclManagerLocal::Reload( bool force ) {
	common->Printf("Reloading %d decl files.\n", loadedFiles.Num());
...
const idDecl *idDeclManagerLocal::FindType( declType_t type, const char *name, bool makeDefault ) {
	...
	if ( !name || !name[0] ) {
		...
		//BC the fx/footstep for NPCs is firing this warning message. Find way to fix this?
...
void idDeclManagerLocal::ReloadDecls_f( const idCmdArgs &args ) {
	...
	if ( !idStr::Icmp( args.Argv( 1 ), "all" ) ) {
		...
		force = false;
		common->Printf("Checking for changed decl files...\n");
	}
