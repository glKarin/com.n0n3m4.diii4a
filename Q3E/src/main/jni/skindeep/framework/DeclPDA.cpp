...
void idDeclPDA::AddEmail( const char *name, bool unique ) const {
	...
	if ( declManager->FindType( DECL_EMAIL, name, false ) == NULL ) {
		common->Warning( "AddEmail: cannot find email '%s'\n", name );
		return;
	}
	emails.Append( name );
}

void idDeclPDA::AddEmailReplied(const char* name) const {
	if (emailsReplied.Find(name) != nullptr) {
		return;
	}
	emailsReplied.Append(name);
}

bool idDeclPDA::IsEmailReplied(const char* name) const {
	return emailsReplied.Find(name) != nullptr;
}

//Return TRUE if the email was read for the first time just now.
bool idDeclPDA::AddEmailRead(const char* name) const {
	if (emailsRead.Find(name) != nullptr) {
		return false;
	}

	emailsRead.Append(name);
	return true;
}

bool idDeclPDA::IsEmailRead(const char* name) const {
	return emailsRead.Find(name) != nullptr;
}

bool idDeclPDA::IsEmailExist(const char* name) const
{
	return emails.Find(name) != nullptr;
}

bool idDeclPDA::HasCriticalUnrepliedEmails() const {
	for (int i = 0; i < emails.Num(); i++)
	{
		const idDeclEmail* email = GetEmailByIndex(i);
		if (email->IsCritical() && ((!IsEmailReplied(emails[i]) && email->HasReplies())
			|| !IsEmailRead(email->GetName())))
		{
			return true;
		}
	}
...
bool idDeclPDA::HasUnreadEmails() const {
	for (int i = 0; i < emails.Num(); i++)
	{
		if (!IsEmailRead(emails[i]))
		{
			return true;
		}
	}
...
bool idDeclEmail::Parse( const char *_text, const int textLength ) {
	...
	while( 1 ) {
		...
		if (!token.Icmp("reply1")) {
			src.ReadToken(&token);
			reply1 = token;
			continue;
		}

		if (!token.Icmp("reply2")) {
			src.ReadToken(&token);
			reply2 = token;
			continue;
		}

		if (!token.Icmp("replyfunc1")) {
			src.ReadToken(&token);
			replyfunc1 = token;
			continue;
		}

		if (!token.Icmp("replyfunc2")) {
			src.ReadToken(&token);
			replyfunc2 = token;
			continue;
		}

		if (!token.Icmp("critical")) {
			isCritical = true;
			continue;
		}

		//BC script function to call when email is read.
		if (!token.Icmp("readfunc")) {
			src.ReadToken(&token);
			readfunc = token;
			continue;
		}
