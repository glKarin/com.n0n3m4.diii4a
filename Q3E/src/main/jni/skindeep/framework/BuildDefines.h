...
// SM: Add optionality support for different version builds
#if !defined(NOSTEAM) && !defined(WINDOWSSTORE) && !defined(EPICSTORE) && !defined(__ANDROID__) //karin: no steam on Android
// If there are none of these defines, then assume we want Steam
#define STEAM 1
#endif

#if !defined(WIN32)
#define OutputDebugString(...) Sys_Printf(__VA_ARGS__)
inline static void MessageBox(void *, const char *msg, const char *title, int) {
	...
	//Sys_Printf("[MessageBox]: %s: %s\n", title, msg);
    Android_OpenDialog(title, msg, 0, 0);
}
enum {
	MB_OK = 1,
	MB_ICONERROR,
