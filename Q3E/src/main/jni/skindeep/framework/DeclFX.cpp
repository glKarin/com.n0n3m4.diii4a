...
void idDeclFX::ParseSingleFXAction( idLexer &src, idFXSingleAction& FXAction ) {
	...
	FXAction.relativeoffset = idVec3(0, 0, 0);
	FXAction.relativeAngle = idVec3(0, 0, 0);
	...
	while (1) {
		...
		//BC offset the position of this effect.
		if (!token.Icmp("relativeoffset")) {
			FXAction.relativeoffset.x = src.ParseFloat();
			src.ExpectTokenString(",");
			FXAction.relativeoffset.y = src.ParseFloat();
			src.ExpectTokenString(",");
			FXAction.relativeoffset.z = src.ParseFloat();
			continue;
		}
		...
		if ( !token.Icmp( "decal" ) ) {
			...
			FXAction.data = token;
			src.ExpectTokenString(",");
			FXAction.explicitAxis = src.ParseBool();
		...
		if (!token.Icmp("relativeangle"))
		{
			...
			src.ExpectTokenString(",");
			a[2] = src.ParseFloat();
			FXAction.relativeAngle = idVec3(a[0], a[1], a[2]);
			continue;
		}
