...
#include "framework/KeyInput.h" //BC
...
public:
	...
	//BC
	virtual const char *	GetKeyFromBinding(const char *bindname, bool joystick = false);
...
void idCmdSystemLocal::Find_f(const idCmdArgs &args)
{
	...
	for (i = 0; i < candidates.Num(); i += 2)
	{
		if (i+1 < candidates.Num())
			common->Printf("%-40s %-30s\n", candidates[i].c_str(), candidates[i+1].c_str());
		else
			common->Printf("%s\n", candidates[i].c_str());
	}

	common->Printf("\n -- Found %i commands with substring '%s' --\n", candidates.Num(), substring.c_str());
}
...
void idCmdSystemLocal::Init( void ) {
	...
	AddCommand("find", Find_f, CMD_FL_SYSTEM, "finds console commands via substring"); //BC
...
const char * idCmdSystemLocal::GetKeyFromBinding(const char *bindname, bool joystick)
{
	return idKeyInput::KeysFromBinding(bindname, joystick);
}